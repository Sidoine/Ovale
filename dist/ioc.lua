...
local __engineActionBar = LibStub:GetLibrary("ovale/engine/ActionBar")
...
local __engineBestAction = LibStub:GetLibrary("ovale/engine/BestAction")
...
__exports.IoC = __class(nil, {
    constructor = function(self)
        ...
        self.compile = OvaleCompileClass(self.azeriteArmor, self.ast, self.condition, self.cooldown, self.paperDoll, self.data, self.profiler, self.debug, self.options, self.ovale, self.score, self.spellBook)
        ...
        self.frame = OvaleFrameModuleClass(self.state, self.compile, self.future, self.baseState, self.enemies, self.ovale, self.options, self.debug, self.spellFlash, self.spellBook, self.bestAction, combat, runner)
