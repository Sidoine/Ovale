...
AddFunction brewmaster_defaultmainactions
{
 ...
 if enemies() >= 3 and spellcooldown(keg_smash) > gcd() and energy() + energy() * { spellcooldown(keg_smash) + gcd() } >= 65 spell(spinning_crane_kick)
...
AddFunction brewmaster_defaultshortcdactions
{
 ...
 unless incomingdamage(1.5) > 0 and buffexpires(fortifying_brew) and spell(dampen_harm) or spell(berserking)
 {
  ...
  unless staggerremaining() / maxhealth() * 100 > 6 * { 1 - spellcharges(purifying_brew count=0) } and staggertick(1) > { 0.02 + 0.001 * { 1 - spellcharges(purifying_brew count=0) } } * staggertick(30) and spell(purifying_brew) or spellcharges(purifying_brew count=0) < 0.5 and spell(black_ox_brew) or energy() + energy() * spellcooldown(keg_smash) < 40 and buffexpires(blackout_combo_buff) and not spellcooldown(keg_smash) > 0 and spell(black_ox_brew) or enemies() >= 2 and spell(keg_smash)
  {
 ...
 unless incomingdamage(1.5) > 0 and buffexpires(fortifying_brew) and spell(dampen_harm)
 {
  ...
  unless spell(berserking)
  {
   ...
   unless spell(bag_of_tricks)
   {
    ...
    unless staggerremaining() / maxhealth() * 100 > 6 * { 1 - spellcharges(purifying_brew count=0) } and staggertick(1) > { 0.02 + 0.001 * { 1 - spellcharges(purifying_brew count=0) } } * staggertick(30) and spell(purifying_brew) or spellcharges(purifying_brew count=0) < 0.5 and spell(black_ox_brew) or energy() + energy() * spellcooldown(keg_smash) < 40 and buffexpires(blackout_combo_buff) and not spellcooldown(keg_smash) > 0 and spell(black_ox_brew) or enemies() >= 2 and spell(keg_smash) or buffexpires(blackout_combo_buff) and incomingdamage(1.999) > message("health.max is not implemented") * 0.1 + staggertick(4) and buffstacks(elusive_brawler) < 2 and spell(celestial_brew) or hastalent(rushing_jade_wind_talent) and buffpresent(blackout_combo_buff) and buffpresent(rushing_jade_wind) and spell(tiger_palm) or { 1 or hastalent(special_delivery_talent) } and buffpresent(blackout_combo_buff) and spell(tiger_palm) or buffstacks(gift_of_the_ox) > 4 and spell(expel_harm) or spell(blackout_kick) or spell(keg_smash) or not target.debuffremaining(concentrated_flame) > 0 and spell(concentrated_flame)
    {
...
AddFunction windwalkerstmainactions
{
 ...
 if buffstacks(chi_energy) > 30 - 5 * enemies() and not previousspell(spinning_crane_kick) and buffexpires(storm_earth_and_fire) and { spellcooldown(rising_sun_kick) > 2 and spellcooldown(fists_of_fury) > 2 or spellcooldown(rising_sun_kick) < 3 and spellcooldown(fists_of_fury) > 3 and chi() > 3 or spellcooldown(rising_sun_kick) > 3 and spellcooldown(fists_of_fury) < 3 and chi() > 4 or maxchi() - chi() <= 1 and energy() < 2 } or buffstacks(chi_energy) > 10 and fightremains() < 7 spell(spinning_crane_kick)
...
AddFunction windwalkeraoemainactions
{
 ...
 if energy() > executetime(fists_of_fury) - 1 or buffpresent(storm_earth_and_fire) spell(fists_of_fury)
 ...
 if not previousspell(spinning_crane_kick) and { { chi() > 3 or spellcooldown(fists_of_fury) > 6 } and { chi() >= 5 or spellcooldown(fists_of_fury) > 2 } or energy() <= 3 } spell(spinning_crane_kick)
 #expel_harm,if=chi.max-chi>=1+essence.conflict_and_strife.major
 if maxchi() - chi() >= 1 + azeriteessenceismajor(conflict_and_strife_essence_id) spell(expel_harm)
 #chi_burst,if=chi.max-chi>=1
 if maxchi() - chi() >= 1 and checkboxon(opt_chi_burst) spell(chi_burst)
 ...
 if not previousspell(blackout_kick) and { buffpresent(bok_proc_buff) or hastalent(hit_combo_talent) and previousgcdspell(tiger_palm) and { maxchi() - chi() >= 14 and energy() < 1 or chi() == 2 and spellcooldown(fists_of_fury) < 3 } } spell(blackout_kick)
}
...
{
 spellcooldown(fists_of_fury) > 0 and spellcooldown(rising_sun_kick) > 0 and spell(whirling_dragon_punch) or { maxchi() - chi() >= 2 and energy() > 3 or maxchi() - chi() >= 4 and { energy() > 2 or not previousgcdspell(tiger_palm) } } and spell(energizing_elixir) or not previousspell(spinning_crane_kick) and { buffpresent(dance_of_chiji_buff) or buffpresent(dance_of_chiji_azerite_buff) } and spell(spinning_crane_kick) or { energy() > executetime(fists_of_fury) - 1 or buffpresent(storm_earth_and_fire) } and spell(fists_of_fury) or hastalent(whirling_dragon_punch_talent) and spellcooldownduration(rising_sun_kick) > spellcooldown(whirling_dragon_punch) + 3 and { spellcooldown(fists_of_fury) > 3 or chi() >= 5 } and spell(rising_sun_kick) or buffexpires(rushing_jade_wind) and spell(rushing_jade_wind) or not previousspell(spinning_crane_kick) and { { chi() > 3 or spellcooldown(fists_of_fury) > 6 } and { chi() >= 5 or spellcooldown(fists_of_fury) > 2 } or energy() <= 3 } and spell(spinning_crane_kick) or maxchi() - chi() >= 1 + azeriteessenceismajor(conflict_and_strife_essence_id) and spell(expel_harm) or maxchi() - chi() >= 1 and checkboxon(opt_chi_burst) and spell(chi_burst) or maxchi() - chi() >= 3 and spell(fist_of_the_white_tiger) or maxchi() - chi() >= 2 and { not hastalent(hit_combo_talent) or not previousspell(tiger_palm) } and spell(tiger_palm) or not previousspell(chi_wave) and spell(chi_wave) or buffexpires(bok_proc_buff) and checkboxon(opt_flying_serpent_kick) and spell(flying_serpent_kick) or not previousspell(blackout_kick) and { buffpresent(bok_proc_buff) or hastalent(hit_combo_talent) and previousgcdspell(tiger_palm) and { maxchi() - chi() >= 14 and energy() < 1 or chi() == 2 and spellcooldown(fists_of_fury) < 3 } } and spell(blackout_kick)
}
...
AddFunction windwalker_defaultmainactions
{
 ...
 unless buffpresent(serenity) and windwalkerserenitymainpostconditions()
 {
  ...
  unless timeincombat() < 5 and chi() < 5 and not pet.present() and windwalkeropenermainpostconditions()
  {
   ...
   if maxchi() - chi() >= 1 + azeriteessenceismajor(conflict_and_strife_essence_id) and { energy() < 1 or spellcooldown(serenity) < 2 or energy() < 4 and spellcooldown(fists_of_fury) < 1.5 } spell(expel_harm)
...
AddFunction windwalker_defaultshortcdactions
{
 ...
 unless buffpresent(serenity) and windwalkerserenityshortcdpostconditions()
 {
  ...
  unless timeincombat() < 5 and chi() < 5 and not pet.present() and windwalkeropenershortcdpostconditions() or maxchi() - chi() >= 3 and { energy() < 1 or energy() < 4 and spellcooldown(fists_of_fury) < 1.5 } and spell(fist_of_the_white_tiger) or maxchi() - chi() >= 1 + azeriteessenceismajor(conflict_and_strife_essence_id) and { energy() < 1 or spellcooldown(serenity) < 2 or energy() < 4 and spellcooldown(fists_of_fury) < 1.5 } and spell(expel_harm) or not previousspell(tiger_palm) and maxchi() - chi() >= 2 and { energy() < 1 or spellcooldown(serenity) < 2 or energy() < 4 and spellcooldown(fists_of_fury) < 1.5 } and spell(tiger_palm)
  {
   #call_action_list,name=cd_sef,if=!talent.serenity.enabled
...
AddFunction windwalker_defaultcdactions
{
 ...
 unless buffpresent(serenity) and windwalkerserenitycdpostconditions()
 {
  ...
  unless timeincombat() < 5 and chi() < 5 and not pet.present() and windwalkeropenercdpostconditions() or maxchi() - chi() >= 3 and { energy() < 1 or energy() < 4 and spellcooldown(fists_of_fury) < 1.5 } and spell(fist_of_the_white_tiger) or maxchi() - chi() >= 1 + azeriteessenceismajor(conflict_and_strife_essence_id) and { energy() < 1 or spellcooldown(serenity) < 2 or energy() < 4 and spellcooldown(fists_of_fury) < 1.5 } and spell(expel_harm) or not previousspell(tiger_palm) and maxchi() - chi() >= 2 and { energy() < 1 or spellcooldown(serenity) < 2 or energy() < 4 and spellcooldown(fists_of_fury) < 1.5 } and spell(tiger_palm)
  {
   #call_action_list,name=cd_sef,if=!talent.serenity.enabled
