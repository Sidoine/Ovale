...
AddFunction firecombustion_phasemainactions
{
 ...
 if hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master_buff) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master_buff) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master_buff) or buffremaining(blaster_master_buff) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } spell(fire_blast)
...
AddFunction firecombustion_phaseshortcdactions
{
 ...
 unless enemies() > 1 and buffexpires(combustion) and spell(living_bomb)
 {
  ...
  unless spell(blood_of_the_enemy) or spell(memory_of_lucid_dreams) or spell(worldvein_resonance) or hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master_buff) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master_buff) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master_buff) or buffremaining(blaster_master_buff) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and not conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(disciplinary_command) == 0 and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__frost_aura_dnt) and spell(frostbolt)
  {
   #call_action_list,name=active_talents
   fireactive_talentsshortcdactions()
  }
...
AddFunction froststmainactions
{
 ...
 if buffstacks(winters_chill) and buffstacks(winters_chill) > buffstacks(fingers_of_frost_buff) and target.debuffremaining(winters_chill) > traveltime(ice_lance) spell(ice_lance)
 #ice_nova
 spell(ice_nova)
 #ice_lance,if=buff.fingers_of_frost.react|debuff.frozen.remains>travel_time
 if buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) spell(ice_lance)
...
{
 unless { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and { previousgcdspell(radiant_spark) or previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) or { target.debuffpresent(mirrors_of_torment) or buffpresent(expanded_potential_buff) or buffpresent(freezing_winds) } and buffstacks(fingers_of_frost_buff) == 0 } } and spell(flurry)
 {
  #frozen_orb
  spell(frozen_orb)
  ...
  unless { buffpresent(freezing_rain_buff) or enemies() >= 3 or enemies() >= 2 and not equippedruneforge(cold_front_runeforge) } and spell(blizzard)
  {
   ...
   unless buffstacks(winters_chill) and target.debuffremaining(winters_chill) > casttime(glacial_spike) + traveltime(glacial_spike) and spell(glacial_spike) or buffstacks(winters_chill) and buffstacks(winters_chill) > buffstacks(fingers_of_frost_buff) and target.debuffremaining(winters_chill) > traveltime(ice_lance) and spell(ice_lance)
   {
    ...
    unless spell(ice_nova)
    {
     ...
     unless { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) } and spell(ice_lance)
     {
      #ebonbolt
      spell(ebonbolt)
...
{
 { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and { previousgcdspell(radiant_spark) or previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) or { target.debuffpresent(mirrors_of_torment) or buffpresent(expanded_potential_buff) or buffpresent(freezing_winds) } and buffstacks(fingers_of_frost_buff) == 0 } } and spell(flurry) or { buffpresent(freezing_rain_buff) or enemies() >= 3 or enemies() >= 2 and not equippedruneforge(cold_front_runeforge) } and spell(blizzard) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > casttime(glacial_spike) + traveltime(glacial_spike) and spell(glacial_spike) or buffstacks(winters_chill) and buffstacks(winters_chill) > buffstacks(fingers_of_frost_buff) and target.debuffremaining(winters_chill) > traveltime(ice_lance) and spell(ice_lance) or spell(ice_nova) or { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) } and spell(ice_lance) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and spell(fire_blast) or buffpresent(brain_freeze_frost) and spell(glacial_spike) or spell(frostbolt)
}
...
AddFunction frostaoemainactions
{
 ...
 if { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and buffstacks(fingers_of_frost_buff) == 0 } spell(flurry)
 #ice_nova
 spell(ice_nova)
 ...
 if buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > traveltime(ice_lance) spell(ice_lance)
 #fire_blast,if=runeforge.disciplinary_command.equipped&cooldown.buff_disciplinary_command.ready&buff.disciplinary_command_fire.down
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) spell(fire_blast)
...
AddFunction frostaoeshortcdactions
{
 ...
 unless spell(blizzard) or { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and buffstacks(fingers_of_frost_buff) == 0 } and spell(flurry) or spell(ice_nova)
 {
  #comet_storm
  spell(comet_storm)

  unless { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > traveltime(ice_lance) } and spell(ice_lance)
  {
   #radiant_spark
   spell(radiant_spark)
   #shifting_power
