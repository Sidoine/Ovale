local __exports = LibStub:NewLibrary("ovale/scripts/ovale_mage", 90000)
...
AddFunction ap_minimum_mana_pct
{
 ...
 {
  0
 } == 30 and equippedruneforge(grisly_icicle_runeforge) 50
 if 
 {
  0
 } == 30 and equippedruneforge(disciplinary_command_runeforge) 50
 0
}
...
AddFunction barrage_mana_pct
{
 ...
 {
  0
 } == 70 and iscovenant(night_fae) 40
 0
}
...
AddFunction totm_max_delay
{
 ...
 {
  0
 } == 5 and azeriteessenceisminor(vision_of_perfection_essence_id) 30
 if 
 {
  0
 } == 5 and conduit(arcane_prodigy_conduit) and enemies() < 3 15
 if 
 {
  0
 } == 5 and iscovenant(night_fae) 15
 if 
 {
  0
 } == 5 and equippedruneforge(disciplinary_command_runeforge) 3
 0
}
...
AddFunction prepull_evo
{
 ...
 {
  0
 } == 0 and equippedruneforge(siphon_storm_runeforge) and iscovenant(night_fae) 1
 if 
 {
  0
 } == 0 and equippedruneforge(siphon_storm_runeforge) and iscovenant(necrolord) and enemies() > 1 1
 if 
 {
  0
 } == 0 and equippedruneforge(siphon_storm_runeforge) and enemies() > 2 1
 0
}

AddFunction final_burn
{
 if buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and not buffpresent(rule_of_threes_buff) and fightremains() <= mana() / powercost(arcane_blast) * executetime(arcane_blast) 1
 0
}
...
AddFunction arcaneshared_cdscdactions
{
 ...
 if buffpresent(arcane_power) and { checkboxon(opt_use_consumables) and target.classification(worldboss) } item(focused_resolve usable=1)
...
AddFunction arcanerotationmainactions
{
 ...
 if not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and iscovenant(kyrian) } and spellcooldown(radiant_spark) <= 8 spell(arcane_barrage)
 ...
 if not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() } and not iscovenant(kyrian) spell(arcane_barrage)
 ...
 if buffpresent(clearcasting_arcane) and buffpresent(expanded_potential_buff) spell(arcane_missiles)
 ...
 if buffpresent(clearcasting_arcane) and { buffpresent(arcane_power) or buffpresent(rune_of_power_buff) or target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) } spell(arcane_missiles)
 #arcane_missiles,if=buff.clearcasting.react&buff.clearcasting.stack=buff.clearcasting.max_stack,chain=1
 if buffpresent(clearcasting_arcane) and buffstacks(clearcasting_arcane) == spelldata(clearcasting_arcane max_stacks) spell(arcane_missiles)
 ...
 if buffpresent(clearcasting_arcane) and buffremaining(clearcasting_arcane) <= buffstacks(clearcasting_arcane) * executetime(arcane_missiles) + gcd() spell(arcane_missiles)
 ...
 if { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) spell(nether_tempest)
 ...
 if buffpresent(rule_of_threes_buff) and buffstacks(arcane_charge_buff) > 3 and mana() > manacost(arcane_blast) spell(arcane_blast)
 ...
 if manapercent() <= barrage_mana_pct() and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spellcooldown(evocation) > 0 spell(arcane_barrage)
 ...
 if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and hastalent(arcane_orb_talent) and spellcooldown(arcane_orb) <= gcd() and manapercent() <= 90 and spellcooldown(evocation) > 0 spell(arcane_barrage)
 ...
 if buffpresent(rune_of_power_buff) and totemremaining(rune_of_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) spell(arcane_barrage)
 ...
 if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) spell(arcane_barrage)
 #arcane_blast
...
{
 unless not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and iscovenant(kyrian) } and spellcooldown(radiant_spark) <= 8 and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() } and not iscovenant(kyrian) and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and not hastalent(rune_of_power_talent) } and spellcooldown(arcane_power) > totm_max_delay() and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) > totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(arcane_barrage) or target.debuffstacks(radiant_spark_vulnerability) == spelldata(radiant_spark_vulnerability max_stacks) and { buffexpires(arcane_power) or buffremaining(arcane_power) <= gcd() } and { buffexpires(rune_of_power_buff) or totemremaining(rune_of_power) <= gcd() } and spell(arcane_barrage) or { target.debuffremaining(radiant_spark) > 5 or target.debuffstacks(radiant_spark_vulnerability) > 0 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(presence_of_mind) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= executetime(arcane_blast) and mana() > manacost(arcane_blast) and spell(arcane_blast) or target.debuffpresent(touch_of_the_magi) and hastalent(arcane_echo_talent) and buffexpires(deathborne) and { target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) or spellcooldown(presence_of_mind) > 0 or iscovenant(kyrian) } and { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffpresent(expanded_potential_buff) and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and { buffpresent(arcane_power) or buffpresent(rune_of_power_buff) or target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffstacks(clearcasting_arcane) == spelldata(clearcasting_arcane max_stacks) and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffremaining(clearcasting_arcane) <= buffstacks(clearcasting_arcane) * executetime(arcane_missiles) + gcd() and spell(arcane_missiles) or { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and spell(nether_tempest) or buffstacks(arcane_charge_buff) <= totm_max_charges() and spell(arcane_orb) or manapercent() <= 95 and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(supernova)
 {
  ...
  if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spellcooldown(evocation) > 0 and spellcooldown(arcane_power) > 0 and spellcooldown(touch_of_the_magi) > 0 and { not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > 0 } spell(shifting_power)
 }
}

AddFunction arcanerotationshortcdpostconditions
{
 not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and iscovenant(kyrian) } and spellcooldown(radiant_spark) <= 8 and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() } and not iscovenant(kyrian) and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and not hastalent(rune_of_power_talent) } and spellcooldown(arcane_power) > totm_max_delay() and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) > totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(arcane_barrage) or target.debuffstacks(radiant_spark_vulnerability) == spelldata(radiant_spark_vulnerability max_stacks) and { buffexpires(arcane_power) or buffremaining(arcane_power) <= gcd() } and { buffexpires(rune_of_power_buff) or totemremaining(rune_of_power) <= gcd() } and spell(arcane_barrage) or { target.debuffremaining(radiant_spark) > 5 or target.debuffstacks(radiant_spark_vulnerability) > 0 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(presence_of_mind) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= executetime(arcane_blast) and mana() > manacost(arcane_blast) and spell(arcane_blast) or target.debuffpresent(touch_of_the_magi) and hastalent(arcane_echo_talent) and buffexpires(deathborne) and { target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) or spellcooldown(presence_of_mind) > 0 or iscovenant(kyrian) } and { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffpresent(expanded_potential_buff) and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and { buffpresent(arcane_power) or buffpresent(rune_of_power_buff) or target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffstacks(clearcasting_arcane) == spelldata(clearcasting_arcane max_stacks) and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffremaining(clearcasting_arcane) <= buffstacks(clearcasting_arcane) * executetime(arcane_missiles) + gcd() and spell(arcane_missiles) or { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and spell(nether_tempest) or buffstacks(arcane_charge_buff) <= totm_max_charges() and spell(arcane_orb) or manapercent() <= 95 and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(supernova) or buffpresent(rule_of_threes_buff) and buffstacks(arcane_charge_buff) > 3 and mana() > manacost(arcane_blast) and spell(arcane_blast) or manapercent() < barrage_mana_pct() and spellcooldown(evocation) > 0 and buffexpires(arcane_power) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and azeriteessenceisminor(vision_of_perfection_essence_id) and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { not spellcooldown(rune_of_power) > 0 or not spellcooldown(arcane_power) > 0 } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or manapercent() <= barrage_mana_pct() and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spellcooldown(evocation) > 0 and spell(arcane_barrage) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and hastalent(arcane_orb_talent) and spellcooldown(arcane_orb) <= gcd() and manapercent() <= 90 and spellcooldown(evocation) > 0 and spell(arcane_barrage) or buffpresent(arcane_power) and buffremaining(arcane_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffpresent(rune_of_power_buff) and totemremaining(rune_of_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(arcane_barrage)
}

AddFunction arcanerotationcdactions
{
 unless not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and iscovenant(kyrian) } and spellcooldown(radiant_spark) <= 8 and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() } and not iscovenant(kyrian) and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { buffstacks(arcane_charge_buff) > totm_max_charges() and not hastalent(rune_of_power_talent) } and spellcooldown(arcane_power) > totm_max_delay() and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) > totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(arcane_barrage) or target.debuffstacks(radiant_spark_vulnerability) == spelldata(radiant_spark_vulnerability max_stacks) and { buffexpires(arcane_power) or buffremaining(arcane_power) <= gcd() } and { buffexpires(rune_of_power_buff) or totemremaining(rune_of_power) <= gcd() } and spell(arcane_barrage) or { target.debuffremaining(radiant_spark) > 5 or target.debuffstacks(radiant_spark_vulnerability) > 0 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(presence_of_mind) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= executetime(arcane_blast) and mana() > manacost(arcane_blast) and spell(arcane_blast) or target.debuffpresent(touch_of_the_magi) and hastalent(arcane_echo_talent) and buffexpires(deathborne) and { target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) or spellcooldown(presence_of_mind) > 0 or iscovenant(kyrian) } and { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffpresent(expanded_potential_buff) and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and { buffpresent(arcane_power) or buffpresent(rune_of_power_buff) or target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffstacks(clearcasting_arcane) == spelldata(clearcasting_arcane max_stacks) and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and buffremaining(clearcasting_arcane) <= buffstacks(clearcasting_arcane) * executetime(arcane_missiles) + gcd() and spell(arcane_missiles) or { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and spell(nether_tempest) or buffstacks(arcane_charge_buff) <= totm_max_charges() and spell(arcane_orb) or manapercent() <= 95 and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(supernova) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spellcooldown(evocation) > 0 and spellcooldown(arcane_power) > 0 and spellcooldown(touch_of_the_magi) > 0 and { not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > 0 } and spell(shifting_power) or buffpresent(rule_of_threes_buff) and buffstacks(arcane_charge_buff) > 3 and mana() > manacost(arcane_blast) and spell(arcane_blast) or manapercent() < barrage_mana_pct() and spellcooldown(evocation) > 0 and buffexpires(arcane_power) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and azeriteessenceisminor(vision_of_perfection_essence_id) and spell(arcane_barrage) or not spellcooldown(touch_of_the_magi) > 0 and { not spellcooldown(rune_of_power) > 0 or not spellcooldown(arcane_power) > 0 } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or manapercent() <= barrage_mana_pct() and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spellcooldown(evocation) > 0 and spell(arcane_barrage) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and hastalent(arcane_orb_talent) and spellcooldown(arcane_orb) <= gcd() and manapercent() <= 90 and spellcooldown(evocation) > 0 and spell(arcane_barrage) or buffpresent(arcane_power) and buffremaining(arcane_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffpresent(rune_of_power_buff) and totemremaining(rune_of_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or mana() > manacost(arcane_blast) and spell(arcane_blast)
 {
...
{
 buffexpires(arcane_familiar_buff) and spell(arcane_familiar) or buffexpires(arcane_intellect) and spell(arcane_intellect) or spell(conjure_mana_gem) or prepull_evo() <= 0 and spell(frostbolt)
}
...
AddFunction arcaneopenermainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and buffpresent(disciplinary_command__frost_aura_dnt) spell(fire_blast)
 #cancel_action,if=action.shifting_power.channeling&gcd.remains=0
 if channeling(shifting_power) and not gcdremaining() > 0 texture(inv_pet_exitbattle text=cancel)
 ...
 if buffpresent(clearcasting_arcane) spell(arcane_missiles)
...
{
 unless equippedruneforge(disciplinary_command_runeforge) and buffpresent(disciplinary_command__frost_aura_dnt) and spell(fire_blast)
 {
  ...
  unless channeling(shifting_power) and not gcdremaining() > 0 and texture(inv_pet_exitbattle text=cancel)
  {
   #shifting_power,if=soulbind.field_of_blossoms.enabled
   if enabledsoulbind(field_of_blossoms_soulbind) spell(shifting_power)
   #touch_of_the_magi
...
{
 equippedruneforge(disciplinary_command_runeforge) and buffpresent(disciplinary_command__frost_aura_dnt) and spell(fire_blast) or channeling(shifting_power) and not gcdremaining() > 0 and texture(inv_pet_exitbattle text=cancel) or buffexpires(rune_of_power_buff) and spell(rune_of_power) or { target.debuffremaining(radiant_spark) > 5 or target.debuffstacks(radiant_spark_vulnerability) > 0 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(presence_of_mind) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= executetime(arcane_blast) and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(arcane_power) and buffremaining(arcane_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or target.debuffpresent(touch_of_the_magi) and hastalent(arcane_echo_talent) and buffexpires(deathborne) and target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) and { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and spell(arcane_missiles) or buffstacks(arcane_charge_buff) <= totm_max_charges() and { spellcooldown(arcane_power) > 10 or enemies() <= 2 } and spell(arcane_orb) or { buffpresent(rune_of_power_buff) or manapercent() > 15 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(arcane_barrage)
}

AddFunction arcaneopenercdactions
{
 unless equippedruneforge(disciplinary_command_runeforge) and buffpresent(disciplinary_command__frost_aura_dnt) and spell(fire_blast) or equippedruneforge(grisly_icicle_runeforge) and manapercent() > 95 and spell(frost_nova) or spell(mirrors_of_torment)
 {
  #deathborne
  spell(deathborne)

  unless manapercent() > 40 and spell(radiant_spark) or channeling(shifting_power) and not gcdremaining() > 0 and texture(inv_pet_exitbattle text=cancel) or enabledsoulbind(field_of_blossoms_soulbind) and spell(shifting_power) or spell(touch_of_the_magi)
  {
   #arcane_power
   spell(arcane_power)
   ...
   unless buffexpires(rune_of_power_buff) and spell(rune_of_power) or spell(presence_of_mind) or { target.debuffremaining(radiant_spark) > 5 or target.debuffstacks(radiant_spark_vulnerability) > 0 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(presence_of_mind) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= executetime(arcane_blast) and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(arcane_power) and buffremaining(arcane_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or target.debuffpresent(touch_of_the_magi) and hastalent(arcane_echo_talent) and buffexpires(deathborne) and target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) and { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and spell(arcane_missiles) or buffstacks(arcane_charge_buff) <= totm_max_charges() and { spellcooldown(arcane_power) > 10 or enemies() <= 2 } and spell(arcane_orb) or { buffpresent(rune_of_power_buff) or manapercent() > 15 } and mana() > manacost(arcane_blast) and spell(arcane_blast)
   {
    ...
    if buffexpires(rune_of_power_buff) spell(evocation)
   }
  }
 }
}
...
{
 equippedruneforge(disciplinary_command_runeforge) and buffpresent(disciplinary_command__frost_aura_dnt) and spell(fire_blast) or equippedruneforge(grisly_icicle_runeforge) and manapercent() > 95 and spell(frost_nova) or spell(mirrors_of_torment) or manapercent() > 40 and spell(radiant_spark) or channeling(shifting_power) and not gcdremaining() > 0 and texture(inv_pet_exitbattle text=cancel) or enabledsoulbind(field_of_blossoms_soulbind) and spell(shifting_power) or spell(touch_of_the_magi) or buffexpires(rune_of_power_buff) and spell(rune_of_power) or spell(presence_of_mind) or { target.debuffremaining(radiant_spark) > 5 or target.debuffstacks(radiant_spark_vulnerability) > 0 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(presence_of_mind) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= executetime(arcane_blast) and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(arcane_power) and buffremaining(arcane_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or target.debuffpresent(touch_of_the_magi) and hastalent(arcane_echo_talent) and buffexpires(deathborne) and target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) and { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and spell(arcane_missiles) or buffstacks(arcane_charge_buff) <= totm_max_charges() and { spellcooldown(arcane_power) > 10 or enemies() <= 2 } and spell(arcane_orb) or { buffpresent(rune_of_power_buff) or manapercent() > 15 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(arcane_barrage)
}
...
{
 buffpresent(clearcasting_arcane) and spell(arcane_missiles) or mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(arcane_barrage)
}
...
AddFunction arcaneessencesmainactions
{
 ...
 if not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() spell(worldvein_resonance)
 ...
 if timesincepreviousspell(concentrated_flame) > 6 and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and timetomaxmana() >= executetime(concentrated_flame) } spell(concentrated_flame)
 #ripple_in_space,if=buff.arcane_power.down&buff.rune_of_power.down&debuff.touch_of_the_magi.down
 if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) spell(ripple_in_space)
 #the_unbound_force,if=buff.arcane_power.down&buff.rune_of_power.down&debuff.touch_of_the_magi.down
 if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) spell(the_unbound_force)
 #memory_of_lucid_dreams,if=buff.arcane_power.down&buff.rune_of_power.down&debuff.touch_of_the_magi.down
 if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) spell(memory_of_lucid_dreams)
}
...
AddFunction arcaneessencesshortcdactions
{
 unless { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(blood_of_the_enemy) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(blood_of_the_enemy) or spellcooldown(arcane_power) >= 50 and spellcooldown(touch_of_the_magi) <= gcd() and buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and spell(worldvein_resonance) or { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(worldvein_resonance) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(worldvein_resonance) or timesincepreviousspell(concentrated_flame) > 6 and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and timetomaxmana() >= executetime(concentrated_flame) } and spell(concentrated_flame)
 {
  ...
  if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and timetomaxmana() >= executetime(reaping_flames) spell(reaping_flames)
  #focused_azerite_beam,if=buff.arcane_power.down&buff.rune_of_power.down&debuff.touch_of_the_magi.down
  if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) spell(focused_azerite_beam)
  #purifying_blast,if=buff.arcane_power.down&buff.rune_of_power.down&debuff.touch_of_the_magi.down
  if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) spell(purifying_blast)
 }
}

AddFunction arcaneessencesshortcdpostconditions
{
 { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(blood_of_the_enemy) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(blood_of_the_enemy) or spellcooldown(arcane_power) >= 50 and spellcooldown(touch_of_the_magi) <= gcd() and buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and spell(worldvein_resonance) or { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(worldvein_resonance) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(worldvein_resonance) or timesincepreviousspell(concentrated_flame) > 6 and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and timetomaxmana() >= executetime(concentrated_flame) } and spell(concentrated_flame) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(ripple_in_space) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(the_unbound_force) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(memory_of_lucid_dreams)
}

AddFunction arcaneessencescdactions
{
 unless { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(blood_of_the_enemy) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(blood_of_the_enemy) or spellcooldown(arcane_power) >= 50 and spellcooldown(touch_of_the_magi) <= gcd() and buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and spell(worldvein_resonance) or { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(worldvein_resonance) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(worldvein_resonance)
 {
  ...
  #guardian_of_azeroth,if=cooldown.arcane_power.remains=0&(!talent.enlightened.enabled|(talent.enlightened.enabled&mana.pct>=70|variable.am_spam=1))&((cooldown.touch_of_the_magi.remains>variable.ap_max_delay&(buff.arcane_charge.stack=buff.arcane_charge.max_stack|variable.am_spam=1))|(cooldown.touch_of_the_magi.remains=0&buff.arcane_charge.stack=0))&buff.rune_of_power.down&mana.pct>=variable.ap_minimum_mana_pct
  if not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() spell(guardian_of_azeroth)
 }
}

AddFunction arcaneessencescdpostconditions
{
 { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(blood_of_the_enemy) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(blood_of_the_enemy) or spellcooldown(arcane_power) >= 50 and spellcooldown(touch_of_the_magi) <= gcd() and buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and spell(worldvein_resonance) or { not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() or fightremains() < spellcooldown(arcane_power) } and spell(worldvein_resonance) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 or am_spam() == 1 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and { buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or am_spam() == 1 } or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(worldvein_resonance) or timesincepreviousspell(concentrated_flame) > 6 and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and timetomaxmana() >= executetime(concentrated_flame) } and spell(concentrated_flame) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and timetomaxmana() >= executetime(reaping_flames) and spell(reaping_flames) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(focused_azerite_beam) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(purifying_blast) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(ripple_in_space) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(the_unbound_force) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(memory_of_lucid_dreams)
}
...
AddFunction arcanecooldownsmainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } spell(frostbolt)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) spell(fire_blast)
 ...
 if buffexpires(rune_of_power_buff) and spellcooldown(touch_of_the_magi) > rop_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } spell(rune_of_power)
}
...
AddFunction arcanecooldownsshortcdactions
{
 ...
 if equippedruneforge(grisly_icicle_runeforge) and not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > 10 and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() spell(frost_nova)

 unless equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) and spell(fire_blast)
 {
  ...
  if not spellcooldown(arcane_power) > 0 and { { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) } and manapercent() >= ap_minimum_mana_pct() spell(radiant_spark)
  ...
  if buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and iscovenant(kyrian) and spellcooldown(radiant_spark) <= 8 spell(touch_of_the_magi)
  ...
  if buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and not iscovenant(kyrian) spell(touch_of_the_magi)
  ...
  unless buffexpires(rune_of_power_buff) and spellcooldown(touch_of_the_magi) > rop_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power)
  {
   #presence_of_mind,if=buff.arcane_charge.stack=0&covenant.kyrian.enabled
   if buffstacks(arcane_charge_buff) == 0 and iscovenant(kyrian) spell(presence_of_mind)
   #presence_of_mind,if=debuff.touch_of_the_magi.up&!covenant.kyrian.enabled
   if target.debuffpresent(touch_of_the_magi) and not iscovenant(kyrian) spell(presence_of_mind)
  }
 }
}
...
{
 equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) and spell(fire_blast) or buffexpires(rune_of_power_buff) and spellcooldown(touch_of_the_magi) > rop_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power)
}

AddFunction arcanecooldownscdactions
{
 unless equippedruneforge(grisly_icicle_runeforge) and spellcooldown(arcane_power) > 30 and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frost_nova) or equippedruneforge(grisly_icicle_runeforge) and not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > 10 and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(frost_nova) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) and spell(fire_blast) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(mirrors_of_torment) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(mirrors_of_torment)
 {
  ...
  unless spellcooldown(touch_of_the_magi) > rs_max_delay() and spellcooldown(arcane_power) > rs_max_delay() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > rs_max_delay() or not hastalent(rune_of_power_talent) } and buffstacks(arcane_charge_buff) > 2 and target.debuffexpires(touch_of_the_magi) and spell(radiant_spark) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(radiant_spark) or not spellcooldown(arcane_power) > 0 and { { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) } and manapercent() >= ap_minimum_mana_pct() and spell(radiant_spark) or spellcooldown(arcane_power) < 50 and azeriteessenceisminor(vision_of_perfection_essence_id) and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and iscovenant(kyrian) and spellcooldown(radiant_spark) <= 8 and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and not iscovenant(kyrian) and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(touch_of_the_magi)
  {
   ...
   if { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() spell(arcane_power)
  }
 }
}
...
{
 equippedruneforge(grisly_icicle_runeforge) and spellcooldown(arcane_power) > 30 and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frost_nova) or equippedruneforge(grisly_icicle_runeforge) and not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > 10 and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(frost_nova) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) and spell(fire_blast) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(mirrors_of_torment) or not spellcooldown(arcane_power) > 0 and { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) and manapercent() >= ap_minimum_mana_pct() and spell(mirrors_of_torment) or spellcooldown(touch_of_the_magi) > rs_max_delay() and spellcooldown(arcane_power) > rs_max_delay() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > rs_max_delay() or not hastalent(rune_of_power_talent) } and buffstacks(arcane_charge_buff) > 2 and target.debuffexpires(touch_of_the_magi) and spell(radiant_spark) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(radiant_spark) or not spellcooldown(arcane_power) > 0 and { { not hastalent(enlightened_talent) or hastalent(enlightened_talent) and manapercent() >= 70 } and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) == 0 } and buffexpires(rune_of_power_buff) } and manapercent() >= ap_minimum_mana_pct() and spell(radiant_spark) or spellcooldown(arcane_power) < 50 and azeriteessenceisminor(vision_of_perfection_essence_id) and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and iscovenant(kyrian) and spellcooldown(radiant_spark) <= 8 and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() and not iscovenant(kyrian) and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() and spell(touch_of_the_magi) or buffstacks(arcane_charge_buff) <= totm_max_charges() and spellcooldown(arcane_power) <= gcd() and spell(touch_of_the_magi) or buffexpires(rune_of_power_buff) and spellcooldown(touch_of_the_magi) > rop_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power) or buffstacks(arcane_charge_buff) == 0 and iscovenant(kyrian) and spell(presence_of_mind) or target.debuffpresent(touch_of_the_magi) and not iscovenant(kyrian) and spell(presence_of_mind)
}
...
AddFunction arcaneaoemainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } spell(frostbolt)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and timeincombat() == 0 spell(fire_blast)
 ...
 if { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) spell(nether_tempest)
 ...
 if buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and hastalent(amplification_talent) and enemies() < 9 spell(arcane_missiles)
 #arcane_missiles,if=buff.clearcasting.react&runeforge.arcane_infinity.equipped&active_enemies<6
 if buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and enemies() < 6 spell(arcane_missiles)
...
{
 unless equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or { equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and timeincombat() == 0 } and spell(fire_blast)
 {
  ...
  if not spellcooldown(arcane_power) > 0 and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and buffexpires(rune_of_power_buff) spell(radiant_spark)
  ...
  unless buffexpires(rune_of_power_buff) and { spellcooldown(touch_of_the_magi) > 20 and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power)
  {
   ...
   unless buffpresent(deathborne) and { hastalent(resonance_talent) and enemies() < 4 or enemies() < 5 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(supernova) or buffstacks(arcane_charge_buff) == 0 and spell(arcane_orb) or { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(nether_tempest)
   {
    ...
    if buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spellcooldown(arcane_power) > 0 and spellcooldown(touch_of_the_magi) > 0 and { not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > 0 } spell(shifting_power)
   }
  }
 }
}
...
{
 equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or { equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and timeincombat() == 0 } and spell(fire_blast) or buffexpires(rune_of_power_buff) and { spellcooldown(touch_of_the_magi) > 20 and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power) or buffpresent(deathborne) and { hastalent(resonance_talent) and enemies() < 4 or enemies() < 5 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(supernova) or buffstacks(arcane_charge_buff) == 0 and spell(arcane_orb) or { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(nether_tempest) or buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and hastalent(amplification_talent) and enemies() < 9 and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and enemies() < 6 and spell(arcane_missiles) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and spell(arcane_explosion) or buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and previousgcdspell(arcane_barrage) and spell(arcane_explosion) or buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage)
}

AddFunction arcaneaoecdactions
{
 unless equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or { equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and timeincombat() == 0 } and spell(fire_blast) or equippedruneforge(grisly_icicle_runeforge) and spellcooldown(arcane_power) > 30 and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frost_nova) or equippedruneforge(grisly_icicle_runeforge) and not spellcooldown(arcane_power) > 0 and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and buffexpires(rune_of_power_buff) and spell(frost_nova) or equippedruneforge(siphon_storm_runeforge) and previousgcdspell(evocation) and spell(touch_of_the_magi)
 {
  ...
  unless { spellcooldown(arcane_power) > 45 or spellcooldown(arcane_power) <= 3 } and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > 5 or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > 5 or spellcooldown(arcane_power) <= gcd() } and spell(mirrors_of_torment) or spellcooldown(touch_of_the_magi) > rs_max_delay() and spellcooldown(arcane_power) > rs_max_delay() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > rs_max_delay() or not hastalent(rune_of_power_talent) } and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and target.debuffexpires(touch_of_the_magi) and spell(radiant_spark) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(radiant_spark) or not spellcooldown(arcane_power) > 0 and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and buffexpires(rune_of_power_buff) and spell(radiant_spark)
  {
   ...
   if not spellcooldown(arcane_power) > 0 and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and buffexpires(rune_of_power_buff) spell(deathborne)
   ...
   unless buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(touch_of_the_magi)
   {
    ...
    if { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and buffexpires(rune_of_power_buff) spell(arcane_power)

    unless buffexpires(rune_of_power_buff) and { spellcooldown(touch_of_the_magi) > 20 and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power) or buffpresent(deathborne) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= spelldata(presence_of_mind max_stacks) * executetime(arcane_blast) and spell(presence_of_mind) or buffpresent(deathborne) and { hastalent(resonance_talent) and enemies() < 4 or enemies() < 5 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(supernova) or buffstacks(arcane_charge_buff) == 0 and spell(arcane_orb) or { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(nether_tempest) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spellcooldown(arcane_power) > 0 and spellcooldown(touch_of_the_magi) > 0 and { not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > 0 } and spell(shifting_power) or buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and hastalent(amplification_talent) and enemies() < 9 and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and enemies() < 6 and spell(arcane_missiles) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and spell(arcane_explosion) or buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and previousgcdspell(arcane_barrage) and spell(arcane_explosion) or buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage)
    {
     #evocation,interrupt_if=mana.pct>=85,interrupt_immediate=1
     spell(evocation)
    }
...
{
 equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and { buffexpires(arcane_power) and buffexpires(rune_of_power_buff) } and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or { equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and previousgcdspell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and timeincombat() == 0 } and spell(fire_blast) or equippedruneforge(grisly_icicle_runeforge) and spellcooldown(arcane_power) > 30 and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frost_nova) or equippedruneforge(grisly_icicle_runeforge) and not spellcooldown(arcane_power) > 0 and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and buffexpires(rune_of_power_buff) and spell(frost_nova) or equippedruneforge(siphon_storm_runeforge) and previousgcdspell(evocation) and spell(touch_of_the_magi) or { spellcooldown(arcane_power) > 45 or spellcooldown(arcane_power) <= 3 } and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > 5 or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > 5 or spellcooldown(arcane_power) <= gcd() } and spell(mirrors_of_torment) or spellcooldown(touch_of_the_magi) > rs_max_delay() and spellcooldown(arcane_power) > rs_max_delay() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > rs_max_delay() or not hastalent(rune_of_power_talent) } and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and target.debuffexpires(touch_of_the_magi) and spell(radiant_spark) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(radiant_spark) or not spellcooldown(arcane_power) > 0 and { spellcooldown(touch_of_the_magi) > ap_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and buffexpires(rune_of_power_buff) and spell(radiant_spark) or buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(touch_of_the_magi) or buffexpires(rune_of_power_buff) and { spellcooldown(touch_of_the_magi) > 20 and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) or not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() } and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power) or buffpresent(deathborne) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= spelldata(presence_of_mind max_stacks) * executetime(arcane_blast) and spell(presence_of_mind) or buffpresent(deathborne) and { hastalent(resonance_talent) and enemies() < 4 or enemies() < 5 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(supernova) or buffstacks(arcane_charge_buff) == 0 and spell(arcane_orb) or { target.refreshable(nether_tempest) or not target.debuffpresent(nether_tempest) } and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(nether_tempest) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spellcooldown(arcane_power) > 0 and spellcooldown(touch_of_the_magi) > 0 and { not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) > 0 } and spell(shifting_power) or buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and hastalent(amplification_talent) and enemies() < 9 and spell(arcane_missiles) or buffpresent(clearcasting_arcane) and equippedruneforge(arcane_infinity_runeforge) and enemies() < 6 and spell(arcane_missiles) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and spell(arcane_explosion) or buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and previousgcdspell(arcane_barrage) and spell(arcane_explosion) or buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage)
}
...
AddFunction arcaneam_spammainactions
{
 ...
 if buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) spell(arcane_orb)
 ...
 if buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) spell(arcane_barrage)
...
{
 buffexpires(rune_of_power_buff) and spellcooldown(arcane_power) > 0 and spell(rune_of_power) or buffpresent(arcane_power) and buffremaining(arcane_power) <= executetime(arcane_missiles) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and spell(arcane_orb) or buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffpresent(clearcasting_arcane) and spell(arcane_missiles) or { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or channeling(evocation) and manapercent() >= 95 and texture(inv_pet_exitbattle text=cancel) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and spell(arcane_orb) or spell(arcane_barrage) or mana() > manacost(arcane_blast) and spell(arcane_blast)
}
...
{
 buffexpires(rune_of_power_buff) and spellcooldown(arcane_power) > 0 and spell(rune_of_power) or { not spellcooldown(arcane_power) > 0 and buffexpires(rune_of_power_buff) or previousgcdspell(rune_of_power) } and spell(touch_of_the_magi) or spellcooldown(arcane_power) < 50 and buffexpires(rune_of_power_buff) and azeriteessenceisenabled(vision_of_perfection_essence_id) and spell(touch_of_the_magi) or buffpresent(arcane_power) and buffremaining(arcane_power) <= executetime(arcane_missiles) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and spell(arcane_orb) or buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffpresent(clearcasting_arcane) and spell(arcane_missiles) or { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or channeling(evocation) and manapercent() >= 95 and texture(inv_pet_exitbattle text=cancel) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and spell(arcane_orb) or spell(arcane_barrage) or mana() > manacost(arcane_blast) and spell(arcane_blast)
}
...
{
 if iscovenant(night_fae) casttime(shifting_power) / currentticktime(shifting_power) * 3
}
...
AddFunction firestandard_rotationmainactions
{
 ...
 if enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } spell(flamestrike)
 ...
 if buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } spell(pyroblast)
 ...
 if not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not fire_blast_pooling() and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } spell(fire_blast)
 ...
 if previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } spell(pyroblast)
 ...
 unless fireactive_talentsmainpostconditions()
 {
  ...
  if enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } spell(flamestrike)
  #fireball
  spell(fireball)
  #scorch
  spell(scorch)
...
{
 unless enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and spell(flamestrike) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak) and buffremaining(hot_streak) < executetime(fireball) and spell(pyroblast) or buffpresent(hot_streak) and { previousgcdspell(fireball) or talent(firestarter_talent) and target.healthpercent() >= 90 or inflighttotarget(pyroblast) } and spell(pyroblast) or buffpresent(sun_kings_blessing_ready_buff) and { spellcooldown(rune_of_power) + executetime(rune_of_power) + casttime(pyroblast) > buffremaining(sun_kings_blessing_ready_buff) or not hastalent(rune_of_power_talent) } and time_to_combustion() + casttime(pyroblast) > buffremaining(sun_kings_blessing_ready_buff) and spell(pyroblast) or buffpresent(hot_streak) and target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(pyroblast) or buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } and spell(pyroblast) or not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not fire_blast_pooling() and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } and spell(fire_blast) or previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and spell(pyroblast) or not phoenix_pooling() and { not hastalent(from_the_ashes_talent) or enemies() > 1 } and { debuffcountonany(ignite) < 2 or enemies() >= hard_cast_flamestrike() or enemies() >= hot_streak_flamestrike() } and spell(phoenix_flames)
 {
  #call_action_list,name=active_talents
...
{
 enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and spell(flamestrike) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak) and buffremaining(hot_streak) < executetime(fireball) and spell(pyroblast) or buffpresent(hot_streak) and { previousgcdspell(fireball) or talent(firestarter_talent) and target.healthpercent() >= 90 or inflighttotarget(pyroblast) } and spell(pyroblast) or buffpresent(sun_kings_blessing_ready_buff) and { spellcooldown(rune_of_power) + executetime(rune_of_power) + casttime(pyroblast) > buffremaining(sun_kings_blessing_ready_buff) or not hastalent(rune_of_power_talent) } and time_to_combustion() + casttime(pyroblast) > buffremaining(sun_kings_blessing_ready_buff) and spell(pyroblast) or buffpresent(hot_streak) and target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(pyroblast) or buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } and spell(pyroblast) or not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not fire_blast_pooling() and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } and spell(fire_blast) or previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and spell(pyroblast) or not phoenix_pooling() and { not hastalent(from_the_ashes_talent) or enemies() > 1 } and { debuffcountonany(ignite) < 2 or enemies() >= hard_cast_flamestrike() or enemies() >= hot_streak_flamestrike() } and spell(phoenix_flames) or fireactive_talentsshortcdpostconditions() or enemies() > 1 and target.distance(less 12) and spell(dragons_breath) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch) or enemies() >= arcane_explosion() and manapercent() >= arcane_explosion_mana() and spell(arcane_explosion) or enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and spell(flamestrike) or spell(fireball) or spell(scorch)
}
...
{
 enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and spell(flamestrike) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak) and buffremaining(hot_streak) < executetime(fireball) and spell(pyroblast) or buffpresent(hot_streak) and { previousgcdspell(fireball) or talent(firestarter_talent) and target.healthpercent() >= 90 or inflighttotarget(pyroblast) } and spell(pyroblast) or buffpresent(sun_kings_blessing_ready_buff) and { spellcooldown(rune_of_power) + executetime(rune_of_power) + casttime(pyroblast) > buffremaining(sun_kings_blessing_ready_buff) or not hastalent(rune_of_power_talent) } and time_to_combustion() + casttime(pyroblast) > buffremaining(sun_kings_blessing_ready_buff) and spell(pyroblast) or buffpresent(hot_streak) and target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(pyroblast) or buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } and spell(pyroblast) or not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not fire_blast_pooling() and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } and spell(fire_blast) or previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and spell(pyroblast) or not phoenix_pooling() and { not hastalent(from_the_ashes_talent) or enemies() > 1 } and { debuffcountonany(ignite) < 2 or enemies() >= hard_cast_flamestrike() or enemies() >= hot_streak_flamestrike() } and spell(phoenix_flames) or fireactive_talentscdpostconditions() or enemies() > 1 and target.distance(less 12) and spell(dragons_breath) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch) or enemies() >= arcane_explosion() and manapercent() >= arcane_explosion_mana() and spell(arcane_explosion) or enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and spell(flamestrike) or spell(fireball) or spell(scorch)
}
...
AddFunction firerop_phasemainactions
{
 ...
 if buffexpires(sun_kings_blessing_ready_buff) and not { enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { not buffpresent(heating_up) and not buffpresent(hot_streak) and not previousoffgcdspell(fire_blast) } and { charges(fire_blast) >= 2 or hastalent(alexstraszas_fury_talent) and spellcooldown(dragons_breath) == 0 or hastalent(searing_touch_talent) and target.healthpercent() <= 30 } spell(fire_blast)
 #fire_blast,use_off_gcd=1,use_while_casting=1,if=!firestarter.active&(((action.fireball.executing|action.pyroblast.executing)&buff.heating_up.react)|(talent.searing_touch.enabled&target.health.pct<=30&(buff.heating_up.react&!action.scorch.executing|!buff.hot_streak.react&!buff.heating_up.react&action.scorch.executing&!hot_streak_spells_in_flight)))
 if not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } spell(fire_blast)
 #call_action_list,name=active_talents
 fireactive_talentsmainactions()
 ...
 unless fireactive_talentsmainpostconditions()
 {
  ...
  if buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and casttime(pyroblast) < totemremaining(rune_of_power) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } spell(pyroblast)
  #pyroblast,if=prev_gcd.1.scorch&buff.heating_up.react&talent.searing_touch.enabled&target.health.pct<=30&!(active_enemies>=variable.hot_streak_flamestrike&(time-buff.combustion.last_expire>variable.delay_flamestrike|variable.disable_combustion))
  if previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } spell(pyroblast)
...
{
 unless enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak) and spell(pyroblast) or buffexpires(sun_kings_blessing_ready_buff) and not { enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { not buffpresent(heating_up) and not buffpresent(hot_streak) and not previousoffgcdspell(fire_blast) } and { charges(fire_blast) >= 2 or hastalent(alexstraszas_fury_talent) and spellcooldown(dragons_breath) == 0 or hastalent(searing_touch_talent) and target.healthpercent() <= 30 } and spell(fire_blast) or not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } and spell(fire_blast)
 {
  #call_action_list,name=active_talents
  fireactive_talentsshortcdactions()
 }
...
{
 enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak) and spell(pyroblast) or buffexpires(sun_kings_blessing_ready_buff) and not { enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { not buffpresent(heating_up) and not buffpresent(hot_streak) and not previousoffgcdspell(fire_blast) } and { charges(fire_blast) >= 2 or hastalent(alexstraszas_fury_talent) and spellcooldown(dragons_breath) == 0 or hastalent(searing_touch_talent) and target.healthpercent() <= 30 } and spell(fire_blast) or not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } and spell(fire_blast) or fireactive_talentsshortcdpostconditions() or buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and casttime(pyroblast) < totemremaining(rune_of_power) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and spell(pyroblast) or not phoenix_pooling() and buffpresent(heating_up) and not buffpresent(hot_streak) and { debuffcountonany(ignite) < 2 or enemies() >= hard_cast_flamestrike() or enemies() >= hot_streak_flamestrike() } and spell(phoenix_flames) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch) or enemies() > 2 and target.distance(less 12) and spell(dragons_breath) or enemies() >= arcane_explosion() and manapercent() >= arcane_explosion_mana() and spell(arcane_explosion) or enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and spell(flamestrike) or spell(fireball)
}
...
{
 enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak) and spell(pyroblast) or buffexpires(sun_kings_blessing_ready_buff) and not { enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { not buffpresent(heating_up) and not buffpresent(hot_streak) and not previousoffgcdspell(fire_blast) } and { charges(fire_blast) >= 2 or hastalent(alexstraszas_fury_talent) and spellcooldown(dragons_breath) == 0 or hastalent(searing_touch_talent) and target.healthpercent() <= 30 } and spell(fire_blast) or not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not inflighttotarget(hot_streak) } } and spell(fire_blast) or fireactive_talentscdpostconditions() or buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and casttime(pyroblast) < totemremaining(rune_of_power) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } } and spell(pyroblast) or not phoenix_pooling() and buffpresent(heating_up) and not buffpresent(hot_streak) and { debuffcountonany(ignite) < 2 or enemies() >= hard_cast_flamestrike() or enemies() >= hot_streak_flamestrike() } and spell(phoenix_flames) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch) or enemies() > 2 and target.distance(less 12) and spell(dragons_breath) or enemies() >= arcane_explosion() and manapercent() >= arcane_explosion_mana() and spell(arcane_explosion) or enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and spell(flamestrike) or spell(fireball)
}
...
AddFunction firecombustion_phasemainactions
{
 ...
 if hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master) or buffremaining(blaster_master) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } spell(fire_blast)
 ...
 if not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 spell(fire_blast)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(disciplinary_command) == 0 spell(arcane_explosion)
 #frostbolt,if=runeforge.disciplinary_command.equipped&buff.disciplinary_command.down&buff.disciplinary_command_frost.down
 if equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__frost_aura_dnt) spell(frostbolt)
 #call_action_list,name=active_talents
 fireactive_talentsmainactions()
 ...
 unless fireactive_talentsmainpostconditions()
 {
  ...
  if bufflastexpire(combustion) <= timesincepreviousspell(combustion) spell(berserking)
...
AddFunction firecombustion_phaseshortcdactions
{
 ...
 unless enemies() > 1 and buffexpires(combustion) and spell(living_bomb)
 {
  ...
  if buffexpires(combustion) and buffexpires(rune_of_power_buff) spell(mirrors_of_torment)

  unless spell(blood_of_the_enemy) or spell(memory_of_lucid_dreams) or spell(worldvein_resonance) or hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master) or buffremaining(blaster_master) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and not conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(disciplinary_command) == 0 and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__frost_aura_dnt) and spell(frostbolt)
  {
   #call_action_list,name=active_talents
   fireactive_talentsshortcdactions()
  }
...
{
 enemies() > 1 and buffexpires(combustion) and spell(living_bomb) or spell(blood_of_the_enemy) or spell(memory_of_lucid_dreams) or spell(worldvein_resonance) or hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master) or buffremaining(blaster_master) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and not conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(disciplinary_command) == 0 and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__frost_aura_dnt) and spell(frostbolt) or fireactive_talentsshortcdpostconditions() or bufflastexpire(combustion) <= timesincepreviousspell(combustion) and spell(berserking) or { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and enemies() >= combustion_flamestrike() and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and { buffremaining(combustion) > casttime(pyroblast) or buffexpires(combustion) } and enemies() < combustion_flamestrike() and spell(pyroblast) or buffpresent(hot_streak) and buffpresent(combustion) and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up) and enemies() < combustion_flamestrike() and spell(pyroblast) or buffpresent(combustion) and { charges(fire_blast) < 1 and hastalent(pyroclasm_talent) and enemies() == 1 or not hastalent(pyroclasm_talent) or enemies() > 1 } and spell(phoenix_flames) or buffexpires(combustion) and spellcooldown(combustion) < casttime(fireball) and not conduit(flame_accretion_conduit) and spell(fireball) or { buffremaining(combustion) > casttime(scorch) and buffpresent(combustion) or buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) } and spell(scorch) or buffremaining(combustion) < gcd() and enemies() > 1 and spell(living_bomb) or buffremaining(combustion) < gcd() and buffpresent(combustion) and target.distance(less 12) and spell(dragons_breath) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch)
}
...
AddFunction firecombustion_phasecdactions
{
 ...
 unless buffexpires(combustion) and spell(bag_of_tricks) or enemies() > 1 and buffexpires(combustion) and spell(living_bomb) or buffexpires(combustion) and buffexpires(rune_of_power_buff) and spell(mirrors_of_torment)
 {
  ...
  unless spell(blood_of_the_enemy) or spell(memory_of_lucid_dreams) or spell(worldvein_resonance) or hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master) or buffremaining(blaster_master) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and not conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast)
  {
   ...
   if equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(disciplinary_command) == 0 fireinterruptactions()

   unless equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(disciplinary_command) == 0 and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__frost_aura_dnt) and spell(frostbolt)
   {
    #call_action_list,name=active_talents
    fireactive_talentscdactions()
    ...
    unless fireactive_talentscdpostconditions()
    {
     ...
     if buffexpires(combustion) and equippedruneforge(disciplinary_command_runeforge) == buffpresent(disciplinary_command) and { inflighttotarget(meteor) and 0 <= 0.5 or executetime(scorch) > 0 and executetime(scorch) < 0.5 or executetime(fireball) > 0 and executetime(fireball) < 0.5 or executetime(pyroblast) > 0 and executetime(pyroblast) < 0.5 } spell(combustion)
     #potion,if=buff.combustion.last_expire<=action.combustion.last_used
     if bufflastexpire(combustion) <= timesincepreviousspell(combustion) and { checkboxon(opt_use_consumables) and target.classification(worldboss) } item(superior_battle_potion_of_intellect_item usable=1)
     #blood_fury,if=buff.combustion.last_expire<=action.combustion.last_used
     if bufflastexpire(combustion) <= timesincepreviousspell(combustion) spell(blood_fury)

     unless bufflastexpire(combustion) <= timesincepreviousspell(combustion) and spell(berserking)
     {
      #fireblood,if=buff.combustion.last_expire<=action.combustion.last_used
      if bufflastexpire(combustion) <= timesincepreviousspell(combustion) spell(fireblood)
      #ancestral_call,if=buff.combustion.last_expire<=action.combustion.last_used
      if bufflastexpire(combustion) <= timesincepreviousspell(combustion) spell(ancestral_call)
      #use_items,if=buff.combustion.last_expire<=action.combustion.last_used
      if bufflastexpire(combustion) <= timesincepreviousspell(combustion) fireuseitemactions()
      #time_warp,if=runeforge.temporal_warp.equipped&buff.combustion.last_expire<=action.combustion.last_used&buff.exhaustion.up
      if equippedruneforge(temporal_warp_runeforge) and bufflastexpire(combustion) <= timesincepreviousspell(combustion) and buffpresent(exhaustion) and { checkboxon(opt_time_warp) and debuffexpires(burst_haste_debuff any=1) } spell(time_warp)
     }
    }
   }
  }
...
{
 buffexpires(combustion) and spell(bag_of_tricks) or enemies() > 1 and buffexpires(combustion) and spell(living_bomb) or buffexpires(combustion) and buffexpires(rune_of_power_buff) and spell(mirrors_of_torment) or spell(blood_of_the_enemy) or spell(memory_of_lucid_dreams) or spell(worldvein_resonance) or hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master) or buffremaining(blaster_master) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and not conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and inflighttotarget(hot_streak) + buffstacks(heating_up) < 2 and spell(fire_blast) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(disciplinary_command) == 0 and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command__frost_aura_dnt) and spell(frostbolt) or fireactive_talentscdpostconditions() or bufflastexpire(combustion) <= timesincepreviousspell(combustion) and spell(berserking) or { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and enemies() >= combustion_flamestrike() and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and { buffremaining(combustion) > casttime(pyroblast) or buffexpires(combustion) } and enemies() < combustion_flamestrike() and spell(pyroblast) or buffpresent(hot_streak) and buffpresent(combustion) and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up) and enemies() < combustion_flamestrike() and spell(pyroblast) or buffpresent(combustion) and { charges(fire_blast) < 1 and hastalent(pyroclasm_talent) and enemies() == 1 or not hastalent(pyroclasm_talent) or enemies() > 1 } and spell(phoenix_flames) or buffexpires(combustion) and spellcooldown(combustion) < casttime(fireball) and not conduit(flame_accretion_conduit) and spell(fireball) or { buffremaining(combustion) > casttime(scorch) and buffpresent(combustion) or buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) } and spell(scorch) or buffremaining(combustion) < gcd() and enemies() > 1 and spell(living_bomb) or buffremaining(combustion) < gcd() and buffpresent(combustion) and target.distance(less 12) and spell(dragons_breath) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch)
}
...
{
 enemies() > 1 and buffexpires(combustion) and { time_to_combustion() > spellcooldownduration(living_bomb) or time_to_combustion() <= 0 or disable_combustion() } and spell(living_bomb) or hastalent(alexstraszas_fury_talent) and buffexpires(combustion) and not buffpresent(hot_streak) and target.distance(less 12) and spell(dragons_breath)
}
...
{
 enemies() > 1 and buffexpires(combustion) and { time_to_combustion() > spellcooldownduration(living_bomb) or time_to_combustion() <= 0 or disable_combustion() } and spell(living_bomb) or { not disable_combustion() and time_to_combustion() <= 0 or spellcooldownduration(meteor) < time_to_combustion() and not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and buffpresent(rune_of_power_buff) and time_to_combustion() > spellcooldown(meteor) or fightremains() < time_to_combustion() or disable_combustion() } and spell(meteor) or hastalent(alexstraszas_fury_talent) and buffexpires(combustion) and not buffpresent(hot_streak) and target.distance(less 12) and spell(dragons_breath)
}
...
AddFunction fire_defaultmainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) spell(arcane_explosion)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) spell(frostbolt)
 ...
 if buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } spell(rune_of_power)
 ...
 unless not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phasemainpostconditions()
 {
  ...
  unless buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phasemainpostconditions()
  {
   ...
   if not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) spell(fire_blast)
...
AddFunction fire_defaultshortcdactions
{
 ...
 if buffexpires(combustion) and buffexpires(rune_of_power_buff) and spellcooldown(combustion) > 0 and { spellcooldown(rune_of_power) > 0 or not hastalent(rune_of_power_talent) } spell(shifting_power)
 ...
 if buffexpires(combustion) and buffexpires(rune_of_power_buff) and { spellcooldown(combustion) < executetime(radiant_spark) or spellcooldown(combustion) > spellcooldownduration(radiant_spark) } or buffpresent(rune_of_power_buff) and spellcooldown(combustion) > 30 spell(radiant_spark)
 ...
 unless spell(concentrated_flame)
 {
  ...
  unless spell(ripple_in_space) or spell(the_unbound_force) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(frostbolt) or buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } and spell(rune_of_power)
  {
   #call_action_list,name=combustion_phase,if=!variable.disable_combustion&variable.time_to_combustion<=0
   ...
   unless not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phaseshortcdpostconditions()
   {
    ...
    unless buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phaseshortcdpostconditions() or not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) and spell(fire_blast) or talent(firestarter_talent) and target.healthpercent() >= 90 and charges(fire_blast) >= 1 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(fireball) > 0 and not buffpresent(hot_streak) or inflighttotarget(pyroblast) and buffpresent(heating_up) and not buffpresent(hot_streak) } and spell(fire_blast)
    {
     #call_action_list,name=standard_rotation,if=(variable.time_to_combustion>0|variable.disable_combustion)&buff.rune_of_power.down
...
{
 spell(concentrated_flame) or spell(ripple_in_space) or spell(the_unbound_force) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(frostbolt) or buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } and spell(rune_of_power) or not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phaseshortcdpostconditions() or buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phaseshortcdpostconditions() or not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) and spell(fire_blast) or talent(firestarter_talent) and target.healthpercent() >= 90 and charges(fire_blast) >= 1 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(fireball) > 0 and not buffpresent(hot_streak) or inflighttotarget(pyroblast) and buffpresent(heating_up) and not buffpresent(hot_streak) } and spell(fire_blast) or { time_to_combustion() > 0 or disable_combustion() } and buffexpires(rune_of_power_buff) and firestandard_rotationshortcdpostconditions()
}

AddFunction fire_defaultcdactions
{
 #counterspell,if=!runeforge.disciplinary_command.equipped
 if not equippedruneforge(disciplinary_command_runeforge) fireinterruptactions()

 unless buffexpires(combustion) and buffexpires(rune_of_power_buff) and spellcooldown(combustion) > 0 and { spellcooldown(rune_of_power) > 0 or not hastalent(rune_of_power_talent) } and spell(shifting_power) or { buffexpires(combustion) and buffexpires(rune_of_power_buff) and { spellcooldown(combustion) < executetime(radiant_spark) or spellcooldown(combustion) > spellcooldownduration(radiant_spark) } or buffpresent(rune_of_power_buff) and spellcooldown(combustion) > 30 } and spell(radiant_spark)
 {
  #deathborne,if=buff.combustion.down&buff.rune_of_power.down&cooldown.combustion.remains<execute_time
  if buffexpires(combustion) and buffexpires(rune_of_power_buff) and spellcooldown(combustion) < executetime(deathborne) spell(deathborne)
  ...
  unless spell(concentrated_flame) or spell(reaping_flames) or spell(focused_azerite_beam) or spell(purifying_blast) or spell(ripple_in_space) or spell(the_unbound_force)
  {
   ...
   if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) fireinterruptactions()
   ...
   unless equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(frostbolt) or buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } and spell(rune_of_power)
   {
    ...
    unless not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phasecdpostconditions()
    {
     ...
     unless buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phasecdpostconditions() or not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) and spell(fire_blast) or talent(firestarter_talent) and target.healthpercent() >= 90 and charges(fire_blast) >= 1 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(fireball) > 0 and not buffpresent(hot_streak) or inflighttotarget(pyroblast) and buffpresent(heating_up) and not buffpresent(hot_streak) } and spell(fire_blast)
     {
      #call_action_list,name=standard_rotation,if=(variable.time_to_combustion>0|variable.disable_combustion)&buff.rune_of_power.down
...
{
 buffexpires(combustion) and buffexpires(rune_of_power_buff) and spellcooldown(combustion) > 0 and { spellcooldown(rune_of_power) > 0 or not hastalent(rune_of_power_talent) } and spell(shifting_power) or { buffexpires(combustion) and buffexpires(rune_of_power_buff) and { spellcooldown(combustion) < executetime(radiant_spark) or spellcooldown(combustion) > spellcooldownduration(radiant_spark) } or buffpresent(rune_of_power_buff) and spellcooldown(combustion) > 30 } and spell(radiant_spark) or spell(concentrated_flame) or spell(reaping_flames) or spell(focused_azerite_beam) or spell(purifying_blast) or spell(ripple_in_space) or spell(the_unbound_force) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__frost_aura_dnt) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(frostbolt) or buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } and spell(rune_of_power) or not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phasecdpostconditions() or buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phasecdpostconditions() or not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - bufflastexpire(combustion) > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) and spell(fire_blast) or talent(firestarter_talent) and target.healthpercent() >= 90 and charges(fire_blast) >= 1 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(fireball) > 0 and not buffpresent(hot_streak) or inflighttotarget(pyroblast) and buffpresent(heating_up) and not buffpresent(hot_streak) } and spell(fire_blast) or { time_to_combustion() > 0 or disable_combustion() } and buffexpires(rune_of_power_buff) and firestandard_rotationcdpostconditions()
}
...
AddFunction froststmainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) spell(arcane_explosion)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) spell(fire_blast)
 #glacial_spike,if=buff.brain_freeze.react
 if buffpresent(brain_freeze_frost) spell(glacial_spike)
 #frostbolt
 spell(frostbolt)
}
...
AddFunction froststshortcdactions
{
 unless { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and { previousgcdspell(radiant_spark) or previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) or { target.debuffpresent(mirrors_of_torment) or buffpresent(expanded_potential_buff) or buffpresent(freezing_winds) } and buffstacks(fingers_of_frost) == 0 } } and spell(flurry)
 {
  ...
  unless { buffpresent(freezing_rain_buff) or enemies() >= 3 or enemies() >= 2 and not equippedruneforge(cold_front_runeforge) } and spell(blizzard)
  {
   ...
   unless buffstacks(winters_chill) and target.debuffremaining(winters_chill) > casttime(glacial_spike) + traveltime(glacial_spike) and spell(glacial_spike) or buffstacks(winters_chill) and buffstacks(winters_chill) > buffstacks(fingers_of_frost) and target.debuffremaining(winters_chill) > traveltime(ice_lance) and spell(ice_lance)
   {
    ...
    unless spell(ice_nova)
    {
     ...
     unless { buffpresent(fingers_of_frost) or target.debuffremaining(frozen) > traveltime(ice_lance) } and spell(ice_lance)
     {
      ...
      if { not equippedruneforge(freezing_winds_runeforge) or enemies() >= 2 } and { buffpresent(brain_freeze_frost) or enabledsoulbind(combat_meditation_soulbind) } spell(radiant_spark)
      ...
      if timesincepreviousspell(shifting_power) > 60 and { { enabledsoulbind(field_of_blossoms_soulbind) or enabledsoulbind(grove_invigoration_soulbind) } and { not hastalent(rune_of_power_talent) or buffexpires(rune_of_power_buff) and spellcooldown(rune_of_power) > 16 } } spell(shifting_power)
      #mirrors_of_torment
      spell(mirrors_of_torment)
      #frost_nova,if=runeforge.grisly_icicle.equipped&target.level<=level&debuff.frozen.down
      if equippedruneforge(grisly_icicle_runeforge) and level() <= level() and target.debuffexpires(frozen) spell(frost_nova)
     }
    }
   }
  }
...
{
 { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and { previousgcdspell(radiant_spark) or previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) or { target.debuffpresent(mirrors_of_torment) or buffpresent(expanded_potential_buff) or buffpresent(freezing_winds) } and buffstacks(fingers_of_frost) == 0 } } and spell(flurry) or { buffpresent(freezing_rain_buff) or enemies() >= 3 or enemies() >= 2 and not equippedruneforge(cold_front_runeforge) } and spell(blizzard) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > casttime(glacial_spike) + traveltime(glacial_spike) and spell(glacial_spike) or buffstacks(winters_chill) and buffstacks(winters_chill) > buffstacks(fingers_of_frost) and target.debuffremaining(winters_chill) > traveltime(ice_lance) and spell(ice_lance) or spell(ice_nova) or { buffpresent(fingers_of_frost) or target.debuffremaining(frozen) > traveltime(ice_lance) } and spell(ice_lance) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__arcane_aura_dnt) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and spell(fire_blast) or buffpresent(brain_freeze_frost) and spell(glacial_spike) or spell(frostbolt)
}
...
AddFunction frostcdsmainactions
{
 ...
 if spellcooldown(icy_veins) > 15 and buffexpires(rune_of_power_buff) spell(rune_of_power)
 #berserking
 spell(berserking)
}
...
AddFunction frostcdsshortcdactions
{
 ...
 if enabledsoulbind(wasteland_propriety_soulbind) spell(mirrors_of_torment)

 unless spellcooldown(icy_veins) > 15 and buffexpires(rune_of_power_buff) and spell(rune_of_power) or spell(berserking)
 {
  #bag_of_tricks
  spell(bag_of_tricks)
 }
...
{
 spellcooldown(icy_veins) > 15 and buffexpires(rune_of_power_buff) and spell(rune_of_power) or spell(berserking)
}
...
AddFunction frostcdscdactions
{
 ...
 unless enabledsoulbind(wasteland_propriety_soulbind) and spell(mirrors_of_torment)
 {
  #deathborne
  spell(deathborne)

  unless spellcooldown(icy_veins) > 15 and buffexpires(rune_of_power_buff) and spell(rune_of_power)
  {
...
{
 enabledsoulbind(wasteland_propriety_soulbind) and spell(mirrors_of_torment) or spellcooldown(icy_veins) > 15 and buffexpires(rune_of_power_buff) and spell(rune_of_power) or spell(berserking) or spell(bag_of_tricks)
}
...
AddFunction frostaoemainactions
{
 ...
 if { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and buffstacks(fingers_of_frost) == 0 } spell(flurry)
 #ice_nova
 spell(ice_nova)
...
AddFunction frostaoeshortcdactions
{
 ...
 unless spell(blizzard) or { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and buffstacks(fingers_of_frost) == 0 } and spell(flurry) or spell(ice_nova)
 {
  #comet_storm
  spell(comet_storm)
  ...
  unless { buffpresent(fingers_of_frost) or target.debuffremaining(frozen) > traveltime(ice_lance) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > traveltime(ice_lance) } and spell(ice_lance)
  {
   ...
   unless equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and spell(fire_blast) or manapercent() > 30 and not equippedruneforge(cold_front_runeforge) and { not equippedruneforge(freezing_winds_runeforge) or buffpresent(freezing_winds) } and spell(arcane_explosion)
   {
    #ebonbolt
    spell(ebonbolt)
   }
...
{
 spell(blizzard) or { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and buffstacks(fingers_of_frost) == 0 } and spell(flurry) or spell(ice_nova) or { buffpresent(fingers_of_frost) or target.debuffremaining(frozen) > traveltime(ice_lance) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > traveltime(ice_lance) } and spell(ice_lance) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and spell(fire_blast) or manapercent() > 30 and not equippedruneforge(cold_front_runeforge) and { not equippedruneforge(freezing_winds_runeforge) or buffpresent(freezing_winds) } and spell(arcane_explosion) or equippedruneforge(glacial_fragments_runeforge) and hastalent(splitting_ice_talent) and spell(ice_lance) or spell(frostbolt)
}
...
{
 spell(frozen_orb) or spell(blizzard) or { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_frost) and buffstacks(fingers_of_frost) == 0 } and spell(flurry) or spell(ice_nova) or spell(comet_storm) or { buffpresent(fingers_of_frost) or target.debuffremaining(frozen) > traveltime(ice_lance) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > traveltime(ice_lance) } and spell(ice_lance) or spell(radiant_spark) or spell(shifting_power) or spell(mirrors_of_torment) or equippedruneforge(grisly_icicle_runeforge) and level() <= level() and target.debuffexpires(frozen) and spell(frost_nova) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) == 0 and buffexpires(disciplinary_command__fire_aura_dnt) and spell(fire_blast) or manapercent() > 30 and not equippedruneforge(cold_front_runeforge) and { not equippedruneforge(freezing_winds_runeforge) or buffpresent(freezing_winds) } and spell(arcane_explosion) or spell(ebonbolt) or equippedruneforge(glacial_fragments_runeforge) and hastalent(splitting_ice_talent) and spell(ice_lance) or spell(frostbolt)
}
