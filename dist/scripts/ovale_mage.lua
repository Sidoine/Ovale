...
AddFunction totm_max_delay
{
 ...
 if message("variable.totm_max_delay is not implemented") == 5 and conduit(arcane_prodigy_conduit) and enemies() < 3 15
...
{
 { hastalent(enlightened_talent) and manapercent() <= 80 and manapercent() >= 65 or not hastalent(enlightened_talent) and manapercent() <= 85 } and spell(use_mana_gem) or buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and spell(bag_of_tricks) or buffpresent(arcane_power) and spell(berserking)
}
...
AddFunction arcanerotationmainactions
{
 ...
 if target.debuffstacks(radiant_spark_vulnerability) == spelldata(radiant_spark_vulnerability max_stacks) and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) spell(strict_sequence)
...
{
 equippedruneforge(disciplinary_command_runeforge) and buffpresent(disciplinary_command_frost_buff) and spell(fire_blast) or equippedruneforge(grisly_icicle_runeforge) and manapercent() > 95 and spell(frost_nova) or spell(mirrors_of_torment) or manapercent() > 40 and spell(radiant_spark) or message("action.shifting_power.channeling is not implemented") and not gcdremaining() > 0 and spell(cancel_action) or message("soulbind.field_of_blossoms.enabled is not implemented") and spell(shifting_power) or spell(touch_of_the_magi) or buffexpires(rune_of_power_buff) and spell(rune_of_power) or spell(presence_of_mind) or { target.debuffremaining(radiant_spark) > 5 or target.debuffstacks(radiant_spark_vulnerability) > 0 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(presence_of_mind) and target.debuffpresent(touch_of_the_magi) and target.debuffremaining(touch_of_the_magi) <= executetime(arcane_blast) and mana() > manacost(arcane_blast) and spell(arcane_blast) or buffpresent(arcane_power) and buffremaining(arcane_power) <= gcd() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or target.debuffpresent(touch_of_the_magi) and hastalent(arcane_echo_talent) and buffexpires(deathborne) and target.debuffremaining(touch_of_the_magi) > executetime(arcane_missiles) and { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or buffpresent(clearcasting_buff) and spell(arcane_missiles) or buffstacks(arcane_charge_buff) <= totm_max_charges() and { spellcooldown(arcane_power) > 10 or enemies() <= 2 } and spell(arcane_orb) or { buffpresent(rune_of_power_buff) or manapercent() > 15 } and mana() > manacost(arcane_blast) and spell(arcane_blast) or spell(arcane_barrage)
}
...
AddFunction arcanecooldownsmainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } spell(frostbolt)
...
AddFunction arcanecooldownsshortcdactions
{
 ...
 unless equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) and previousgcdspell(frostbolt) and spell(fire_blast)
 {
...
{
 equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) and previousgcdspell(frostbolt) and spell(fire_blast) or buffexpires(rune_of_power_buff) and spellcooldown(touch_of_the_magi) > rop_max_delay() and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and { spellcooldown(arcane_power) > 15 or target.debuffpresent(touch_of_the_magi) } and spell(rune_of_power)
}
...
AddFunction arcaneaoemainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } spell(frostbolt)
...
{
 unless equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and buffexpires(arcane_power) and buffexpires(rune_of_power_buff) and target.debuffexpires(touch_of_the_magi) and not spellcooldown(touch_of_the_magi) > 0 and buffstacks(arcane_charge_buff) <= aoe_totm_max_charges() and { hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) <= gcd() and spellcooldown(arcane_power) > totm_max_delay() or not hastalent(rune_of_power_talent) and spellcooldown(arcane_power) > totm_max_delay() or spellcooldown(arcane_power) <= gcd() } and spell(frostbolt) or { equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) and previousgcdspell(frostbolt) or equippedruneforge(disciplinary_command_runeforge) and timeincombat() == 0 } and spell(fire_blast)
 {
...
AddFunction arcaneam_spamcdactions
{
 unless buffexpires(rune_of_power_buff) and spellcooldown(arcane_power) > 0 and spell(rune_of_power) or { not spellcooldown(arcane_power) > 0 and buffexpires(rune_of_power_buff) or previousgcdspell(rune_of_power) } and spell(touch_of_the_magi) or spellcooldown(arcane_power) < 50 and buffexpires(rune_of_power_buff) and azeriteessenceisenabled(vision_of_perfection_essence_id) and spell(touch_of_the_magi)
 {
  ...
  unless buffpresent(arcane_power) and buffremaining(arcane_power) <= executetime(arcane_missiles) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffstacks(arcane_charge_buff) < spelldata(arcane_charge_buff max_stacks) and buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and spell(arcane_orb) or buffexpires(rune_of_power_buff) and buffexpires(arcane_power) and target.debuffexpires(touch_of_the_magi) and buffstacks(arcane_charge_buff) == spelldata(arcane_charge_buff max_stacks) and spell(arcane_barrage) or buffpresent(clearcasting_buff) and spell(arcane_missiles) or { not hasazeritetrait(arcane_pummeling_trait) or buffexpires(clearcasting_channel_buff) } and spell(arcane_missiles) or message("action.evocation.channeling is not implemented") and manapercent() >= 95 and spell(cancel_action)
  {
   #evocation
   spell(evocation)
  }
...
{
 enemies() >= hot_streak_flamestrike() and { timeincombat() - message("buff.combustion.last_expire is not implemented") > delay_flamestrike() or disable_combustion() } and { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak) and spell(pyroblast) or buffexpires(sun_kings_blessing_ready_buff) and not { enemies() >= hard_cast_flamestrike() and { timeincombat() - message("buff.combustion.last_expire is not implemented") > delay_flamestrike() or disable_combustion() } } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(heating_up) and not buffpresent(hot_streak) and not previousoffgcdspell(fire_blast) and { charges(fire_blast) >= 2 or hastalent(alexstraszas_fury_talent) and spellcooldown(dragons_breath) == 0 or hastalent(searing_touch_talent) and target.healthpercent() <= 30 } and spell(fire_blast) or not { talent(firestarter_talent) and target.healthpercent() >= 90 } and { { executetime(fireball) > 0 or executetime(pyroblast) > 0 } and buffpresent(heating_up) or hastalent(searing_touch_talent) and target.healthpercent() <= 30 and { buffpresent(heating_up) and not executetime(scorch) > 0 or not buffpresent(hot_streak) and not buffpresent(heating_up) and executetime(scorch) > 0 and not message("hot_streak_spells_in_flight is not implemented") } } and spell(fire_blast) or fireactive_talentscdpostconditions() or buffpresent(pyroclasm_buff) and casttime(pyroblast) < buffremaining(pyroclasm_buff) and casttime(pyroblast) < totemremaining(rune_of_power) and { buffstacks(pyroclasm_buff) == spelldata(pyroclasm_buff max_stacks) or buffremaining(pyroclasm_buff) < casttime(pyroblast) + executetime(fireball) or buffpresent(alexstraszas_fury_buff) or not equippedruneforge(sun_kings_blessing_runeforge) } and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up) and hastalent(searing_touch_talent) and target.healthpercent() <= 30 and not { enemies() >= hot_streak_flamestrike() and { timeincombat() - message("buff.combustion.last_expire is not implemented") > delay_flamestrike() or disable_combustion() } } and spell(pyroblast) or not phoenix_pooling() and buffpresent(heating_up) and not buffpresent(hot_streak) and { debuffcountonany(ignite) < 2 or enemies() >= hard_cast_flamestrike() or enemies() >= hot_streak_flamestrike() } and spell(phoenix_flames) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch) or enemies() > 2 and target.distance(less 12) and spell(dragons_breath) or enemies() >= arcane_explosion() and manapercent() >= arcane_explosion_mana() and spell(arcane_explosion) or enemies() >= hard_cast_flamestrike() and { timeincombat() - message("buff.combustion.last_expire is not implemented") > delay_flamestrike() or disable_combustion() } and spell(flamestrike) or spell(fireball)
}
...
AddFunction firecombustion_phasemainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(buff_disciplinary_command) == 0 spell(arcane_explosion)
 #frostbolt,if=runeforge.disciplinary_command.equipped&buff.disciplinary_command.down&buff.disciplinary_command_frost.down
 if equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command_frost_buff) spell(frostbolt)
 #call_action_list,name=active_talents
 fireactive_talentsmainactions()
...
AddFunction firecombustion_phasecdactions
{
 ...
 unless buffexpires(combustion) and spell(bag_of_tricks) or enemies() > 1 and buffexpires(combustion) and spell(living_bomb) or buffexpires(combustion) and buffexpires(rune_of_power_buff) and spell(mirrors_of_torment)
 {
  ...
  unless spell(blood_of_the_enemy) or spell(memory_of_lucid_dreams) or spell(worldvein_resonance) or hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master_buff) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master_buff) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master_buff) or buffremaining(blaster_master_buff) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps_300142) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and message("hot_streak_spells_in_flight is not implemented") + buffstacks(heating_up) < 2 and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and not conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and message("hot_streak_spells_in_flight is not implemented") + buffstacks(heating_up) < 2 and spell(fire_blast)
  {
   ...
   if equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(buff_disciplinary_command) == 0 fireinterruptactions()

   unless equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(buff_disciplinary_command) == 0 and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command_frost_buff) and spell(frostbolt)
   {
    #call_action_list,name=active_talents
    fireactive_talentscdactions()
...
{
 buffexpires(combustion) and spell(bag_of_tricks) or enemies() > 1 and buffexpires(combustion) and spell(living_bomb) or buffexpires(combustion) and buffexpires(rune_of_power_buff) and spell(mirrors_of_torment) or spell(blood_of_the_enemy) or spell(memory_of_lucid_dreams) or spell(worldvein_resonance) or hasazeritetrait(blaster_master_trait) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { buffremaining(combustion) - baseduration(blaster_master_buff) } / spellcooldownduration(fire_blast) - buffremaining(combustion) / { baseduration(blaster_master_buff) - 0.5 } >= 0 or not hasazeritetrait(blaster_master_trait) or not hastalent(flame_on_talent) or buffremaining(combustion) <= baseduration(blaster_master_buff) or buffremaining(blaster_master_buff) < 0.5 or hasequippeditem(hyperthread_wristwraps_item) and spellcooldown(hyperthread_wristwraps_300142) < 5 } and buffpresent(combustion) and { not executetime(scorch) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(scorch) > 0 and buffexpires(hot_streak) and { buffexpires(heating_up) or hasazeritetrait(blaster_master_trait) } or hasazeritetrait(blaster_master_trait) and hastalent(flame_on_talent) and inflighttotarget(pyroblast) and buffexpires(heating_up) and buffexpires(hot_streak) } and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and { charges(fire_blast count=0) + { extended_combustion_remains() - baseduration(infernal_cascade) } / spellcooldownduration(fire_blast) - extended_combustion_remains() / { baseduration(infernal_cascade) - 0.5 } >= 0 or extended_combustion_remains() <= baseduration(infernal_cascade) or buffremaining(infernal_cascade) < 0.5 } and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and message("hot_streak_spells_in_flight is not implemented") + buffstacks(heating_up) < 2 and spell(fire_blast) or not hasazeritetrait(blaster_master_trait) and { enemies() <= debuffcountonany(ignite) or not spellcooldown(phoenix_flames) == 0 } and not conduit(infernal_cascade_conduit) and charges(fire_blast) >= 1 and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak) and message("hot_streak_spells_in_flight is not implemented") + buffstacks(heating_up) < 2 and spell(fire_blast) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(buff_disciplinary_command) == 0 and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and buffexpires(disciplinary_command) and buffexpires(disciplinary_command_frost_buff) and spell(frostbolt) or fireactive_talentscdpostconditions() or message("buff.combustion.last_expire is not implemented") <= message("action.combustion.last_used is not implemented") and spell(berserking) or { buffpresent(hot_streak) or buffpresent(firestorm_buff) } and enemies() >= combustion_flamestrike() and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and { buffremaining(combustion) > casttime(pyroblast) or buffexpires(combustion) } and enemies() < combustion_flamestrike() and spell(pyroblast) or buffpresent(hot_streak) and buffpresent(combustion) and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up) and enemies() < combustion_flamestrike() and spell(pyroblast) or buffpresent(combustion) and { charges(fire_blast) < 1 and hastalent(pyroclasm_talent) and enemies() == 1 or not hastalent(pyroclasm_talent) or enemies() > 1 } and spell(phoenix_flames) or buffexpires(combustion) and spellcooldown(combustion) < casttime(fireball) and not conduit(flame_accretion_conduit) and spell(fireball) or { buffremaining(combustion) > casttime(scorch) and buffpresent(combustion) or buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) } and spell(scorch) or buffremaining(combustion) < gcd() and enemies() > 1 and spell(living_bomb) or buffremaining(combustion) < gcd() and buffpresent(combustion) and target.distance(less 12) and spell(dragons_breath) or target.healthpercent() <= 30 and hastalent(searing_touch_talent) and spell(scorch)
}
...
AddFunction fire_defaultmainactions
{
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) spell(arcane_explosion)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) spell(frostbolt)
...
AddFunction fire_defaultshortcdactions
{
 ...
 unless spell(concentrated_flame)
 {
  ...
  unless spell(ripple_in_space) or spell(the_unbound_force) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(frostbolt) or buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } and spell(rune_of_power)
  {
   #call_action_list,name=combustion_phase,if=!variable.disable_combustion&variable.time_to_combustion<=0
   ...
   unless not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phaseshortcdpostconditions()
   {
    ...
    unless buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phaseshortcdpostconditions() or not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - message("buff.combustion.last_expire is not implemented") > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) and spell(fire_blast) or talent(firestarter_talent) and target.healthpercent() >= 90 and charges(fire_blast) >= 1 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(fireball) > 0 and not buffpresent(hot_streak) or inflighttotarget(pyroblast) and buffpresent(heating_up) and not buffpresent(hot_streak) } and spell(fire_blast)
    {
     #call_action_list,name=standard_rotation,if=(variable.time_to_combustion>0|variable.disable_combustion)&buff.rune_of_power.down
...
{
 spell(concentrated_flame) or spell(ripple_in_space) or spell(the_unbound_force) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(frostbolt) or buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } and spell(rune_of_power) or not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phaseshortcdpostconditions() or buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phaseshortcdpostconditions() or not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - message("buff.combustion.last_expire is not implemented") > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) and spell(fire_blast) or talent(firestarter_talent) and target.healthpercent() >= 90 and charges(fire_blast) >= 1 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(fireball) > 0 and not buffpresent(hot_streak) or inflighttotarget(pyroblast) and buffpresent(heating_up) and not buffpresent(hot_streak) } and spell(fire_blast) or { time_to_combustion() > 0 or disable_combustion() } and buffexpires(rune_of_power_buff) and firestandard_rotationshortcdpostconditions()
}
...
AddFunction fire_defaultcdactions
{
 ...
 unless buffexpires(combustion) and buffexpires(rune_of_power_buff) and spellcooldown(combustion) > 0 and { spellcooldown(rune_of_power) > 0 or not hastalent(rune_of_power_talent) } and spell(shifting_power) or { buffexpires(combustion) and buffexpires(rune_of_power_buff) and { spellcooldown(combustion) < executetime(radiant_spark) or spellcooldown(combustion) > spellcooldownduration(radiant_spark) } or buffpresent(rune_of_power_buff) and spellcooldown(combustion) > 30 } and spell(radiant_spark)
 {
  ...
  unless spell(concentrated_flame) or spell(reaping_flames) or spell(focused_azerite_beam) or spell(purifying_blast) or spell(ripple_in_space) or spell(the_unbound_force)
  {
   ...
   if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) fireinterruptactions()
...
{
 buffexpires(combustion) and buffexpires(rune_of_power_buff) and spellcooldown(combustion) > 0 and { spellcooldown(rune_of_power) > 0 or not hastalent(rune_of_power_talent) } and spell(shifting_power) or { buffexpires(combustion) and buffexpires(rune_of_power_buff) and { spellcooldown(combustion) < executetime(radiant_spark) or spellcooldown(combustion) > spellcooldownduration(radiant_spark) } or buffpresent(rune_of_power_buff) and spellcooldown(combustion) > 30 } and spell(radiant_spark) or spell(concentrated_flame) or spell(reaping_flames) or spell(focused_azerite_beam) or spell(purifying_blast) or spell(ripple_in_space) or spell(the_unbound_force) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_frost_buff) and spellcooldown(combustion) > 30 and not buffpresent(disciplinary_command) and spell(frostbolt) or buffexpires(rune_of_power_buff) and { time_to_combustion() > baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast) or time_to_combustion() > fightremains() or disable_combustion() } and spell(rune_of_power) or not disable_combustion() and time_to_combustion() <= 0 and firecombustion_phasecdpostconditions() or buffpresent(rune_of_power_buff) and { time_to_combustion() > 0 or disable_combustion() } and firerop_phasecdpostconditions() or not fire_blast_pooling() and { time_to_combustion() > 0 or disable_combustion() } and enemies() >= hard_cast_flamestrike() and { timeincombat() - message("buff.combustion.last_expire is not implemented") > delay_flamestrike() or disable_combustion() } and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak) and spell(fire_blast) or talent(firestarter_talent) and target.healthpercent() >= 90 and charges(fire_blast) >= 1 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up) or executetime(fireball) > 0 and not buffpresent(hot_streak) or inflighttotarget(pyroblast) and buffpresent(heating_up) and not buffpresent(hot_streak) } and spell(fire_blast) or { time_to_combustion() > 0 or disable_combustion() } and buffexpires(rune_of_power_buff) and firestandard_rotationcdpostconditions()
}
...
AddFunction froststmainactions
{
 ...
 if message("remaining_winters_chill is not implemented") and message("remaining_winters_chill is not implemented") > buffstacks(fingers_of_frost_buff) and target.debuffremaining(winters_chill) > traveltime(ice_lance) spell(ice_lance)
 #comet_storm
 spell(comet_storm)
 #ice_nova
 spell(ice_nova)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) spell(arcane_explosion)
 ...
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) spell(fire_blast)
...
AddFunction froststshortcdactions
{
 unless { message("remaining_winters_chill is not implemented") == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_buff) and { previousgcdspell(radiant_spark) or previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) or { target.debuffpresent(mirrors_of_torment) or buffpresent(expanded_potential_buff) or buffpresent(freezing_winds) } and buffstacks(fingers_of_frost_buff) == 0 } } and spell(flurry)
 {
  ...
  unless { buffpresent(freezing_rain_buff) or enemies() >= 3 or enemies() >= 2 and not equippedruneforge(cold_front_runeforge) } and spell(blizzard) or message("remaining_winters_chill is not implemented") == 1 and target.debuffpresent(winters_chill) and spell(ray_of_frost) or message("remaining_winters_chill is not implemented") and target.debuffremaining(winters_chill) > casttime(glacial_spike) + traveltime(glacial_spike) and spell(glacial_spike) or message("remaining_winters_chill is not implemented") and message("remaining_winters_chill is not implemented") > buffstacks(fingers_of_frost_buff) and target.debuffremaining(winters_chill) > traveltime(ice_lance) and spell(ice_lance) or spell(comet_storm) or spell(ice_nova)
  {
   ...
   unless { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) } and spell(ice_lance) or spell(ebonbolt)
   {
    ...
    if { not equippedruneforge(freezing_winds_runeforge) or enemies() >= 2 } and { buffpresent(brain_freeze_buff) or message("soulbind.combat_meditation.enabled is not implemented") } spell(radiant_spark)
    ...
    if timesincepreviousspell(shifting_power) > 60 and { message("soulbind.field_of_blossoms.enabled is not implemented") or message("soulbind.grove_invigoration.enabled is not implemented") } and { not hastalent(rune_of_power_talent) or buffexpires(rune_of_power_buff) and spellcooldown(rune_of_power) > 16 } spell(shifting_power)
    #mirrors_of_torment
    spell(mirrors_of_torment)
...
{
 { message("remaining_winters_chill is not implemented") == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_buff) and { previousgcdspell(radiant_spark) or previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) or { target.debuffpresent(mirrors_of_torment) or buffpresent(expanded_potential_buff) or buffpresent(freezing_winds) } and buffstacks(fingers_of_frost_buff) == 0 } } and spell(flurry) or { buffpresent(freezing_rain_buff) or enemies() >= 3 or enemies() >= 2 and not equippedruneforge(cold_front_runeforge) } and spell(blizzard) or message("remaining_winters_chill is not implemented") == 1 and target.debuffpresent(winters_chill) and spell(ray_of_frost) or message("remaining_winters_chill is not implemented") and target.debuffremaining(winters_chill) > casttime(glacial_spike) + traveltime(glacial_spike) and spell(glacial_spike) or message("remaining_winters_chill is not implemented") and message("remaining_winters_chill is not implemented") > buffstacks(fingers_of_frost_buff) and target.debuffremaining(winters_chill) > traveltime(ice_lance) and spell(ice_lance) or spell(comet_storm) or spell(ice_nova) or { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) } and spell(ice_lance) or spell(ebonbolt) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) and spell(fire_blast) or buffpresent(brain_freeze_buff) and spell(glacial_spike) or spell(frostbolt)
}
...
{
 { message("remaining_winters_chill is not implemented") == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_buff) and { previousgcdspell(radiant_spark) or previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) or { target.debuffpresent(mirrors_of_torment) or buffpresent(expanded_potential_buff) or buffpresent(freezing_winds) } and buffstacks(fingers_of_frost_buff) == 0 } } and spell(flurry) or spell(frozen_orb) or { buffpresent(freezing_rain_buff) or enemies() >= 3 or enemies() >= 2 and not equippedruneforge(cold_front_runeforge) } and spell(blizzard) or message("remaining_winters_chill is not implemented") == 1 and target.debuffpresent(winters_chill) and spell(ray_of_frost) or message("remaining_winters_chill is not implemented") and target.debuffremaining(winters_chill) > casttime(glacial_spike) + traveltime(glacial_spike) and spell(glacial_spike) or message("remaining_winters_chill is not implemented") and message("remaining_winters_chill is not implemented") > buffstacks(fingers_of_frost_buff) and target.debuffremaining(winters_chill) > traveltime(ice_lance) and spell(ice_lance) or spell(comet_storm) or spell(ice_nova) or buffpresent(freezing_winds) and enemies() == 1 and spell(radiant_spark) or { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) } and spell(ice_lance) or spell(ebonbolt) or { not equippedruneforge(freezing_winds_runeforge) or enemies() >= 2 } and { buffpresent(brain_freeze_buff) or message("soulbind.combat_meditation.enabled is not implemented") } and spell(radiant_spark) or enemies() >= 3 and spell(shifting_power) or timesincepreviousspell(shifting_power) > 60 and { message("soulbind.field_of_blossoms.enabled is not implemented") or message("soulbind.grove_invigoration.enabled is not implemented") } and { not hastalent(rune_of_power_talent) or buffexpires(rune_of_power_buff) and spellcooldown(rune_of_power) > 16 } and spell(shifting_power) or spell(mirrors_of_torment) or equippedruneforge(grisly_icicle_runeforge) and level() <= level() and target.debuffexpires(frozen) and spell(frost_nova) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_arcane_buff) and spell(arcane_explosion) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) and spell(fire_blast) or buffpresent(brain_freeze_buff) and spell(glacial_spike) or spell(frostbolt)
}
...
{
 message("soulbind.wasteland_propriety.enabled is not implemented") and spell(mirrors_of_torment) or spellcooldown(icy_veins) > 15 and buffexpires(rune_of_power_buff) and spell(rune_of_power) or spell(berserking) or spell(bag_of_tricks)
}
...
AddFunction frostaoemainactions
{
 ...
 if { message("remaining_winters_chill is not implemented") == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_buff) and buffstacks(fingers_of_frost_buff) == 0 } spell(flurry)
 #ice_nova
 spell(ice_nova)
 #comet_storm
 spell(comet_storm)
 ...
 if buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) or message("remaining_winters_chill is not implemented") and target.debuffremaining(winters_chill) > traveltime(ice_lance) spell(ice_lance)
 #fire_blast,if=runeforge.disciplinary_command.equipped&cooldown.buff_disciplinary_command.ready&buff.disciplinary_command_fire.down
 if equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) spell(fire_blast)
...
AddFunction frostaoeshortcdactions
{
 ...
 unless spell(blizzard) or { message("remaining_winters_chill is not implemented") == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_buff) and buffstacks(fingers_of_frost_buff) == 0 } and spell(flurry) or spell(ice_nova) or spell(comet_storm) or { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) or message("remaining_winters_chill is not implemented") and target.debuffremaining(winters_chill) > traveltime(ice_lance) } and spell(ice_lance)
 {
  #radiant_spark
  spell(radiant_spark)
  #shifting_power
...
{
 spell(frozen_orb) or spell(blizzard) or { message("remaining_winters_chill is not implemented") == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze_buff) and buffstacks(fingers_of_frost_buff) == 0 } and spell(flurry) or spell(ice_nova) or spell(comet_storm) or { buffpresent(fingers_of_frost_buff) or target.debuffremaining(frozen) > traveltime(ice_lance) or message("remaining_winters_chill is not implemented") and target.debuffremaining(winters_chill) > traveltime(ice_lance) } and spell(ice_lance) or spell(radiant_spark) or spell(shifting_power) or spell(mirrors_of_torment) or equippedruneforge(grisly_icicle_runeforge) and level() <= level() and target.debuffexpires(frozen) and spell(frost_nova) or equippedruneforge(disciplinary_command_runeforge) and spellcooldown(buff_disciplinary_command) == 0 and buffexpires(disciplinary_command_fire_buff) and spell(fire_blast) or manapercent() > 30 and not equippedruneforge(cold_front_runeforge) and { not equippedruneforge(freezing_winds_runeforge) or buffpresent(freezing_winds) } and spell(arcane_explosion) or spell(ebonbolt) or equippedruneforge(glacial_fragments_runeforge) and hastalent(splitting_ice_talent) and spell(ice_lance) or spell(frostbolt)
}
