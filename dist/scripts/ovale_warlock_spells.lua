...
# Launches s1 bolts of felfire over d at random targets afflicted by your Immolate within 196449A1 yds. Each bolt deals 196448s1 Fire damage to the target and 196448s2 Fire damage to nearby enemies.
  SpellInfo(channel_demonfire cd=25 duration=3 channel=3 talent=channel_demonfire_talent tick=0.2)
...
# Inflicts impending doom upon the target, causing 265469s1 Shadow damage after d. rnrnIf Doom kills the target, there is a s2 chance to summon a Doomguard to fight for you for 280365d.rnrn|cFFFFFFFFGenerates 1 Soul Shard when dealing damage.|r
  SpellInfo(doom duration=30 talent=doom_talent tick=30)
...
Define(immolate 348)
# Burns the enemy, causing s1 Fire damage immediately and an additional 157736o1 Fire damage over 157736d.rnrn|cFFFFFFFFPeriodic damage generates 1 Soul Shard Fragment and has a s2 chance to generate an additional 1 on critical strikes.|r
  SpellInfo(immolate)
...
# Draws fire toward the enemy, dealing s2 Fire damage.rnrn|cFFFFFFFFGenerates 244670s1 Soul Shard Fragments and an additional 1 on critical strikes.|r
  SpellInfo(incinerate max_stacks=5 replace=shadow_bolt)
...
# Places a phantom singularity above the target, which consumes the life of all enemies within 205246A2 yards, dealing 8*205246s2 damage over d, healing you for 205246e2*100 of the damage done.
  SpellInfo(phantom_singularity cd=45 duration=16 talent=phantom_singularity_talent tick=2)
...
# Embeds a demon seed in the enemy target that will explode after d, dealing 27285s1 Shadow damage to all enemies within 27285A1 yards and applying Corruption to them.rnrnThe seed will detonate early if the target is hit by other detonations, or takes SPS*s1/100 damage from your spells.
...
# Sends a shadowy bolt at the enemy, causing s1 Shadow damage.?c2[rnrn|cFFFFFFFFGenerates 1 Soul Shard.|r][]
