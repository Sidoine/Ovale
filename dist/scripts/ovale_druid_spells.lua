...
Define(ashamanes_frenzy 210722)
	SpellInfo(ashamanes_frenzy cd=75 combopoints=-3)
...
Define(barkskin 22812)
	...
	SpellInfo(barkskin add_cd=30 specialization=guardian talent=!survival_of_the_fittest)
...
Define(brutal_slash 202028)
	...
	SpellInfo(brutal_slash combopoints=-1 energy=20 physical=1)
...
Define(cenarion_ward_buff 102351)
	SpellInfo(cenarion_ward_buff duration=30)
...
Define(elunes_guidance 202060)
	SpellInfo(elunes_guidance cd=30 combopoints=-5)
Define(elunes_guidance_buff 202060)
	#TODO 1 combopoints per s
...
Define(force_of_nature_caster 205636)
	SpellInfo(force_of_nature_caster cd=60)
Define(frenzied_regeneration 22842)
	...
	SpellAddBuff(frenzied_regeneration guardian_of_elune_buff=0 specialization=guardian)
...
Define(healing_touch 5185)
	SpellInfo(healing_touch mana=19000)
Define(healing_touch 5185)
...
Define(innervate_buff 29166)
	SpellInfo(innervate_buff duration=10)
...
Define(ironfur 192081)
	...
	SpellAddBuff(ironfur guardian_of_elune_buff=0 specialization=guardian)
...
Define(maim 22570)
	SpellInfo(maim cd=10 combopoints=1 max_combopoints=5 energy=35 interrupt=1 physical=1 stance=druid_cat_form)
...
Define(moonfire_cat 155625)
	SpellInfo(moonfire_cat combopoints=-1 energy=30 stance=druid_cat_form)
...
Define(regrowth 8936)
	SpellAddTargetBuff(regrowth regrowth_buff=1 specialization=restoration)
	SpellAddBuff(regrowth clearcasting_restoration_buff=0 specialization=restoration)
...
Define(savage_roar 52610)
	SpellInfo(savage_roar combopoints=1 max_combopoints=5 energy=40 stance=druid_cat_form)
	SpellInfo(savage_roar duration=12 add_durationcp=6)
...
Define(shred 5221)
	SpellInfo(shred combopoints=-1 energy=40 physical=1 stance=druid_cat_form)
...
Define(solar_wrath 190984)
	SpellInfo(solar_wrath travel_time=1 astralpower=-8 specialization=balance)
	SpellRequire(solar_wrath astralpower_percent 125=buff,blessing_of_elune_buff specialization=balance)
	SpellRequire(solar_wrath astralpower_percent 150=buff,celestial_alignment_buff specialization=balance)
...
Define(survival_instincts 61336)
	...
	SpellInfo(survival_instincts add_cd=-80 specialization=guardian talent=survival_of_the_fittest)
...
Define(swipe_cat 106785) 
	SpellInfo(swipe_cat combopoints=-1 energy=40 physical=1 stance=druid_cat_form)
	SpellRequire(swipe_cat add_energy_from_aura value3=buff,scent_of_blood_buff)
...
Define(thrash_cat 106830)
	SpellInfo(thrash_cat combopoints=-1 energy=45 stance=druid_cat_form)
