...
AddFunction afflictionspendersmainactions
{
 ...
 if { spellcooldown(summon_darkglare) < timetoshard() * { 5 - souldshards() } or not spellcooldown(summon_darkglare) > 0 } and target.timetodie() > spellcooldown(summon_darkglare) afflictionfillersmainactions()

 unless { spellcooldown(summon_darkglare) < timetoshard() * { 5 - souldshards() } or not spellcooldown(summon_darkglare) > 0 } and target.timetodie() > spellcooldown(summon_darkglare) and afflictionfillersmainpostconditions()
 {
  ...
  if not use_seed() and { not hastalent(deathbolt_talent) or spellcooldown(deathbolt) > timetoshard() or souldshards() > 1 } and { not hastalent(vile_taint_talent) or souldshards() > 1 } and buffremaining(unstable_affliction_buff) <= casttime(unstable_affliction) + padding() and { not hasazeritetrait(cascading_calamity_trait) or buffremaining(cascading_calamity_buff) > timetoshard() } spell(unstable_affliction)
 }
}

AddFunction afflictionspendersmainpostconditions
{
 { spellcooldown(summon_darkglare) < timetoshard() * { 5 - souldshards() } or not spellcooldown(summon_darkglare) > 0 } and target.timetodie() > spellcooldown(summon_darkglare) and afflictionfillersmainpostconditions()
}
...
AddFunction afflictionspendersshortcdactions
{
 ...
 {
  #call_action_list,name=fillers,if=(cooldown.summon_darkglare.remains<time_to_shard*(5-soul_shard)|cooldown.summon_darkglare.up)&time_to_die>cooldown.summon_darkglare.remains
  if { spellcooldown(summon_darkglare) < timetoshard() * { 5 - souldshards() } or not spellcooldown(summon_darkglare) > 0 } and target.timetodie() > spellcooldown(summon_darkglare) afflictionfillersshortcdactions()
 }
}

AddFunction afflictionspendersshortcdpostconditions
{
 spellcooldown(summon_darkglare) <= souldshards() * { executetime(unstable_affliction) + azeritetraitrank(dreadful_calling_trait) } and { not hastalent(deathbolt_talent) or spellcooldown(deathbolt) <= souldshards() * executetime(unstable_affliction) } and { hastalent(sow_the_seeds_talent) or target.debuffremaining(phantom_singularity) or target.debuffremaining(vile_taint) } and spell(unstable_affliction) or { spellcooldown(summon_darkglare) < timetoshard() * { 5 - souldshards() } or not spellcooldown(summon_darkglare) > 0 } and target.timetodie() > spellcooldown(summon_darkglare) and afflictionfillersshortcdpostconditions() or use_seed() and spell(seed_of_corruption) or not use_seed() and not previousgcdspell(summon_darkglare) and { hastalent(deathbolt_talent) and spellcooldown(deathbolt) <= executetime(unstable_affliction) and not hasazeritetrait(cascading_calamity_trait) or { souldshards() >= 5 and enemies() < 2 or souldshards() >= 2 and enemies() >= 2 } and target.timetodie() > 4 + executetime(unstable_affliction) and enemies() == 1 or target.timetodie() <= 8 + executetime(unstable_affliction) * souldshards() } and spell(unstable_affliction) or not use_seed() and buffremaining(unstable_affliction_buff) <= casttime(unstable_affliction) + padding() and spell(unstable_affliction) or not use_seed() and { not hastalent(deathbolt_talent) or spellcooldown(deathbolt) > timetoshard() or souldshards() > 1 } and { not hastalent(vile_taint_talent) or souldshards() > 1 } and buffremaining(unstable_affliction_buff) <= casttime(unstable_affliction) + padding() and { not hasazeritetrait(cascading_calamity_trait) or buffremaining(cascading_calamity_buff) > timetoshard() } and spell(unstable_affliction)
}
...
AddFunction afflictionspenderscdactions
{
 ...
 {
  #call_action_list,name=fillers,if=(cooldown.summon_darkglare.remains<time_to_shard*(5-soul_shard)|cooldown.summon_darkglare.up)&time_to_die>cooldown.summon_darkglare.remains
  if { spellcooldown(summon_darkglare) < timetoshard() * { 5 - souldshards() } or not spellcooldown(summon_darkglare) > 0 } and target.timetodie() > spellcooldown(summon_darkglare) afflictionfillerscdactions()
 }
}

AddFunction afflictionspenderscdpostconditions
{
 spellcooldown(summon_darkglare) <= souldshards() * { executetime(unstable_affliction) + azeritetraitrank(dreadful_calling_trait) } and { not hastalent(deathbolt_talent) or spellcooldown(deathbolt) <= souldshards() * executetime(unstable_affliction) } and { hastalent(sow_the_seeds_talent) or target.debuffremaining(phantom_singularity) or target.debuffremaining(vile_taint) } and spell(unstable_affliction) or { spellcooldown(summon_darkglare) < timetoshard() * { 5 - souldshards() } or not spellcooldown(summon_darkglare) > 0 } and target.timetodie() > spellcooldown(summon_darkglare) and afflictionfillerscdpostconditions() or use_seed() and spell(seed_of_corruption) or not use_seed() and not previousgcdspell(summon_darkglare) and { hastalent(deathbolt_talent) and spellcooldown(deathbolt) <= executetime(unstable_affliction) and not hasazeritetrait(cascading_calamity_trait) or { souldshards() >= 5 and enemies() < 2 or souldshards() >= 2 and enemies() >= 2 } and target.timetodie() > 4 + executetime(unstable_affliction) and enemies() == 1 or target.timetodie() <= 8 + executetime(unstable_affliction) * souldshards() } and spell(unstable_affliction) or not use_seed() and buffremaining(unstable_affliction_buff) <= casttime(unstable_affliction) + padding() and spell(unstable_affliction) or not use_seed() and { not hastalent(deathbolt_talent) or spellcooldown(deathbolt) > timetoshard() or souldshards() > 1 } and { not hastalent(vile_taint_talent) or souldshards() > 1 } and buffremaining(unstable_affliction_buff) <= casttime(unstable_affliction) + padding() and { not hasazeritetrait(cascading_calamity_trait) or buffremaining(cascading_calamity_buff) > timetoshard() } and spell(unstable_affliction)
}
