...
{
 not pet.present() and spell(summon_felguard) or hastalent(inner_demons_talent) and spell(inner_demons) or checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1) or spell(demonbolt)
}
...
AddFunction demonology_defaultmainactions
{
 ...
 if { demonduration(demonic_tyrant) > 0 and { not azeriteessenceismajor(vision_of_perfection_essence_id) or not hastalent(demonic_consumption_talent) or spellcooldown(summon_demonic_tyrant) >= spellcooldownduration(summon_demonic_tyrant) - 5 } and { not hastalent(nether_portal_talent) or spellcooldown(nether_portal) > 160 } or target.timetodie() < 30 } and checkboxon(opt_use_consumables) and target.classification(worldboss) item(disabled_item usable=1)
...
{
 unless { demonduration(demonic_tyrant) > 0 and { not azeriteessenceismajor(vision_of_perfection_essence_id) or not hastalent(demonic_consumption_talent) or spellcooldown(summon_demonic_tyrant) >= spellcooldownduration(summon_demonic_tyrant) - 5 } and { not hastalent(nether_portal_talent) or spellcooldown(nether_portal) > 160 } or target.timetodie() < 30 } and checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1)
 {
