...
AddFunction RetributionGeneratorsCdActions
{
 ...
 unless { HolyPower() >= 5 or BuffPresent(memory_of_lucid_dreams) or BuffPresent(seething_rage) } and RetributionFinishersCdPostConditions() or { not False(raid_event_adds_exists) or 600 > 15 or Enemies() >= 2 } and { HolyPower() <= 0 or HolyPower() == 1 and SpellCooldown(blade_of_justice) > GCD() } and { SpellCooldown(avenging_wrath) > 10 or Talent(crusade_talent) and SpellCooldown(crusade) > 10 } and Spell(wake_of_ashes) or { HolyPower() <= 2 or HolyPower() == 3 and { SpellCooldown(hammer_of_wrath) > GCD() * 2 or HoW() } } and Spell(blade_of_justice) or { HolyPower() <= 2 or HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or HoW() } } and Spell(judgment) or HolyPower() <= 4 and Spell(hammer_of_wrath) or { HolyPower() <= 2 or HolyPower() <= 3 and SpellCooldown(blade_of_justice) > GCD() * 2 or HolyPower() == 4 and SpellCooldown(blade_of_justice) > GCD() * 2 and SpellCooldown(judgment) > GCD() * 2 } and Spell(consecration_retribution)
 {
  ...
  unless Talent(hammer_of_wrath_talent) and { target.HealthPercent() <= 20 or BuffPresent(avenging_wrath_buff) or BuffPresent(crusade_buff) } and RetributionFinishersCdPostConditions() or SpellCharges(crusader_strike count=0) >= 1.75 and { HolyPower() <= 2 or HolyPower() <= 3 and SpellCooldown(blade_of_justice) > GCD() * 2 or HolyPower() == 4 and SpellCooldown(blade_of_justice) > GCD() * 2 and SpellCooldown(judgment) > GCD() * 2 and SpellCooldown(consecration_retribution) > GCD() * 2 } and Spell(crusader_strike)
  {
   ...
   unless RetributionFinishersCdPostConditions() or Spell(concentrated_flame) or HolyPower() <= 4 and Spell(crusader_strike)
   {
...
AddFunction RetributionFinishersMainActions
{
 ...
 if ds_castable() and { not Talent(crusade_talent) or SpellCooldown(crusade) > GCD() * 2 } or BuffPresent(empyrean_power_buff) and target.DebuffExpires(judgment) and BuffExpires(divine_purpose_buff) Spell(divine_storm)
 ...
 if { not Talent(crusade_talent) and SpellCooldown(avenging_wrath) > GCD() * 3 or SpellCooldown(crusade) > GCD() * 3 } and { not Talent(execution_sentence_talent) or SpellCooldown(execution_sentence) > GCD() * 2 or SpellCooldown(avenging_wrath) > GCD() * 3 and SpellCooldown(avenging_wrath) < 10 or BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 10 } Spell(templars_verdict)
}
...
{
 { BuffExpires(inquisition_buff) or BuffRemaining(inquisition_buff) < 5 and HolyPower() >= 3 or Talent(execution_sentence_talent) and SpellCooldown(execution_sentence) < 10 and BuffRemaining(inquisition_buff) < 15 or SpellCooldown(avenging_wrath) < 15 and BuffRemaining(inquisition_buff) < 20 and HolyPower() >= 3 } and Spell(inquisition) or Enemies() <= 2 and { not Talent(crusade_talent) and SpellCooldown(avenging_wrath) > 10 or Talent(crusade_talent) and BuffExpires(crusade_buff) and SpellCooldown(crusade) > 10 or BuffStacks(crusade_buff) >= 7 } and Spell(execution_sentence) or { ds_castable() and { not Talent(crusade_talent) or SpellCooldown(crusade) > GCD() * 2 } or BuffPresent(empyrean_power_buff) and target.DebuffExpires(judgment) and BuffExpires(divine_purpose_buff) } and Spell(divine_storm) or { not Talent(crusade_talent) and SpellCooldown(avenging_wrath) > GCD() * 3 or SpellCooldown(crusade) > GCD() * 3 } and { not Talent(execution_sentence_talent) or SpellCooldown(execution_sentence) > GCD() * 2 or SpellCooldown(avenging_wrath) > GCD() * 3 and SpellCooldown(avenging_wrath) < 10 or BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 10 } and Spell(templars_verdict)
}
...
AddFunction RetributionCooldownsMainActions
{
 ...
 if SpellCooldown(avenging_wrath) < GCD() and HolyPower() >= 3 or SpellCooldown(crusade) < GCD() and HolyPower() >= 4 or SpellCooldown(avenging_wrath) >= 45 or SpellCooldown(crusade) >= 45 Spell(worldvein_resonance)
...
{
 { SpellCooldown(avenging_wrath) < GCD() and HolyPower() >= 3 or SpellCooldown(crusade) < GCD() and HolyPower() >= 4 or SpellCooldown(avenging_wrath) >= 45 or SpellCooldown(crusade) >= 45 } and Spell(worldvein_resonance) or { not False(raid_event_adds_exists) or 600 > 30 or Enemies() >= 2 } and { BuffExpires(avenging_wrath_buff) or BuffExpires(crusade_buff) } and SpellCooldown(blade_of_justice) > GCD() * 3 and SpellCooldown(judgment) > GCD() * 3 and Spell(focused_azerite_beam)
}
...
AddFunction RetributionCooldownsCdActions
{
 ...
 unless BuffExpires(seething_rage) and BuffExpires(memory_of_lucid_dreams) and Spell(shield_of_vengeance) or { TimeInCombat() <= 2 or BuffPresent(reckless_force_buff) } and Spell(the_unbound_force)
 {
  ...
  if not Talent(crusade_talent) and { SpellCooldown(avenging_wrath) < GCD() and HolyPower() >= 3 or SpellCooldown(avenging_wrath) >= 45 } or Talent(crusade_talent) and SpellCooldown(crusade) < GCD() and HolyPower() >= 4 or SpellCooldown(crusade) >= 45 Spell(guardian_of_azeroth)
