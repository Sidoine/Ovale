...
do
    ...
    local desc = "[8.2] Simulationcraft: T24_Paladin_Protection"
    local code = [[
...
do
    ...
    local desc = "[8.2] Simulationcraft: T24_Paladin_Retribution"
    local code = [[
...
AddFunction RetributionCooldownsShortCdActions
{
 ...
 if TimeInCombat() <= 2 or BuffPresent(reckless_force_buff) Spell(the_unbound_force_essence)
...
AddFunction RetributionCooldownsCdActions
{
 ...
 if { SpellCooldown(guardian_of_azeroth) > 90 or not AzeriteEssenceIsMajor(condensed_lifeforce_essence_id) } and { BuffPresent(burst_haste_buff any=1) or BuffPresent(avenging_wrath_buff) or BuffPresent(crusade_buff) and BuffRemaining(crusade_buff) < 25 } and CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(item_focused_resolve usable=1)
 ...
 unless BuffExpires(seething_rage) and BuffExpires(memory_of_lucid_dreams_essence_buff) and CheckBoxOn(opt_shield_of_vengeance) and Spell(shield_of_vengeance)
 {
  ...
  if target.DebuffExpires(razor_coral) or BuffRemaining(avenging_wrath_buff) >= 20 or BuffPresent(crusade_buff) and BuffStacks(crusade_buff) == 10 and BuffRemaining(crusade_buff) > 15 RetributionUseItemActions()

  unless { TimeInCombat() <= 2 or BuffPresent(reckless_force_buff) } and Spell(the_unbound_force_essence)
  {
