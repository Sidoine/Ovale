...
AddFunction RetributionGeneratorsMainActions
{
 ...
 unless { holypower() >= 5 or buffpresent(memory_of_lucid_dreams_essence_buff) or buffpresent(seething_rage) or buffexpires(inquisition_buff) and holypower() >= 3 } and RetributionFinishersMainPostConditions()
 {
  ...
  if holypower() <= 2 or holypower() == 3 and { spellcooldown(hammer_of_wrath) > gcd() * 2 or undefined() } spell(blade_of_justice)
  #judgment,if=holy_power<=2|(holy_power<=4&(cooldown.blade_of_justice.remains>gcd*2|variable.HoW))
  if holypower() <= 2 or holypower() <= 4 and { spellcooldown(blade_of_justice) > gcd() * 2 or undefined() } spell(judgment)
...
AddFunction RetributionFinishersMainActions
{
 ...
 if undefined() and undefined() and { not hastalent(execution_sentence_talent) or enemies() >= 2 or spellcooldown(execution_sentence) > gcd() * 2 or spellcooldown(avenging_wrath) > gcd() * 3 and spellcooldown(avenging_wrath) < 10 or spellcooldown(crusade) > gcd() * 3 and spellcooldown(crusade) < 10 or buffpresent(crusade_buff) and buffstacks(crusade_buff) < 10 } spell(divine_storm)
 ...
 if undefined() and { not hastalent(execution_sentence_talent) or spellcooldown(execution_sentence) > gcd() * 2 or spellcooldown(avenging_wrath) > gcd() * 3 and spellcooldown(avenging_wrath) < 10 or spellcooldown(crusade) > gcd() * 3 and spellcooldown(crusade) < 10 or buffpresent(crusade_buff) and buffstacks(crusade_buff) < 10 } spell(templars_verdict)
}
...
{
 buffexpires(avenging_wrath_buff) and { buffexpires(inquisition_buff) or buffremaining(inquisition_buff) < 8 and holypower() >= 3 or hastalent(execution_sentence_talent) and spellcooldown(execution_sentence) < 10 and buffremaining(inquisition_buff) < 15 or spellcooldown(avenging_wrath) < 15 and buffremaining(inquisition_buff) < 20 and holypower() >= 3 } and spell(inquisition) or enemies() <= 2 and { not hastalent(crusade_talent) and spellcooldown(avenging_wrath) > 10 or hastalent(crusade_talent) and buffexpires(crusade_buff) and spellcooldown(crusade) > 10 or buffstacks(crusade_buff) >= 7 } and spell(execution_sentence) or undefined() and undefined() and { not hastalent(execution_sentence_talent) or enemies() >= 2 or spellcooldown(execution_sentence) > gcd() * 2 or spellcooldown(avenging_wrath) > gcd() * 3 and spellcooldown(avenging_wrath) < 10 or spellcooldown(crusade) > gcd() * 3 and spellcooldown(crusade) < 10 or buffpresent(crusade_buff) and buffstacks(crusade_buff) < 10 } and spell(divine_storm) or undefined() and { not hastalent(execution_sentence_talent) or spellcooldown(execution_sentence) > gcd() * 2 or spellcooldown(avenging_wrath) > gcd() * 3 and spellcooldown(avenging_wrath) < 10 or spellcooldown(crusade) > gcd() * 3 and spellcooldown(crusade) < 10 or buffpresent(crusade_buff) and buffstacks(crusade_buff) < 10 } and spell(templars_verdict)
}
