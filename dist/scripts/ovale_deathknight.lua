...
AddFunction BloodStandardMainActions
{
 ...
 if Charges(blood_boil count=0) >= 1 and BuffStacks(haemostasis_buff) < 5 and { BuffStacks(haemostasis_buff) < 3 or not BuffPresent(dancing_rune_weapon_buff) } Spell(blood_boil)
...
AddFunction BloodStandardShortCdActions
{
 unless RunicPowerDeficit() < 10 and Spell(death_strike)
 {
  ...
  unless BuffRemaining(bone_shield_buff) <= GCD() * 2 and Spell(marrowrend) or Charges(blood_boil count=0) >= 1 and BuffStacks(haemostasis_buff) < 5 and { BuffStacks(haemostasis_buff) < 3 or not BuffPresent(dancing_rune_weapon_buff) } and Spell(blood_boil) or { BuffStacks(bone_shield_buff) < 5 and Talent(ossuary_talent) or BuffRemaining(bone_shield_buff) < GCD() * 3 } and Spell(marrowrend)
  {
   #bonestorm,if=runic_power>=100&spell_targets.bonestorm>=3
   if RunicPower() >= 100 and Enemies() >= 3 Spell(bonestorm)

   unless { BuffPresent(blood_shield_buff) or RunicPowerDeficit() < 15 and { RunicPowerDeficit() < 25 or not BuffPresent(dancing_rune_weapon_buff) } } and Spell(death_strike) or BuffPresent(dancing_rune_weapon_buff) and Spell(heart_strike)
   {
...
{
 RunicPowerDeficit() < 10 and Spell(death_strike) or BuffRemaining(bone_shield_buff) <= GCD() * 2 and Spell(marrowrend) or Charges(blood_boil count=0) >= 1 and BuffStacks(haemostasis_buff) < 5 and { BuffStacks(haemostasis_buff) < 3 or not BuffPresent(dancing_rune_weapon_buff) } and Spell(blood_boil) or { BuffStacks(bone_shield_buff) < 5 and Talent(ossuary_talent) or BuffRemaining(bone_shield_buff) < GCD() * 3 } and Spell(marrowrend) or { BuffPresent(blood_shield_buff) or RunicPowerDeficit() < 15 and { RunicPowerDeficit() < 25 or not BuffPresent(dancing_rune_weapon_buff) } } and Spell(death_strike) or BuffPresent(dancing_rune_weapon_buff) and Spell(heart_strike) or { TimeToRunes(3) < GCD() or BuffStacks(bone_shield_buff) > 6 } and Spell(heart_strike)
}
...
AddFunction FrostPrecombatCdActions
{
 ...
 #snapshot_stats
 #potion
 if CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(prolonged_power usable=1)
}
...
AddFunction FrostCooldownsMainActions
{
 ...
 if { HasEquippedItem(cold_heart_item) or Talent(cold_heart_talent) } and { { BuffStacks(cold_heart_item_buff) >= 10 or BuffStacks(cold_heart_buff) >= 10 } and target.DebuffStacks(razorice_debuff) == 5 or target.TimeToDie() <= GCD() } FrostColdheartMainActions()
}

AddFunction FrostCooldownsMainPostConditions
{
 { HasEquippedItem(cold_heart_item) or Talent(cold_heart_talent) } and { { BuffStacks(cold_heart_item_buff) >= 10 or BuffStacks(cold_heart_buff) >= 10 } and target.DebuffStacks(razorice_debuff) == 5 or target.TimeToDie() <= GCD() } and FrostColdheartMainPostConditions()
}
...
AddFunction FrostCooldownsShortCdActions
{
 ...
 #call_action_list,name=cold_heart,if=(equipped.cold_heart|talent.cold_heart.enabled)&(((buff.cold_heart_item.stack>=10|buff.cold_heart_talent.stack>=10)&debuff.razorice.stack=5)|target.time_to_die<=gcd)
 if { HasEquippedItem(cold_heart_item) or Talent(cold_heart_talent) } and { { BuffStacks(cold_heart_item_buff) >= 10 or BuffStacks(cold_heart_buff) >= 10 } and target.DebuffStacks(razorice_debuff) == 5 or target.TimeToDie() <= GCD() } FrostColdheartShortCdActions()
}

AddFunction FrostCooldownsShortCdPostConditions
{
 { HasEquippedItem(cold_heart_item) or Talent(cold_heart_talent) } and { { BuffStacks(cold_heart_item_buff) >= 10 or BuffStacks(cold_heart_buff) >= 10 } and target.DebuffStacks(razorice_debuff) == 5 or target.TimeToDie() <= GCD() } and FrostColdheartShortCdPostConditions()
}
...
AddFunction FrostCooldownsCdActions
{
 ...
 if BuffPresent(pillar_of_frost_buff) and BuffPresent(empower_rune_weapon_buff) and CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(prolonged_power usable=1)
 ...
 unless SpellCooldown(empower_rune_weapon) > 0 and Spell(pillar_of_frost)
 {
  ...
  #call_action_list,name=cold_heart,if=(equipped.cold_heart|talent.cold_heart.enabled)&(((buff.cold_heart_item.stack>=10|buff.cold_heart_talent.stack>=10)&debuff.razorice.stack=5)|target.time_to_die<=gcd)
  if { HasEquippedItem(cold_heart_item) or Talent(cold_heart_talent) } and { { BuffStacks(cold_heart_item_buff) >= 10 or BuffStacks(cold_heart_buff) >= 10 } and target.DebuffStacks(razorice_debuff) == 5 or target.TimeToDie() <= GCD() } FrostColdheartCdActions()

  unless { HasEquippedItem(cold_heart_item) or Talent(cold_heart_talent) } and { { BuffStacks(cold_heart_item_buff) >= 10 or BuffStacks(cold_heart_buff) >= 10 } and target.DebuffStacks(razorice_debuff) == 5 or target.TimeToDie() <= GCD() } and FrostColdheartCdPostConditions()
  {
...
{
 SpellCooldown(empower_rune_weapon) > 0 and Spell(pillar_of_frost) or { HasEquippedItem(cold_heart_item) or Talent(cold_heart_talent) } and { { BuffStacks(cold_heart_item_buff) >= 10 or BuffStacks(cold_heart_buff) >= 10 } and target.DebuffStacks(razorice_debuff) == 5 or target.TimeToDie() <= GCD() } and FrostColdheartCdPostConditions()
}
...
{
 SpellCooldown(summon_gargoyle) < 5 and { SpellCooldown(dark_transformation) < 5 or not HasEquippedItem(137075 _item) } and Talent(summon_gargoyle_talent)
}
...
AddFunction UnholyCooldownsMainActions
{
 ...
 if HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 UnholyColdheartMainActions()
}

AddFunction UnholyCooldownsMainPostConditions
{
 HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 and UnholyColdheartMainPostConditions()
}

AddFunction UnholyCooldownsShortCdActions
{
 #call_action_list,name=cold_heart,if=equipped.cold_heart&buff.cold_heart_item.stack>10
 if HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 UnholyColdheartShortCdActions()

 unless HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 and UnholyColdheartShortCdPostConditions()
 {
  ...
  if HasEquippedItem(137075 _item) and SpellCooldown(summon_gargoyle) > 40 or not HasEquippedItem(137075 _item) or not Talent(summon_gargoyle_talent) Spell(dark_transformation)
...
{
 HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 and UnholyColdheartShortCdPostConditions()
}

AddFunction UnholyCooldownsCdActions
{
 #call_action_list,name=cold_heart,if=equipped.cold_heart&buff.cold_heart_item.stack>10
 if HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 UnholyColdheartCdActions()

 unless HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 and UnholyColdheartCdPostConditions()
 {
  #army_of_the_dead
  Spell(army_of_the_dead)

  unless target.DebuffStacks(festering_wound_debuff) >= 4 and Spell(apocalypse) or { HasEquippedItem(137075 _item) and SpellCooldown(summon_gargoyle) > 40 or not HasEquippedItem(137075 _item) or not Talent(summon_gargoyle_talent) } and Spell(dark_transformation)
  {
...
{
 HasEquippedItem(cold_heart_item) and BuffStacks(cold_heart_item_buff) > 10 and UnholyColdheartCdPostConditions() or target.DebuffStacks(festering_wound_debuff) >= 4 and Spell(apocalypse) or { HasEquippedItem(137075 _item) and SpellCooldown(summon_gargoyle) > 40 or not HasEquippedItem(137075 _item) or not Talent(summon_gargoyle_talent) } and Spell(dark_transformation) or target.DebuffStacks(festering_wound_debuff) < 4 and Spell(unholy_frenzy) or Enemies() >= 2 and { SpellCooldown(death_and_decay) <= GCD() and not Talent(defile_talent) or SpellCooldown(defile) <= GCD() and Talent(defile_talent) } and Spell(unholy_frenzy) or { target.TimeToDie() < 8 or Rune() < 3 } and not BuffPresent(unholy_frenzy_buff) and Spell(soul_reaper) or Spell(unholy_blight)
}
...
AddFunction UnholyColdheartMainActions
{
 ...
 if DebuffRemaining(unholy_strength) < GCD() and DebuffPresent(unholy_strength) and BuffStacks(cold_heart_item_buff) > 16 Spell(chains_of_ice)
 #chains_of_ice,if=buff.master_of_ghouls.remains<gcd&buff.master_of_ghouls.up&buff.cold_heart_item.stack>17
 if DebuffRemaining(master_of_ghouls) < GCD() and DebuffPresent(master_of_ghouls) and BuffStacks(cold_heart_item_buff) > 17 Spell(chains_of_ice)
 #chains_of_ice,if=buff.cold_heart_item.stack=20&buff.unholy_strength.react
 if BuffStacks(cold_heart_item_buff) == 20 and DebuffPresent(unholy_strength) Spell(chains_of_ice)
}
...
{
 DebuffRemaining(unholy_strength) < GCD() and DebuffPresent(unholy_strength) and BuffStacks(cold_heart_item_buff) > 16 and Spell(chains_of_ice) or DebuffRemaining(master_of_ghouls) < GCD() and DebuffPresent(master_of_ghouls) and BuffStacks(cold_heart_item_buff) > 17 and Spell(chains_of_ice) or BuffStacks(cold_heart_item_buff) == 20 and DebuffPresent(unholy_strength) and Spell(chains_of_ice)
}
...
AddFunction UnholyAoeMainActions
{
 ...
 if FIXME_death_and_decay.ticking and Rune() < 2 and not pooling_for_gargoyle() Spell(epidemic)
 #death_coil,if=death_and_decay.ticking&rune<2&!variable.pooling_for_gargoyle
 if FIXME_death_and_decay.ticking and Rune() < 2 and not pooling_for_gargoyle() Spell(death_coil)
 #scourge_strike,if=death_and_decay.ticking&cooldown.apocalypse.remains
 if FIXME_death_and_decay.ticking and SpellCooldown(apocalypse) > 0 Spell(scourge_strike)
 #clawing_shadows,if=death_and_decay.ticking&cooldown.apocalypse.remains
 if FIXME_death_and_decay.ticking and SpellCooldown(apocalypse) > 0 Spell(clawing_shadows)
...
{
 SpellCooldown(apocalypse) > 0 and Spell(death_and_decay) or Spell(defile) or FIXME_death_and_decay.ticking and Rune() < 2 and not pooling_for_gargoyle() and Spell(epidemic) or FIXME_death_and_decay.ticking and Rune() < 2 and not pooling_for_gargoyle() and Spell(death_coil) or FIXME_death_and_decay.ticking and SpellCooldown(apocalypse) > 0 and Spell(scourge_strike) or FIXME_death_and_decay.ticking and SpellCooldown(apocalypse) > 0 and Spell(clawing_shadows) or not pooling_for_gargoyle() and Spell(epidemic) or Talent(bursting_sores_talent) and Enemies() >= 2 and target.DebuffStacks(festering_wound_debuff) <= 1 and Spell(festering_strike) or BuffPresent(sudden_doom_buff) and RuneDeficit() >= 4 and Spell(death_coil) or { BuffPresent(sudden_doom_buff) and not pooling_for_gargoyle() or pet.Present() } and Spell(death_coil) or RunicPowerDeficit() < 14 and { SpellCooldown(apocalypse) > 5 or target.DebuffStacks(festering_wound_debuff) > 4 } and not pooling_for_gargoyle() and Spell(death_coil) or { target.DebuffPresent(festering_wound_debuff) and SpellCooldown(apocalypse) > 5 or target.DebuffStacks(festering_wound_debuff) > 4 } and SpellCooldown(army_of_the_dead) > 5 and Spell(scourge_strike) or { target.DebuffPresent(festering_wound_debuff) and SpellCooldown(apocalypse) > 5 or target.DebuffStacks(festering_wound_debuff) > 4 } and SpellCooldown(army_of_the_dead) > 5 and Spell(clawing_shadows) or RunicPowerDeficit() < 20 and not pooling_for_gargoyle() and Spell(death_coil) or { { target.DebuffStacks(festering_wound_debuff) < 4 and not BuffPresent(unholy_frenzy_buff) or target.DebuffStacks(festering_wound_debuff) < 3 } and SpellCooldown(apocalypse) < 3 or target.DebuffStacks(festering_wound_debuff) < 1 } and SpellCooldown(army_of_the_dead) > 5 and Spell(festering_strike) or not pooling_for_gargoyle() and Spell(death_coil)
}
...
AddFunction UnholyDefaultCdActions
{
 ...
 if { SpellCooldown(army_of_the_dead) == 0 or pet.Present() or BuffPresent(unholy_frenzy_buff) } and CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(prolonged_power usable=1)
