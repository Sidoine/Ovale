...
AddFunction beast_masterystmainactions
{
 ...
 if { focus() - powercost(cobra_shot) + focusregenrate() * { spellcooldown(kill_command) - 1 } > powercost(kill_command) or spellcooldown(kill_command) > 1 + gcd() and spellcooldown(bestial_wrath) > timetomaxfocus() or buffpresent(memory_of_lucid_dreams) } and spellcooldown(kill_command) > 1 or target.timetodie() < 3 spell(cobra_shot)
...
{
 spell(kill_shot) or spell(bloodshed) or { pet.buffpresent(main_frenzy_buff) and pet.buffremaining(main_frenzy_buff) < gcd() or spellcooldown(bestial_wrath) > 0 and { spellfullrecharge(barbed_shot) < gcd() or hasazeritetrait(primal_instincts_trait) and spellcooldown(aspect_of_the_wild) < gcd() } or spellcooldown(bestial_wrath) < 12 + gcd() and hastalent(scent_of_blood_talent_beast_mastery) } and spell(barbed_shot) or { focus() + focusregenrate() * gcd() < maxfocus() and buffexpires(bestial_wrath) and not target.debuffremaining(concentrated_flame_burn_debuff) and not inflighttotarget(concentrated_flame) or spellfullrecharge(concentrated_flame) < gcd() or target.timetodie() < 5 } and spell(concentrated_flame) or { buffpresent(aspect_of_the_wild) and buffpresent(bestial_wrath) or target.timetodie() < 15 } and spell(stampede) or spell(a_murder_of_crows) or { buffpresent(reckless_force_buff) or buffstacks(reckless_force_counter) < 10 or target.timetodie() < 5 } and spell(the_unbound_force) or azeritetraitrank(dance_of_death_trait) > 1 and buffremaining(dance_of_death_buff) < gcd() and spell(barbed_shot) or { buffremaining(aspect_of_the_wild) > 10 + gcd() or target.timetodie() < 10 + gcd() } and spell(blood_of_the_enemy) or pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or spell(chimaera_shot) or spell(dire_beast) or { hastalent(one_with_the_pack_talent) and charges(barbed_shot count=0) > 1.5 or charges(barbed_shot count=0) > 1.8 or spellcooldown(aspect_of_the_wild) < baseduration(main_frenzy_buff) - gcd() and hasazeritetrait(primal_instincts_trait) or target.timetodie() < 9 } and spell(barbed_shot) or spell(barrage) or { { focus() - powercost(cobra_shot) + focusregenrate() * { spellcooldown(kill_command) - 1 } > powercost(kill_command) or spellcooldown(kill_command) > 1 + gcd() and spellcooldown(bestial_wrath) > timetomaxfocus() or buffpresent(memory_of_lucid_dreams) } and spellcooldown(kill_command) > 1 or target.timetodie() < 3 } and spell(cobra_shot) or baseduration(main_frenzy_buff) - gcd() > spellfullrecharge(barbed_shot) and spell(barbed_shot)
}
...
AddFunction beast_masteryprecombatcdactions
{
 ...
 unless spell(worldvein_resonance)
 {
  ...
  unless spell(memory_of_lucid_dreams)
  {
   ...
   if not message("raid_event.invulnerable.exists is not implemented") and { true(trinket_has_cooldown_undefined) + true(trinket_has_cooldown_undefined) < 2 or hasequippeditem(variable_intensity_gigavolt_oscillating_reactor_item) } beast_masteryuseitemactions()
...
AddFunction marksmanshipstmainactions
{
 ...
 if hastalent(steady_focus_talent) and previousgcdspell(steady_shot) and buffremaining(steady_focus) < 5 spell(steady_shot)
 #kill_shot
 spell(kill_shot)
 #explosive_shot
 spell(explosive_shot)
 ...
 if buffpresent(trueshot) or { buffexpires(double_tap) or message("ca_active is not implemented") } and buffexpires(precise_shots) or spellfullrecharge(aimed_shot) < casttime(aimed_shot) and spellcooldown(trueshot) > 0 spell(aimed_shot)
...
{
 unless hastalent(steady_focus_talent) and previousgcdspell(steady_shot) and buffremaining(steady_focus) < 5 and spell(steady_shot) or spell(kill_shot) or spell(explosive_shot) or enemies() > 1 and spell(barrage) or spell(a_murder_of_crows) or checkboxon(opt_volley) and spell(volley) or target.refreshable(serpent_sting) and not inflighttotarget(serpent_sting) and spell(serpent_sting) or { buffexpires(trueshot) or focus() < 35 or focus() < 60 and not hastalent(lethal_shots_talent) or buffremaining(in_the_rhythm) < executetime(rapid_fire) } and spell(rapid_fire) or { buffpresent(trueshot) and { buffstacks(unerring_vision_buff) > 4 or not hasazeritetrait(unerring_vision_trait) } or target.timetodie() < 11 } and spell(blood_of_the_enemy)
 {
...
{
 hastalent(steady_focus_talent) and previousgcdspell(steady_shot) and buffremaining(steady_focus) < 5 and spell(steady_shot) or spell(kill_shot) or spell(explosive_shot) or enemies() > 1 and spell(barrage) or spell(a_murder_of_crows) or checkboxon(opt_volley) and spell(volley) or target.refreshable(serpent_sting) and not inflighttotarget(serpent_sting) and spell(serpent_sting) or { buffexpires(trueshot) or focus() < 35 or focus() < 60 and not hastalent(lethal_shots_talent) or buffremaining(in_the_rhythm) < executetime(rapid_fire) } and spell(rapid_fire) or { buffpresent(trueshot) and { buffstacks(unerring_vision_buff) > 4 or not hasazeritetrait(unerring_vision_trait) } or target.timetodie() < 11 } and spell(blood_of_the_enemy) or { not buffpresent(trueshot) or target.timetodie() < 5 } and spell(focused_azerite_beam) or { buffpresent(trueshot) or { buffexpires(double_tap) or message("ca_active is not implemented") } and buffexpires(precise_shots) or spellfullrecharge(aimed_shot) < casttime(aimed_shot) and spellcooldown(trueshot) > 0 } and spell(aimed_shot) or { not buffpresent(trueshot) or target.timetodie() < 8 } and spell(purifying_blast) or { focus() + focusregenrate() * gcd() < maxfocus() and buffexpires(trueshot) and not target.debuffremaining(concentrated_flame_burn_debuff) and not inflighttotarget(concentrated_flame) or spellfullrecharge(concentrated_flame) < gcd() or target.timetodie() < 5 } and spell(concentrated_flame) or { buffpresent(reckless_force_buff) or buffstacks(reckless_force_counter) < 10 or target.timetodie() < 5 } and spell(the_unbound_force) or buffexpires(trueshot) and { buffpresent(precise_shots) and focus() > 55 or focus() > 75 or target.timetodie() < 5 } and spell(arcane_shot) or buffexpires(trueshot) and { buffpresent(precise_shots) and focus() > 55 or focus() > 75 or target.timetodie() < 5 } and spell(chimaera_shot_marksmanship) or spell(steady_shot)
}
...
AddFunction marksmanshipcdscdactions
{
 unless { spellcooldown(rapid_fire) < gcd() or spellcooldown(rapid_fire) < spellcooldown(aimed_shot) or target.timetodie() < 20 } and spell(double_tap) or { buffremaining(trueshot) > 14 and { target.timetodie() > spellcooldownduration(berserking) + baseduration(berserking) or target.healthpercent() < 20 or not hastalent(careful_aim_talent) } or target.timetodie() < 13 } and spell(berserking)
 {
  ...
  unless buffexpires(trueshot) and spell(bag_of_tricks) or buffexpires(trueshot) and { target.healthpercent() > 80 or target.healthpercent() <= 20 or target.timetohealthpercent(20) > 30 } and spell(reaping_flames) or { { buffremaining(trinket_azsharas_font_of_power_cooldown_buff) > 20 or not hasequippeditem(azsharas_font_of_power_item) or target.timetodie() < message("trinket.azsharas_font_of_power.cooldown.duration is not implemented") + 34 and target.healthpercent() > 20 } and { spellcooldown(trueshot) < 3 or azeriteessenceisminor(vision_of_perfection_essence_id) and target.timetodie() > spellcooldown(worldvein_resonance) + baseduration(worldvein_resonance_buff) } or target.timetodie() < 20 } and spell(worldvein_resonance)
  {
...
AddFunction survivalcleavemainactions
{
 ...
 if charges(butchery count=0) > 2.5 or target.debuffpresent(shrapnel_bomb_debuff) or spellcooldown(wildfire_bomb) > enemies() - gcd() or target.debuffpresent(blood_of_the_enemy) or message("raid_event.adds.remains is not implemented") < 5 and false(raid_event_adds_exists) spell(butchery)
