...
AddFunction beast_masterystmainactions
{
 ...
 if hastalent(one_with_the_pack_talent) and charges(barbed_shot count=0) > 1.5 or charges(barbed_shot count=0) > 1.8 or spellcooldown(aspect_of_the_wild) < baseduration(main_frenzy_buff) - gcd() and hasazeritetrait(primal_instincts_trait) or target.timetodie() < 9 spell(barbed_shot)
 #barrage
 spell(barrage)
 ...
 if { focus() - powercost(cobra_shot) + focus() * { spellcooldown(kill_command) - 1 } > powercost(kill_command) or spellcooldown(kill_command) > 1 + gcd() and spellcooldown(bestial_wrath) > focus() or buffpresent(memory_of_lucid_dreams) } and spellcooldown(kill_command) > 1 or target.timetodie() < 3 spell(cobra_shot)
 #barbed_shot,if=pet.main.buff.frenzy.duration-gcd>full_recharge_time
 if baseduration(main_frenzy_buff) - gcd() > spellfullrecharge(barbed_shot) spell(barbed_shot)
}
...
AddFunction beast_masteryprecombatshortcdactions
{
 ...
 unless spell(worldvein_resonance) or spell(memory_of_lucid_dreams)
 {
  ...
  if not FIXME_raid_event.invulnerable.exists spell(focused_azerite_beam)
...
AddFunction beast_masteryprecombatcdactions
{
 ...
 unless spell(worldvein_resonance)
 {
  ...
  unless spell(memory_of_lucid_dreams)
  {
   ...
   if not FIXME_raid_event.invulnerable.exists and { true(trinket_has_cooldown_undefined) + true(trinket_has_cooldown_undefined) < 2 or hasequippeditem(variable_intensity_gigavolt_oscillating_reactor_item) } beast_masteryuseitemactions()

   unless not FIXME_raid_event.invulnerable.exists and spell(focused_azerite_beam)
   {
...
AddFunction beast_masterycleavemainactions
{
 ...
 if pet.buffpresent(main_frenzy_buff) and pet.buffremaining(main_frenzy_buff) <= gcd() spell(barbed_shot)
 ...
 if pet.buffexpires(main_frenzy_buff) and { charges(barbed_shot count=0) > 1.8 or buffpresent(bestial_wrath) } or spellcooldown(aspect_of_the_wild) < baseduration(main_frenzy_buff) - gcd() and hasazeritetrait(primal_instincts_trait) or charges(barbed_shot count=0) > 1.4 or target.timetodie() < 9 spell(barbed_shot)
 #concentrated_flame
 ...
 if spellcooldown(kill_command) > focus() and { enemies() < 3 or not hasazeritetrait(rapid_reload_trait) } spell(cobra_shot)
}
...
AddFunction marksmanshiptrickshotsmainactions
{
 ...
 if buffpresent(trick_shots) and FIXME_ca_active and buffpresent(double_tap) spell(aimed_shot)
 ...
 if buffpresent(trick_shots) and { hasazeritetrait(focused_fire_trait) or azeritetraitrank(in_the_rhythm_trait) > 1 or hasazeritetrait(surging_shots_trait) or hastalent(streamline_talent) } spell(rapid_fire)
 ...
 if buffpresent(trick_shots) and { buffexpires(precise_shots) or spellcooldown(aimed_shot) < casttime(aimed_shot) or buffpresent(trueshot) } spell(aimed_shot)
 #rapid_fire,if=buff.trick_shots.up
 if buffpresent(trick_shots) spell(rapid_fire)
 #multishot,if=buff.trick_shots.down|buff.precise_shots.up&!buff.trueshot.up|focus>70
 if buffexpires(trick_shots) or buffpresent(precise_shots) and not buffpresent(trueshot) or focus() > 70 spell(multishot)
 #concentrated_flame
 spell(concentrated_flame)
 #blood_of_the_enemy
 spell(blood_of_the_enemy)
...
{
 spell(kill_shot) or checkboxon(opt_volley) and spell(volley) or spell(barrage) or spell(explosive_shot) or buffpresent(trick_shots) and FIXME_ca_active and buffpresent(double_tap) and spell(aimed_shot) or buffpresent(trick_shots) and { hasazeritetrait(focused_fire_trait) or azeritetraitrank(in_the_rhythm_trait) > 1 or hasazeritetrait(surging_shots_trait) or hastalent(streamline_talent) } and spell(rapid_fire) or buffpresent(trick_shots) and { buffexpires(precise_shots) or spellcooldown(aimed_shot) < casttime(aimed_shot) or buffpresent(trueshot) } and spell(aimed_shot) or buffpresent(trick_shots) and spell(rapid_fire) or { buffexpires(trick_shots) or buffpresent(precise_shots) and not buffpresent(trueshot) or focus() > 70 } and spell(multishot) or spell(focused_azerite_beam) or spell(purifying_blast) or spell(concentrated_flame) or spell(blood_of_the_enemy) or { buffpresent(reckless_force_buff) or buffstacks(reckless_force_counter) < 10 } and spell(the_unbound_force) or spell(a_murder_of_crows) or target.refreshable(serpent_sting) and not inflighttotarget(serpent_sting) and spell(serpent_sting) or spell(steady_shot)
}
...
AddFunction marksmanshipstmainactions
{
 ...
 if buffpresent(trueshot) and { buffstacks(unerring_vision_buff) > 4 or not hasazeritetrait(unerring_vision_trait) } or target.timetodie() < 11 spell(blood_of_the_enemy)
 #arcane_shot,if=buff.trueshot.up&!buff.memory_of_lucid_dreams.up
 if buffpresent(trueshot) and not buffpresent(memory_of_lucid_dreams) spell(arcane_shot)
 #chimaera_shot,if=buff.trueshot.up&!buff.memory_of_lucid_dreams.up
 if buffpresent(trueshot) and not buffpresent(memory_of_lucid_dreams) spell(chimaera_shot_marksmanship)
 ...
 if buffpresent(trueshot) or { buffexpires(double_tap) or FIXME_ca_active } and buffexpires(precise_shots) or spellfullrecharge(aimed_shot) < casttime(aimed_shot) and spellcooldown(trueshot) > 0 spell(aimed_shot)
 #arcane_shot,if=buff.trueshot.up&buff.memory_of_lucid_dreams.up
 if buffpresent(trueshot) and buffpresent(memory_of_lucid_dreams) spell(arcane_shot)
 #chimaera_shot,if=buff.trueshot.up&buff.memory_of_lucid_dreams.up
 if buffpresent(trueshot) and buffpresent(memory_of_lucid_dreams) spell(chimaera_shot_marksmanship)
 ...
 if focus() + focus() * gcd() < maxfocus() and buffexpires(trueshot) and not target.debuffremaining(concentrated_flame_burn_debuff) and not inflighttotarget(concentrated_flame) or spellfullrecharge(concentrated_flame) < gcd() or target.timetodie() < 5 spell(concentrated_flame)
 #the_unbound_force,if=buff.reckless_force.up|buff.reckless_force_counter.stack<10|target.time_to_die<5
 if buffpresent(reckless_force_buff) or buffstacks(reckless_force_counter) < 10 or target.timetodie() < 5 spell(the_unbound_force)
 ...
 unless spell(kill_shot)
 {
  ...
  unless enemies() > 1 and spell(barrage)
  {
   ...
   unless target.refreshable(serpent_sting) and not inflighttotarget(serpent_sting) and spell(serpent_sting) or { buffexpires(trueshot) or focus() < 35 or focus() < 60 and not hastalent(lethal_shots_talent) or buffremaining(in_the_rhythm) < executetime(rapid_fire) } and spell(rapid_fire) or { buffpresent(trueshot) and { buffstacks(unerring_vision_buff) > 4 or not hasazeritetrait(unerring_vision_trait) } or target.timetodie() < 11 } and spell(blood_of_the_enemy)
   {
    ...
    unless buffpresent(trueshot) and not buffpresent(memory_of_lucid_dreams) and spell(arcane_shot) or buffpresent(trueshot) and not buffpresent(memory_of_lucid_dreams) and spell(chimaera_shot_marksmanship) or { buffpresent(trueshot) or { buffexpires(double_tap) or FIXME_ca_active } and buffexpires(precise_shots) or spellfullrecharge(aimed_shot) < casttime(aimed_shot) and spellcooldown(trueshot) > 0 } and spell(aimed_shot) or buffpresent(trueshot) and buffpresent(memory_of_lucid_dreams) and spell(arcane_shot) or buffpresent(trueshot) and buffpresent(memory_of_lucid_dreams) and spell(chimaera_shot_marksmanship)
    {
...
{
 spell(kill_shot) or spell(explosive_shot) or enemies() > 1 and spell(barrage) or spell(a_murder_of_crows) or checkboxon(opt_volley) and spell(volley) or target.refreshable(serpent_sting) and not inflighttotarget(serpent_sting) and spell(serpent_sting) or { buffexpires(trueshot) or focus() < 35 or focus() < 60 and not hastalent(lethal_shots_talent) or buffremaining(in_the_rhythm) < executetime(rapid_fire) } and spell(rapid_fire) or { buffpresent(trueshot) and { buffstacks(unerring_vision_buff) > 4 or not hasazeritetrait(unerring_vision_trait) } or target.timetodie() < 11 } and spell(blood_of_the_enemy) or { not buffpresent(trueshot) or target.timetodie() < 5 } and spell(focused_azerite_beam) or buffpresent(trueshot) and not buffpresent(memory_of_lucid_dreams) and spell(arcane_shot) or buffpresent(trueshot) and not buffpresent(memory_of_lucid_dreams) and spell(chimaera_shot_marksmanship) or { buffpresent(trueshot) or { buffexpires(double_tap) or FIXME_ca_active } and buffexpires(precise_shots) or spellfullrecharge(aimed_shot) < casttime(aimed_shot) and spellcooldown(trueshot) > 0 } and spell(aimed_shot) or buffpresent(trueshot) and buffpresent(memory_of_lucid_dreams) and spell(arcane_shot) or buffpresent(trueshot) and buffpresent(memory_of_lucid_dreams) and spell(chimaera_shot_marksmanship) or { not buffpresent(trueshot) or target.timetodie() < 8 } and spell(purifying_blast) or { focus() + focus() * gcd() < maxfocus() and buffexpires(trueshot) and not target.debuffremaining(concentrated_flame_burn_debuff) and not inflighttotarget(concentrated_flame) or spellfullrecharge(concentrated_flame) < gcd() or target.timetodie() < 5 } and spell(concentrated_flame) or { buffpresent(reckless_force_buff) or buffstacks(reckless_force_counter) < 10 or target.timetodie() < 5 } and spell(the_unbound_force) or buffexpires(trueshot) and { buffpresent(precise_shots) and focus() > 55 or focus() > 75 or target.timetodie() < 5 } and spell(arcane_shot) or buffexpires(trueshot) and { buffpresent(precise_shots) and focus() > 55 or focus() > 75 or target.timetodie() < 5 } and spell(chimaera_shot_marksmanship) or spell(steady_shot)
}
...
AddFunction marksmanshipcdsmainactions
{
 ...
 if { buffremaining(trinket_azsharas_font_of_power_cooldown_buff) > 20 or not hasequippeditem(azsharas_font_of_power_item) or target.timetodie() < FIXME_trinket.azsharas_font_of_power.cooldown.duration + 34 and target.healthpercent() > 20 } and { spellcooldown(trueshot) < 3 or azeriteessenceisminor(vision_of_perfection_essence_id) and target.timetodie() > spellcooldown(worldvein_resonance) + baseduration(worldvein_resonance_buff) } or target.timetodie() < 20 spell(worldvein_resonance)
...
AddFunction marksmanshipcdscdactions
{
 unless target.debuffexpires(hunters_mark) and not buffpresent(trueshot) and spell(hunters_mark) or { spellcooldown(rapid_fire) < gcd() or spellcooldown(rapid_fire) < spellcooldown(aimed_shot) or target.timetodie() < 20 } and spell(double_tap) or { previousgcdspell(trueshot) and { target.timetodie() > spellcooldownduration(berserking) + baseduration(berserking) or target.healthpercent() < 20 or not hastalent(careful_aim_talent) } or target.timetodie() < 13 } and spell(berserking)
 {
  ...
  unless buffexpires(trueshot) and spell(bag_of_tricks) or buffexpires(trueshot) and { target.healthpercent() > 80 or target.healthpercent() <= 20 or target.timetohealthpercent(20) > 30 } and spell(reaping_flames) or { { buffremaining(trinket_azsharas_font_of_power_cooldown_buff) > 20 or not hasequippeditem(azsharas_font_of_power_item) or target.timetodie() < FIXME_trinket.azsharas_font_of_power.cooldown.duration + 34 and target.healthpercent() > 20 } and { spellcooldown(trueshot) < 3 or azeriteessenceisminor(vision_of_perfection_essence_id) and target.timetodie() > spellcooldown(worldvein_resonance) + baseduration(worldvein_resonance_buff) } or target.timetodie() < 20 } and spell(worldvein_resonance)
  {
   ...
   if { FIXME_ca_active or target.timetodie() > spellcooldown(guardian_of_azeroth) + 30 } and { buffpresent(trueshot) or spellcooldown(trueshot) < 16 } or target.timetodie() < 31 spell(guardian_of_azeroth)
...
{
 target.debuffexpires(hunters_mark) and not buffpresent(trueshot) and spell(hunters_mark) or { spellcooldown(rapid_fire) < gcd() or spellcooldown(rapid_fire) < spellcooldown(aimed_shot) or target.timetodie() < 20 } and spell(double_tap) or { previousgcdspell(trueshot) and { target.timetodie() > spellcooldownduration(berserking) + baseduration(berserking) or target.healthpercent() < 20 or not hastalent(careful_aim_talent) } or target.timetodie() < 13 } and spell(berserking) or buffexpires(trueshot) and spell(bag_of_tricks) or buffexpires(trueshot) and { target.healthpercent() > 80 or target.healthpercent() <= 20 or target.timetohealthpercent(20) > 30 } and spell(reaping_flames) or { { buffremaining(trinket_azsharas_font_of_power_cooldown_buff) > 20 or not hasequippeditem(azsharas_font_of_power_item) or target.timetodie() < FIXME_trinket.azsharas_font_of_power.cooldown.duration + 34 and target.healthpercent() > 20 } and { spellcooldown(trueshot) < 3 or azeriteessenceisminor(vision_of_perfection_essence_id) and target.timetodie() > spellcooldown(worldvein_resonance) + baseduration(worldvein_resonance_buff) } or target.timetodie() < 20 } and spell(worldvein_resonance) or spellcooldown(trueshot) < 7 and spell(ripple_in_space) or not buffpresent(trueshot) and spell(memory_of_lucid_dreams) or { buffpresent(trueshot) and buffpresent(bloodlust) or previousgcdspell(trueshot) and target.healthpercent() < 20 or { buffpresent(unbridled_fury) or buffpresent(unbridled_fury) } and target.timetodie() < 61 or target.timetodie() < 26 } and checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1)
}
...
AddFunction marksmanship_defaultcdactions
{
 ...
 if previousgcdspell(trueshot) and { buffpresent(guardian_of_azeroth_buff) or not azeriteessenceismajor(condensed_lifeforce_essence_id) and FIXME_ca_active } or target.debuffexpires(razor_coral) or target.timetodie() < 20 marksmanshipuseitemactions()
...
AddFunction survivalwfimainactions
{
 ...
 if spellfullrecharge(wildfire_bomb) < 1.5 * gcd() and focus() + focuscastingregen(wildfire_bomb) < maxfocus() or FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 spell(wildfire_bomb)
 ...
 if FIXME_next_wi_bomb.shrapnel and target.debuffremaining(serpent_sting) > 5 * gcd() spell(wildfire_bomb)
 ...
 if FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) or FIXME_next_wi_bomb.pheromone or FIXME_next_wi_bomb.shrapnel spell(wildfire_bomb)
}
...
{
 unless spell(kill_shot) or focus() + focuscastingregen(harpoon) < maxfocus() and hastalent(terms_of_engagement_talent) and checkboxon(opt_harpoon) and spell(harpoon) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(mongoose_bite) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(raptor_strike) or { buffpresent(vipers_venom_buff) and buffremaining(vipers_venom_buff) < 1.5 * gcd() or not target.debuffpresent(serpent_sting) } and spell(serpent_sting) or { spellfullrecharge(wildfire_bomb) < 1.5 * gcd() and focus() + focuscastingregen(wildfire_bomb) < maxfocus() or FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 } and spell(wildfire_bomb) or focus() + focuscastingregen(kill_command) < maxfocus() - focus() and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command)
 {
  #a_murder_of_crows
  spell(a_murder_of_crows)
...
{
 spell(kill_shot) or focus() + focuscastingregen(harpoon) < maxfocus() and hastalent(terms_of_engagement_talent) and checkboxon(opt_harpoon) and spell(harpoon) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(mongoose_bite) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(raptor_strike) or { buffpresent(vipers_venom_buff) and buffremaining(vipers_venom_buff) < 1.5 * gcd() or not target.debuffpresent(serpent_sting) } and spell(serpent_sting) or { spellfullrecharge(wildfire_bomb) < 1.5 * gcd() and focus() + focuscastingregen(wildfire_bomb) < maxfocus() or FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 } and spell(wildfire_bomb) or focus() + focuscastingregen(kill_command) < maxfocus() - focus() and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or spellfullrecharge(wildfire_bomb) < 1.5 * gcd() and spell(wildfire_bomb) or buffpresent(vipers_venom_buff) and target.debuffremaining(serpent_sting) < 4 * gcd() and spell(serpent_sting) or { target.debuffpresent(shrapnel_bomb_debuff) or buffstacks(mongoose_fury) == 5 } and spell(mongoose_bite) or FIXME_next_wi_bomb.shrapnel and target.debuffremaining(serpent_sting) > 5 * gcd() and spell(wildfire_bomb) or target.refreshable(serpent_sting) and spell(serpent_sting) or not buffpresent(mongoose_fury) and spell(chakrams) or spell(mongoose_bite) or spell(raptor_strike) or buffpresent(vipers_venom_buff) and spell(serpent_sting) or { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) or FIXME_next_wi_bomb.pheromone or FIXME_next_wi_bomb.shrapnel } and spell(wildfire_bomb)
}
...
AddFunction survivalcleavemainactions
{
 ...
 if not hastalent(guerrilla_tactics_talent) or spellfullrecharge(wildfire_bomb) < gcd() or FIXME_raid_event.adds.remains < 6 and false(raid_event_adds_exists) spell(wildfire_bomb)
 ...
 if charges(butchery count=0) > 2.5 or target.debuffpresent(shrapnel_bomb_debuff) or spellcooldown(wildfire_bomb) > enemies() - gcd() or target.debuffpresent(blood_of_the_enemy) or FIXME_raid_event.adds.remains < 5 and false(raid_event_adds_exists) spell(butchery)
 ...
 if target.refreshable(serpent_sting) and buffstacks(tip_of_the_spear) < 3 and FIXME_next_wi_bomb.volatile or target.refreshable(serpent_sting) and azeritetraitrank(latent_poison_trait) > 0 spell(serpent_sting)
...
AddFunction survivalcleaveshortcdactions
{
 unless azeritetraitrank(blur_of_talons_trait) > 0 and buffpresent(coordinated_assault) and { buffremaining(coordinated_assault) < 1.5 * gcd() or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < 1.5 * gcd() or buffpresent(coordinated_assault) and not buffpresent(blur_of_talons_buff) } and spell(mongoose_bite) or target.debuffstacks(latent_poison) > { enemies() or 9 } and target.timetodie() < enemies() * gcd() and spell(mongoose_bite)
 {
  ...
  unless { target.debuffpresent(shrapnel_bomb_debuff) and not hastalent(hydras_bite_talent) or target.debuffpresent(shrapnel_bomb_debuff) and enemies() > 5 } and spell(carve) or { not hastalent(guerrilla_tactics_talent) or spellfullrecharge(wildfire_bomb) < gcd() or FIXME_raid_event.adds.remains < 6 and false(raid_event_adds_exists) } and spell(wildfire_bomb) or { charges(butchery count=0) > 2.5 or target.debuffpresent(shrapnel_bomb_debuff) or spellcooldown(wildfire_bomb) > enemies() - gcd() or target.debuffpresent(blood_of_the_enemy) or FIXME_raid_event.adds.remains < 5 and false(raid_event_adds_exists) } and spell(butchery) or target.debuffstacks(latent_poison) > 8 and spell(mongoose_bite) or spell(chakrams) or focus() + focuscastingregen(kill_command) < maxfocus() and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or hastalent(terms_of_engagement_talent) and checkboxon(opt_harpoon) and spell(harpoon) or hastalent(guerrilla_tactics_talent) and spell(carve) or spellcooldown(wildfire_bomb) > { enemies() or 5 } and spell(butchery)
  {
   #flanking_strike,if=focus+cast_regen<focus.max
   if focus() + focuscastingregen(flanking_strike) < maxfocus() spell(flanking_strike)
...
AddFunction survivalcdsmainactions
{
 ...
 if { FIXME_raid_event.adds.remains > 90 or not false(raid_event_adds_exists) or enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and focus() < maxfocus() spell(blood_of_the_enemy)
 #ripple_in_space
 spell(ripple_in_space)
...
AddFunction survivalcdsshortcdactions
{
 unless { spellcooldown(coordinated_assault) > 60 or target.timetodie() < 13 } and spell(berserking) or { buffpresent(guardian_of_azeroth_buff) and { buffpresent(berserking_buff) or buffpresent(blood_fury) or not race(troll) } or buffpresent(unbridled_fury) and target.timetodie() < 61 or target.timetodie() < 26 or not azeriteessenceismajor(condensed_lifeforce_essence_id) and buffpresent(coordinated_assault) } and checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1)
 {
  ...
  if 600 > 90 and focus() < maxfocus() - 25 or { enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and { buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) > 3 * gcd() or not buffpresent(blur_of_talons_buff) } spell(focused_azerite_beam)

  unless { FIXME_raid_event.adds.remains > 90 or not false(raid_event_adds_exists) or enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and focus() < maxfocus() and spell(blood_of_the_enemy)
  {
   ...
   if { FIXME_raid_event.adds.remains > 60 or not false(raid_event_adds_exists) or enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and focus() < maxfocus() spell(purifying_blast)
...
AddFunction survivalcdscdactions
{
 ...
 unless { spellcooldown(coordinated_assault) > 60 or target.timetodie() < 13 } and spell(berserking) or { buffpresent(guardian_of_azeroth_buff) and { buffpresent(berserking_buff) or buffpresent(blood_fury) or not race(troll) } or buffpresent(unbridled_fury) and target.timetodie() < 61 or target.timetodie() < 26 or not azeriteessenceismajor(condensed_lifeforce_essence_id) and buffpresent(coordinated_assault) } and checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1) or target.distance() >= 6 and spell(aspect_of_the_eagle)
 {
  ...
  unless { 600 > 90 and focus() < maxfocus() - 25 or { enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and { buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) > 3 * gcd() or not buffpresent(blur_of_talons_buff) } } and spell(focused_azerite_beam) or { FIXME_raid_event.adds.remains > 90 or not false(raid_event_adds_exists) or enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and focus() < maxfocus() and spell(blood_of_the_enemy) or { FIXME_raid_event.adds.remains > 60 or not false(raid_event_adds_exists) or enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and focus() < maxfocus() and spell(purifying_blast)
  {
   #guardian_of_azeroth
   spell(guardian_of_azeroth)
  }
...
{
 { spellcooldown(coordinated_assault) > 60 or target.timetodie() < 13 } and spell(berserking) or { buffpresent(guardian_of_azeroth_buff) and { buffpresent(berserking_buff) or buffpresent(blood_fury) or not race(troll) } or buffpresent(unbridled_fury) and target.timetodie() < 61 or target.timetodie() < 26 or not azeriteessenceismajor(condensed_lifeforce_essence_id) and buffpresent(coordinated_assault) } and checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1) or target.distance() >= 6 and spell(aspect_of_the_eagle) or { 600 > 90 and focus() < maxfocus() - 25 or { enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and { buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) > 3 * gcd() or not buffpresent(blur_of_talons_buff) } } and spell(focused_azerite_beam) or { FIXME_raid_event.adds.remains > 90 or not false(raid_event_adds_exists) or enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and focus() < maxfocus() and spell(blood_of_the_enemy) or { FIXME_raid_event.adds.remains > 60 or not false(raid_event_adds_exists) or enemies() > 1 and not hastalent(birds_of_prey_talent) or enemies() > 2 } and focus() < maxfocus() and spell(purifying_blast) or spell(ripple_in_space) or spellfullrecharge(concentrated_flame) < 1 * gcd() and spell(concentrated_flame) or buffpresent(reckless_force_buff) and spell(the_unbound_force) or spell(worldvein_resonance) or { target.healthpercent() > 80 or target.healthpercent() <= 20 or target.timetohealthpercent(20) > 30 } and spell(reaping_flames) or azeriteessenceismajor(memory_of_lucid_dreams_essence_id) and target.refreshable(serpent_sting) and buffpresent(vipers_venom_buff) and not spellcooldown(memory_of_lucid_dreams) > 0 and spell(serpent_sting) or azeriteessenceismajor(memory_of_lucid_dreams_essence_id) and not spellcooldown(memory_of_lucid_dreams) > 0 and spell(mongoose_bite) or azeriteessenceismajor(memory_of_lucid_dreams_essence_id) and spellfullrecharge(wildfire_bomb) < 1.5 * gcd() and focus() < powercost(mongoose_bite) and not spellcooldown(memory_of_lucid_dreams) > 0 and spell(wildfire_bomb) or focus() < powercost(mongoose_bite) and buffpresent(coordinated_assault) and spell(memory_of_lucid_dreams)
}
...
AddFunction survivalapwfimainactions
{
 ...
 #wildfire_bomb,if=full_recharge_time<1.5*gcd|focus+cast_regen<focus.max&(next_wi_bomb.volatile&dot.serpent_sting.ticking&dot.serpent_sting.refreshable|next_wi_bomb.pheromone&!buff.mongoose_fury.up&focus+cast_regen<focus.max-action.kill_command.cast_regen*3)
 if spellfullrecharge(wildfire_bomb) < 1.5 * gcd() or focus() + focuscastingregen(wildfire_bomb) < maxfocus() and { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 } spell(wildfire_bomb)
 #mongoose_bite,if=buff.mongoose_fury.remains&next_wi_bomb.pheromone
 if buffpresent(mongoose_fury) and FIXME_next_wi_bomb.pheromone spell(mongoose_bite)
 ...
 if FIXME_next_wi_bomb.shrapnel and focus() > 30 and target.debuffremaining(serpent_sting) > 5 * gcd() spell(wildfire_bomb)
 #chakrams,if=!buff.mongoose_fury.remains
 if not buffpresent(mongoose_fury) spell(chakrams)
 #serpent_sting,if=refreshable
 if target.refreshable(serpent_sting) spell(serpent_sting)
 ...
 if FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) or FIXME_next_wi_bomb.pheromone or FIXME_next_wi_bomb.shrapnel and focus() > 50 spell(wildfire_bomb)
}
...
AddFunction survivalapwfishortcdactions
{
 unless spell(kill_shot) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(mongoose_bite) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(raptor_strike) or not target.debuffpresent(serpent_sting) and spell(serpent_sting)
 {
  ...
  unless { spellfullrecharge(wildfire_bomb) < 1.5 * gcd() or focus() + focuscastingregen(wildfire_bomb) < maxfocus() and { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 } } and spell(wildfire_bomb) or buffpresent(mongoose_fury) and FIXME_next_wi_bomb.pheromone and spell(mongoose_bite) or spellfullrecharge(kill_command) < 1.5 * gcd() and focus() + focuscastingregen(kill_command) < maxfocus() - 20 and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command)
  {
   #steel_trap,if=focus+cast_regen<focus.max
   if focus() + focuscastingregen(steel_trap) < maxfocus() spell(steel_trap)
  }
...
{
 spell(kill_shot) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(mongoose_bite) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(raptor_strike) or not target.debuffpresent(serpent_sting) and spell(serpent_sting) or { spellfullrecharge(wildfire_bomb) < 1.5 * gcd() or focus() + focuscastingregen(wildfire_bomb) < maxfocus() and { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 } } and spell(wildfire_bomb) or buffpresent(mongoose_fury) and FIXME_next_wi_bomb.pheromone and spell(mongoose_bite) or spellfullrecharge(kill_command) < 1.5 * gcd() and focus() + focuscastingregen(kill_command) < maxfocus() - 20 and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or { buffstacks(tip_of_the_spear) == 3 or target.debuffpresent(shrapnel_bomb_debuff) } and spell(raptor_strike) or target.debuffpresent(shrapnel_bomb_debuff) and spell(mongoose_bite) or FIXME_next_wi_bomb.shrapnel and focus() > 30 and target.debuffremaining(serpent_sting) > 5 * gcd() and spell(wildfire_bomb) or not buffpresent(mongoose_fury) and spell(chakrams) or target.refreshable(serpent_sting) and spell(serpent_sting) or focus() + focuscastingregen(kill_command) < maxfocus() and { buffstacks(mongoose_fury) < 5 or focus() < powercost(mongoose_bite) } and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or spell(raptor_strike) or { buffpresent(mongoose_fury) or focus() > 40 or target.debuffpresent(shrapnel_bomb_debuff) } and spell(mongoose_bite) or { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) or FIXME_next_wi_bomb.pheromone or FIXME_next_wi_bomb.shrapnel and focus() > 50 } and spell(wildfire_bomb)
}

AddFunction survivalapwficdactions
{
 unless spell(kill_shot) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(mongoose_bite) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(raptor_strike) or not target.debuffpresent(serpent_sting) and spell(serpent_sting) or spell(a_murder_of_crows) or { spellfullrecharge(wildfire_bomb) < 1.5 * gcd() or focus() + focuscastingregen(wildfire_bomb) < maxfocus() and { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 } } and spell(wildfire_bomb)
 {
  #coordinated_assault
  spell(coordinated_assault)
 }
...
{
 spell(kill_shot) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(mongoose_bite) or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < gcd() and spell(raptor_strike) or not target.debuffpresent(serpent_sting) and spell(serpent_sting) or spell(a_murder_of_crows) or { spellfullrecharge(wildfire_bomb) < 1.5 * gcd() or focus() + focuscastingregen(wildfire_bomb) < maxfocus() and { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) and target.debuffrefreshable(serpent_sting) or FIXME_next_wi_bomb.pheromone and not buffpresent(mongoose_fury) and focus() + focuscastingregen(wildfire_bomb) < maxfocus() - focuscastingregen(kill_command) * 3 } } and spell(wildfire_bomb) or buffpresent(mongoose_fury) and FIXME_next_wi_bomb.pheromone and spell(mongoose_bite) or spellfullrecharge(kill_command) < 1.5 * gcd() and focus() + focuscastingregen(kill_command) < maxfocus() - 20 and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or focus() + focuscastingregen(steel_trap) < maxfocus() and spell(steel_trap) or { buffstacks(tip_of_the_spear) == 3 or target.debuffpresent(shrapnel_bomb_debuff) } and spell(raptor_strike) or target.debuffpresent(shrapnel_bomb_debuff) and spell(mongoose_bite) or FIXME_next_wi_bomb.shrapnel and focus() > 30 and target.debuffremaining(serpent_sting) > 5 * gcd() and spell(wildfire_bomb) or not buffpresent(mongoose_fury) and spell(chakrams) or target.refreshable(serpent_sting) and spell(serpent_sting) or focus() + focuscastingregen(kill_command) < maxfocus() and { buffstacks(mongoose_fury) < 5 or focus() < powercost(mongoose_bite) } and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or spell(raptor_strike) or { buffpresent(mongoose_fury) or focus() > 40 or target.debuffpresent(shrapnel_bomb_debuff) } and spell(mongoose_bite) or { FIXME_next_wi_bomb.volatile and target.debuffpresent(serpent_sting) or FIXME_next_wi_bomb.pheromone or FIXME_next_wi_bomb.shrapnel and focus() > 50 } and spell(wildfire_bomb)
}
...
{
 unless spell(kill_shot) or buffpresent(coordinated_assault) and { buffremaining(coordinated_assault) < 1.5 * gcd() or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < 1.5 * gcd() } and spell(mongoose_bite) or buffpresent(coordinated_assault) and { buffremaining(coordinated_assault) < 1.5 * gcd() or buffpresent(blur_of_talons_buff) and buffremaining(blur_of_talons_buff) < 1.5 * gcd() } and spell(raptor_strike) or focus() + focuscastingregen(flanking_strike) < maxfocus() and spell(flanking_strike) or spellfullrecharge(kill_command) < 1.5 * gcd() and focus() + focuscastingregen(kill_command) < maxfocus() and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or focus() + focuscastingregen(steel_trap) < maxfocus() and spell(steel_trap) or { focus() + focuscastingregen(wildfire_bomb) < maxfocus() and not target.debuffpresent(wildfire_bomb_debuff) and not buffpresent(memory_of_lucid_dreams) and { spellfullrecharge(wildfire_bomb) < 1.5 * gcd() or not target.debuffpresent(wildfire_bomb_debuff) and not buffpresent(coordinated_assault) or not target.debuffpresent(wildfire_bomb_debuff) and buffstacks(mongoose_fury) < 1 } or target.timetodie() < 18 and not target.debuffpresent(wildfire_bomb_debuff) } and spell(wildfire_bomb) or not target.debuffpresent(serpent_sting) and not buffpresent(coordinated_assault) and spell(serpent_sting) or focus() + focuscastingregen(kill_command) < maxfocus() and { buffstacks(mongoose_fury) < 5 or focus() < powercost(mongoose_bite) } and pet.present() and not pet.isincapacitated() and not pet.isfeared() and not pet.isstunned() and spell(kill_command) or target.refreshable(serpent_sting) and not buffpresent(coordinated_assault) and buffstacks(mongoose_fury) < 5 and spell(serpent_sting) or not buffpresent(coordinated_assault) and spell(a_murder_of_crows)
 {
  #coordinated_assault,if=!buff.coordinated_assault.up
  if not buffpresent(coordinated_assault) spell(coordinated_assault)
 }
