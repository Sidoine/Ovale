...
AddFunction MarksmanshipPatientsniperMainActions
{
 ...
 unless target.TimeToDie() < vuln_window() and Enemies() == 1 and MarksmanshipTargetdieMainPostConditions()
 {
  ...
  if not Talent(sidewinders_talent) and not pooling_for_piercing() and not InFlightToTarget(windburst) and { Focus() > 65 or BuffPresent(trueshot_buff) or 1 / { 100 / { 100 + MeleeAttackSpeedPercent() } } > 1 } Spell(marked_shot)
