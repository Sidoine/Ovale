...
AddFunction ElementalSingletargetMainActions
{
 ...
 if Talent(totem_mastery_talent_elemental) and { TotemRemaining(totem_mastery) < 6 or TotemRemaining(totem_mastery) < BaseDuration(ascendance_elemental_buff) + SpellCooldown(ascendance_elemental) and SpellCooldown(ascendance_elemental) < 15 } and { not BuffPresent(ele_resonance_totem_buff) or InCombat() } Spell(totem_mastery_elemental)
...
AddFunction ElementalPrecombatMainActions
{
 ...
 #snapshot_stats
 #totem_mastery
 if not BuffPresent(ele_resonance_totem_buff) or InCombat() Spell(totem_mastery_elemental)
 #elemental_blast
...
{
 { not BuffPresent(ele_resonance_totem_buff) or InCombat() } and Spell(totem_mastery_elemental) or Spell(elemental_blast)
}

AddFunction ElementalPrecombatCdActions
{
 unless { not BuffPresent(ele_resonance_totem_buff) or InCombat() } and Spell(totem_mastery_elemental)
 {
  #fire_elemental
...
AddFunction ElementalDefaultMainActions
{
 ...
 if Talent(totem_mastery_talent_elemental) and TotemRemaining(totem_mastery) < 2 and { not BuffPresent(ele_resonance_totem_buff) or InCombat() } Spell(totem_mastery_elemental)
...
{
 unless Talent(totem_mastery_talent_elemental) and TotemRemaining(totem_mastery) < 2 and { not BuffPresent(ele_resonance_totem_buff) or InCombat() } and Spell(totem_mastery_elemental)
 {
  #run_action_list,name=aoe,if=active_enemies>2&(spell_targets.chain_lightning>2|spell_targets.lava_beam>2)
...
{
 Talent(totem_mastery_talent_elemental) and TotemRemaining(totem_mastery) < 2 and { not BuffPresent(ele_resonance_totem_buff) or InCombat() } and Spell(totem_mastery_elemental) or Enemies() > 2 and { Enemies() > 2 or Enemies() > 2 } and ElementalAoeShortCdPostConditions() or ElementalSingletargetShortCdPostConditions()
}
...
{
 Talent(totem_mastery_talent_elemental) and TotemRemaining(totem_mastery) < 2 and { not BuffPresent(ele_resonance_totem_buff) or InCombat() } and Spell(totem_mastery_elemental) or Enemies() > 2 and { Enemies() > 2 or Enemies() > 2 } and ElementalAoeCdPostConditions() or ElementalSingletargetCdPostConditions()
}
...
AddFunction EnhancementBuffsMainActions
{
 ...
 if TotemRemaining(totem_mastery) < 2 and { not BuffPresent(enh_resonance_totem_buff) or InCombat() } Spell(totem_mastery_enhancement)
}
