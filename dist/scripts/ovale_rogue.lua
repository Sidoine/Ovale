...
{
 checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1) or { not hasequippeditem(pocketsized_computation_device_item) or not spellcooldownduration(cyclotronic_blast) or message("raid_event.invulnerable.exists is not implemented") } and spell(stealth) or spell(slice_and_dice)
}

AddFunction outlawprecombatcdactions
{
 unless checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1) or 600 > 40 and spell(marked_for_death) or { not hasequippeditem(pocketsized_computation_device_item) or not spellcooldownduration(cyclotronic_blast) or message("raid_event.invulnerable.exists is not implemented") } and spell(stealth) or spell(roll_the_bones) or spell(slice_and_dice)
 {
  ...
  if { not hasequippeditem(pocketsized_computation_device_item) or not spellcooldownduration(cyclotronic_blast) or message("raid_event.invulnerable.exists is not implemented") } and energydeficit() > 1 spell(adrenaline_rush)
  #use_item,name=azsharas_font_of_power
...
AddFunction outlawfinishmainactions
{
 ...
 if buffremaining(slice_and_dice) < message("fight_remains is not implemented") and buffremaining(slice_and_dice) < { 1 + combopoints() } * 1.8 spell(slice_and_dice)
 #dispatch
 spell(dispatch)
}
...
{
 buffremaining(slice_and_dice) < message("fight_remains is not implemented") and buffremaining(slice_and_dice) < { 1 + combopoints() } * 1.8 and spell(slice_and_dice) or spell(dispatch)
}
...
{
 spell(between_the_eyes) or buffremaining(slice_and_dice) < message("fight_remains is not implemented") and buffremaining(slice_and_dice) < { 1 + combopoints() } * 1.8 and spell(slice_and_dice) or spell(dispatch)
}
   ...
   unless blade_flurry_sync() and energy() > 2 and spell(blade_rush) or not stealthed() and ambush_condition() and spell(vanish) or not stealthed() and combopoints() <= 1 and spell(dreadblades)
   {
    ...
    unless not stealthed() and spell(sepsis) or { buffpresent(bloodlust) or buffpresent(adrenaline_rush) } and checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1)
    {
     ...
     unless spell(berserking)
     {
      ...
      if buffpresent(bloodlust) or message("fight_remains is not implemented") <= 20 or combopointsdeficit() <= 2 outlawuseitemactions()
     }
    }
   }
  }
...
{
 if message("conduit.slaughter_scars.enabled is not implemented") and message("conduit.slaughter_scars.rank is not implemented") >= 1 + talentpoints(weaponmaster_talent) 20 + talentpoints(vigor_talent) * 5 + talentpoints(master_of_shadows_talent) * 5 + talentpoints(shadow_focus_talent) * 10 + talentpoints(alacrity_talent) * 20 + 30 * { enemies() >= 3 }
 ...
 unless buffpresent(shuriken_tornado) and combopointsdeficit() <= 2 and subtletyfinishmainpostconditions()
 {
  ...
  unless enemies() == 4 and combopoints() >= 4 and subtletyfinishmainpostconditions()
  {
   ...
   unless combopointsdeficit() <= 1 - { hastalent(deeper_stratagem_talent) and { buffpresent(vanish) or hasazeritetrait(the_first_dance_trait) and enemies() < 3 } } and subtletyfinishmainpostconditions()
   {
    ...
    if not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and message("conduit.slaughter_scars.enabled is not implemented") and message("conduit.slaughter_scars.rank is not implemented") >= 1 + talentpoints(weaponmaster_talent) spell(slaughter)
    unless not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and message("conduit.slaughter_scars.enabled is not implemented") and message("conduit.slaughter_scars.rank is not implemented") >= 1 + talentpoints(weaponmaster_talent) and spellusable(slaughter) and spellcooldown(slaughter) < timetoenergyfor(slaughter)
    {
     ...
     if not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and { azeritetraitrank(perforate_trait) >= 2 and enemies() <= 2 and true(position_back) or buffstacks(perforated_veins_buff) >= 3 and message("conduit.perforated_veins.rank is not implemented") >= 3 - message("conduit.deeper_daggers.enabled is not implemented") * 2 } spell(gloomblade)
     ...
     if not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and buffstacks(perforated_veins_buff) >= 3 and message("conduit.perforated_veins.rank is not implemented") >= 8 - talentpoints(weaponmaster_talent) * 2 spell(backstab)
     #shadowstrike
     spell(shadowstrike)
    }
   }
...
{
 timeincombat() < 1 and spell(slaughter) or { buffpresent(stealth) or buffpresent(vanish) } and spell(shadowstrike) or buffpresent(shuriken_tornado) and combopointsdeficit() <= 2 and subtletyfinishshortcdpostconditions() or enemies() == 4 and combopoints() >= 4 and subtletyfinishshortcdpostconditions() or combopointsdeficit() <= 1 - { hastalent(deeper_stratagem_talent) and { buffpresent(vanish) or hasazeritetrait(the_first_dance_trait) and enemies() < 3 } } and subtletyfinishshortcdpostconditions() or hastalent(secret_technique_talent) and target.debuffremaining(find_weakness) < 1 and enemies() == 2 and target.timetodie() - buffremaining(shadowstrike) > 6 and spell(shadowstrike) or not hastalent(deeper_stratagem_talent) and azeritetraitrank(blade_in_the_shadows_trait) == 3 and enemies() == 3 and spell(shadowstrike) or use_priority_rotation() and { target.debuffremaining(find_weakness) < 1 or hastalent(weaponmaster_talent) and enemies() <= 4 or hasazeritetrait(inevitability_trait) and buffpresent(symbols_of_death) and enemies() <= 3 + hasazeritetrait(blade_in_the_shadows_trait) } and spell(shadowstrike) or enemies() >= 3 and spell(shuriken_storm) or { target.debuffremaining(find_weakness) <= 1 or spellcooldown(symbols_of_death) < 18 and target.debuffremaining(find_weakness) < spellcooldown(symbols_of_death) } and spell(shadowstrike) or not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and message("conduit.slaughter_scars.enabled is not implemented") and message("conduit.slaughter_scars.rank is not implemented") >= 1 + talentpoints(weaponmaster_talent) and spell(slaughter) or not { not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and message("conduit.slaughter_scars.enabled is not implemented") and message("conduit.slaughter_scars.rank is not implemented") >= 1 + talentpoints(weaponmaster_talent) and spellusable(slaughter) and spellcooldown(slaughter) < timetoenergyfor(slaughter) } and { not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and { azeritetraitrank(perforate_trait) >= 2 and enemies() <= 2 and true(position_back) or buffstacks(perforated_veins_buff) >= 3 and message("conduit.perforated_veins.rank is not implemented") >= 3 - message("conduit.deeper_daggers.enabled is not implemented") * 2 } and spell(gloomblade) or not message("runeforge.akaaris_soul_fragment.equipped is not implemented") and buffstacks(perforated_veins_buff) >= 3 and message("conduit.perforated_veins.rank is not implemented") >= 8 - talentpoints(weaponmaster_talent) * 2 and spell(backstab) or spell(shadowstrike) }
}
...
AddFunction subtletycdsmainactions
{
 ...
 if { message("runeforge.mark_of_the_master_assassin.equipped is not implemented") or message("runeforge.deathly_shadows.equipped is not implemented") } and buffpresent(symbols_of_death) and buffpresent(shadow_dance) and buffremaining(master_assassin_buff) == 0 and buffexpires(deathly_shadows_buff) spell(vanish)
 ...
 unless { not stealthed() and buffpresent(slice_and_dice) or azeriteessenceismajor(breath_of_the_dying_essence_id) and timeincombat() >= 2 } and subtletyessencesmainpostconditions()
 {
  ...
  unless not hastalent(shadow_focus_talent)
  {
   ...
   if buffpresent(slice_and_dice) and not target.debuffpresent(serrated_bone_spike_dot_debuff) or message("fight_remains is not implemented") <= 5 spell(serrated_bone_spike)
...
AddFunction subtletycdsshortcdactions
{
 ...
 unless { message("runeforge.mark_of_the_master_assassin.equipped is not implemented") or message("runeforge.deathly_shadows.equipped is not implemented") } and buffpresent(symbols_of_death) and buffpresent(shadow_dance) and buffremaining(master_assassin_buff) == 0 and buffexpires(deathly_shadows_buff) and spell(vanish)
 {
  #call_action_list,name=essences,if=!stealthed.all&buff.slice_and_dice.up|essence.breath_of_the_dying.major&time>=2
  ...
  unless { not stealthed() and buffpresent(slice_and_dice) or azeriteessenceismajor(breath_of_the_dying_essence_id) and timeincombat() >= 2 } and subtletyessencesshortcdpostconditions()
  {
   ...
   unless not hastalent(shadow_focus_talent)
   {
    ...
    unless { buffpresent(slice_and_dice) and not target.debuffpresent(serrated_bone_spike_dot_debuff) or message("fight_remains is not implemented") <= 5 } and spell(serrated_bone_spike)
    {
...
{
 { message("runeforge.mark_of_the_master_assassin.equipped is not implemented") or message("runeforge.deathly_shadows.equipped is not implemented") } and buffpresent(symbols_of_death) and buffpresent(shadow_dance) and buffremaining(master_assassin_buff) == 0 and buffexpires(deathly_shadows_buff) and spell(vanish) or { not stealthed() and buffpresent(slice_and_dice) or azeriteessenceismajor(breath_of_the_dying_essence_id) and timeincombat() >= 2 } and subtletyessencesshortcdpostconditions() or not { not hastalent(shadow_focus_talent) } and { { buffpresent(slice_and_dice) and not target.debuffpresent(serrated_bone_spike_dot_debuff) or message("fight_remains is not implemented") <= 5 } and spell(serrated_bone_spike) or { buffpresent(bloodlust) or buffpresent(symbols_of_death) and { buffpresent(shadow_blades_buff) or spellcooldown(shadow_blades) <= 10 } } and checkboxon(opt_use_consumables) and target.classification(worldboss) and item(disabled_item usable=1) or buffpresent(symbols_of_death) and spell(berserking) }
}

AddFunction subtletycdscdactions
{
 unless not buffpresent(shadow_dance) and buffpresent(shuriken_tornado) and buffremaining(shuriken_tornado) <= 3.5 and spell(shadow_dance) or buffpresent(shuriken_tornado) and buffremaining(shuriken_tornado) <= 3.5 and spell(symbols_of_death) or { message("runeforge.mark_of_the_master_assassin.equipped is not implemented") or message("runeforge.deathly_shadows.equipped is not implemented") } and buffpresent(symbols_of_death) and buffpresent(shadow_dance) and buffremaining(master_assassin_buff) == 0 and buffexpires(deathly_shadows_buff) and spell(vanish)
 {
  #call_action_list,name=essences,if=!stealthed.all&buff.slice_and_dice.up|essence.breath_of_the_dying.major&time>=2
  ...
  unless { not stealthed() and buffpresent(slice_and_dice) or azeriteessenceismajor(breath_of_the_dying_essence_id) and timeincombat() >= 2 } and subtletyessencescdpostconditions()
  {
   ...
   unless not hastalent(shadow_focus_talent)
   {
    unless energy() >= 60 and buffpresent(slice_and_dice) and not spellcooldown(symbols_of_death) > 0 and spellcharges(shadow_dance) >= 1 and spell(shuriken_tornado) or { buffpresent(slice_and_dice) and not target.debuffpresent(serrated_bone_spike_dot_debuff) or message("fight_remains is not implemented") <= 5 } and spell(serrated_bone_spike) or buffpresent(slice_and_dice) and not { not spellcooldown(shadow_blades) > 0 } and { hastalent(enveloping_shadows_talent) or spellcharges(shadow_dance) >= 1 } and { not hastalent(shuriken_tornado_talent) or hastalent(shadow_focus_talent) or spellcooldown(shuriken_tornado) > 2 } and { not azeriteessenceismajor(blood_of_the_enemy_essence_id) or spellcooldown(blood_of_the_enemy) > 2 } and spell(symbols_of_death) or false(raid_event_adds_exists) and { target.timetodie() < combopointsdeficit() or not stealthed() and combopointsdeficit() >= maxcombopoints() } and spell(marked_for_death) or 600 > 30 - 10 and not stealthed() and combopointsdeficit() >= maxcombopoints() and spell(marked_for_death)
    {
...
{
 subtletycdsmainpostconditions() or stealthed() and subtletystealthedmainpostconditions() or use_priority_rotation() and subtletystealth_cdsmainpostconditions() or energydeficit() <= stealth_threshold() and subtletystealth_cdsmainpostconditions() or combopoints() == message("animacharged_cp is not implemented") and subtletyfinishmainpostconditions() or { combopointsdeficit() <= 1 or target.timetodie() <= 1 and combopoints() >= 3 } and subtletyfinishmainpostconditions() or enemies() == 4 and combopoints() >= 4 and subtletyfinishmainpostconditions() or energydeficit() <= stealth_threshold() and subtletybuildmainpostconditions()
}
...
{
 spell(stealth) or subtletycdsshortcdpostconditions() or stealthed() and subtletystealthedshortcdpostconditions() or target.timetodie() > 6 and buffremaining(slice_and_dice) < gcd() and combopoints() >= 4 - { timeincombat() < 10 } * 2 and spell(slice_and_dice) or use_priority_rotation() and subtletystealth_cdsshortcdpostconditions() or energydeficit() <= stealth_threshold() and subtletystealth_cdsshortcdpostconditions() or combopoints() == message("animacharged_cp is not implemented") and subtletyfinishshortcdpostconditions() or { combopointsdeficit() <= 1 or target.timetodie() <= 1 and combopoints() >= 3 } and subtletyfinishshortcdpostconditions() or enemies() == 4 and combopoints() >= 4 and subtletyfinishshortcdpostconditions() or energydeficit() <= stealth_threshold() and subtletybuildshortcdpostconditions()
}
