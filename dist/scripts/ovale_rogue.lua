...
do
    ...
    local desc = "[8.2] Simulationcraft: T24_Rogue_Assassination"
    local code = [[
...
AddFunction AssassinationEssencesCdActions
{
 unless Spell(concentrated_flame_essence)
 {
  ...
  unless { Enemies() >= 2 or 600 > 60 and Energy() < 70 } and Spell(focused_azerite_beam) or { Enemies() >= 2 or 600 > 60 } and Spell(purifying_blast) or Spell(the_unbound_force_essence) or Spell(ripple_in_space_essence) or BuffStacks(lifeblood_buff) < 3 and Spell(worldvein_resonance_essence)
  {
...
{
 Spell(concentrated_flame_essence) or { Enemies() >= 2 or 600 > 60 and Energy() < 70 } and Spell(focused_azerite_beam) or { Enemies() >= 2 or 600 > 60 } and Spell(purifying_blast) or Spell(the_unbound_force_essence) or Spell(ripple_in_space_essence) or BuffStacks(lifeblood_buff) < 3 and Spell(worldvein_resonance_essence)
}
...
AddFunction AssassinationDotMainActions
{
 ...
 unless { not Talent(subterfuge_talent) or not { not SpellCooldown(vanish) > 0 and SpellCooldown(vendetta) <= 4 } } and ComboPointsDeficit() >= 1 + 3 * { HasAzeriteTrait(shrouded_suffocation_trait) and not SpellCooldown(vanish) > 0 } and target.Refreshable(garrote_debuff) and { PersistentMultiplier(garrote_debuff) <= 1 or target.DebuffRemaining(garrote_debuff) <= target.CurrentTickTime(garrote_debuff) and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and { not target.DebuffPresent(exsanguinated) or target.DebuffRemaining(garrote_debuff) <= target.CurrentTickTime(garrote_debuff) * 2 and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and not False(ss_buffed) and target.TimeToDie() - target.DebuffRemaining(garrote_debuff) > 4 and { BuffRemaining(master_assassin_buff) == 0 or not target.DebuffPresent(garrote_debuff) and HasAzeriteTrait(shrouded_suffocation_trait) } and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote)
 {
  ...
  if not skip_cycle_garrote() and True(target_isnot_target) and { not Talent(subterfuge_talent) or not { not SpellCooldown(vanish) > 0 and SpellCooldown(vendetta) <= 4 } } and ComboPointsDeficit() >= 1 + 3 * { HasAzeriteTrait(shrouded_suffocation_trait) and not SpellCooldown(vanish) > 0 } and target.Refreshable(garrote_debuff) and { PersistentMultiplier(garrote_debuff) <= 1 or target.DebuffRemaining(garrote_debuff) <= target.CurrentTickTime(garrote_debuff) and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and { not target.DebuffPresent(exsanguinated) or target.DebuffRemaining(garrote_debuff) <= target.CurrentTickTime(garrote_debuff) * 2 and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and not False(ss_buffed) and target.TimeToDie() - target.DebuffRemaining(garrote_debuff) > 12 and { BuffRemaining(master_assassin_buff) == 0 or not target.DebuffPresent(garrote_debuff) and HasAzeriteTrait(shrouded_suffocation_trait) } Spell(garrote)
  unless not skip_cycle_garrote() and True(target_isnot_target) and { not Talent(subterfuge_talent) or not { not SpellCooldown(vanish) > 0 and SpellCooldown(vendetta) <= 4 } } and ComboPointsDeficit() >= 1 + 3 * { HasAzeriteTrait(shrouded_suffocation_trait) and not SpellCooldown(vanish) > 0 } and target.Refreshable(garrote_debuff) and { PersistentMultiplier(garrote_debuff) <= 1 or target.DebuffRemaining(garrote_debuff) <= target.CurrentTickTime(garrote_debuff) and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and { not target.DebuffPresent(exsanguinated) or target.DebuffRemaining(garrote_debuff) <= target.CurrentTickTime(garrote_debuff) * 2 and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and not False(ss_buffed) and target.TimeToDie() - target.DebuffRemaining(garrote_debuff) > 12 and { BuffRemaining(master_assassin_buff) == 0 or not target.DebuffPresent(garrote_debuff) and HasAzeriteTrait(shrouded_suffocation_trait) } and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote)
  {
   ...
   if not skip_cycle_rupture() and not skip_rupture() and True(target_isnot_target) and ComboPoints() >= 4 and target.Refreshable(rupture_debuff) and { PersistentMultiplier(rupture_debuff) <= 1 or target.DebuffRemaining(rupture_debuff) <= target.CurrentTickTime(rupture_debuff) and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and { not target.DebuffPresent(exsanguinated) or target.DebuffRemaining(rupture_debuff) <= target.CurrentTickTime(rupture_debuff) * 2 and Enemies() >= 3 + HasAzeriteTrait(shrouded_suffocation_trait) } and target.TimeToDie() - target.DebuffRemaining(rupture_debuff) > 4 Spell(rupture)
  }
 }
}
...
AddFunction AssassinationCdsCdActions
{
 ...
 if { BuffPresent(burst_haste_buff any=1) or target.DebuffPresent(vendetta_debuff) } and CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(item_focused_resolve usable=1)
 #use_item,name=azsharas_font_of_power
 AssassinationUseItemActions()
 #use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|debuff.vendetta.remains>10|target.time_to_die<20
 if target.DebuffExpires(razor_coral) or target.DebuffRemaining(vendetta_debuff) > 10 or target.TimeToDie() < 20 AssassinationUseItemActions()
...
do
    ...
    local desc = "[8.2] Simulationcraft: T24_Rogue_Outlaw"
    local code = [[
...
{
 unless Spell(concentrated_flame_essence) or { Enemies() >= 2 or 600 > 60 and not BuffPresent(adrenaline_rush_buff) } and Spell(focused_azerite_beam)
 {
...
{
 Spell(concentrated_flame_essence) or { Enemies() >= 2 or 600 > 60 and not BuffPresent(adrenaline_rush_buff) } and Spell(focused_azerite_beam)
}
...
AddFunction OutlawEssencesCdActions
{
 unless Spell(concentrated_flame_essence)
 {
  ...
  unless { Enemies() >= 2 or 600 > 60 and not BuffPresent(adrenaline_rush_buff) } and Spell(focused_azerite_beam) or { Enemies() >= 2 or 600 > 60 } and Spell(purifying_blast) or Spell(the_unbound_force_essence) or Spell(ripple_in_space_essence) or BuffStacks(lifeblood_buff) < 3 and Spell(worldvein_resonance_essence)
  {
...
{
 Spell(concentrated_flame_essence) or { Enemies() >= 2 or 600 > 60 and not BuffPresent(adrenaline_rush_buff) } and Spell(focused_azerite_beam) or { Enemies() >= 2 or 600 > 60 } and Spell(purifying_blast) or Spell(the_unbound_force_essence) or Spell(ripple_in_space_essence) or BuffStacks(lifeblood_buff) < 3 and Spell(worldvein_resonance_essence)
}
...
AddFunction OutlawCdsCdActions
{
 ...
 if { BuffPresent(burst_haste_buff any=1) or BuffPresent(adrenaline_rush_buff) } and CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(item_unbridled_fury usable=1)
 #use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|buff.adrenaline_rush.up&(target.health.pct<30|target.time_to_die<60)
 if target.DebuffExpires(razor_coral) or BuffPresent(adrenaline_rush_buff) and { target.HealthPercent() < 30 or target.TimeToDie() < 60 } OutlawUseItemActions()
 #blood_fury
 Spell(blood_fury_ap)
 #berserking
 Spell(berserking)
...
do
    ...
    local desc = "[8.2] Simulationcraft: T24_Rogue_Subtlety"
    local code = [[
...
AddFunction SubtletyEssencesCdActions
{
 unless Spell(concentrated_flame_essence)
 {
  ...
  unless { Enemies() >= 2 or 600 > 60 } and not { not SpellCooldown(symbols_of_death) > 0 } and not BuffPresent(symbols_of_death_buff) and EnergyDeficit() >= 30 and Spell(focused_azerite_beam) or { Enemies() >= 2 or 600 > 60 } and Spell(purifying_blast) or Spell(the_unbound_force_essence) or Spell(ripple_in_space_essence) or BuffStacks(lifeblood_buff) < 3 and Spell(worldvein_resonance_essence)
  {
...
{
 Spell(concentrated_flame_essence) or { Enemies() >= 2 or 600 > 60 } and not { not SpellCooldown(symbols_of_death) > 0 } and not BuffPresent(symbols_of_death_buff) and EnergyDeficit() >= 30 and Spell(focused_azerite_beam) or { Enemies() >= 2 or 600 > 60 } and Spell(purifying_blast) or Spell(the_unbound_force_essence) or Spell(ripple_in_space_essence) or BuffStacks(lifeblood_buff) < 3 and Spell(worldvein_resonance_essence)
}
...
AddFunction SubtletyCdsCdActions
{
 ...
 if { BuffPresent(burst_haste_buff any=1) or BuffPresent(symbols_of_death_buff) and { BuffPresent(shadow_blades_buff) or SpellCooldown(shadow_blades) <= 10 } } and CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(item_unbridled_fury usable=1)
 #use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|buff.symbols_of_death.up&(target.health.pct<30|target.time_to_die<60)
 if target.DebuffExpires(razor_coral) or BuffPresent(symbols_of_death_buff) and { target.HealthPercent() < 30 or target.TimeToDie() < 60 } SubtletyUseItemActions()
