...
AddFunction assassinationstealthedmainactions
{
 ...
 unless hasazeritetrait(shrouded_suffocation_trait) and buffpresent(subterfuge) and buffremaining(subterfuge) < 1.3 and not false(ss_buffed) and { spellusable(garrote) and spellcooldown(garrote) < timetoenergyfor(garrote) }
 {
  ...
  unless hastalent(subterfuge_talent) and { target.debuffremaining(garrote) < 12 or persistentmultiplier(garrote) <= 1 } and target.timetodie() - target.debuffremaining(garrote) > 2 and { spellusable(garrote) and spellcooldown(garrote) < timetoenergyfor(garrote) }
  {
   ...
   if hastalent(subterfuge_talent) and hasazeritetrait(shrouded_suffocation_trait) and { enemies() > 1 or not hastalent(exsanguinate_talent) } and target.timetodie() > target.debuffremaining(garrote) and { target.debuffremaining(garrote) < 18 or not false(ss_buffed) } spell(garrote)
   unless hastalent(subterfuge_talent) and hasazeritetrait(shrouded_suffocation_trait) and { enemies() > 1 or not hastalent(exsanguinate_talent) } and target.timetodie() > target.debuffremaining(garrote) and { target.debuffremaining(garrote) < 18 or not false(ss_buffed) } and { spellusable(garrote) and spellcooldown(garrote) < timetoenergyfor(garrote) }
   {
    #pool_resource,for_next=1
...
AddFunction assassinationdotmainactions
{
 ...
 unless target.refreshable(garrote) and combopointsdeficit() >= 1 + 3 * { hasazeritetrait(shrouded_suffocation_trait) and not spellcooldown(vanish) > 0 } and { persistentmultiplier(garrote) <= 1 or target.debuffremaining(garrote) <= target.currentticktime(garrote) and enemies() >= 3 + hasazeritetrait(shrouded_suffocation_trait) } and { not target.debuffpresent(exsanguinated) or target.debuffremaining(garrote) <= target.currentticktime(garrote) * 2 and enemies() >= 3 + hasazeritetrait(shrouded_suffocation_trait) } and not false(ss_buffed) and target.timetodie() - target.debuffremaining(garrote) > 4 and { buffremaining(master_assassin_buff) == 0 or not target.debuffpresent(garrote) and hasazeritetrait(shrouded_suffocation_trait) } and { spellusable(garrote) and spellcooldown(garrote) < timetoenergyfor(garrote) }
 {
  ...
  if not skip_cycle_garrote() and not false(target_is_target) and target.refreshable(garrote) and combopointsdeficit() >= 1 + 3 * { hasazeritetrait(shrouded_suffocation_trait) and not spellcooldown(vanish) > 0 } and { persistentmultiplier(garrote) <= 1 or target.debuffremaining(garrote) <= target.currentticktime(garrote) and enemies() >= 3 + hasazeritetrait(shrouded_suffocation_trait) } and { not target.debuffpresent(exsanguinated) or target.debuffremaining(garrote) <= target.currentticktime(garrote) * 2 and enemies() >= 3 + hasazeritetrait(shrouded_suffocation_trait) } and not false(ss_buffed) and target.timetodie() - target.debuffremaining(garrote) > 12 and { buffremaining(master_assassin_buff) == 0 or not target.debuffpresent(garrote) and hasazeritetrait(shrouded_suffocation_trait) } spell(garrote)
...
AddFunction outlawprecombatshortcdactions
{
 ...
 unless { not hasequippeditem(pocketsized_computation_device_item) or not spellcooldownduration(cyclotronic_blast) or false(raid_event_invulnerable_exists) } and spell(stealth)
 {
...
{
 { not hasequippeditem(pocketsized_computation_device_item) or not spellcooldownduration(cyclotronic_blast) or false(raid_event_invulnerable_exists) } and spell(stealth) or spell(slice_and_dice)
}

AddFunction outlawprecombatcdactions
{
 unless 600 > 40 and buffexpires(marked_for_death_buff) and spell(marked_for_death) or { not hasequippeditem(pocketsized_computation_device_item) or not spellcooldownduration(cyclotronic_blast) or false(raid_event_invulnerable_exists) } and spell(stealth) or spell(roll_the_bones) or spell(slice_and_dice)
 {
  #use_item,name=azsharas_font_of_power
  outlawuseitemactions()
  #use_item,effect_name=cyclotronic_blast,if=!raid_event.invulnerable.exists
  if not false(raid_event_invulnerable_exists) outlawuseitemactions()
 }
}

AddFunction outlawprecombatcdpostconditions
{
 600 > 40 and buffexpires(marked_for_death_buff) and spell(marked_for_death) or { not hasequippeditem(pocketsized_computation_device_item) or not spellcooldownduration(cyclotronic_blast) or false(raid_event_invulnerable_exists) } and spell(stealth) or spell(roll_the_bones) or spell(slice_and_dice)
}
...
{
 not hastalent(nightstalker_talent) and equippedruneforge(tiny_toxic_blade_runeforge) and spell(shiv) or enemies() >= 2 + { hastalent(gloomblade_talent) and azeritetraitrank(perforate_trait) >= 2 and true(position_back) } and spell(shuriken_storm) or spellcharges(serrated_bone_spike count=0) >= 2.75 and spell(serrated_bone_spike) or spell(gloomblade) or spell(backstab)
}
