...
AddFunction balancestmainactions
{
 ...
 if target.refreshable(moonfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(moonfire) and dot_requirements() spell(moonfire)
 ...
 if buffpresent(oneths_perception) and message("buff.starfall.refreshable is not implemented") spell(starfall)
 ...
 if hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and message("buff.starfall.refreshable is not implemented") and { buffremaining(eclipse_lunar) > 6 and message("eclipse.in_lunar is not implemented") and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 spell(starfall)
 ...
 if hastalent(starlord_talent) and { buffpresent(starlord) or astralpower() > 90 } and buffstacks(starlord) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(ca_inc) > 10 or not convoke_condition() and covenant(night_fae) } spell(starsurge)
...
{
 { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(moonfire) and dot_requirements() and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(sunfire) and dot_requirements() and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and astralpower() >= astralpowercost(stellar_flare) and dot_requirements() and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * message("conduit.precise_alignment.enabled is not implemented") } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or buffpresent(oneths_perception) and message("buff.starfall.refreshable is not implemented") and spell(starfall) or buffremaining(starlord) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord) and texture(starlord text=cancel) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and message("buff.starfall.refreshable is not implemented") and { buffremaining(eclipse_lunar) > 6 and message("eclipse.in_lunar is not implemented") and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord) or astralpower() > 90 } and buffstacks(starlord) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(ca_inc) > 10 or not convoke_condition() and covenant(night_fae) } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balancestcdactions
{
 unless { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(moonfire) and dot_requirements() and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(sunfire) and dot_requirements() and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and astralpower() >= astralpowercost(stellar_flare) and dot_requirements() and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits)
  {
   ...
   if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * message("conduit.precise_alignment.enabled is not implemented") } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 22 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)
...
AddFunction balanceprepatch_stmainactions
{
 ...
 if target.refreshable(moonfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) spell(moonfire)
 ...
 if buffremaining(starlord) < 6 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and astralpower() > 90 and buffpresent(starlord) texture(starlord text=cancel)
...
{
 target.refreshable(moonfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { astralpower() > 90 or buffpresent(bloodlust) and buffremaining(bloodlust) < 36 } and not buffpresent(ca_inc_buff) and spell(incarnation) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and save_for_ca_inc() and spell(fury_of_elune) or buffremaining(starlord) < 6 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and astralpower() > 90 and buffpresent(starlord) and texture(starlord text=cancel) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and { buffpresent(ca_inc_buff) or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and hastalent(starlord_talent) and { buffpresent(starlord) or astralpower() > 90 } and buffstacks(starlord) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and spellcooldown(ca_inc) > 7 and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { azeritetraitrank(streaking_stars_trait) and buffremaining(ca_inc_buff) > executetime(starfire) and prev_wrath() or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starfire) or not prev_starfire() } and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } or azeritetraitrank(dawning_sun_trait) > 2 and buffremaining(eclipse_solar) > 5 and not buffremaining(dawning_sun_buff) > executetime(wrath) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balancefallthrumainactions
{
 ...
 if target.debuffremaining(moonfire) > buffremaining(sunfire) spell(sunfire)
 #moonfire
 spell(moonfire)
}
...
{
 not equippedruneforge(balance_of_all_things_runeforge) and spell(starsurge) or target.debuffremaining(moonfire) > buffremaining(sunfire) and spell(sunfire) or spell(moonfire)
}
...
AddFunction balancedreambindermainactions
{
 ...
 if target.refreshable(moonfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) spell(moonfire)
 ...
 if buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * message("conduit.precise_alignment.enabled is not implemented") and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(incarnation)
...
{
 { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits) or { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * message("conduit.precise_alignment.enabled is not implemented") and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balancedreambindercdactions
{
 unless { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits)
  {
   ...
   if buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 4 * message("conduit.precise_alignment.enabled is not implemented") and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 22 or message("interpolated_fight_remains is not implemented") % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)
...
AddFunction balanceboatmainactions
{
 ...
 if target.refreshable(moonfire) and target.timetodie() > 13.5 and astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) spell(moonfire)
 ...
 if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * message("conduit.precise_alignment.enabled is not implemented") } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and message("interpolated_fight_remains is not implemented") > 180 + 20 + 4 * message("conduit.precise_alignment.enabled is not implemented") or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") < 30 + 4 * message("conduit.precise_alignment.enabled is not implemented") } spell(incarnation)
...
AddFunction balanceboatcdactions
{
 ...
 unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and spell(sunfire) or target.refreshable(moonfire) and target.timetodie() > 13.5 and astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { message("eclipse.in_any is not implemented") or message("eclipse.solar_in_1 is not implemented") or message("eclipse.lunar_in_1 is not implemented") } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(ca_inc) > 0 } } } and { spellcooldown(ca_inc) > 30 or astralpower() > 90 and not spellcooldown(ca_inc) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or message("interpolated_fight_remains is not implemented") < 10 } and { target.debuffremaining(adaptive_swarm_damage_debuff) > 4 or not covenant(necrolord) } and spell(fury_of_elune) or { { message("eclipse.lunar_next is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or message("interpolated_fight_remains is not implemented") < 10 } and spell(kindred_spirits)
 {
  ...
  if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 4 * message("conduit.precise_alignment.enabled is not implemented") } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") % 180 < 20 + 4 * message("conduit.precise_alignment.enabled is not implemented") } spell(celestial_alignment)

  unless { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * message("conduit.precise_alignment.enabled is not implemented") } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and message("interpolated_fight_remains is not implemented") > 180 + 20 + 4 * message("conduit.precise_alignment.enabled is not implemented") or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") < 30 + 4 * message("conduit.precise_alignment.enabled is not implemented") } and spell(incarnation)
  {
...
AddFunction balanceaoemainactions
{
 ...
 if message("buff.starfall.refreshable is not implemented") and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } spell(starfall)
 ...
 if equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and message("buff.starfall.refreshable is not implemented") or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } spell(starfall)
 ...
 if not buffpresent(adaptive_swarm) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 spell(adaptive_swarm)
 ...
 if target.refreshable(moonfire) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + buffremaining(moonfire) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) spell(moonfire)
 #force_of_nature,if=ap_check
 if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)
 ...
 if covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and message("buff.starfall.refreshable is not implemented") spell(starfall)
...
{
 unless message("buff.starfall.refreshable is not implemented") and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and message("buff.starfall.refreshable is not implemented") or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and astralpower() >= astralpowercost(sunfire) and message("eclipse.in_any is not implemented") and spell(sunfire) or { not buffpresent(adaptive_swarm) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + buffremaining(moonfire) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { message("interpolated_fight_remains is not implemented") < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } and spell(stellar_flare) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { message("buff.starfall.refreshable is not implemented") or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and message("buff.starfall.refreshable is not implemented") and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.lunar_next is not implemented") or message("eclipse.any_next is not implemented") and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or message("eclipse.in_solar is not implemented") and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire)
 {
  #run_action_list,name=fallthru
  balancefallthrushortcdactions()
 }
...
AddFunction balanceaoecdactions
{
 unless message("buff.starfall.refreshable is not implemented") and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and message("buff.starfall.refreshable is not implemented") or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and astralpower() >= astralpowercost(sunfire) and message("eclipse.in_any is not implemented") and spell(sunfire) or { not buffpresent(adaptive_swarm) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + buffremaining(moonfire) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { message("interpolated_fight_remains is not implemented") < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } and spell(stellar_flare)
  {
   ...
   unless message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { message("buff.starfall.refreshable is not implemented") or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and message("buff.starfall.refreshable is not implemented") and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.lunar_next is not implemented") or message("eclipse.any_next is not implemented") and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or message("eclipse.in_solar is not implemented") and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire)
   {
    #run_action_list,name=fallthru
    balancefallthrucdactions()
   }
  ...
  unless feralcooldownmainpostconditions()
  {
   ...
   unless combopoints() >= 5 - 4cp_bite() and feralfinishermainpostconditions()
   {
    ...
    unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthmainpostconditions()
    {
     ...
     unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0
     {
...
{
 { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthmainpostconditions() or feralcooldownmainpostconditions() or combopoints() >= 5 - 4cp_bite() and feralfinishermainpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthmainpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0 } and { hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonsmainpostconditions() or feralfillermainpostconditions() }
}
...
{
 buffexpires(cat_form) and spell(cat_form) or spell(prowl) or { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthshortcdpostconditions() or feralcooldownshortcdpostconditions() or combopoints() >= 5 - 4cp_bite() and feralfinishershortcdpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthshortcdpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0 } and { hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonsshortcdpostconditions() or { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and spell(rake) or combopoints() == 0 and spell(feral_frenzy) or target.refreshable(moonfire_cat_debuff) and spell(moonfire_cat) or target.refreshable(thrash_cat_debuff) and message("druid.thrash_cat.ticks_gained_on_refresh is not implemented") > thrash_ticks() and spell(thrash_cat) or buffpresent(tigers_fury) and 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies() * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) and spell(brutal_slash) or enemies() > 2 and spell(swipe_cat) or buffpresent(clearcasting) and spell(shred) or feralfillershortcdpostconditions() }
}
...
{
 buffexpires(cat_form) and spell(cat_form) or spell(prowl) or { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthcdpostconditions() or feralcooldowncdpostconditions() or combopoints() >= 5 - 4cp_bite() and feralfinishercdpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthcdpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0 } and { hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonscdpostconditions() or { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and spell(rake) or combopoints() == 0 and spell(feral_frenzy) or target.refreshable(moonfire_cat_debuff) and spell(moonfire_cat) or target.refreshable(thrash_cat_debuff) and message("druid.thrash_cat.ticks_gained_on_refresh is not implemented") > thrash_ticks() and spell(thrash_cat) or buffpresent(tigers_fury) and 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies() * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) and spell(brutal_slash) or enemies() > 2 and spell(swipe_cat) or buffpresent(clearcasting) and spell(shred) or feralfillercdpostconditions() }
}
...
{
 not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not covenant(venthyr) } and spell(incarnation) or { not message("druid.catweave_bear is not implemented") and not message("druid.owlweave_bear is not implemented") or enemies() >= 2 } and spell(empower_bond) or hastalent(brambles_talent) and buffpresent(bear_form) and spell(barkskin) or buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm) and spell(adaptive_swarm) or buffpresent(galactic_guardian) and message("druid.owlweave_bear is not implemented") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian) and message("druid.catweave_bear is not implemented") and enemies() <= 3 or buffpresent(galactic_guardian) and not message("druid.catweave_bear is not implemented") and not message("druid.owlweave_bear is not implemented") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle) or target.debuffstacks(thrash_bear_debuff) > 2 and target.debuffgain(thrash_bear_debuff) <= baseduration(thrash_bear_debuff) and spell(pulverize) or spell(thrash_bear) or spell(maul) or spell(swipe_bear)
}
...
AddFunction guardian_defaultmainactions
{
 ...
 if message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardiancatweavemainactions()

 unless message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavemainpostconditions()
 {
  ...
  unless message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavemainpostconditions()
  {
   ...
   if equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 guardianlycaraomainactions()

   unless equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraomainpostconditions()
   {
    ...
    if equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 guardianlycaracmainactions()

    unless equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaracmainpostconditions()
    {
...
{
 message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavemainpostconditions() or message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavemainpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraomainpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaracmainpostconditions() or guardianbearmainpostconditions()
}
...
AddFunction guardian_defaultshortcdactions
{
 ...
 #run_action_list,name=catweave,if=druid.catweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&dot.moonfire.remains>=gcd+0.5&rage<40&buff.incarnation_guardian_of_ursoc.down&buff.berserk_bear.down&buff.galactic_guardian.down)|(buff.cat_form.up&energy>25)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up&(buff.cat_form.up&energy>20))|(runeforge.oath_of_the_elder_druid.equipped&buff.heart_of_the_wild.remains<10)&(buff.cat_form.up&energy>20)|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
 if message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardiancatweaveshortcdactions()

 unless message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweaveshortcdpostconditions()
 {
  #run_action_list,name=owlweave,if=druid.owlweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&rage<20&buff.incarnation.down&buff.berserk_bear.down)|(buff.moonkin_form.up&dot.sunfire.refreshable)|(buff.moonkin_form.up&buff.heart_of_the_wild.up)|(buff.moonkin_form.up&(buff.eclipse_lunar.up|buff.eclipse_solar.up)&!runeforge.oath_of_the_elder_druid.equipped)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up)|(covenant.night_fae&cooldown.convoke_the_spirits.remains<=1)|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
  ...
  {
   #run_action_list,name=lycarao,if=((runeforge.lycaras_fleeting_glimpse.equipped)&(talent.balance_affinity.enabled)&(buff.lycaras_fleeting_glimpse.up)&(buff.lycaras_fleeting_glimpse.remains<=2))
   if equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 guardianlycaraoshortcdactions()

   unless equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraoshortcdpostconditions()
   {
    #run_action_list,name=lycarac,if=((runeforge.lycaras_fleeting_glimpse.equipped)&(talent.feral_affinity.enabled)&(buff.lycaras_fleeting_glimpse.up)&(buff.lycaras_fleeting_glimpse.remains<=2))
    if equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 guardianlycaracshortcdactions()

    unless equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaracshortcdpostconditions()
    {
     #run_action_list,name=bear
...
AddFunction guardian_defaultcdactions
{
 ...
 unless message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavecdpostconditions()
 {
  #run_action_list,name=owlweave,if=druid.owlweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&rage<20&buff.incarnation.down&buff.berserk_bear.down)|(buff.moonkin_form.up&dot.sunfire.refreshable)|(buff.moonkin_form.up&buff.heart_of_the_wild.up)|(buff.moonkin_form.up&(buff.eclipse_lunar.up|buff.eclipse_solar.up)&!runeforge.oath_of_the_elder_druid.equipped)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up)|(covenant.night_fae&cooldown.convoke_the_spirits.remains<=1)|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
  ...
  {
   #run_action_list,name=lycarao,if=((runeforge.lycaras_fleeting_glimpse.equipped)&(talent.balance_affinity.enabled)&(buff.lycaras_fleeting_glimpse.up)&(buff.lycaras_fleeting_glimpse.remains<=2))
   if equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 guardianlycaraocdactions()

   unless equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraocdpostconditions()
   {
    #run_action_list,name=lycarac,if=((runeforge.lycaras_fleeting_glimpse.equipped)&(talent.feral_affinity.enabled)&(buff.lycaras_fleeting_glimpse.up)&(buff.lycaras_fleeting_glimpse.remains<=2))
    if equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 guardianlycaraccdactions()

    unless equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraccdpostconditions()
    {
     #run_action_list,name=bear
...
{
 message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavecdpostconditions() or message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) or equippedruneforge(oath_of_the_elder_druid_runeforge_guardian) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavecdpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraocdpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge_guardian) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraccdpostconditions() or guardianbearcdpostconditions()
}
...
# lycaras_fleeting_glimpse_runeforge_guardian
# mangle
# maul
# mighty_bash
# moonfire
