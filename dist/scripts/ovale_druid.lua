...
{
 not spellcooldown(celestial_alignment) == 0 or not convoke_condition() and covenant(night_fae)
...
{
 covenant(night_fae) and { buffamount(primordial_arcanic_pulsar) < 240 and { spellcooldown(celestial_alignment) + 10 > fightremains() or spellcooldown(celestial_alignment) + 30 < fightremains() and fightremains() > 130 or buffremaining(celestial_alignment) > 7 } and buffremaining(eclipse_solar) > 10 or fightremains() % 120 < 15 }
 covenant(night_fae) and { buffamount(primordial_arcanic_pulsar) < 240 and { spellcooldown(celestial_alignment) + 10 > fightremains() or spellcooldown(celestial_alignment) + 30 < fightremains() and fightremains() > 130 or buffremaining(celestial_alignment) > 7 } and buffremaining(eclipse_solar) > 10 or fightremains() % 120 < 15 }
...
AddFunction balancestmainactions
{
 ...
 if not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) spell(adaptive_swarm)
 ...
 if buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize) or buffpresent(celestial_alignment) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and buffpresent(eclipse_any) spell(starsurge)
 ...
 if { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(celestial_alignment) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } spell(starsurge)
 ...
 if { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) spell(half_moon)
 ...
 if { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) spell(full_moon)
 ...
 if buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) spell(starfire)
 #wrath
 spell(wrath)
...
AddFunction balancestshortcdactions
{
 unless { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(celestial_alignment) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation)
  {
   ...
   if buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage) or not covenant(necrolord) } and save_for_ca_inc() spell(fury_of_elune)

   unless buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord_buff) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and buffpresent(eclipse_lunar) and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(celestial_alignment) == 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize) or buffpresent(celestial_alignment) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and buffpresent(eclipse_any) } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(celestial_alignment) > 10 or not convoke_condition() and covenant(night_fae) } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(celestial_alignment) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)

    unless { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
...
AddFunction balancestcdactions
{
 unless { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(celestial_alignment) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits)
  {
   ...
   if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 22 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or fightremains() % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)

   unless { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation)
   {
...
AddFunction balancedreambindermainactions
{
 ...
 if buffpresent(kindred_empowerment_energize) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(incarnation)
...
{
 { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } and spell(kindred_spirits) or { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balancedreambindercdactions
{
 unless { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } and spell(kindred_spirits)
  {
   ...
   if buffpresent(kindred_empowerment_energize) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 4 * conduit(precise_alignment_conduit) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 22 or fightremains() % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)

   unless { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation)
   {
    #variable,name=convoke_condition,value=covenant.night_fae&(buff.primordial_arcanic_pulsar.value<240&(cooldown.ca_inc.remains+10>interpolated_fight_remains|cooldown.ca_inc.remains+30<interpolated_fight_remains&interpolated_fight_remains>130|buff.ca_inc.remains>7)&buff.eclipse_solar.remains>10|interpolated_fight_remains%%120<15)
    #variable,name=save_for_ca_inc,value=(!cooldown.ca_inc.ready|!variable.convoke_condition&covenant.night_fae)
    ...
    unless buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
     balancefallthrucdactions()
    }
...
{
 { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } and spell(kindred_spirits) or { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation) or buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath) or balancefallthrucdpostconditions()
}
...
AddFunction balanceboatmainactions
{
 ...
 if { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } and buffpresent(starlord_buff) texture(starlord text=cancel)
 ...
 if not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(celestial_alignment) == 0 and covenant(night_fae) spell(starsurge)
 #adaptive_swarm,target_if=!dot.adaptive_swarm_damage.ticking&!action.adaptive_swarm_damage.in_flight&(!dot.adaptive_swarm_heal.ticking|dot.adaptive_swarm_heal.remains>5)|dot.adaptive_swarm_damage.stack<3&dot.adaptive_swarm_damage.remains<3&dot.adaptive_swarm_damage.ticking
 if not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) spell(adaptive_swarm)
 ...
 if { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) == 0 } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } or fightremains() < 10 spell(kindred_spirits)
 ...
 if covenant(night_fae) and { convoke_desync() or spellcooldown(celestial_alignment) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 spell(starsurge)
 ...
 if fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(celestial_alignment) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(celestial_alignment) or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) spell(starsurge)
...
AddFunction balanceboatshortcdactions
{
 unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(celestial_alignment) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } and not buffpresent(kindred_empowerment_energize) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(celestial_alignment) == 0 } } and spell(stellar_flare)
 {
  ...
  if { buffpresent(eclipse_any) or eclipse() or eclipse() } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(celestial_alignment) > 0 } } } and { spellcooldown(celestial_alignment) > 30 or astralpower() > 90 and not spellcooldown(celestial_alignment) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage) > 4 or not covenant(necrolord) } spell(fury_of_elune)

  unless { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) == 0 } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } or fightremains() < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(celestial_alignment) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(celestial_alignment) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(celestial_alignment) or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon)
  {
   #warrior_of_elune
   spell(warrior_of_elune)
...
{
 { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(celestial_alignment) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } and not buffpresent(kindred_empowerment_energize) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(celestial_alignment) == 0 } } and spell(stellar_flare) or { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) == 0 } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } or fightremains() < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(celestial_alignment) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(celestial_alignment) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(celestial_alignment) or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceboatcdactions
{
 ...
 unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(celestial_alignment) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } and not buffpresent(kindred_empowerment_energize) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(celestial_alignment) == 0 } } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(eclipse_any) or eclipse() or eclipse() } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(celestial_alignment) > 0 } } } and { spellcooldown(celestial_alignment) > 30 or astralpower() > 90 and not spellcooldown(celestial_alignment) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage) > 4 or not covenant(necrolord) } and spell(fury_of_elune) or { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) == 0 } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } or fightremains() < 10 } and spell(kindred_spirits)
 {
  ...
  if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 20 + 4 * conduit(precise_alignment_conduit) } spell(celestial_alignment)

  unless { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation)
  {
...
{
 { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(celestial_alignment) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal) or target.debuffremaining(adaptive_swarm_heal) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) == 0 } and not buffpresent(kindred_empowerment_energize) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(celestial_alignment) == 0 } } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(eclipse_any) or eclipse() or eclipse() } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(celestial_alignment) > 0 } } } and { spellcooldown(celestial_alignment) > 30 or astralpower() > 90 and not spellcooldown(celestial_alignment) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage) > 4 or not covenant(necrolord) } and spell(fury_of_elune) or { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) == 0 } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) == 0 } or fightremains() < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(celestial_alignment) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(celestial_alignment) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(celestial_alignment) or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath) or balancefallthrucdpostconditions()
}
...
AddFunction balanceaoemainactions
{
 ...
 if not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 spell(adaptive_swarm)
 ...
 if { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(incarnation)
...
{
 unless buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)

  unless { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(celestial_alignment) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } and spell(stellar_flare)
  {
   ...
   if buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage) or not covenant(necrolord) or enemies() > 2 } spell(fury_of_elune)
...
{
 buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(celestial_alignment) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } and spell(stellar_flare) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not astralpower() >= astralpowercost(starfire) or { buffremaining(celestial_alignment) < 5 and buffpresent(celestial_alignment) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { eclipse() or eclipse() and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(celestial_alignment) < executetime(starfire) and not is_cleave() and buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(celestial_alignment) > executetime(wrath) } and spell(wrath) or spell(starfire) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceaoecdactions
{
 unless buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  if { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)
  ...
  unless { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(celestial_alignment) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } and spell(stellar_flare)
  {
   ...
   unless buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not astralpower() >= astralpowercost(starfire) or { buffremaining(celestial_alignment) < 5 and buffpresent(celestial_alignment) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { eclipse() or eclipse() and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(celestial_alignment) < executetime(starfire) and not is_cleave() and buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(celestial_alignment) > executetime(wrath) } and spell(wrath) or spell(starfire)
   {
    #run_action_list,name=fallthru
    balancefallthrucdactions()
   }
...
{
 buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(celestial_alignment) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } and spell(stellar_flare) or buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not astralpower() >= astralpowercost(starfire) or { buffremaining(celestial_alignment) < 5 and buffpresent(celestial_alignment) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { eclipse() or eclipse() and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(celestial_alignment) < executetime(starfire) and not is_cleave() and buffremaining(celestial_alignment) < executetime(wrath) and buffpresent(celestial_alignment) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(celestial_alignment) > executetime(wrath) } and spell(wrath) or spell(starfire) or balancefallthrucdpostconditions()
}
...
AddFunction balance_defaultmainactions
{
 ...
 if { not covenant(night_fae) or not { not spellcooldown(convoke_the_spirits) > 0 } } and buffpresent(celestial_alignment) spell(berserking)
...
{
 unless { not covenant(night_fae) or not { not spellcooldown(convoke_the_spirits) > 0 } } and buffpresent(celestial_alignment) and spell(berserking) or level() == 50 and spell(296208)
 {
  #run_action_list,name=aoe,if=variable.is_aoe
...
AddFunction balance_defaultcdactions
{
 ...
 unless { not covenant(night_fae) or not { not spellcooldown(convoke_the_spirits) > 0 } } and buffpresent(celestial_alignment) and spell(berserking)
 {
