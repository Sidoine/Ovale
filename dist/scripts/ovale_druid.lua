...
{
 { buffremaining(timeworn_dreambinder_buff) < gcd() + 0.1 or buffremaining(timeworn_dreambinder_buff) < executetime(starfire) + 0.1 and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") } } and buffpresent(timeworn_dreambinder_buff) and equippedruneforge(timeworn_dreambinder_runeforge)
}

AddFunction aspPerSec
{
 message("eclipse.in_lunar is not implemented") * 8 / executetime(starfire) + { not message("eclipse.in_lunar is not implemented") * 6 / executetime(wrath) + 0.2 / { 100 / { 100 + spellcastspeedpercent() } } }
}
...
{
 buffremaining(timeworn_dreambinder_buff) > gcd() + 0.1 and { message("eclipse.in_both is not implemented") or message("eclipse.in_solar is not implemented") or message("eclipse.lunar_next is not implemented") } or buffremaining(timeworn_dreambinder_buff) > executetime(starfire) + 0.1 and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") } or not buffpresent(timeworn_dreambinder_buff)
}

AddFunction convoke_desync
{
 { message("interpolated_fight_remains is not implemented") - 20 } / 120 > { message("interpolated_fight_remains is not implemented") - 25 - 10 * talentpoints(incarnation_talent) - 4 * conduit(precise_alignment_conduit) } / 180
}

AddFunction save_for_ca_inc
{
 not spellcooldown(ca_inc) == 0 or not convoke_condition() and covenant(night_fae)
 not spellcooldown(ca_inc) == 0
 not spellcooldown(ca_inc) == 0 or not convoke_condition() and covenant(night_fae)
}

AddFunction convoke_condition
{
 covenant(night_fae) and { buffamount(primordial_arcanic_pulsar) < 240 and { spellcooldown(ca_inc) + 10 > message("interpolated_fight_remains is not implemented") or spellcooldown(ca_inc) + 30 < message("interpolated_fight_remains is not implemented") and message("interpolated_fight_remains is not implemented") > 130 or buffremaining(ca_inc_buff) > 7 } and buffremaining(eclipse_solar) > 10 or message("interpolated_fight_remains is not implemented") % 120 < 15 }
 covenant(night_fae) and { buffamount(primordial_arcanic_pulsar) < 240 and { spellcooldown(ca_inc) + 10 > message("interpolated_fight_remains is not implemented") or spellcooldown(ca_inc) + 30 < message("interpolated_fight_remains is not implemented") and message("interpolated_fight_remains is not implemented") > 130 or buffremaining(ca_inc_buff) > 7 } and buffremaining(eclipse_solar) > 10 or message("interpolated_fight_remains is not implemented") % 120 < 15 }
...
{
 { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and { buffremaining(eclipse_solar) > gcd() or buffremaining(eclipse_lunar) > gcd() }
}
...
AddFunction balancestmainactions
{
 ...
 if not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) spell(adaptive_swarm)
 ...
 if { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } spell(kindred_spirits)
 ...
 if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(incarnation)
 ...
 if covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } spell(starsurge)
 ...
 if hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and message("eclipse.in_lunar is not implemented") and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 spell(starfall)
 ...
 if buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and message("eclipse.in_any is not implemented") spell(starsurge)
 ...
 if { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } spell(starsurge)
 ...
 if { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) spell(half_moon)
 ...
 if { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) spell(full_moon)
 ...
 if message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) spell(starfire)
 #wrath
 spell(wrath)
...
{
 unless { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and astralpower() >= astralpowercost(moonfire) and dot_requirements() and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(sunfire) and dot_requirements() and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and astralpower() >= astralpowercost(stellar_flare) and dot_requirements() and spell(stellar_flare)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)

  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation)
  {
   ...
   if message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() spell(fury_of_elune)

   unless buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord_buff) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and message("eclipse.in_lunar is not implemented") and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(ca_inc) > 10 or not convoke_condition() and covenant(night_fae) } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)

    unless { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
...
{
 { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and astralpower() >= astralpowercost(moonfire) and dot_requirements() and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(sunfire) and dot_requirements() and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and astralpower() >= astralpowercost(stellar_flare) and dot_requirements() and spell(stellar_flare) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord_buff) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and message("eclipse.in_lunar is not implemented") and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(ca_inc) > 10 or not convoke_condition() and covenant(night_fae) } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balancestcdactions
{
 unless { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and astralpower() >= astralpowercost(moonfire) and dot_requirements() and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(sunfire) and dot_requirements() and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and astralpower() >= astralpowercost(stellar_flare) and dot_requirements() and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits)
  {
   ...
   if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 22 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)

   unless { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation)
   {
    ...
    unless message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord_buff) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and message("eclipse.in_lunar is not implemented") and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(ca_inc) > 10 or not convoke_condition() and covenant(night_fae) } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
...
{
 { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and astralpower() >= astralpowercost(moonfire) and dot_requirements() and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and astralpower() >= astralpowercost(sunfire) and dot_requirements() and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and astralpower() >= astralpowercost(stellar_flare) and dot_requirements() and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord_buff) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and message("eclipse.in_lunar is not implemented") and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(ca_inc) > 10 or not convoke_condition() and covenant(night_fae) } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrucdpostconditions()
}
...
AddFunction balanceprepatch_stmainactions
{
 ...
 if target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) spell(moonfire)
 #sunfire,target_if=refreshable&target.time_to_die>12,if=(buff.ca_inc.remains>5|!buff.ca_inc.up|astral_power<30)&ap_check
 if target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) spell(sunfire)
 ...
 if target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) spell(stellar_flare)
 #incarnation,if=(astral_power>90|buff.bloodlust.up&buff.bloodlust.remains<36)&!buff.ca_inc.up
 if { astralpower() > 90 or buffpresent(bloodlust) and buffremaining(bloodlust) < 36 } and not buffpresent(ca_inc_buff) spell(incarnation)
 ...
 if { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and { buffpresent(ca_inc_buff) or astralpower() > 90 and message("eclipse.in_any is not implemented") } spell(starsurge)
 ...
 if { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 spell(starsurge)
 #new_moon,if=(buff.eclipse_lunar.up|(charges=2&recharge_time<5)|charges=3)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) spell(new_moon)
 #half_moon,if=(buff.eclipse_lunar.up|(charges=2&recharge_time<5)|charges=3|buff.ca_inc.up)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) spell(half_moon)
 #full_moon,if=(buff.eclipse_lunar.up|(charges=2&recharge_time<5)|charges=3|buff.ca_inc.up)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) spell(full_moon)
 ...
 if azeritetraitrank(streaking_stars_trait) and buffremaining(ca_inc_buff) > executetime(starfire) and prev_wrath() or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starfire) or not prev_starfire() } and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } or azeritetraitrank(dawning_sun_trait) > 2 and buffremaining(eclipse_solar) > 5 and not buffremaining(dawning_sun_buff) > executetime(wrath) spell(starfire)
 #wrath
 spell(wrath)
 #run_action_list,name=fallthru
 balancefallthrumainactions()
...
{
 unless target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)

  unless { astralpower() > 90 or buffpresent(bloodlust) and buffremaining(bloodlust) < 36 } and not buffpresent(ca_inc_buff) and spell(incarnation)
  {
   ...
   if message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and save_for_ca_inc() spell(fury_of_elune)

   unless buffremaining(starlord_buff) < 6 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and { buffpresent(ca_inc_buff) or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and spellcooldown(ca_inc) > 7 and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)

    unless { azeritetraitrank(streaking_stars_trait) and buffremaining(ca_inc_buff) > executetime(starfire) and prev_wrath() or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starfire) or not prev_starfire() } and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } or azeritetraitrank(dawning_sun_trait) > 2 and buffremaining(eclipse_solar) > 5 and not buffremaining(dawning_sun_buff) > executetime(wrath) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
     balancefallthrushortcdactions()
    }
...
{
 target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or { astralpower() > 90 or buffpresent(bloodlust) and buffremaining(bloodlust) < 36 } and not buffpresent(ca_inc_buff) and spell(incarnation) or buffremaining(starlord_buff) < 6 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and { buffpresent(ca_inc_buff) or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and spellcooldown(ca_inc) > 7 and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or { azeritetraitrank(streaking_stars_trait) and buffremaining(ca_inc_buff) > executetime(starfire) and prev_wrath() or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starfire) or not prev_starfire() } and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } or azeritetraitrank(dawning_sun_trait) > 2 and buffremaining(eclipse_solar) > 5 and not buffremaining(dawning_sun_buff) > executetime(wrath) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}

AddFunction balanceprepatch_stcdactions
{
 unless target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  #celestial_alignment,if=(astral_power>90|buff.bloodlust.up&buff.bloodlust.remains<26)&!buff.ca_inc.up
  if { astralpower() > 90 or buffpresent(bloodlust) and buffremaining(bloodlust) < 26 } and not buffpresent(ca_inc_buff) spell(celestial_alignment)

  unless { astralpower() > 90 or buffpresent(bloodlust) and buffremaining(bloodlust) < 36 } and not buffpresent(ca_inc_buff) and spell(incarnation) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and save_for_ca_inc() and spell(fury_of_elune) or buffremaining(starlord_buff) < 6 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and { buffpresent(ca_inc_buff) or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and spellcooldown(ca_inc) > 7 and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { azeritetraitrank(streaking_stars_trait) and buffremaining(ca_inc_buff) > executetime(starfire) and prev_wrath() or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starfire) or not prev_starfire() } and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } or azeritetraitrank(dawning_sun_trait) > 2 and buffremaining(eclipse_solar) > 5 and not buffremaining(dawning_sun_buff) > executetime(wrath) } and spell(starfire) or spell(wrath)
  {
   #run_action_list,name=fallthru
   balancefallthrucdactions()
  }
...
{
 target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 or not buffpresent(ca_inc_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { astralpower() > 90 or buffpresent(bloodlust) and buffremaining(bloodlust) < 36 } and not buffpresent(ca_inc_buff) and spell(incarnation) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and save_for_ca_inc() and spell(fury_of_elune) or buffremaining(starlord_buff) < 6 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and { buffpresent(ca_inc_buff) or astralpower() > 90 and message("eclipse.in_any is not implemented") } and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and spellcooldown(ca_inc) > 7 and spell(starsurge) or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and buffremaining(eclipse_solar) > 7 and message("eclipse.in_solar is not implemented") and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { azeritetraitrank(streaking_stars_trait) and buffremaining(ca_inc_buff) > executetime(starfire) and prev_wrath() or { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starfire) or not prev_starfire() } and { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } or azeritetraitrank(dawning_sun_trait) > 2 and buffremaining(eclipse_solar) > 5 and not buffremaining(dawning_sun_buff) > executetime(wrath) } and spell(starfire) or spell(wrath) or balancefallthrucdpostconditions()
}
...
AddFunction balancedreambindermainactions
{
 ...
 if target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) spell(moonfire)
 ...
 if target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) spell(sunfire)
 ...
 if target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) spell(stellar_flare)
 ...
 if { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 spell(kindred_spirits)
 ...
 if buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(incarnation)
 ...
 if covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } spell(starsurge)
 #new_moon,if=(buff.eclipse_lunar.up|(charges=2&recharge_time<5)|charges=3)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) spell(new_moon)
 #half_moon,if=(buff.eclipse_lunar.up&!covenant.kyrian|(buff.kindred_empowerment_energize.up&covenant.kyrian)|(charges=2&recharge_time<5)|charges=3|buff.ca_inc.up)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) spell(half_moon)
...
{
 unless { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)

  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits) or { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation)
  {
   #fury_of_elune,if=eclipse.in_any&ap_check&(dot.adaptive_swarm_damage.ticking|!covenant.necrolord)&variable.save_for_ca_inc
   if message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() spell(fury_of_elune)

   unless covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)
...
{
 { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits) or { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}

AddFunction balancedreambindercdactions
{
 unless { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  #ravenous_frenzy,if=buff.ca_inc.up
  if buffpresent(ca_inc_buff) spell(ravenous_frenzy)

  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits)
  {
   ...
   if buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 22 or message("interpolated_fight_remains is not implemented") % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)

   unless { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation)
   {
    #variable,name=convoke_condition,value=covenant.night_fae&(buff.primordial_arcanic_pulsar.value<240&(cooldown.ca_inc.remains+10>interpolated_fight_remains|cooldown.ca_inc.remains+30<interpolated_fight_remains&interpolated_fight_remains>130|buff.ca_inc.remains>7)&buff.eclipse_solar.remains>10|interpolated_fight_remains%%120<15)
    #variable,name=save_for_ca_inc,value=(!cooldown.ca_inc.ready|!variable.convoke_condition&covenant.night_fae)
    ...
    unless message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
     balancefallthrucdactions()
    }
...
{
 { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits) or { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") < 32 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrucdpostconditions()
}
...
AddFunction balanceboatmainactions
{
 ...
 if { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) texture(starlord text=cancel)
 ...
 if not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) spell(starsurge)
 #adaptive_swarm,target_if=!dot.adaptive_swarm_damage.ticking&!action.adaptive_swarm_damage.in_flight&(!dot.adaptive_swarm_heal.ticking|dot.adaptive_swarm_heal.remains>5)|dot.adaptive_swarm_damage.stack<3&dot.adaptive_swarm_damage.remains<3&dot.adaptive_swarm_damage.ticking
 if not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) spell(adaptive_swarm)
 ...
 if target.refreshable(sunfire) and target.timetodie() > 16 and astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } spell(sunfire)
 ...
 if target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) spell(moonfire)
 ...
 if target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } spell(stellar_flare)
 ...
 if { message("eclipse.lunar_next is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or message("interpolated_fight_remains is not implemented") < 10 spell(kindred_spirits)
 ...
 if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and message("interpolated_fight_remains is not implemented") > 180 + 20 + 4 * conduit(precise_alignment_conduit) or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") < 30 + 4 * conduit(precise_alignment_conduit) } spell(incarnation)
 ...
 if covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 spell(starsurge)
 ...
 if message("interpolated_fight_remains is not implemented") < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and message("eclipse.in_any is not implemented") and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) spell(starsurge)
 ...
 if message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and message("eclipse.in_lunar is not implemented") or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) spell(starfire)
 #wrath
 spell(wrath)
 #run_action_list,name=fallthru
 balancefallthrumainactions()
...
{
 unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } and spell(stellar_flare)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)
  ...
  if { message("eclipse.in_any is not implemented") or message("eclipse.solar_in_1 is not implemented") or message("eclipse.lunar_in_1 is not implemented") } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(ca_inc) > 0 } } } and { spellcooldown(ca_inc) > 30 or astralpower() > 90 and not spellcooldown(ca_inc) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or message("interpolated_fight_remains is not implemented") < 10 } and { target.debuffremaining(adaptive_swarm_damage_debuff) > 4 or not covenant(necrolord) } spell(fury_of_elune)

  unless { { message("eclipse.lunar_next is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or message("interpolated_fight_remains is not implemented") < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and message("interpolated_fight_remains is not implemented") > 180 + 20 + 4 * conduit(precise_alignment_conduit) or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { message("interpolated_fight_remains is not implemented") < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and message("eclipse.in_any is not implemented") and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon)
  {
   #warrior_of_elune
   spell(warrior_of_elune)

   unless { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and message("eclipse.in_lunar is not implemented") or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath)
   {
    #run_action_list,name=fallthru
    balancefallthrushortcdactions()
   }
...
{
 { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } and spell(stellar_flare) or { { message("eclipse.lunar_next is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or message("interpolated_fight_remains is not implemented") < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and message("interpolated_fight_remains is not implemented") > 180 + 20 + 4 * conduit(precise_alignment_conduit) or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { message("interpolated_fight_remains is not implemented") < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and message("eclipse.in_any is not implemented") and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and message("eclipse.in_lunar is not implemented") or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceboatcdactions
{
 ...
 unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { message("eclipse.in_any is not implemented") or message("eclipse.solar_in_1 is not implemented") or message("eclipse.lunar_in_1 is not implemented") } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(ca_inc) > 0 } } } and { spellcooldown(ca_inc) > 30 or astralpower() > 90 and not spellcooldown(ca_inc) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or message("interpolated_fight_remains is not implemented") < 10 } and { target.debuffremaining(adaptive_swarm_damage_debuff) > 4 or not covenant(necrolord) } and spell(fury_of_elune) or { { message("eclipse.lunar_next is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or message("interpolated_fight_remains is not implemented") < 10 } and spell(kindred_spirits)
 {
  ...
  if { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") % 180 < 20 + 4 * conduit(precise_alignment_conduit) } spell(celestial_alignment)
  ...
  unless { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and message("interpolated_fight_remains is not implemented") > 180 + 20 + 4 * conduit(precise_alignment_conduit) or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation)
  {
   ...
   if { convoke_desync() and message("interpolated_fight_remains is not implemented") > 130 or buffpresent(ca_inc_buff) } and { message("buff.balance_of_all_things_nature.stack_value is not implemented") > 30 or message("buff.balance_of_all_things_arcane.stack_value is not implemented") > 30 } or message("interpolated_fight_remains is not implemented") < 10 spell(convoke_the_spirits)

   unless covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { message("interpolated_fight_remains is not implemented") < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and message("eclipse.in_any is not implemented") and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and message("eclipse.in_lunar is not implemented") or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath)
   {
    #run_action_list,name=fallthru
    balancefallthrucdactions()
   }
...
AddFunction balanceaoemainactions
{
 ...
 if not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 spell(adaptive_swarm)
 ...
 if target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) spell(moonfire)
 ...
 if covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) spell(starfall)
 ...
 if covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) spell(starsurge)
 ...
 if buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } spell(starsurge)
 ...
 if message("eclipse.lunar_next is not implemented") or message("eclipse.any_next is not implemented") and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or message("eclipse.in_solar is not implemented") and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) spell(wrath)
 #starfire
 spell(starfire)
 #run_action_list,name=fallthru
 balancefallthrumainactions()
...
{
 unless buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and astralpower() >= astralpowercost(sunfire) and message("eclipse.in_any is not implemented") and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) and spell(moonfire)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)

  unless { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { message("interpolated_fight_remains is not implemented") < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } and spell(stellar_flare)
  {
   ...
   if message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } spell(fury_of_elune)

   unless buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)

    unless { message("eclipse.lunar_next is not implemented") or message("eclipse.any_next is not implemented") and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or message("eclipse.in_solar is not implemented") and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire)
    {
     #run_action_list,name=fallthru
     balancefallthrushortcdactions()
    }
...
{
 buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and astralpower() >= astralpowercost(sunfire) and message("eclipse.in_any is not implemented") and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { message("interpolated_fight_remains is not implemented") < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } and spell(stellar_flare) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { message("eclipse.lunar_next is not implemented") or message("eclipse.any_next is not implemented") and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or message("eclipse.in_solar is not implemented") and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceaoecdactions
{
 unless buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and astralpower() >= astralpowercost(sunfire) and message("eclipse.in_any is not implemented") and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { message("interpolated_fight_remains is not implemented") < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } and spell(stellar_flare)
  {
   ...
   unless message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.lunar_next is not implemented") or message("eclipse.any_next is not implemented") and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or message("eclipse.in_solar is not implemented") and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire)
   {
    #run_action_list,name=fallthru
    balancefallthrucdactions()
   }
...
{
 buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and astralpower() >= astralpowercost(sunfire) and message("eclipse.in_any is not implemented") and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { spellcooldown(ca_inc) == 0 or enemies() < 3 or { message("eclipse.in_solar is not implemented") or message("eclipse.in_both is not implemented") or message("eclipse.in_lunar is not implemented") and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 7 or message("interpolated_fight_remains is not implemented") % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { message("interpolated_fight_remains is not implemented") < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } and spell(stellar_flare) or message("eclipse.in_any is not implemented") and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and not spellknown(half_moon) and not spellknown(full_moon) and spell(new_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { message("eclipse.in_any is not implemented") and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.lunar_next is not implemented") or message("eclipse.any_next is not implemented") and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or message("eclipse.in_solar is not implemented") and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire) or balancefallthrucdpostconditions()
}
...
{
 hastalent(bloodtalons_talent) and buffexpires(bloodtalons_buff) and feralbloodtalonsshortcdpostconditions() or target.debuffpersistentmultiplier(rake_debuff) < 1.6 and message("druid.rake.ticks_gained_on_refresh is not implemented") > 2 and spell(rake) or spell(shred)
}
...
{
 hastalent(bloodtalons_talent) and buffexpires(bloodtalons_buff) and feralbloodtalonscdpostconditions() or target.debuffpersistentmultiplier(rake_debuff) < 1.6 and message("druid.rake.ticks_gained_on_refresh is not implemented") > 2 and spell(rake) or spell(shred)
}
...
{
 { buffexpires(savage_roar) or buffremaining(savage_roar) < { combopoints() * 5 + 1 } * 0.3 } and spell(savage_roar) or { message("druid.primal_wrath.ticks_gained_on_refresh is not implemented") > rip_ticks() >? best_rip() or enemies() > 3 + 1 * talentpoints(sabertooth_talent) } and spell(primal_wrath) or { not buffpresent(rip) or buffremaining(rip) + combopoints() * talentpoints(sabertooth_talent) < baseduration(rip) * 0.3 or target.debuffpersistentmultiplier(rip) < persistentmultiplier(rip) } and message("druid.rip.ticks_gained_on_refresh is not implemented") > rip_ticks() and spell(rip) or buffpresent(iron_jaws) and spell(maim) or energy() >= energycost(ferocious_bite max=1) and spell(ferocious_bite)
}
...
{
 spell(incarnation) or { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking) or feralessenceshortcdpostconditions()
}
...
AddFunction feralcooldowncdactions
{
 ...
 unless spell(incarnation) or { energydeficit() > 55 or buffpresent(bs_inc_buff) and buffremaining(bs_inc_buff) < 13 } and spell(tigers_fury)
 {
...
{
 spell(incarnation) or { energydeficit() > 55 or buffpresent(bs_inc_buff) and buffremaining(bs_inc_buff) < 13 } and spell(tigers_fury) or { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking) or feralessencecdpostconditions()
}
  ...
  unless feralcooldownmainpostconditions()
  {
   ...
   unless combopoints() >= 5 - 4cp_bite() and feralfinishermainpostconditions()
   {
    ...
    unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthmainpostconditions()
    {
     ...
     unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons_buff) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting_buff) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0
     {
      #run_action_list,name=bloodtalons,if=talent.bloodtalons.enabled&(buff.bloodtalons.down|active_bt_triggers=2)
      if hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } feralbloodtalonsmainactions()

      unless hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonsmainpostconditions()
      {
...
{
 { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthmainpostconditions() or feralcooldownmainpostconditions() or combopoints() >= 5 - 4cp_bite() and feralfinishermainpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthmainpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons_buff) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting_buff) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0 } and { hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonsmainpostconditions() or feralfillermainpostconditions() }
}
   ...
   unless feralcooldownshortcdpostconditions()
   {
    ...
    unless combopoints() >= 5 - 4cp_bite() and feralfinishershortcdpostconditions()
    {
     ...
     unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthshortcdpostconditions()
     {
      ...
      {
       #run_action_list,name=bloodtalons,if=talent.bloodtalons.enabled&(buff.bloodtalons.down|active_bt_triggers=2)
       if hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } feralbloodtalonsshortcdactions()

       unless hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonsshortcdpostconditions() or { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and spell(rake)
       {
...
{
 buffexpires(cat_form) and spell(cat_form) or spell(prowl) or { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthshortcdpostconditions() or feralcooldownshortcdpostconditions() or combopoints() >= 5 - 4cp_bite() and feralfinishershortcdpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthshortcdpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons_buff) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting_buff) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0 } and { hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonsshortcdpostconditions() or { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and spell(rake) or target.refreshable(moonfire_cat_debuff) and spell(moonfire_cat) or target.refreshable(thrash_cat_debuff) and message("druid.thrash_cat.ticks_gained_on_refresh is not implemented") > thrash_ticks() and spell(thrash_cat) or buffpresent(tigers_fury) and 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies() * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) and spell(brutal_slash) or enemies() > 2 and spell(swipe_cat) or buffpresent(clearcasting_buff) and spell(shred) or feralfillershortcdpostconditions() }
}
   ...
   unless feralcooldowncdpostconditions()
   {
    ...
    unless combopoints() >= 5 - 4cp_bite() and feralfinishercdpostconditions()
    {
     ...
     unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthcdpostconditions()
     {
      ...
      {
       #run_action_list,name=bloodtalons,if=talent.bloodtalons.enabled&(buff.bloodtalons.down|active_bt_triggers=2)
       if hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } feralbloodtalonscdactions()

       unless hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonscdpostconditions() or { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and spell(rake) or combopoints() == 0 and spell(feral_frenzy) or target.refreshable(moonfire_cat_debuff) and spell(moonfire_cat) or target.refreshable(thrash_cat_debuff) and message("druid.thrash_cat.ticks_gained_on_refresh is not implemented") > thrash_ticks() and spell(thrash_cat) or buffpresent(tigers_fury) and 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies() * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) and spell(brutal_slash) or enemies() > 2 and spell(swipe_cat) or buffpresent(clearcasting_buff) and spell(shred)
       {
        #call_action_list,name=filler
...
{
 buffexpires(cat_form) and spell(cat_form) or spell(prowl) or { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthcdpostconditions() or feralcooldowncdpostconditions() or combopoints() >= 5 - 4cp_bite() and feralfinishercdpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthcdpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons_buff) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting_buff) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and message("active_bt_triggers is not implemented") == 0 } and { hastalent(bloodtalons_talent) and { buffexpires(bloodtalons_buff) or message("active_bt_triggers is not implemented") == 2 } and feralbloodtalonscdpostconditions() or { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and spell(rake) or combopoints() == 0 and spell(feral_frenzy) or target.refreshable(moonfire_cat_debuff) and spell(moonfire_cat) or target.refreshable(thrash_cat_debuff) and message("druid.thrash_cat.ticks_gained_on_refresh is not implemented") > thrash_ticks() and spell(thrash_cat) or buffpresent(tigers_fury) and 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies() * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) and spell(brutal_slash) or enemies() > 2 and spell(swipe_cat) or buffpresent(clearcasting_buff) and spell(shred) or feralfillercdpostconditions() }
}
...
AddFunction guardianowlweavemainactions
{
 ...
 if target.refreshable(adaptive_swarm_debuff) spell(adaptive_swarm)
...
{
 not buffpresent(moonkin_form) and spell(moonkin_form) or message("druid.owlweave_bear is not implemented") and spell(empower_bond) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or { target.refreshable(moonfire_debuff) or buffpresent(galactic_guardian_buff) } and spell(moonfire) or target.refreshable(sunfire) and spell(sunfire) or { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and spell(starsurge) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.in_lunar is not implemented") and buffpresent(starsurge_empowerment_buff) } and spell(starfire) or spell(wrath)
}
...
{
 not buffpresent(cat_form) and spell(cat_form) or buffpresent(prowl) and spell(rake) or target.debuffrefreshable(rip) and combopoints() >= 4 and spell(rip) or combopoints() >= 4 and spell(ferocious_bite) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or target.debuffrefreshable(rake_debuff) and combopoints() < 4 and spell(rake) or spell(shred)
}
...
{
 not buffpresent(cat_form) and spell(cat_form) or buffpresent(prowl) and spell(rake) or message("druid.catweave_bear is not implemented") and spell(empower_bond) or target.debuffrefreshable(rip) and combopoints() >= 4 and spell(rip) or combopoints() >= 4 and spell(ferocious_bite) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or target.debuffrefreshable(rake_debuff) and combopoints() < 4 and spell(rake) or spell(shred)
}
...
AddFunction guardianbearmainactions
{
 ...
 if buffpresent(ravenous_frenzy) or not covenant(venthyr) spell(incarnation)
 ...
 if buffpresent(galactic_guardian_buff) and message("druid.owlweave_bear is not implemented") and enemies() <= 3 spell(moonfire)
 ...
 if buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and enemies() >= 4 spell(swipe)
 ...
 if buffpresent(galactic_guardian_buff) and message("druid.catweave_bear is not implemented") and enemies() <= 3 or buffpresent(galactic_guardian_buff) and not message("druid.catweave_bear is not implemented") and not message("druid.owlweave_bear is not implemented") and enemies() <= 3 spell(moonfire)
...
{
 unless not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not covenant(venthyr) } and spell(incarnation)
 {
  ...
  unless buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or buffpresent(galactic_guardian_buff) and message("druid.owlweave_bear is not implemented") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire_debuff) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian_buff) and message("druid.catweave_bear is not implemented") and enemies() <= 3 or buffpresent(galactic_guardian_buff) and not message("druid.catweave_bear is not implemented") and not message("druid.owlweave_bear is not implemented") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle)
  {
...
{
 not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not covenant(venthyr) } and spell(incarnation) or buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or buffpresent(galactic_guardian_buff) and message("druid.owlweave_bear is not implemented") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire_debuff) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian_buff) and message("druid.catweave_bear is not implemented") and enemies() <= 3 or buffpresent(galactic_guardian_buff) and not message("druid.catweave_bear is not implemented") and not message("druid.owlweave_bear is not implemented") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle) or spell(thrash_bear) or spell(maul) or spell(swipe_bear)
}
...
AddFunction guardianbearcdactions
{
 unless not buffpresent(bear_form) and spell(bear_form)
 {
  ...
  if buffpresent(ravenous_frenzy) or not covenant(venthyr) spell(berserk)
 }
}

AddFunction guardianbearcdpostconditions
{
 not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not covenant(venthyr) } and spell(incarnation) or { not message("druid.catweave_bear is not implemented") and not message("druid.owlweave_bear is not implemented") or enemies() >= 2 } and spell(empower_bond) or hastalent(brambles_talent) and buffpresent(bear_form) and spell(barkskin) or buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or buffpresent(galactic_guardian_buff) and message("druid.owlweave_bear is not implemented") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire_debuff) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian_buff) and message("druid.catweave_bear is not implemented") and enemies() <= 3 or buffpresent(galactic_guardian_buff) and not message("druid.catweave_bear is not implemented") and not message("druid.owlweave_bear is not implemented") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle) or target.debuffstacks(thrash_bear_debuff) > 2 and target.debuffgain(thrash_bear_debuff) <= baseduration(thrash_bear_debuff) and spell(pulverize) or spell(thrash_bear) or spell(maul) or spell(swipe_bear)
}
...
AddFunction guardian_defaultmainactions
{
 ...
 if message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardiancatweavemainactions()

 unless message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavemainpostconditions()
 {
  ...
  if message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardianowlweavemainactions()

  unless message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavemainpostconditions()
  {
...
{
 message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavemainpostconditions() or message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavemainpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraomainpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaracmainpostconditions() or guardianbearmainpostconditions()
}
...
AddFunction guardian_defaultshortcdactions
{
 ...
 #run_action_list,name=catweave,if=druid.catweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&dot.moonfire.remains>=gcd+0.5&rage<40&buff.incarnation_guardian_of_ursoc.down&buff.berserk_bear.down&buff.galactic_guardian.down)|(buff.cat_form.up&energy>25)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up&(buff.cat_form.up&energy>20))|(runeforge.oath_of_the_elder_druid.equipped&buff.heart_of_the_wild.remains<10)&(buff.cat_form.up&energy>20)|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
 if message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardiancatweaveshortcdactions()

 unless message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweaveshortcdpostconditions()
 {
  #run_action_list,name=owlweave,if=druid.owlweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&rage<20&buff.incarnation.down&buff.berserk_bear.down)|(buff.moonkin_form.up&dot.sunfire.refreshable)|(buff.moonkin_form.up&buff.heart_of_the_wild.up)|(buff.moonkin_form.up&(buff.eclipse_lunar.up|buff.eclipse_solar.up)&!runeforge.oath_of_the_elder_druid.equipped)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up)|(covenant.night_fae&cooldown.convoke_the_spirits.remains<=1)|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
  if message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardianowlweaveshortcdactions()

  unless message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweaveshortcdpostconditions()
  {
   #run_action_list,name=lycarao,if=((runeforge.lycaras_fleeting_glimpse.equipped)&(talent.balance_affinity.enabled)&(buff.lycaras_fleeting_glimpse.up)&(buff.lycaras_fleeting_glimpse.remains<=2))
...
{
 message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweaveshortcdpostconditions() or message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweaveshortcdpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraoshortcdpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaracshortcdpostconditions() or guardianbearshortcdpostconditions()
}
...
AddFunction guardian_defaultcdactions
{
 ...
 #run_action_list,name=catweave,if=druid.catweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&dot.moonfire.remains>=gcd+0.5&rage<40&buff.incarnation_guardian_of_ursoc.down&buff.berserk_bear.down&buff.galactic_guardian.down)|(buff.cat_form.up&energy>25)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up&(buff.cat_form.up&energy>20))|(runeforge.oath_of_the_elder_druid.equipped&buff.heart_of_the_wild.remains<10)&(buff.cat_form.up&energy>20)|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
 if message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardiancatweavecdactions()

 unless message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavecdpostconditions()
 {
  #run_action_list,name=owlweave,if=druid.owlweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&rage<20&buff.incarnation.down&buff.berserk_bear.down)|(buff.moonkin_form.up&dot.sunfire.refreshable)|(buff.moonkin_form.up&buff.heart_of_the_wild.up)|(buff.moonkin_form.up&(buff.eclipse_lunar.up|buff.eclipse_solar.up)&!runeforge.oath_of_the_elder_druid.equipped)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up)|(covenant.night_fae&cooldown.convoke_the_spirits.remains<=1)|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
  if message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardianowlweavecdactions()

  unless message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavecdpostconditions()
  {
   #run_action_list,name=lycarao,if=((runeforge.lycaras_fleeting_glimpse.equipped)&(talent.balance_affinity.enabled)&(buff.lycaras_fleeting_glimpse.up)&(buff.lycaras_fleeting_glimpse.remains<=2))
...
{
 message("druid.catweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavecdpostconditions() or message("druid.owlweave_bear is not implemented") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavecdpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraocdpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraccdpostconditions() or guardianbearcdpostconditions()
}
