...
{
 buffpresent(ca_inc_buff) and spell(ravenous_frenzy) or { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { message("eclipse.in_any is not implemented") or message("eclipse.solar_in_1 is not implemented") or message("eclipse.lunar_in_1 is not implemented") } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(ca_inc) > 0 } } } and { spellcooldown(ca_inc) > 30 or astralpower() > 90 and not spellcooldown(ca_inc) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or message("interpolated_fight_remains is not implemented") < 10 } and { target.debuffremaining(adaptive_swarm_damage_debuff) > 4 or not covenant(necrolord) } and spell(fury_of_elune) or { { message("eclipse.lunar_next is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or message("interpolated_fight_remains is not implemented") < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") % 180 < 20 + 4 * conduit(precise_alignment_conduit) } and spell(celestial_alignment) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and message("interpolated_fight_remains is not implemented") > 180 + 20 + 4 * conduit(precise_alignment_conduit) or message("interpolated_fight_remains is not implemented") < spellcooldown(convoke_the_spirits) + 6 or message("interpolated_fight_remains is not implemented") < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or { { convoke_desync() and message("interpolated_fight_remains is not implemented") > 130 or buffpresent(ca_inc_buff) } and { message("buff.balance_of_all_things_nature.stack_value is not implemented") > 30 or message("buff.balance_of_all_things_arcane.stack_value is not implemented") > 30 } or message("interpolated_fight_remains is not implemented") < 10 } and spell(convoke_the_spirits) or covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { message("interpolated_fight_remains is not implemented") < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and message("eclipse.in_any is not implemented") and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { message("eclipse.in_lunar is not implemented") or message("eclipse.solar_next is not implemented") or message("eclipse.any_next is not implemented") or buffpresent(warrior_of_elune) and message("eclipse.in_lunar is not implemented") or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
{
 unless spell(berserk) or spell(incarnation) or { energydeficit() > 55 or buffpresent(bs_inc_buff) and buffremaining(bs_inc_buff) < 13 } and spell(tigers_fury) or buffpresent(tigers_fury) and buffexpires(bs_inc_buff) and combopoints() < 4 and target.debuffpersistentmultiplier(rake_debuff) < 1.6 and energy() > 40 and spell(shadowmeld) or { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking)
 {
  #call_action_list,name=essence
...
{
 spell(berserk) or spell(incarnation) or { energydeficit() > 55 or buffpresent(bs_inc_buff) and buffremaining(bs_inc_buff) < 13 } and spell(tigers_fury) or buffpresent(tigers_fury) and buffexpires(bs_inc_buff) and combopoints() < 4 and target.debuffpersistentmultiplier(rake_debuff) < 1.6 and energy() > 40 and spell(shadowmeld) or { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking) or feralessenceshortcdpostconditions()
}
...
{
 spell(berserk) or spell(incarnation) or { energydeficit() > 55 or buffpresent(bs_inc_buff) and buffremaining(bs_inc_buff) < 13 } and spell(tigers_fury) or buffpresent(tigers_fury) and buffexpires(bs_inc_buff) and combopoints() < 4 and target.debuffpersistentmultiplier(rake_debuff) < 1.6 and energy() > 40 and spell(shadowmeld) or { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking) or feralessencecdpostconditions()
}
...
{
 checkboxon("catweave_bear") and hastalent(feral_affinity_talent_guardian) and spell(cat_form) or checkboxon("catweave_bear") and hastalent(feral_affinity_talent_guardian) and spell(prowl) or checkboxon("owlweave_bear") and hastalent(balance_affinity_talent) and spell(moonkin_form) or not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") and spell(bear_form) or hastalent(heart_of_the_wild_talent) and checkboxon("owlweave_bear") and spell(heart_of_the_wild) or checkboxon("owlweave_bear") and spell(wrath)
}
...
{
 not buffpresent(cat_form) and spell(cat_form) or buffpresent(prowl) and spell(rake) or hastalent(heart_of_the_wild_talent) and not buffpresent(heart_of_the_wild) and spell(heart_of_the_wild) or checkboxon("catweave_bear") and spell(empower_bond) or checkboxon("catweave_bear") and spell(convoke_the_spirits) or target.debuffrefreshable(rip) and combopoints() >= 4 and spell(rip) or combopoints() >= 4 and spell(ferocious_bite) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or target.debuffrefreshable(rake_debuff) and combopoints() < 4 and spell(rake) or spell(shred)
}
...
AddFunction guardianbearcdactions
{
 unless not buffpresent(bear_form) and spell(bear_form)
 {
  ...
  if { buffpresent(berserk) or buffpresent(incarnation_guardian_of_ursoc) } and not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") and { checkboxon(opt_use_consumables) and target.classification(worldboss) } item(superior_battle_potion_of_agility_item usable=1)
 }
}
...
AddFunction guardian_defaultmainactions
{
 ...
 unless checkboxon("catweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavemainpostconditions()
 {
  ...
  if checkboxon("owlweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardianowlweavemainactions()

  unless checkboxon("owlweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavemainpostconditions()
  {
...
{
 checkboxon("catweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk) and buffexpires(galactic_guardian_buff) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or equippedruneforge(oath_of_the_elder_druid_runeforge) and buffremaining(heart_of_the_wild) < 10 and buffpresent(cat_form) and energy() > 20 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavemainpostconditions() or checkboxon("owlweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 20 and buffexpires(incarnation) and buffexpires(berserk) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or buffpresent(moonkin_form) and { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and not equippedruneforge(oath_of_the_elder_druid_runeforge) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or covenant(night_fae) and spellcooldown(convoke_the_spirits) <= 1 or covenant(kyrian) and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavemainpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(balance_affinity_talent) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaraomainpostconditions() or equippedruneforge(lycaras_fleeting_glimpse_runeforge) and hastalent(feral_affinity_talent_guardian) and buffpresent(lycaras_fleeting_glimpse) and buffremaining(lycaras_fleeting_glimpse) <= 2 and guardianlycaracmainpostconditions() or guardianbearmainpostconditions()
}
