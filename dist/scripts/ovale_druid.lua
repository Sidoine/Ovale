...
{
 { buffremaining(timeworn_dreambinder_buff) < gcd() + 0.1 or buffremaining(timeworn_dreambinder_buff) < executetime(starfire) + 0.1 and { buffpresent(eclipse_lunar) or eclipse() or eclipse() } } and buffpresent(timeworn_dreambinder_buff) and equippedruneforge(timeworn_dreambinder_runeforge)
}
...
{
 buffremaining(timeworn_dreambinder_buff) > gcd() + 0.1 and { buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) or eclipse() } or buffremaining(timeworn_dreambinder_buff) > executetime(starfire) + 0.1 and { buffpresent(eclipse_lunar) or eclipse() or eclipse() } or not buffpresent(timeworn_dreambinder_buff)
}

AddFunction convoke_desync
{
 { fightremains() - 20 } / 120 > { fightremains() - 25 - 10 * talentpoints(incarnation_talent) - 4 * conduit(precise_alignment_conduit) } / 180
}
...
{
 covenant(night_fae) and { buffamount(primordial_arcanic_pulsar) < 240 and { spellcooldown(ca_inc) + 10 > fightremains() or spellcooldown(ca_inc) + 30 < fightremains() and fightremains() > 130 or buffremaining(ca_inc_buff) > 7 } and buffremaining(eclipse_solar) > 10 or fightremains() % 120 < 15 }
 covenant(night_fae) and { buffamount(primordial_arcanic_pulsar) < 240 and { spellcooldown(ca_inc) + 10 > fightremains() or spellcooldown(ca_inc) + 30 < fightremains() and fightremains() > 130 or buffremaining(ca_inc_buff) > 7 } and buffremaining(eclipse_solar) > 10 or fightremains() % 120 < 15 }
 buffamount(primordial_arcanic_pulsar) < 250 - astralpower() and { spellcooldown(ca_inc) + 10 > fightremains() or spellcooldown(ca_inc) + 30 < fightremains() and fightremains() > 130 or buffremaining(ca_inc_buff) > 7 } and buffpresent(eclipse_any) or fightremains() % 120 < 15
}
...
AddFunction balancestmainactions
{
 ...
 if buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and buffpresent(eclipse_any) spell(starsurge)
 ...
 if { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } spell(starsurge)
 ...
 if buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) spell(starfire)
 #wrath
 spell(wrath)
...
AddFunction balancestshortcdactions
{
 unless { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(ca_inc) == 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + 4 * conduit(precise_alignment_conduit) } and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 + 9 * { buffamount(primordial_arcanic_pulsar) > 100 } or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation)
  {
   ...
   if buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() spell(fury_of_elune)

   unless buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord_buff) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord_buff) and texture(starlord text=cancel) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and buffpresent(eclipse_lunar) and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage_debuff) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(ca_inc) == 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize_buff) or buffpresent(ca_inc_buff) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) == 0 or not covenant(venthyr) } or astralpower() > 90 and buffpresent(eclipse_any) } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord_buff) or astralpower() > 90 } and buffstacks(starlord_buff) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(ca_inc) > 10 or not convoke_condition() and covenant(night_fae) } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 and { spellcooldown(kindred_spirits) > 7 or not covenant(kyrian) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)

    unless { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
...
AddFunction balanceprepatch_stmainactions
{
 ...
 if { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and { buffpresent(ca_inc_buff) or astralpower() > 90 and buffpresent(eclipse_any) } spell(starsurge)
 ...
 if { not azeritetraitrank(streaking_stars_trait) or buffremaining(ca_inc_buff) < executetime(starsurge) or not prev_starsurge() } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not hastalent(starlord_talent) and spellcooldown(ca_inc) > 7 spell(starsurge)
 #new_moon,if=(buff.eclipse_lunar.up|(charges=2&recharge_time<5)|charges=3)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } spell(new_moon)
...
AddFunction balancedreambindershortcdactions
{
 unless { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits) or { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation)
  {
   ...
   if buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() spell(fury_of_elune)
 ...
 unless { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire) and target.timetodie() > 12 and { { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(ca_inc_buff) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(ca_inc_buff) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize_buff) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } and spell(kindred_spirits)
  {
   ...
   unless { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) or covenant(night_fae) or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() < 32 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } } and spell(incarnation)
   {
    ...
    unless buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) } and save_for_ca_inc() and spell(fury_of_elune) or covenant(night_fae) and convoke_condition() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not covenant(kyrian) or buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(ca_inc_buff) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath)
    {
     #run_action_list,name=fallthru
     balancefallthrucdactions()
    }
...
AddFunction balanceboatmainactions
{
 ...
 if { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or fightremains() < 10 spell(kindred_spirits)
 ...
 if fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) spell(starsurge)
...
AddFunction balanceboatshortcdactions
{
 unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } } and spell(stellar_flare)
 {
  ...
  if { buffpresent(eclipse_any) or message("eclipse.solar_in_1 is not implemented") or message("eclipse.lunar_in_1 is not implemented") } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(ca_inc) > 0 } } } and { spellcooldown(ca_inc) > 30 or astralpower() > 90 and not spellcooldown(ca_inc) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage_debuff) > 4 or not covenant(necrolord) } spell(fury_of_elune)

  unless { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or fightremains() < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon)
  {
   #warrior_of_elune
   spell(warrior_of_elune)
...
{
 { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } } and spell(stellar_flare) or { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or fightremains() < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceboatcdactions
{
 ...
 unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(eclipse_any) or message("eclipse.solar_in_1 is not implemented") or message("eclipse.lunar_in_1 is not implemented") } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(ca_inc) > 0 } } } and { spellcooldown(ca_inc) > 30 or astralpower() > 90 and not spellcooldown(ca_inc) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage_debuff) > 4 or not covenant(necrolord) } and spell(fury_of_elune) or { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or fightremains() < 10 } and spell(kindred_spirits)
 {
  ...
  unless { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation)
  {
   ...
   if { convoke_desync() and fightremains() > 130 or buffpresent(ca_inc_buff) } and { message("buff.balance_of_all_things_nature.stack_value is not implemented") > 30 or message("buff.balance_of_all_things_arcane.stack_value is not implemented") > 30 } or fightremains() < 10 spell(convoke_the_spirits)
...
{
 { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and astralpower() >= 90 and { spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } and buffpresent(starlord_buff) and texture(starlord text=cancel) or { not critnotup() and { not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_condition() or not covenant(night_fae) } and { covenant(night_fae) or spellcooldown(ca_inc) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize_buff) and covenant(kyrian) } or not spellcooldown(convoke_the_spirits) > 0 and spellcooldown(ca_inc) == 0 and covenant(night_fae) } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage_debuff) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm_heal_debuff) or target.debuffremaining(adaptive_swarm_heal_debuff) > 5 } or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 and target.debuffpresent(adaptive_swarm_damage_debuff) } and spell(adaptive_swarm) or target.refreshable(sunfire) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and { critnotup() or astralpower() < 30 and not buffpresent(ca_inc_buff) or spellcooldown(ca_inc) == 0 } and not buffpresent(kindred_empowerment_energize_buff) } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + buffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and { critnotup() or astralpower() < 30 or spellcooldown(ca_inc) == 0 } } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(eclipse_any) or message("eclipse.solar_in_1 is not implemented") or message("eclipse.lunar_in_1 is not implemented") } and { not covenant(night_fae) or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(ca_inc) > 0 } } } and { spellcooldown(ca_inc) > 30 or astralpower() > 90 and not spellcooldown(ca_inc) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not covenant(kyrian) } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage_debuff) > 4 or not covenant(necrolord) } and spell(fury_of_elune) or { { eclipse() or eclipse() or eclipse() or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(ca_inc) == 0 } and { spellcooldown(ca_inc) > 30 or spellcooldown(ca_inc) == 0 } or fightremains() < 10 } and spell(kindred_spirits) or { astralpower() > 90 and { buffpresent(kindred_empowerment_energize_buff) or not covenant(kyrian) } or covenant(night_fae) or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 4 * conduit(precise_alignment_conduit) } and { not covenant(night_fae) or not spellcooldown(convoke_the_spirits) > 0 or convoke_desync() and fightremains() > 180 + 20 + 4 * conduit(precise_alignment_conduit) or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() < 30 + 4 * conduit(precise_alignment_conduit) } and spell(incarnation) or covenant(night_fae) and { convoke_desync() or spellcooldown(ca_inc) < 10 } and astralpower() > 50 and spellcooldown(convoke_the_spirits) < 10 and spell(starsurge) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not { not spellcooldown(ca_inc) > 0 } or covenant(kyrian) and not { not spellcooldown(empower_bond) > 0 } or covenant(night_fae) } and { not covenant(venthyr) or not buffpresent(ca_inc_buff) or astralpower() > 90 } or buffremaining(ca_inc_buff) > 8 and not buffpresent(ravenous_frenzy) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or eclipse() or eclipse() or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) } and spell(starfire) or spell(wrath) or balancefallthrucdpostconditions()
}
...
AddFunction balanceaoemainactions
{
 ...
 if target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(ca_inc) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) } spell(moonfire)
 ...
 if { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(incarnation)
 ...
 if fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 spell(kindred_spirits)
 ...
 if { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } spell(new_moon)
 #half_moon,if=(eclipse.in_any&cooldown.ca_inc.remains>50|(charges=2&recharge_time<5)|charges=3)&ap_check
 if { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) spell(half_moon)
 #full_moon,if=(eclipse.in_any&cooldown.ca_inc.remains>50|(charges=2&recharge_time<5)|charges=3)&ap_check
 if { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) spell(full_moon)
...
{
 unless buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(ca_inc) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)

  unless { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } } and spell(stellar_flare)
  {
   ...
   if buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } spell(fury_of_elune)

   unless buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)

    unless { eclipse() or eclipse() and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire)
    {
     #run_action_list,name=fallthru
     balancefallthrushortcdactions()
    }
...
{
 buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(ca_inc) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } } and spell(stellar_flare) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { eclipse() or eclipse() and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceaoecdactions
{
 unless buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(ca_inc) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  ...
  if { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 22 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } spell(celestial_alignment)
  ...
  unless { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } } and spell(stellar_flare)
  {
   ...
   unless buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { eclipse() or eclipse() and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire)
   {
    #run_action_list,name=fallthru
    balancefallthrucdactions()
   }
...
{
 buffrefreshable(starfall) and { enemies() < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies() >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies() > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or dream_will_fall_off() and starfall_wont_fall_off() and spell(starsurge) or target.refreshable(sunfire) and target.timetodie() > 14 - enemies() + buffremaining(sunfire) and { astralpower() >= astralpowercost(sunfire) and buffpresent(eclipse_any) } and spell(sunfire) or { not target.debuffpresent(adaptive_swarm_debuff) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage_debuff) < 3 and target.debuffremaining(adaptive_swarm_damage_debuff) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies() * 1.5 } / enemies() + target.debuffremaining(moonfire_debuff) and { { spellcooldown(ca_inc) == 0 or enemies() < 3 or { buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies() < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and not buffpresent(kindred_empowerment_energize_buff) and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(ca_inc_buff) and { fightremains() < spellcooldown(convoke_the_spirits) + 7 or fightremains() % 180 < 32 or not spellcooldown(convoke_the_spirits) > 0 or not covenant(night_fae) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and spellcooldown(ca_inc) > 50 } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies() < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(ca_inc_buff) > 10 or not buffpresent(ca_inc_buff) } } and spell(stellar_flare) or buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage_debuff) or not covenant(necrolord) or enemies() > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffrefreshable(starfall) and spell(starfall) or covenant(night_fae) and convoke_condition() and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and buffpresent(starfall) and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not message("starfire.ap_check is not implemented") or { buffremaining(ca_inc_buff) < 5 and buffpresent(ca_inc_buff) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies() < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies() < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(ca_inc) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { eclipse() or eclipse() and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(ca_inc_buff) < executetime(starfire) and not is_cleave() and buffremaining(ca_inc_buff) < executetime(wrath) and buffpresent(ca_inc_buff) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies() <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(ca_inc_buff) > executetime(wrath) } and spell(wrath) or spell(starfire) or balancefallthrucdpostconditions()
}
...
AddFunction guardianowlweavemainactions
{
 ...
 if buffpresent(eclipse_lunar) or eclipse() or buffpresent(eclipse_lunar) and buffpresent(starsurge_empowerment_buff) spell(starfire)
 #wrath
 spell(wrath)
}
...
{
 not buffpresent(moonkin_form) and spell(moonkin_form) or checkboxon("owlweave_bear") and spell(empower_bond) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or { target.refreshable(moonfire_debuff) or buffpresent(galactic_guardian_buff) } and spell(moonfire) or target.refreshable(sunfire) and spell(sunfire) or { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and spell(starsurge) or { buffpresent(eclipse_lunar) or eclipse() or buffpresent(eclipse_lunar) and buffpresent(starsurge_empowerment_buff) } and spell(starfire) or spell(wrath)
}
...
{
 not buffpresent(cat_form) and spell(cat_form) or buffpresent(prowl) and spell(rake) or checkboxon("catweave_bear") and spell(empower_bond) or target.debuffrefreshable(rip) and combopoints() >= 4 and spell(rip) or combopoints() >= 4 and spell(ferocious_bite) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or target.debuffrefreshable(rake_debuff) and combopoints() < 4 and spell(rake) or spell(shred)
}
...
{
 not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not covenant(venthyr) } and spell(incarnation) or buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or buffpresent(galactic_guardian_buff) and checkboxon("owlweave_bear") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk_bear) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire_debuff) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian_buff) and checkboxon("catweave_bear") and enemies() <= 3 or buffpresent(galactic_guardian_buff) and not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle) or spell(thrash_bear) or spell(maul) or spell(swipe)
}
...
AddFunction guardianbearcdactions
{
 unless not buffpresent(bear_form) and spell(bear_form)
 {
  ...
  if buffpresent(ravenous_frenzy) or not covenant(venthyr) spell(berserk_bear)
 }
}

AddFunction guardianbearcdpostconditions
{
 not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not covenant(venthyr) } and spell(incarnation) or { not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") or enemies() >= 2 } and spell(empower_bond) or hastalent(brambles_talent) and buffpresent(bear_form) and spell(barkskin) or buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm_debuff) and spell(adaptive_swarm) or buffpresent(galactic_guardian_buff) and checkboxon("owlweave_bear") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk_bear) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire_debuff) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian_buff) and checkboxon("catweave_bear") and enemies() <= 3 or buffpresent(galactic_guardian_buff) and not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle) or target.debuffstacks(thrash_bear_debuff) > 2 and target.debuffgain(thrash_bear_debuff) <= baseduration(thrash_bear_debuff) and spell(pulverize) or spell(thrash_bear) or spell(maul) or spell(swipe)
}
