    ...
    AddMissingVariantSpells = function(self, annotation)
        if annotation.functionReference then
            for _, node in ipairs(annotation.functionReference) do
                if self.ovaleCondition:IsSpellBookCondition(node.name) then
                    local positionalParams = node.rawPositionalParams, node.rawNamedParams
                    local spellIdParam = positionalParams[1]
                    if  not spellIdParam then
                        self.tracer:Error("No spell argument in %s.", getFunctionCallString(node))
                    elseif spellIdParam.type == "value" then
                    ...
                    elseif spellIdParam.type == "string" then
                        if  not self.ovaleData.buffSpellList[spellIdParam.value] then
                            self.tracer:Error("Unknown spell list %s in %s.", spellIdParam.value, getFunctionCallString(node))
                        end
                    else
