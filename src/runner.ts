...
export class Runner {
    ...
    private ComputeBool(element: AstNode, atTime: number) {
        let newElement = this.Compute(element, atTime);
        if (
    ...
    private ComputeAction: ComputerFunction<AstFunctionNode> = (
        ...
        let timeSpan = this.GetTimeSpan(node);
    ...
    private ComputeArithmetic: ComputerFunction<AstExpressionNode> = (
        ...
        let [a, b, c, timeSpanA] = this.AsValue(atTime, nodeA);
        const nodeB = this.Compute(element.child[2], atTime);
        let [x, y, z, timeSpanB] = this.AsValue(atTime, nodeB);
    ...
    private ComputeCompare: ComputerFunction<AstExpressionNode> = (
        ...
        let [a, b, c, timeSpanA] = this.AsValue(atTime, elementA);
        const elementB = this.Compute(element.child[2], atTime);
        let [x, y, z, timeSpanB] = this.AsValue(atTime, elementB);
        timeSpanA.Intersect(timeSpanB, timeSpan);
        if (timeSpan.Measure() == 0) {
            this.tracer.Log(
    ...
    private ComputeCustomFunction: ComputerFunction<AstFunctionNode> = (
        ...
        if (node) {
            ...
            let elementA = this.Compute(node.child[1], atTime);
            if (this.tracer.debug.trace)
                this.tracer.Log(
    ...
    private ComputeLua: ComputerFunction<AstLuaNode> = (element, atTime) => {
    ...
    private ComputeValue: ComputerFunction<AstValueNode> = (
        element,
        atTime
    ) => {
    ...
    private computeString: ComputerFunction<AstStringNode> = (
        element,
        atTime
    ) => {
        this.tracer.Log("[%d]    value is %s", element.nodeId, element.value);
    ...
    private computeVariable: ComputerFunction<AstVariableNode> = (
        element,
        atTime
    ) => {
    ...
    private AsValue(
        ...
        let value: number | string, origin: number, rate: number, timeSpan;
    ...
    COMPUTE_VISITOR: LuaObj<ComputerFunction<any>> = {
