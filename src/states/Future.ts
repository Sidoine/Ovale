...
export class OvaleFutureClass extends States<OvaleFutureData> {
    ...
    UpdateStateCounters(
        ...
        let inccounter = this.ovaleData.GetSpellInfoProperty(
            spellId,
            atTime,
            "inccounter",
            targetGUID
        ...
        let resetcounter = this.ovaleData.GetSpellInfoProperty(
            spellId,
            atTime,
            "resetcounter",
            targetGUID
    ...
    private UNIT_SPELLCAST_CHANNEL_STOP = (
        ...
        if ((unitId == "player" || unitId == "pet") && !WHITE_ATTACK[spellId]) {
            ...
            this.tracer.DebugTimestamp(event, unitId, spell, lineId, spellId);
            let now = GetTime();
            let [spellcast, index] = this.GetSpellcast(
                spell,
                spellId,
                undefined,
                now
    ...
    private UNIT_SPELLCAST_SENT = (
        ...
        if ((unitId == "player" || unitId == "pet") && !WHITE_ATTACK[spellId]) {
            ...
            if (targetName == "") {
                ...
                let [targetGUID, nextGUID] = this.ovaleGuid.NameGUID(
                    targetName
                );
                if (nextGUID) {
        ...
        if ((unitId == "player" || unitId == "pet") && !WHITE_ATTACK[spellId]) {
            ...
            if (spellcast) {
                ...
                if (lineId == castId && name == spellName) {
                    ...
                    let [auraId, auraGUID] = this.GetAuraFinish(
                        spellId,
                        spellcast.target,
                        now
                    );
    ...
    GetSpellcast(
        ...
        spellId: number,
        lineId: number | undefined | string,
        atTime: number
    ...
    GetGCD(spellId?: number, atTime?: number, targetGUID?: string) {
        ...
        if (!atTime) {
            if (
                this.next.currentCast.stop &&
                this.next.currentCast.stop > this.baseState.next.currentTime
            ) {
                atTime = this.next.currentCast.stop;
            } else {
                atTime =
                    this.baseState.next.currentTime ||
                    this.baseState.current.currentTime;
            }
        }
        targetGUID =
            targetGUID ||
            this.ovaleGuid.UnitGUID(this.baseState.next.defaultTarget);
        let gcd =
            spellId &&
