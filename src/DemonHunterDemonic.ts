...
export class OvaleDemonHunterDemonicClass {
    ...
    COMBAT_LOG_EVENT_UNFILTERED(event: string, ...__args: any[]) {
        ...
        if (sourceGUID == this.playerGUID && cleuEvent == "SPELL_CAST_SUCCESS") {
            ...
            if (HAVOC_EYE_BEAM_SPELL_ID == spellId) {
                this.debug.Debug("Spell %d (%s) has successfully been cast. Gaining Aura (only during meta).", spellId, spellName);
                this.GainAura();
            }
        }
        if (sourceGUID == this.playerGUID && cleuEvent == "SPELL_AURA_REMOVED") {
            ...
            if (HAVOC_META_BUFF_ID == spellId) {
                this.debug.Debug("Aura %d (%s) is removed. Dropping Aura.", spellId, spellName);
                this.DropAura();
            }
        }
    }
    ...
    DropAura() {
        let now = GetTime();
        this.debug.Debug("Removing '%s' (%d) buff on player %s.", HIDDEN_BUFF_EXTENDED_BY_DEMONIC, HIDDEN_BUFF_ID, this.playerGUID);
        this.ovaleAura.LostAuraOnGUID(this.playerGUID, now, HIDDEN_BUFF_ID, this.playerGUID);
    }
}
