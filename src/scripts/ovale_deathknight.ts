...
AddFunction froststandardmainactions
{
 ...
 if not buffpresent(frozen_pulse) and hastalent(frozen_pulse_talent) or buffpresent(killing_machine_frost) or buffpresent(death_and_decay) and iscovenant("night_fae") and buffstacks(deaths_due_buff) > 8 or timetorunes(4) <= gcd() spell(obliterate)
 ...
 unless buffpresent(pillar_of_frost) and spell(berserking)
 {
  ...
  unless buffpresent(pillar_of_frost) and enemies() == 1 and { buffremaining(pillar_of_frost) < 5 and hastalent(cold_heart_talent) or not hastalent(cold_heart_talent) and buffremaining(pillar_of_frost) < 3 } and spell(bag_of_tricks)
  {
   ...
   unless hastalent(breath_of_sindragosa_talent) and { spellcooldown(breath_of_sindragosa) > 0 or spellcooldown(breath_of_sindragosa) <= 0 and runicpowerdeficit() < 60 } and spell(pillar_of_frost) or hastalent(icecap_talent) and not buffpresent(pillar_of_frost) and spell(pillar_of_frost) or hastalent(obliteration_talent) and { hastalent(gathering_storm_talent) and buffpresent(remorseless_winter) or not hastalent(gathering_storm_talent) } and spell(pillar_of_frost)
   {
    ...
    unless { hastalent(breath_of_sindragosa_talent) and runicpowerdeficit() > 40 and runecount() >= 3 and buffpresent(pillar_of_frost) or not hastalent(breath_of_sindragosa_talent) and runicpowerdeficit() >= 25 } and spell(hypothermic_presence) or buffpresent(pillar_of_frost) and spell(raise_dead)
    {
...
AddFunction unholyprecombatshortcdactions
{
 ...
 #snapshot_stats
 #raise_dead
 if buffexpires(raise_dead) spell(raise_dead_unholy)
}
...
{
 buffexpires(raise_dead) and spell(raise_dead_unholy)
}
...
{
 { buffpresent(sudden_doom_buff) and not pooling_for_gargoyle() or pet.present() } and spell(death_coil) or runicpowerdeficit() < 13 and not pooling_for_gargoyle() and spell(death_coil) or spellcooldown(apocalypse) > 0 and { hastalent(defile_talent) or iscovenant("night_fae") or runeforge(phearomones_runeforge) } and spell(death_and_decay) or target.debuffstacks(festering_wound) > 4 and spell(scourge_strike) or target.debuffpresent(festering_wound) and spellcooldown(apocalypse) > 5 and { not hastalent(unholy_blight_talent) or hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) < 5 or not hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) >= 5 or not conduit(convocation_of_the_dead_conduit) } and spell(scourge_strike) or target.debuffpresent(festering_wound) and hastalent(unholy_blight_talent) and { not hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) < 5 or hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) >= 5 } and { spellcooldown(unholy_blight) > 10 and not target.debuffremaining(unholy_blight) or spellcooldown(apocalypse) > 10 } and spell(scourge_strike) or runicpowerdeficit() < 20 and not pooling_for_gargoyle() and spell(death_coil) or target.debuffstacks(festering_wound) < 1 and spell(festering_strike) or target.debuffstacks(festering_wound) < 4 and spellcooldown(apocalypse) < 5 and { not hastalent(unholy_blight_talent) or hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) < 5 or not hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) >= 5 or not conduit(convocation_of_the_dead_conduit) } and spell(festering_strike) or target.debuffstacks(festering_wound) < 4 and hastalent(unholy_blight_talent) and { not hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) < 5 or hastalent(army_of_the_damned_talent) and conduitrank(convocation_of_the_dead_conduit) >= 5 } and { spellcooldown(unholy_blight) < 10 or spellcooldown(apocalypse) < 10 and target.debuffremaining(unholy_blight) } and spell(festering_strike) or not pooling_for_gargoyle() and spell(death_coil)
}
...
AddFunction unholycooldownscdactions
{
 ...
 if { pet.present() or buffpresent(unholy_assault) or hastalent(army_of_the_damned_talent) and { pet.present() or pet.present() or spellcooldown(army_of_the_dead) > target.timetodie() } } and { checkboxon(opt_use_consumables) and target.classification(worldboss) } item(potion_of_spectral_strength_item usable=1)
 ...
 if { spellcooldown(unholy_blight) > 0 or spellcooldown(dark_transformation) > 0 } and { pet.present() or pet.present() and not hastalent(army_of_the_damned_talent) or target.timetohealthpercent(20) < 5 } unholyuseitemactions()
...
AddFunction unholy_defaultmainactions
{
 ...
 if pet.present() or buffpresent(unholy_assault) or hastalent(army_of_the_damned_talent) and pet.present() and { pet.present() or spellcooldown(army_of_the_dead) > spellcooldownduration(berserking) / 3 } or target.timetodie() <= baseduration(berserking_buff) spell(berserking)
...
AddFunction unholy_defaultshortcdactions
{
 ...
 unless { pet.present() or buffpresent(unholy_assault) or hastalent(army_of_the_damned_talent) and pet.present() and { pet.present() or spellcooldown(army_of_the_dead) > spellcooldownduration(berserking) / 3 } or target.timetodie() <= baseduration(berserking_buff) } and spell(berserking)
 {
...
AddFunction unholy_defaultcdactions
{
 ...
 if pet.present() or buffpresent(unholy_assault) or hastalent(army_of_the_damned_talent) and pet.present() and { pet.present() or spellcooldown(army_of_the_dead) > spellcooldownduration(blood_fury) / 3 } or target.timetodie() <= baseduration(blood_fury) spell(blood_fury)
 ...
 unless { pet.present() or buffpresent(unholy_assault) or hastalent(army_of_the_damned_talent) and pet.present() and { pet.present() or spellcooldown(army_of_the_dead) > spellcooldownduration(berserking) / 3 } or target.timetodie() <= baseduration(berserking_buff) } and spell(berserking)
 {
  ...
  if pet.present() or buffpresent(unholy_assault) or hastalent(army_of_the_damned_talent) and pet.present() and { pet.present() or spellcooldown(army_of_the_dead) > spellcooldownduration(ancestral_call) / 3 } or target.timetodie() <= 15 spell(ancestral_call)
  ...
  if pet.present() or buffpresent(unholy_assault) or hastalent(army_of_the_damned_talent) and pet.present() and { pet.present() or spellcooldown(army_of_the_dead) > spellcooldownduration(fireblood) / 3 } or target.timetodie() <= baseduration(fireblood_buff) spell(fireblood)
