...
AddFunction windwalkerweapons_of_ordermainactions
{
 ...
 unless not hastalent(serenity_talent) and windwalkercd_sefmainpostconditions()
 {
  ...
  unless hastalent(serenity_talent) and windwalkercd_serenitymainpostconditions()
  {
   ...
   if enemies() >= 2 and buffremaining(weapons_of_order_ww_buff) < 1 spell(fists_of_fury)
   ...
   if { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and enemies() >= 3 and buffpresent(weapons_of_order_ww_buff) spell(spinning_crane_kick)
   ...
   if chi() >= 3 or buffpresent(weapons_of_order_ww_buff) spell(blackout_kick)
...
AddFunction windwalkerweapons_of_ordershortcdactions
{
 ...
 unless not hastalent(serenity_talent) and windwalkercd_sefshortcdpostconditions()
 {
  ...
  unless hastalent(serenity_talent) and windwalkercd_serenityshortcdpostconditions()
  {
   ...
   unless spell(rising_sun_kick) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and buffpresent(dance_of_chiji_windwalker) and spell(spinning_crane_kick) or enemies() >= 2 and buffremaining(weapons_of_order_ww_buff) < 1 and spell(fists_of_fury) or enemies() >= 2 and { spellcooldown(fists_of_fury) > 0 and spellcooldown(rising_sun_kick) > 0 } and spell(whirling_dragon_punch) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and enemies() >= 3 and buffpresent(weapons_of_order_ww_buff) and spell(spinning_crane_kick) or not previousspell(blackout_kick) and enemies() <= 2 and spell(blackout_kick) or spellcooldown(fists_of_fury) > 0 and spellcooldown(rising_sun_kick) > 0 and spell(whirling_dragon_punch) or buffpresent(storm_earth_and_fire) and 600 > spellcooldownduration(fists_of_fury) * 0.6 and spell(fists_of_fury) or buffstacks(chi_energy) > 30 - 5 * enemies() and spell(spinning_crane_kick)
   {
...
{
 not hastalent(serenity_talent) and windwalkercd_sefshortcdpostconditions() or hastalent(serenity_talent) and windwalkercd_serenityshortcdpostconditions() or spell(rising_sun_kick) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and buffpresent(dance_of_chiji_windwalker) and spell(spinning_crane_kick) or enemies() >= 2 and buffremaining(weapons_of_order_ww_buff) < 1 and spell(fists_of_fury) or enemies() >= 2 and { spellcooldown(fists_of_fury) > 0 and spellcooldown(rising_sun_kick) > 0 } and spell(whirling_dragon_punch) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and enemies() >= 3 and buffpresent(weapons_of_order_ww_buff) and spell(spinning_crane_kick) or not previousspell(blackout_kick) and enemies() <= 2 and spell(blackout_kick) or spellcooldown(fists_of_fury) > 0 and spellcooldown(rising_sun_kick) > 0 and spell(whirling_dragon_punch) or buffpresent(storm_earth_and_fire) and 600 > spellcooldownduration(fists_of_fury) * 0.6 and spell(fists_of_fury) or buffstacks(chi_energy) > 30 - 5 * enemies() and spell(spinning_crane_kick) or maxchi() - chi() >= 1 and spell(expel_harm) or { not hastalent(hit_combo_talent) or not previousspell(tiger_palm) } and maxchi() - chi() >= 2 and spell(tiger_palm) or spell(chi_wave) or { chi() >= 3 or buffpresent(weapons_of_order_ww_buff) } and spell(blackout_kick) or checkboxon(opt_flying_serpent_kick) and spell(flying_serpent_kick)
}
...
{
 not hastalent(serenity_talent) and windwalkercd_sefcdpostconditions() or hastalent(serenity_talent) and windwalkercd_serenitycdpostconditions() or maxchi() - chi() >= 2 and timetomaxenergy() > 3 and spell(energizing_elixir) or spell(rising_sun_kick) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and buffpresent(dance_of_chiji_windwalker) and spell(spinning_crane_kick) or enemies() >= 2 and buffremaining(weapons_of_order_ww_buff) < 1 and spell(fists_of_fury) or enemies() >= 2 and { spellcooldown(fists_of_fury) > 0 and spellcooldown(rising_sun_kick) > 0 } and spell(whirling_dragon_punch) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and enemies() >= 3 and buffpresent(weapons_of_order_ww_buff) and spell(spinning_crane_kick) or not previousspell(blackout_kick) and enemies() <= 2 and spell(blackout_kick) or spellcooldown(fists_of_fury) > 0 and spellcooldown(rising_sun_kick) > 0 and spell(whirling_dragon_punch) or buffpresent(storm_earth_and_fire) and 600 > spellcooldownduration(fists_of_fury) * 0.6 and spell(fists_of_fury) or buffstacks(chi_energy) > 30 - 5 * enemies() and spell(spinning_crane_kick) or chi() < 3 and spell(fist_of_the_white_tiger) or maxchi() - chi() >= 1 and spell(expel_harm) or maxchi() - chi() >= { 1 + enemies() > 1 } and checkboxon(opt_chi_burst) and spell(chi_burst) or { not hastalent(hit_combo_talent) or not previousspell(tiger_palm) } and maxchi() - chi() >= 2 and spell(tiger_palm) or spell(chi_wave) or { chi() >= 3 or buffpresent(weapons_of_order_ww_buff) } and spell(blackout_kick) or checkboxon(opt_flying_serpent_kick) and spell(flying_serpent_kick)
}
...
AddFunction windwalkerserenitymainactions
{
 ...
 if { not previousspell(blackout_kick) or not hastalent(hit_combo_talent) } and buffpresent(weapons_of_order_ww_buff) and spellcooldown(rising_sun_kick) > 2 spell(blackout_kick)
...
{
 unless buffremaining(serenity) < 1 and spell(fists_of_fury) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and { enemies() >= 3 or enemies() > 1 and not { not spellcooldown(rising_sun_kick) > 0 } } and spell(spinning_crane_kick) or not previousspell(rising_sun_kick) and spell(rising_sun_kick) or enemies() >= 3 and spell(fists_of_fury) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and buffpresent(dance_of_chiji_windwalker) and spell(spinning_crane_kick) or { not previousspell(blackout_kick) or not hastalent(hit_combo_talent) } and buffpresent(weapons_of_order_ww_buff) and spellcooldown(rising_sun_kick) > 2 and spell(blackout_kick)
 {
...
{
 buffremaining(serenity) < 1 and spell(fists_of_fury) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and { enemies() >= 3 or enemies() > 1 and not { not spellcooldown(rising_sun_kick) > 0 } } and spell(spinning_crane_kick) or not previousspell(rising_sun_kick) and spell(rising_sun_kick) or enemies() >= 3 and spell(fists_of_fury) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and buffpresent(dance_of_chiji_windwalker) and spell(spinning_crane_kick) or { not previousspell(blackout_kick) or not hastalent(hit_combo_talent) } and buffpresent(weapons_of_order_ww_buff) and spellcooldown(rising_sun_kick) > 2 and spell(blackout_kick) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and target.debuffpresent(bonedust_brew) and spell(spinning_crane_kick) or { not previousspell(blackout_kick) or not hastalent(hit_combo_talent) } and spell(blackout_kick) or spell(spinning_crane_kick)
}
...
{
 buffremaining(serenity) < 1 and spell(fists_of_fury) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and { enemies() >= 3 or enemies() > 1 and not { not spellcooldown(rising_sun_kick) > 0 } } and spell(spinning_crane_kick) or not previousspell(rising_sun_kick) and spell(rising_sun_kick) or enemies() >= 3 and spell(fists_of_fury) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and buffpresent(dance_of_chiji_windwalker) and spell(spinning_crane_kick) or { not previousspell(blackout_kick) or not hastalent(hit_combo_talent) } and buffpresent(weapons_of_order_ww_buff) and spellcooldown(rising_sun_kick) > 2 and spell(blackout_kick) or spell(fist_of_the_white_tiger) or { not hastalent(hit_combo_talent) and conduit(calculated_strikes_conduit) or not previousspell(spinning_crane_kick) } and target.debuffpresent(bonedust_brew) and spell(spinning_crane_kick) or chi() < 3 and spell(fist_of_the_white_tiger) or { not previousspell(blackout_kick) or not hastalent(hit_combo_talent) } and spell(blackout_kick) or spell(spinning_crane_kick)
}
