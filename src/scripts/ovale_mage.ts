...
AddFunction firecombustion_phasemainactions
{
 ...
 unless fireactive_talentsmainpostconditions()
 {
  ...
  unless bufflastexpire(combustion) <= timesincepreviousspell(combustion) and firecombustion_cooldownsmainpostconditions()
  {
   ...
   if buffpresent(hot_streak) and enemies() >= combustion_flamestrike() or buffpresent(firestorm_buff) and enemies() >= combustion_flamestrike() - runeforge(firestorm_runeforge) spell(flamestrike)
   #pyroblast,if=buff.sun_kings_blessing_ready.up&buff.sun_kings_blessing_ready.remains>cast_time
   if buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) spell(pyroblast)
   #pyroblast,if=buff.firestorm.react
   if buffpresent(firestorm_buff) spell(pyroblast)
...
AddFunction fireactive_talentsmainactions
{
 ...
 if hastalent(alexstraszas_fury_talent) and buffexpires(combustion) and not buffpresent(hot_streak) and target.distance() < 12 spell(dragons_breath)
}
...
{
 enemies() > 1 and buffexpires(combustion) and { time_to_combustion() > spellcooldownduration(living_bomb) or time_to_combustion() <= 0 or disable_combustion() } and spell(living_bomb) or hastalent(alexstraszas_fury_talent) and buffexpires(combustion) and not buffpresent(hot_streak) and target.distance() < 12 and spell(dragons_breath)
}
...
{
 enemies() > 1 and buffexpires(combustion) and { time_to_combustion() > spellcooldownduration(living_bomb) or time_to_combustion() <= 0 or disable_combustion() } and spell(living_bomb) or { not disable_combustion() and time_to_combustion() <= 0 or spellcooldownduration(meteor) < time_to_combustion() and not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and buffpresent(rune_of_power_buff) and time_to_combustion() > spellcooldown(meteor) or fightremains() < time_to_combustion() or disable_combustion() } and spell(meteor) or hastalent(alexstraszas_fury_talent) and buffexpires(combustion) and not buffpresent(hot_streak) and target.distance() < 12 and spell(dragons_breath)
}
...
AddFunction fire_defaultcdactions
{
 ...
 unless buffexpires(combustion) and time_to_combustion() > 0 and { spellcooldown(rune_of_power) > 0 or not hastalent(rune_of_power_talent) } and not { buffpresent(infernal_cascade) and buffpresent(hot_streak) } and { enemies() < combustion_shifting_power() or enemies() < combustion_flamestrike() or time_to_combustion() - 0 > spellcooldown(shifting_power) } and spell(shifting_power) or { buffexpires(combustion) and buffexpires(rune_of_power_buff) and { time_to_combustion() < executetime(radiant_spark) or time_to_combustion() > spellcooldownduration(radiant_spark) } or buffpresent(rune_of_power_buff) and time_to_combustion() > 30 } and spell(radiant_spark)
 {
  ...
  unless time_to_combustion() <= 3 and buffexpires(combustion) and spell(mirrors_of_torment) or executetime(mirrors_of_torment) > 0 and spellfullrecharge(fire_blast_fire) - executetime(mirrors_of_torment) < 4 and not inflighttotarget(hot_streak) and not buffpresent(hot_streak) and spell(fire_blast_fire)
  {
   ...
   if buffexpires(combustion) and target.debuffexpires(radiant_spark_vulnerability) and buffexpires(rune_of_power_buff) spell(mirror_image)
   ...
   if time_to_combustion() >= on_use_cutoff() and { buffstacks(infernal_cascade) == spelldata(infernal_cascade max_stacks) or not conduit(infernal_cascade_conduit) or combustion_on_use() or time_to_combustion() + 5 > fightremains() % itemcooldown(slot="trinket0slot") } fireuseitemactions()
...
AddFunction froststmainactions
{
 ...
 if buffpresent(freezing_rain_buff) or enemies() >= 2 or runeforge(glacial_fragments_runeforge) and buffstacks(winters_chill) == 2 spell(blizzard)
...
AddFunction froststshortcdactions
{
 unless { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze) and { previousgcdspell(glacial_spike) or previousgcdspell(frostbolt) and { not conduit(ire_of_the_ascended_conduit) or spellcooldown(radiant_spark) > 0 or runeforge(freezing_winds_runeforge) } or previousgcdspell(radiant_spark) or buffstacks(fingers_of_frost) == 0 and { target.debuffpresent(mirrors_of_torment) or buffpresent(freezing_winds) or buffpresent(expanded_potential_buff) } } } and spell(flurry)
 {
  ...
  unless { buffpresent(freezing_rain_buff) or enemies() >= 2 or runeforge(glacial_fragments_runeforge) and buffstacks(winters_chill) == 2 } and spell(blizzard)
  {
...
AddFunction frostaoeshortcdactions
{
 ...
 unless spell(blizzard) or { buffstacks(winters_chill) == 0 or target.debuffexpires(winters_chill) } and { previousgcdspell(ebonbolt) or buffpresent(brain_freeze) and buffstacks(fingers_of_frost) == 0 } and spell(flurry) or spell(ice_nova)
 {
  ...
  unless { buffpresent(fingers_of_frost) or target.debuffremaining(frozen) > traveltime(ice_lance) or buffstacks(winters_chill) and target.debuffremaining(winters_chill) > traveltime(ice_lance) } and spell(ice_lance)
  {
   ...
   unless runeforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) <= 0 and buffexpires(disciplinary_command__fire_aura_dnt) and spell(fire_blast) or manapercent() > 30 and enemies() >= 6 and not runeforge(glacial_fragments_runeforge) and spell(arcane_explosion)
   {
    #ebonbolt
    spell(ebonbolt)
   }
