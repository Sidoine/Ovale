...
{
 if not { extended_combustion_remains() < time_to_combustion() and charges(fire_blast_fire count=0) + { time_to_combustion() + 0 * shifting_power_before_combustion() + { spelldata(mirrors_of_torment max_stacks) - 1 } * mot_recharge_amount() * iscovenant("venthyr") * { spellcooldown(mirrors_of_torment) <= time_to_combustion() } } / spellcooldownduration(fire_blast_fire) - 1 < spellmaxcharges(fire_blast_fire) + overpool_fire_blasts() / spellcooldownduration(fire_blast_fire) - baseduration(combustion) / spellcooldownduration(fire_blast_fire) % 1 and time_to_combustion() < fightremains() } and hastalent(rune_of_power_talent) and buffexpires(rune_of_power_buff) { not runeforge(sun_kings_blessing_runeforge) or buffstacks(sun_kings_blessing_buff) > spelldata(sun_kings_blessing_buff max_stacks) - 1 } and spellcooldown(rune_of_power) < spellfullrecharge(fire_blast_fire) - 0 * { shifting_power_before_combustion() and spellcooldown(shifting_power) < spellcooldown(rune_of_power) } and spellcooldown(rune_of_power) < fightremains()
 if not { extended_combustion_remains() < time_to_combustion() and charges(fire_blast_fire count=0) + { time_to_combustion() + 0 * shifting_power_before_combustion() + { spelldata(mirrors_of_torment max_stacks) - 1 } * mot_recharge_amount() * iscovenant("venthyr") * { spellcooldown(mirrors_of_torment) <= time_to_combustion() } } / spellcooldownduration(fire_blast_fire) - 1 < spellmaxcharges(fire_blast_fire) + overpool_fire_blasts() / spellcooldownduration(fire_blast_fire) - baseduration(combustion) / spellcooldownduration(fire_blast_fire) % 1 and time_to_combustion() < fightremains() } and runeforge(sun_kings_blessing_runeforge) talent(searing_touch_talent) and target.healthpercent() < 30 and spellfullrecharge(fire_blast_fire) > 3 * gcd()
 extended_combustion_remains() < time_to_combustion() and charges(fire_blast_fire count=0) + { time_to_combustion() + 0 * shifting_power_before_combustion() + { spelldata(mirrors_of_torment max_stacks) - 1 } * mot_recharge_amount() * iscovenant("venthyr") * { spellcooldown(mirrors_of_torment) <= time_to_combustion() } } / spellcooldownduration(fire_blast_fire) - 1 < spellmaxcharges(fire_blast_fire) + overpool_fire_blasts() / spellcooldownduration(fire_blast_fire) - baseduration(combustion) / spellcooldownduration(fire_blast_fire) % 1 and time_to_combustion() < fightremains()
}
...
AddFunction use_shifting_power
{
 if iscovenant("night_fae") TargetTimeToHealthPercent(90) < extended_combustion_remains() and conduit(infernal_cascade_conduit) and expected_fire_blasts() < needed_fire_blasts() and { not hastalent(rune_of_power_talent) or spellcooldown(rune_of_power) > extended_combustion_remains() } or enemies() >= combustion_shifting_power()
}
...
AddFunction time_to_combustion_max
{
 ...
 if hasequippeditem(gladiators_badge_item) and itemcooldown(gladiators_badge_item) - 20 < time_to_combustion() itemcooldown(gladiators_badge_item) > 0
 ...
 if iscovenant("necrolord") and spellcooldown(deathborne) - 10 < time_to_combustion() spellcooldown(deathborne) + { baseduration(deathborne) - baseduration(combustion) } * runeforge(deaths_fathom_runeforge)
 if iscovenant("venthyr") and spellcooldown(mirrors_of_torment) - 25 < time_to_combustion() spellcooldown(mirrors_of_torment) > 0
 if iscovenant("kyrian") and spellcooldown(radiant_spark) - 10 < time_to_combustion() spellcooldown(radiant_spark) > 0
 if hastalent(firestarter_talent) and not firestarter_combustion() TargetTimeToHealthPercent(90)
 buffpresent(combustion)
}
...
AddFunction mot_recharge_amount
{
 6
}
...
{
 if 0 < 0 not hastalent(pyroclasm_talent)
}
...
AddFunction firerop_phasemainactions
{
 ...
 unless fireactive_talentsmainpostconditions()
 {
  ...
  if not phoenix_pooling() and { debuffcountonany(ignite_debuff) < 2 or enemies() >= hard_cast_flamestrike() or enemies() >= hot_streak_flamestrike() } spell(phoenix_flames)
  #scorch,if=searing_touch.active
  if talent(searing_touch_talent) and target.healthpercent() < 30 spell(scorch)
...
AddFunction firecombustion_phasemainactions
{
 ...
 #variable,use_off_gcd=1,use_while_casting=1,name=extended_combustion_remains,op=add,value=variable.skb_duration,if=runeforge.sun_kings_blessing&(buff.sun_kings_blessing_ready.up|variable.extended_combustion_remains>gcd.remains+1.5*gcd.max*(buff.sun_kings_blessing.max_stack-buff.sun_kings_blessing.stack))
 #call_action_list,name=combustion_cooldowns,if=variable.extended_combustion_remains>variable.skb_duration
 if extended_combustion_remains() > skb_duration() firecombustion_cooldownsmainactions()

 unless extended_combustion_remains() > skb_duration() and firecombustion_cooldownsmainpostconditions()
 {
  #call_action_list,name=active_talents
  fireactive_talentsmainactions()
  ...
  unless fireactive_talentsmainpostconditions()
  {
   ...
   if buffexpires(combustion) and buffstacks(sun_kings_blessing_buff) > 2 and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) < 20 and buffpresent(sun_kings_blessing_buff) texture(sun_kings_blessing_buff text=cancel)
   ...
   if buffexpires(combustion) and buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) spell(pyroblast)
   #pyroblast,if=buff.combustion.down&buff.pyroclasm.react&buff.pyroclasm.remains>cast_time
   if buffexpires(combustion) and buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) spell(pyroblast)
   ...
   if buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) spell(scorch)
   ...
   if not conduit(infernal_cascade_conduit) and not fire_blast_pooling() and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 spell(fire_blast_fire)
   ...
   if conduit(infernal_cascade_conduit) and not fire_blast_pooling() and { expected_fire_blasts() >= needed_fire_blasts() or buffremaining(combustion) < gcd() or extended_combustion_remains() <= baseduration(infernal_cascade_buff) or buffstacks(infernal_cascade_buff) < 2 or buffremaining(infernal_cascade_buff) < gcd() or spellcooldown(shifting_power) <= 0 and use_shifting_power() } and buffpresent(combustion) and { not buffpresent(firestorm_buff) or buffremaining(infernal_cascade_buff) < 0.5 } and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 spell(fire_blast_fire)
   ...
   if buffpresent(sun_kings_blessing_ready_buff) and { needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and enemies() >= combustion_flamestrike() and buffremaining(sun_kings_blessing_ready_buff) > casttime(flamestrike) and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(flamestrike) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } spell(flamestrike)
   ...
   if buffpresent(sun_kings_blessing_ready_buff) and { not hastalent(pyroclasm_talent) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) or needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(pyroblast) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } spell(pyroblast)
   ...
   if buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and buffremaining(combustion) > casttime(pyroblast) and enemies() < combustion_flamestrike() and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(pyroblast) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } spell(pyroblast)
   ...
   if buffremaining(combustion) > casttime(scorch) spell(scorch)
...
{
 extended_combustion_remains() > skb_duration() and firecombustion_cooldownsmainpostconditions() or fireactive_talentsmainpostconditions()
}
...
AddFunction firecombustion_phaseshortcdactions
{
 ...
 unless enemies() > 1 and buffexpires(combustion) and spell(living_bomb)
 {
  ...
  #variable,use_off_gcd=1,use_while_casting=1,name=extended_combustion_remains,op=add,value=variable.skb_duration,if=runeforge.sun_kings_blessing&(buff.sun_kings_blessing_ready.up|variable.extended_combustion_remains>gcd.remains+1.5*gcd.max*(buff.sun_kings_blessing.max_stack-buff.sun_kings_blessing.stack))
  #call_action_list,name=combustion_cooldowns,if=variable.extended_combustion_remains>variable.skb_duration
  if extended_combustion_remains() > skb_duration() firecombustion_cooldownsshortcdactions()

  unless extended_combustion_remains() > skb_duration() and firecombustion_cooldownsshortcdpostconditions()
  {
   #call_action_list,name=active_talents
   fireactive_talentsshortcdactions()

   unless fireactive_talentsshortcdpostconditions() or buffexpires(combustion) and buffstacks(sun_kings_blessing_buff) > 2 and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) < 20 and buffpresent(sun_kings_blessing_buff) and texture(sun_kings_blessing_buff text=cancel) or buffexpires(combustion) and spellcooldown(combustion) < casttime(flamestrike) and enemies() >= combustion_flamestrike() and spell(flamestrike) or buffexpires(combustion) and buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and spellcooldown(combustion) < casttime(fireball) and not conduit(flame_accretion_conduit) and spell(fireball) or buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) and spell(scorch) or not conduit(infernal_cascade_conduit) and not fire_blast_pooling() and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 and spell(fire_blast_fire) or conduit(infernal_cascade_conduit) and not fire_blast_pooling() and { expected_fire_blasts() >= needed_fire_blasts() or buffremaining(combustion) < gcd() or extended_combustion_remains() <= baseduration(infernal_cascade_buff) or buffstacks(infernal_cascade_buff) < 2 or buffremaining(infernal_cascade_buff) < gcd() or spellcooldown(shifting_power) <= 0 and use_shifting_power() } and buffpresent(combustion) and { not buffpresent(firestorm_buff) or buffremaining(infernal_cascade_buff) < 0.5 } and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 and spell(fire_blast_fire) or { buffpresent(hot_streak_buff) and enemies() >= combustion_flamestrike() or buffpresent(firestorm_buff) and enemies() >= combustion_flamestrike() - runeforge(firestorm_runeforge) } and spell(flamestrike)
   {
    ...
    unless buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak_buff) and buffpresent(combustion) and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up_buff) and enemies() < combustion_flamestrike() and buffpresent(combustion) and spell(pyroblast)
    {
     ...
     if use_shifting_power() and buffpresent(combustion) and not charges(fire_blast_fire) and charges(phoenix_flames) < spellmaxcharges(phoenix_flames) spell(shifting_power)
     ...
     if buffpresent(sun_kings_blessing_ready_buff) and { needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and enemies() >= combustion_flamestrike() and buffremaining(sun_kings_blessing_ready_buff) > executetime(rune_of_power) + casttime(flamestrike) spell(rune_of_power)

     unless buffpresent(sun_kings_blessing_ready_buff) and { needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and enemies() >= combustion_flamestrike() and buffremaining(sun_kings_blessing_ready_buff) > casttime(flamestrike) and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(flamestrike) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } and spell(flamestrike)
     {
      ...
      if buffpresent(sun_kings_blessing_ready_buff) and { not hastalent(pyroclasm_talent) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > executetime(rune_of_power) + casttime(pyroblast) or needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and buffremaining(sun_kings_blessing_ready_buff) > executetime(rune_of_power) + casttime(pyroblast) spell(rune_of_power)
     }
    }
   }
  }
...
{
 enemies() > 1 and buffexpires(combustion) and spell(living_bomb) or extended_combustion_remains() > skb_duration() and firecombustion_cooldownsshortcdpostconditions() or fireactive_talentsshortcdpostconditions() or buffexpires(combustion) and buffstacks(sun_kings_blessing_buff) > 2 and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) < 20 and buffpresent(sun_kings_blessing_buff) and texture(sun_kings_blessing_buff text=cancel) or buffexpires(combustion) and spellcooldown(combustion) < casttime(flamestrike) and enemies() >= combustion_flamestrike() and spell(flamestrike) or buffexpires(combustion) and buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and spellcooldown(combustion) < casttime(fireball) and not conduit(flame_accretion_conduit) and spell(fireball) or buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) and spell(scorch) or not conduit(infernal_cascade_conduit) and not fire_blast_pooling() and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 and spell(fire_blast_fire) or conduit(infernal_cascade_conduit) and not fire_blast_pooling() and { expected_fire_blasts() >= needed_fire_blasts() or buffremaining(combustion) < gcd() or extended_combustion_remains() <= baseduration(infernal_cascade_buff) or buffstacks(infernal_cascade_buff) < 2 or buffremaining(infernal_cascade_buff) < gcd() or spellcooldown(shifting_power) <= 0 and use_shifting_power() } and buffpresent(combustion) and { not buffpresent(firestorm_buff) or buffremaining(infernal_cascade_buff) < 0.5 } and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 and spell(fire_blast_fire) or { buffpresent(hot_streak_buff) and enemies() >= combustion_flamestrike() or buffpresent(firestorm_buff) and enemies() >= combustion_flamestrike() - runeforge(firestorm_runeforge) } and spell(flamestrike) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak_buff) and buffpresent(combustion) and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up_buff) and enemies() < combustion_flamestrike() and buffpresent(combustion) and spell(pyroblast) or buffpresent(sun_kings_blessing_ready_buff) and { needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and enemies() >= combustion_flamestrike() and buffremaining(sun_kings_blessing_ready_buff) > casttime(flamestrike) and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(flamestrike) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and { not hastalent(pyroclasm_talent) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) or needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(pyroblast) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } and spell(pyroblast) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and buffremaining(combustion) > casttime(pyroblast) and enemies() < combustion_flamestrike() and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(pyroblast) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } and spell(pyroblast) or buffpresent(combustion) and traveltime(phoenix_flames) < buffremaining(combustion) and buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 and spell(phoenix_flames) or buffremaining(combustion) > casttime(scorch) and spell(scorch) or buffremaining(combustion) < gcd() and enemies() > 1 and spell(living_bomb) or buffremaining(combustion) < gcd() and buffpresent(combustion) and target.distance() < 12 and spell(dragons_breath)
}
...
AddFunction firecombustion_phasecdactions
{
 ...
 unless buffexpires(combustion) and spell(bag_of_tricks) or enemies() > 1 and buffexpires(combustion) and spell(living_bomb)
 {
  #variable,use_off_gcd=1,use_while_casting=1,name=extended_combustion_remains,value=buff.combustion.remains+buff.combustion.duration*(cooldown.combustion.remains<buff.combustion.remains)
  #variable,use_off_gcd=1,use_while_casting=1,name=extended_combustion_remains,op=add,value=variable.skb_duration,if=runeforge.sun_kings_blessing&(buff.sun_kings_blessing_ready.up|variable.extended_combustion_remains>gcd.remains+1.5*gcd.max*(buff.sun_kings_blessing.max_stack-buff.sun_kings_blessing.stack))
  #call_action_list,name=combustion_cooldowns,if=variable.extended_combustion_remains>variable.skb_duration
  if extended_combustion_remains() > skb_duration() firecombustion_cooldownscdactions()

  unless extended_combustion_remains() > skb_duration() and firecombustion_cooldownscdpostconditions()
  {
   #call_action_list,name=active_talents
   fireactive_talentscdactions()

   unless fireactive_talentscdpostconditions() or buffexpires(combustion) and buffstacks(sun_kings_blessing_buff) > 2 and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) < 20 and buffpresent(sun_kings_blessing_buff) and texture(sun_kings_blessing_buff text=cancel) or buffexpires(combustion) and spellcooldown(combustion) < casttime(flamestrike) and enemies() >= combustion_flamestrike() and spell(flamestrike) or buffexpires(combustion) and buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and spellcooldown(combustion) < casttime(fireball) and not conduit(flame_accretion_conduit) and spell(fireball) or buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) and spell(scorch)
   {
...
{
 buffexpires(combustion) and spell(bag_of_tricks) or enemies() > 1 and buffexpires(combustion) and spell(living_bomb) or extended_combustion_remains() > skb_duration() and firecombustion_cooldownscdpostconditions() or fireactive_talentscdpostconditions() or buffexpires(combustion) and buffstacks(sun_kings_blessing_buff) > 2 and hastalent(rune_of_power_talent) and spellcooldown(rune_of_power) < 20 and buffpresent(sun_kings_blessing_buff) and texture(sun_kings_blessing_buff text=cancel) or buffexpires(combustion) and spellcooldown(combustion) < casttime(flamestrike) and enemies() >= combustion_flamestrike() and spell(flamestrike) or buffexpires(combustion) and buffpresent(sun_kings_blessing_ready_buff) and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and spell(pyroblast) or buffexpires(combustion) and spellcooldown(combustion) < casttime(fireball) and not conduit(flame_accretion_conduit) and spell(fireball) or buffexpires(combustion) and spellcooldown(combustion) < casttime(scorch) and spell(scorch) or not conduit(infernal_cascade_conduit) and not fire_blast_pooling() and buffpresent(combustion) and not buffpresent(firestorm_buff) and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 and spell(fire_blast_fire) or conduit(infernal_cascade_conduit) and not fire_blast_pooling() and { expected_fire_blasts() >= needed_fire_blasts() or buffremaining(combustion) < gcd() or extended_combustion_remains() <= baseduration(infernal_cascade_buff) or buffstacks(infernal_cascade_buff) < 2 or buffremaining(infernal_cascade_buff) < gcd() or spellcooldown(shifting_power) <= 0 and use_shifting_power() } and buffpresent(combustion) and { not buffpresent(firestorm_buff) or buffremaining(infernal_cascade_buff) < 0.5 } and not buffpresent(hot_streak_buff) and inflighttotarget(hot_streak_spells) + buffstacks(heating_up_buff) * { gcdremaining() > 0 } < 2 and spell(fire_blast_fire) or { buffpresent(hot_streak_buff) and enemies() >= combustion_flamestrike() or buffpresent(firestorm_buff) and enemies() >= combustion_flamestrike() - runeforge(firestorm_runeforge) } and spell(flamestrike) or buffpresent(combustion) and 2 * buffstacks(hot_streak_buff) + buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) == 2 and spell(radiant_spark) or buffpresent(firestorm_buff) and spell(pyroblast) or buffpresent(hot_streak_buff) and buffpresent(combustion) and spell(pyroblast) or previousgcdspell(scorch) and buffpresent(heating_up_buff) and enemies() < combustion_flamestrike() and buffpresent(combustion) and spell(pyroblast) or use_shifting_power() and buffpresent(combustion) and not charges(fire_blast_fire) and charges(phoenix_flames) < spellmaxcharges(phoenix_flames) and spell(shifting_power) or buffpresent(sun_kings_blessing_ready_buff) and { needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and enemies() >= combustion_flamestrike() and buffremaining(sun_kings_blessing_ready_buff) > executetime(rune_of_power) + casttime(flamestrike) and spell(rune_of_power) or buffpresent(sun_kings_blessing_ready_buff) and { needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and enemies() >= combustion_flamestrike() and buffremaining(sun_kings_blessing_ready_buff) > casttime(flamestrike) and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(flamestrike) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } and spell(flamestrike) or buffpresent(sun_kings_blessing_ready_buff) and { not hastalent(pyroclasm_talent) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > executetime(rune_of_power) + casttime(pyroblast) or needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and buffremaining(sun_kings_blessing_ready_buff) > executetime(rune_of_power) + casttime(pyroblast) and spell(rune_of_power) or buffpresent(sun_kings_blessing_ready_buff) and { not hastalent(pyroclasm_talent) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) or needed_fire_blasts() <= expected_fire_blasts() or target.debuffpresent(mirrors_of_torment) } and buffremaining(sun_kings_blessing_ready_buff) > casttime(pyroblast) and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(pyroblast) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } and spell(pyroblast) or buffpresent(pyroclasm_buff) and buffremaining(pyroclasm_buff) > casttime(pyroblast) and buffremaining(combustion) > casttime(pyroblast) and enemies() < combustion_flamestrike() and { not conduit(infernal_cascade_conduit) or buffremaining(infernal_cascade_buff) > executetime(pyroblast) or buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 } and spell(pyroblast) or buffpresent(combustion) and traveltime(phoenix_flames) < buffremaining(combustion) and buffstacks(heating_up_buff) + inflighttotarget(hot_streak_spells) < 2 and spell(phoenix_flames) or buffremaining(combustion) > casttime(scorch) and spell(scorch) or buffremaining(combustion) < gcd() and enemies() > 1 and spell(living_bomb) or buffremaining(combustion) < gcd() and buffpresent(combustion) and target.distance() < 12 and spell(dragons_breath)
}
...
AddFunction fireactive_talentsshortcdactions
{
 unless enemies() > 1 and buffexpires(combustion) and { time_to_combustion() > spellcooldownduration(living_bomb) or time_to_combustion() <= 0 } and spell(living_bomb)
 {
  ...
  if time_to_combustion() <= 0 or buffremaining(combustion) > traveltime(meteor) or spellcooldownduration(meteor) < time_to_combustion() and not hastalent(rune_of_power_talent) or hastalent(rune_of_power_talent) and buffpresent(rune_of_power_buff) and time_to_combustion() > spellcooldown(meteor) or fightremains() < time_to_combustion() spell(meteor)
 }
}
...
AddFunction fire_defaultmainactions
{
 ...
 unless not disable_combustion() and firecombustion_timingmainpostconditions()
 {
  ...
  if time_to_combustion() <= 0 or buffpresent(combustion) or time_to_combustion() < combustion_precast_time() and spellcooldown(combustion) < combustion_precast_time() firecombustion_phasemainactions()

  unless { time_to_combustion() <= 0 or buffpresent(combustion) or time_to_combustion() < combustion_precast_time() and spellcooldown(combustion) < combustion_precast_time() } and firecombustion_phasemainpostconditions()
  {
   ...
   if buffpresent(rune_of_power_buff) and buffexpires(combustion) and time_to_combustion() > 0 firerop_phasemainactions()

   unless buffpresent(rune_of_power_buff) and buffexpires(combustion) and time_to_combustion() > 0 and firerop_phasemainpostconditions()
   {
    ...
    if talent(firestarter_talent) and target.healthpercent() >= 90 and time_to_combustion() > 0 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up_buff) or executetime(fireball) > 0 and not buffpresent(hot_streak_buff) or inflighttotarget(pyroblast) and buffpresent(heating_up_buff) and not buffpresent(hot_streak_buff) } spell(fire_blast_fire)
    ...
    if executetime(shifting_power) > 0 and spellfullrecharge(fire_blast_fire) < 0 spell(fire_blast_fire)
    #call_action_list,name=standard_rotation,if=variable.time_to_combustion>0&buff.rune_of_power.down&buff.combustion.down
    if time_to_combustion() > 0 and buffexpires(rune_of_power_buff) and buffexpires(combustion) firestandard_rotationmainactions()

    unless time_to_combustion() > 0 and buffexpires(rune_of_power_buff) and buffexpires(combustion) and firestandard_rotationmainpostconditions()
    {
     #scorch
     spell(scorch)
    }
...
{
 not disable_combustion() and firecombustion_timingmainpostconditions() or { time_to_combustion() <= 0 or buffpresent(combustion) or time_to_combustion() < combustion_precast_time() and spellcooldown(combustion) < combustion_precast_time() } and firecombustion_phasemainpostconditions() or buffpresent(rune_of_power_buff) and buffexpires(combustion) and time_to_combustion() > 0 and firerop_phasemainpostconditions() or time_to_combustion() > 0 and buffexpires(rune_of_power_buff) and buffexpires(combustion) and firestandard_rotationmainpostconditions()
}
...
AddFunction fire_defaultshortcdactions
{
 unless not buffpresent(newfound_resolve_buff) and debuffpresent(trial_of_doubt_debuff) and debuffremains(trial_of_doubt_debuff) < 10 and texture(inv_enchant_essencemagiclarge text=face)
 {
  ...
  unless not disable_combustion() and firecombustion_timingshortcdpostconditions()
  {
   ...
   if time_to_combustion() > spellcooldown(mirrors_of_torment) - 30 * runeforge(sinful_delight_runeforge) spell(mirrors_of_torment)
   ...
   unless executetime(mirrors_of_torment) > 0 and spellfullrecharge(fire_blast_fire) - executetime(mirrors_of_torment) < 4 and not inflighttotarget(hot_streak_spells) and not buffpresent(hot_streak_buff) and spell(fire_blast_fire) or { not conduit(infernal_cascade_conduit) and timeincombat() < 5 or buffstacks(infernal_cascade_buff) == spelldata(infernal_cascade_buff max_stacks) } and buffpresent(soul_ignition_buff) and texture(soul_ignition_buff text=cancel)
   {
    ...
    unless runeforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) <= 0 and buffexpires(disciplinary_command_arcane_buff) and not buffpresent(disciplinary_command_fire_buff) and { time_to_combustion() + executetime(arcane_explosion) + casttime(frostbolt) > spellcooldownduration(disciplinary_command) or time_to_combustion() < 5 and not hastalent(rune_of_power_talent) } and spell(arcane_explosion) or runeforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) < casttime(frostbolt) and buffexpires(disciplinary_command_frost_buff) and not buffpresent(disciplinary_command_fire_buff) and { time_to_combustion() + casttime(frostbolt) > spellcooldownduration(disciplinary_command) or time_to_combustion() < 5 } and spell(frostbolt)
    {
     ...
     #variable,use_off_gcd=1,use_while_casting=1,name=fire_blast_pooling,value=variable.extended_combustion_remains<variable.time_to_combustion&action.fire_blast.charges_fractional+(variable.time_to_combustion+action.shifting_power.full_reduction*variable.shifting_power_before_combustion+(debuff.mirrors_of_torment.max_stack-1)*variable.mot_recharge_amount*covenant.venthyr*(cooldown.mirrors_of_torment.remains<=variable.time_to_combustion))%cooldown.fire_blast.duration-1<cooldown.fire_blast.max_charges+variable.overpool_fire_blasts%cooldown.fire_blast.duration-(buff.combustion.duration%cooldown.fire_blast.duration)%%1&variable.time_to_combustion<fight_remains
     #call_action_list,name=combustion_phase,if=variable.time_to_combustion<=0|buff.combustion.up|variable.time_to_combustion<variable.combustion_precast_time&cooldown.combustion.remains<variable.combustion_precast_time
     if time_to_combustion() <= 0 or buffpresent(combustion) or time_to_combustion() < combustion_precast_time() and spellcooldown(combustion) < combustion_precast_time() firecombustion_phaseshortcdactions()

     unless { time_to_combustion() <= 0 or buffpresent(combustion) or time_to_combustion() < combustion_precast_time() and spellcooldown(combustion) < combustion_precast_time() } and firecombustion_phaseshortcdpostconditions()
     {
      ...
      if buffexpires(combustion) and buffexpires(rune_of_power_buff) and not buffpresent(firestorm_buff) and { time_to_combustion() >= baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast_fire) or time_to_combustion() > fightremains() } and { not runeforge(sun_kings_blessing_runeforge) or enemies() >= hard_cast_flamestrike() or buffpresent(sun_kings_blessing_ready_buff) or buffstacks(sun_kings_blessing_buff) >= spelldata(sun_kings_blessing_buff max_stacks) - 1 or fightremains() < baseduration(rune_of_power_buff) } spell(rune_of_power)
      #variable,use_off_gcd=1,use_while_casting=1,name=fire_blast_pooling,value=searing_touch.active&action.fire_blast.full_recharge_time>3*gcd.max,if=!variable.fire_blast_pooling&runeforge.sun_kings_blessing
      #variable,name=phoenix_pooling,if=active_enemies<variable.combustion_flamestrike,value=variable.time_to_combustion+buff.combustion.duration-5<action.phoenix_flames.full_recharge_time+cooldown.phoenix_flames.duration-action.shifting_power.full_reduction*variable.shifting_power_before_combustion&variable.time_to_combustion<fight_remains|runeforge.sun_kings_blessing|time<5
      #variable,name=phoenix_pooling,if=active_enemies>=variable.combustion_flamestrike,value=variable.time_to_combustion<action.phoenix_flames.full_recharge_time-action.shifting_power.full_reduction*variable.shifting_power_before_combustion&variable.time_to_combustion<fight_remains|runeforge.sun_kings_blessing|time<5
      #call_action_list,name=rop_phase,if=buff.rune_of_power.up&buff.combustion.down&variable.time_to_combustion>0
      if buffpresent(rune_of_power_buff) and buffexpires(combustion) and time_to_combustion() > 0 firerop_phaseshortcdactions()

      unless buffpresent(rune_of_power_buff) and buffexpires(combustion) and time_to_combustion() > 0 and firerop_phaseshortcdpostconditions() or not fire_blast_pooling() and time_to_combustion() > 0 and enemies() >= hard_cast_flamestrike() and not { talent(firestarter_talent) and target.healthpercent() >= 90 } and not buffpresent(hot_streak_buff) and { buffpresent(heating_up_buff) and executetime(flamestrike) < 0.5 or charges(fire_blast_fire count=0) >= 2 } and spell(fire_blast_fire) or talent(firestarter_talent) and target.healthpercent() >= 90 and time_to_combustion() > 0 and not fire_blast_pooling() and { not executetime(fireball) > 0 and not inflighttotarget(pyroblast) and buffpresent(heating_up_buff) or executetime(fireball) > 0 and not buffpresent(hot_streak_buff) or inflighttotarget(pyroblast) and buffpresent(heating_up_buff) and not buffpresent(hot_streak_buff) } and spell(fire_blast_fire) or executetime(shifting_power) > 0 and spellfullrecharge(fire_blast_fire) < 0 and spell(fire_blast_fire)
      {
       #call_action_list,name=standard_rotation,if=variable.time_to_combustion>0&buff.rune_of_power.down&buff.combustion.down
       if time_to_combustion() > 0 and buffexpires(rune_of_power_buff) and buffexpires(combustion) firestandard_rotationshortcdactions()
      }
     }
    }
   }
 ...
 unless not buffpresent(newfound_resolve_buff) and debuffpresent(trial_of_doubt_debuff) and debuffremains(trial_of_doubt_debuff) < 10 and texture(inv_enchant_essencemagiclarge text=face)
 {
  ...
  unless not disable_combustion() and firecombustion_timingcdpostconditions() or buffexpires(combustion) and charges(fire_blast_fire) <= 1 and not { buffpresent(infernal_cascade_buff) and buffpresent(hot_streak_buff) } and shifting_power_before_combustion() and spell(shifting_power) or buffexpires(combustion) and time_to_combustion() > spellcooldown(radiant_spark) - 5 and spell(radiant_spark)
  {
   ...
   unless time_to_combustion() < combustion_precast_time() + executetime(mirrors_of_torment) and buffexpires(combustion) and spell(mirrors_of_torment) or time_to_combustion() > spellcooldown(mirrors_of_torment) - 30 * runeforge(sinful_delight_runeforge) and spell(mirrors_of_torment) or executetime(mirrors_of_torment) > 0 and spellfullrecharge(fire_blast_fire) - executetime(mirrors_of_torment) < 4 and not inflighttotarget(hot_streak_spells) and not buffpresent(hot_streak_buff) and spell(fire_blast_fire)
   {
    ...
    if { buffexpires(combustion) and time_to_combustion() >= on_use_cutoff() or on_use_cutoff() == 0 } and hastrinket(glyph_of_assimilation_item) item(glyph_of_assimilation_item usable=1)
    ...
    unless { not conduit(infernal_cascade_conduit) and timeincombat() < 5 or buffstacks(infernal_cascade_buff) == spelldata(infernal_cascade_buff max_stacks) } and buffpresent(soul_ignition_buff) and texture(soul_ignition_buff text=cancel)
    {
     ...
     if hasequippeditem(gladiators_badge_item) and { buffexpires(combustion) and time_to_combustion() >= on_use_cutoff() or on_use_cutoff() == 0 } and hastrinket(inscrutable_quantum_device_item) item(inscrutable_quantum_device_item usable=1)
     ...
     if hasequippeditem(gladiators_badge_item) and { buffexpires(combustion) and time_to_combustion() >= on_use_cutoff() or on_use_cutoff() == 0 } and hastrinket(flame_of_battle_item) item(flame_of_battle_item usable=1)
     ...
     if hasequippeditem(gladiators_badge_item) and { buffexpires(combustion) and time_to_combustion() >= on_use_cutoff() or on_use_cutoff() == 0 } and hastrinket(wakeners_frond_item) item(wakeners_frond_item usable=1)
     ...
     if hasequippeditem(gladiators_badge_item) and { buffexpires(combustion) and time_to_combustion() >= on_use_cutoff() or on_use_cutoff() == 0 } and hastrinket(instructors_divine_bell_item) item(instructors_divine_bell_item usable=1)
     ...
     if hasequippeditem(gladiators_badge_item) and { buffexpires(combustion) and time_to_combustion() >= on_use_cutoff() or on_use_cutoff() == 0 } and hastrinket(sunblood_amethyst_item) item(sunblood_amethyst_item usable=1)
     #use_items,if=buff.combustion.down&variable.time_to_combustion>=variable.on_use_cutoff|variable.on_use_cutoff=0
     if buffexpires(combustion) and time_to_combustion() >= on_use_cutoff() or on_use_cutoff() == 0 fireuseitemactions()
     ...
     unless runeforge(grisly_icicle_runeforge) and buffexpires(combustion) and { time_to_combustion() > spellcooldown(frost_nova) or time_to_combustion() < combustion_precast_time() + executetime(frost_nova) } and spell(frost_nova)
     {
      ...
      unless runeforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) <= 0 and buffexpires(disciplinary_command_arcane_buff) and not buffpresent(disciplinary_command_fire_buff) and { time_to_combustion() + executetime(arcane_explosion) + casttime(frostbolt) > spellcooldownduration(disciplinary_command) or time_to_combustion() < 5 and not hastalent(rune_of_power_talent) } and spell(arcane_explosion) or runeforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) < casttime(frostbolt) and buffexpires(disciplinary_command_frost_buff) and not buffpresent(disciplinary_command_fire_buff) and { time_to_combustion() + casttime(frostbolt) > spellcooldownduration(disciplinary_command) or time_to_combustion() < 5 } and spell(frostbolt) or runeforge(disciplinary_command_runeforge) and spellcooldown(disciplinary_command) <= 0 and buffexpires(disciplinary_command_frost_buff) and not buffpresent(disciplinary_command_fire_buff) and { time_to_combustion() > spellcooldownduration(disciplinary_command) or time_to_combustion() < 5 } and spell(frost_nova)
      {
       #variable,use_off_gcd=1,use_while_casting=1,name=fire_blast_pooling,value=variable.extended_combustion_remains<variable.time_to_combustion&action.fire_blast.charges_fractional+(variable.time_to_combustion+action.shifting_power.full_reduction*variable.shifting_power_before_combustion+(debuff.mirrors_of_torment.max_stack-1)*variable.mot_recharge_amount*covenant.venthyr*(cooldown.mirrors_of_torment.remains<=variable.time_to_combustion))%cooldown.fire_blast.duration-1<cooldown.fire_blast.max_charges+variable.overpool_fire_blasts%cooldown.fire_blast.duration-(buff.combustion.duration%cooldown.fire_blast.duration)%%1&variable.time_to_combustion<fight_remains
       #call_action_list,name=combustion_phase,if=variable.time_to_combustion<=0|buff.combustion.up|variable.time_to_combustion<variable.combustion_precast_time&cooldown.combustion.remains<variable.combustion_precast_time
       if time_to_combustion() <= 0 or buffpresent(combustion) or time_to_combustion() < combustion_precast_time() and spellcooldown(combustion) < combustion_precast_time() firecombustion_phasecdactions()

       unless { time_to_combustion() <= 0 or buffpresent(combustion) or time_to_combustion() < combustion_precast_time() and spellcooldown(combustion) < combustion_precast_time() } and firecombustion_phasecdpostconditions() or buffexpires(combustion) and buffexpires(rune_of_power_buff) and not buffpresent(firestorm_buff) and { time_to_combustion() >= baseduration(rune_of_power_buff) and time_to_combustion() > spellfullrecharge(fire_blast_fire) or time_to_combustion() > fightremains() } and { not runeforge(sun_kings_blessing_runeforge) or enemies() >= hard_cast_flamestrike() or buffpresent(sun_kings_blessing_ready_buff) or buffstacks(sun_kings_blessing_buff) >= spelldata(sun_kings_blessing_buff max_stacks) - 1 or fightremains() < baseduration(rune_of_power_buff) } and spell(rune_of_power)
       {
        #variable,use_off_gcd=1,use_while_casting=1,name=fire_blast_pooling,value=searing_touch.active&action.fire_blast.full_recharge_time>3*gcd.max,if=!variable.fire_blast_pooling&runeforge.sun_kings_blessing
        #variable,name=phoenix_pooling,if=active_enemies<variable.combustion_flamestrike,value=variable.time_to_combustion+buff.combustion.duration-5<action.phoenix_flames.full_recharge_time+cooldown.phoenix_flames.duration-action.shifting_power.full_reduction*variable.shifting_power_before_combustion&variable.time_to_combustion<fight_remains|runeforge.sun_kings_blessing|time<5
        #variable,name=phoenix_pooling,if=active_enemies>=variable.combustion_flamestrike,value=variable.time_to_combustion<action.phoenix_flames.full_recharge_time-action.shifting_power.full_reduction*variable.shifting_power_before_combustion&variable.time_to_combustion<fight_remains|runeforge.sun_kings_blessing|time<5
        #call_action_list,name=rop_phase,if=buff.rune_of_power.up&buff.combustion.down&variable.time_to_combustion>0
        if buffpresent(rune_of_power_buff) and buffexpires(combustion) and time_to_combustion() > 0 firerop_phasecdactions()
        ...
        {
         #call_action_list,name=standard_rotation,if=variable.time_to_combustion>0&buff.rune_of_power.down&buff.combustion.down
         if time_to_combustion() > 0 and buffexpires(rune_of_power_buff) and buffexpires(combustion) firestandard_rotationcdactions()
        }
       }
      }
     }
