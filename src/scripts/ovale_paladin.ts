...
AddFunction ProtectionDefaultMainActions
{
 ...
 if SpellCooldown(judgment_prot) < GCD() and SpellCharges(judgment_prot count=0) > 1 and not SpellCooldown(judgment_prot) > 0 or not Talent(crusaders_judgment_talent) Spell(judgment_prot)
 ...
 if SpellCooldown(judgment_prot) <= GCD() and not Talent(crusaders_judgment_talent) or SpellCooldown(avenger_shield) <= GCD() and FIXME_consecration.remains < GCD() Spell(consecration)
 ...
 if not Talent(crusaders_judgment_talent) and FIXME_consecration.remains < SpellCooldown(judgment_prot) + SpellCooldown(avengers_shield) and FIXME_consecration.remains < 3 * GCD() Spell(consecration)
 #judgment,if=cooldown_react|!talent.crusaders_judgment.enabled
 if not SpellCooldown(judgment_prot) > 0 or not Talent(crusaders_judgment_talent) Spell(judgment_prot)
 #blessed_hammer
...
AddFunction ProtectionDefaultCdActions
{
 ...
 unless SpellCharges(shield_of_the_righteous count=0) >= 2 and Spell(seraphim)
 {
  ...
  unless BuffPresent(avengers_valor_buff) and SpellCharges(shield_of_the_righteous count=0) >= 2 and { SpellCooldown(seraphim) > GCD() or not Talent(seraphim_talent) } and Spell(shield_of_the_righteous) or SpellCharges(shield_of_the_righteous count=0) == 3 and SpellCooldown(avenger_shield) > 2 * GCD() and Spell(shield_of_the_righteous) or { BuffPresent(avenging_wrath_buff) and not Talent(seraphim_talent) or BuffPresent(seraphim_buff) and BuffPresent(avengers_valor_buff) } and Spell(shield_of_the_righteous) or { BuffPresent(avenging_wrath_buff) and BuffRemaining(avenging_wrath_buff) < 4 and not Talent(seraphim_talent) or BuffRemaining(seraphim_buff) < 4 and BuffPresent(seraphim_buff) } and Spell(shield_of_the_righteous)
  {
   ...
   unless { SpellCharges(shield_of_the_righteous count=0) > 2 and not BuffPresent(avengers_valor_buff) or Enemies() >= 2 } and not SpellCooldown(avengers_shield) > 0 and Spell(avengers_shield) or { SpellCooldown(judgment_prot) < GCD() and SpellCharges(judgment_prot count=0) > 1 and not SpellCooldown(judgment_prot) > 0 or not Talent(crusaders_judgment_talent) } and Spell(judgment_prot) or not SpellCooldown(avengers_shield) > 0 and Spell(avengers_shield) or { SpellCooldown(judgment_prot) <= GCD() and not Talent(crusaders_judgment_talent) or SpellCooldown(avenger_shield) <= GCD() and FIXME_consecration.remains < GCD() } and Spell(consecration) or not Talent(crusaders_judgment_talent) and FIXME_consecration.remains < SpellCooldown(judgment_prot) + SpellCooldown(avengers_shield) and FIXME_consecration.remains < 3 * GCD() and Spell(consecration) or { not SpellCooldown(judgment_prot) > 0 or not Talent(crusaders_judgment_talent) } and Spell(judgment_prot)
   {
...
AddFunction RetributionGeneratorsMainActions
{
 ...
 unless HolyPower() >= 5 and RetributionFinishersMainPostConditions()
 {
  ...
  unless Talent(hammer_of_wrath_talent) and { target.HealthPercent() <= 20 or BuffPresent(avenging_wrath_buff) or BuffPresent(crusade_buff) } and { BuffPresent(divine_purpose_buff) or BuffStacks(crusade_buff) < 10 } and RetributionFinishersMainPostConditions()
  {
   ...
   if SpellCharges(crusader_strike count=0) >= 1 and { HolyPower() <= 2 or HolyPower() <= 3 and SpellCooldown(blade_of_justice) > GCD() * 2 or HolyPower() == 4 and SpellCooldown(blade_of_justice) > GCD() * 2 and SpellCooldown(judgment) > GCD() * 2 and SpellCooldown(consecration) > GCD() * 2 } Spell(crusader_strike)
