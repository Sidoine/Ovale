...
AddFunction assassinationprecombatshortcdactions
{
 unless buffremaining(lethal_poison_buff) < 1200 and spell(deadly_poison)
 {
  ...
  #food
  #snapshot_stats
  #marked_for_death,precombat_seconds=10,if=!covenant.venthyr&raid_event.adds.in>15
  if not iscovenant("venthyr") and 600 > 15 spell(marked_for_death)
 }
}
...
{
 unless buffremaining(lethal_poison_buff) < 1200 and spell(deadly_poison) or not iscovenant("venthyr") and 600 > 15 and spell(marked_for_death)
 {
...
{
 buffremaining(lethal_poison_buff) < 1200 and spell(deadly_poison) or not iscovenant("venthyr") and 600 > 15 and spell(marked_for_death) or spell(stealth) or spell(slice_and_dice)
}
...
AddFunction assassinationdotmainactions
{
 ...
 unless target.refreshable(garrote) and combopointsdeficit() >= 1 and { persistentmultiplier(garrote) <= 1 or target.debuffremaining(garrote) <= target.currentticktime(garrote) and enemies(tagged=1) >= 3 } and { not target.debuffpresent(exsanguinated) or target.debuffremaining(garrote) <= target.currentticktime(garrote) * 2 and enemies(tagged=1) >= 3 } and target.timetodie() - target.debuffremaining(garrote) > 4 and buffremaining(master_assassin_buff) == 0 and { spellusable(garrote) and spellcooldown(garrote) < timetoenergyfor(garrote) }
 {
  ...
  unless not skip_cycle_garrote() and not never(target_is_target) and target.refreshable(garrote) and combopointsdeficit() >= 1 and { persistentmultiplier(garrote) <= 1 or target.debuffremaining(garrote) <= target.currentticktime(garrote) and enemies(tagged=1) >= 3 } and { not target.debuffpresent(exsanguinated) or target.debuffremaining(garrote) <= target.currentticktime(garrote) * 2 and enemies(tagged=1) >= 3 } and target.timetodie() - target.debuffremaining(garrote) > 12 and buffremaining(master_assassin_buff) == 0 and { spellusable(garrote) and spellcooldown(garrote) < timetoenergyfor(garrote) }
  {
   ...
   #crimson_tempest,if=spell_targets=1&(!runeforge.dashing_scoundrel|rune_word.frost.enabled)&master_assassin_remains=0&effective_combo_points>=(cp_max_spend-1)&refreshable&!exsanguinated&!debuff.shiv.up&target.time_to_die-remains>4
   if enemies(tagged=1) == 1 and { not runeforge(dashing_scoundrel_runeforge) or always(rune_word) } and buffremaining(master_assassin_buff) == 0 and combopoints() >= maxcombopoints() - 1 and target.refreshable(crimson_tempest) and not target.debuffpresent(exsanguinated) and not target.debuffpresent(shiv_debuff) and target.timetodie() - target.debuffremaining(crimson_tempest) > 4 spell(crimson_tempest)
  }
 }
}
...
{
 #envenom,if=effective_combo_points>=4+talent.deeper_stratagem.enabled&(debuff.vendetta.up|debuff.shiv.up|debuff.flagellation.up|energy.deficit<=25+energy.regen_combined|!variable.single_target|effective_combo_points>cp_max_spend)&(!talent.exsanguinate.enabled|cooldown.exsanguinate.remains>2)
 if combopoints() >= 4 + talentpoints(deeper_stratagem_talent) and { target.debuffpresent(vendetta) or target.debuffpresent(shiv_debuff) or target.debuffpresent(flagellation) or energydeficit() <= 25 + energyregenrate() or not single_target() or combopoints() > maxcombopoints() } and { not hastalent(exsanguinate_talent) or spellcooldown(exsanguinate) > 2 } spell(envenom)
...
AddFunction assassinationcdsshortcdactions
{
 ...
 if timesincepreviousspell(marked_for_death) > 1.5 and { never(raid_event_adds_exists) and { not single_target() or target.timetodie() < 30 } and { target.timetodie() < combopointsdeficit() * 1.5 or combopointsdeficit() >= maxcombopoints() } } spell(marked_for_death)
 #marked_for_death,if=raid_event.adds.in>30-raid_event.adds.duration&combo_points.deficit>=cp_max_spend&!cooldown.shiv.ready&(!covenant.venthyr|debuff.flagellation.up|cooldown.flagellation.remains>15)
 if 600 > 30 - 10 and combopointsdeficit() >= maxcombopoints() and not spellcooldown(shiv) <= 0 and { not iscovenant("venthyr") or target.debuffpresent(flagellation) or spellcooldown(flagellation) > 15 } spell(marked_for_death)
...
{
 unless timesincepreviousspell(marked_for_death) > 1.5 and { never(raid_event_adds_exists) and { not single_target() or target.timetodie() < 30 } and { target.timetodie() < combopointsdeficit() * 1.5 or combopointsdeficit() >= maxcombopoints() } } and spell(marked_for_death) or 600 > 30 - 10 and combopointsdeficit() >= maxcombopoints() and not spellcooldown(shiv) <= 0 and { not iscovenant("venthyr") or target.debuffpresent(flagellation) or spellcooldown(flagellation) > 15 } and spell(marked_for_death)
 {
...
AddFunction effective_combo_points
{
 if iscovenant("kyrian") and combopoints() > combopoints() and combopointsdeficit() > 2 and 1 < 0.5 and not { combopoints() == 2 and buffpresent(echoing_reprimand_3_buff) or combopoints() == 3 and buffpresent(echoing_reprimand_4_buff) or combopoints() == 4 and buffpresent(echoing_reprimand_5_buff) } combopoints()
 combopoints()
}
...
AddFunction subtletystealthedmainactions
{
 ...
 if effective_combo_points() >= maxcombopoints() subtletyfinishmainactions()

 unless effective_combo_points() >= maxcombopoints() and subtletyfinishmainpostconditions()
 {
  ...
  unless buffpresent(shuriken_tornado) and combopointsdeficit() <= 2 and subtletyfinishmainpostconditions()
  {
   ...
   if enemies(tagged=1) >= 4 and effective_combo_points() >= 4 subtletyfinishmainactions()

   unless enemies(tagged=1) >= 4 and effective_combo_points() >= 4 and subtletyfinishmainpostconditions()
   {
    ...
    unless combopointsdeficit() <= 1 - { hastalent(deeper_stratagem_talent) and buffpresent(vanish) } and subtletyfinishmainpostconditions()
    {
     ...
     if not use_priority_rotation() and target.debuffremaining(find_weakness_debuff) < 1 and enemies(tagged=1) <= 3 and target.timetodie() - buffremaining(shadowstrike) > 6 spell(shadowstrike text=cycle)
...
{
 effective_combo_points() >= maxcombopoints() and subtletyfinishmainpostconditions() or buffpresent(shuriken_tornado) and combopointsdeficit() <= 2 and subtletyfinishmainpostconditions() or enemies(tagged=1) >= 4 and effective_combo_points() >= 4 and subtletyfinishmainpostconditions() or combopointsdeficit() <= 1 - { hastalent(deeper_stratagem_talent) and buffpresent(vanish) } and subtletyfinishmainpostconditions()
}
...
AddFunction subtletystealthedshortcdactions
{
 ...
 {
  #call_action_list,name=finish,if=variable.effective_combo_points>=cp_max_spend
  if effective_combo_points() >= maxcombopoints() subtletyfinishshortcdactions()

  unless effective_combo_points() >= maxcombopoints() and subtletyfinishshortcdpostconditions()
  {
   #call_action_list,name=finish,if=buff.shuriken_tornado.up&combo_points.deficit<=2
   ...
   {
    #call_action_list,name=finish,if=spell_targets.shuriken_storm>=4&variable.effective_combo_points>=4
    if enemies(tagged=1) >= 4 and effective_combo_points() >= 4 subtletyfinishshortcdactions()

    unless enemies(tagged=1) >= 4 and effective_combo_points() >= 4 and subtletyfinishshortcdpostconditions()
    {
     #call_action_list,name=finish,if=combo_points.deficit<=1-(talent.deeper_stratagem.enabled&buff.vanish.up)
...
AddFunction subtletystealthedcdactions
{
 ...
 {
  #call_action_list,name=finish,if=variable.effective_combo_points>=cp_max_spend
  if effective_combo_points() >= maxcombopoints() subtletyfinishcdactions()
  ...
  unless effective_combo_points() >= maxcombopoints() and subtletyfinishcdpostconditions()
  {
   ...
   {
    #call_action_list,name=finish,if=spell_targets.shuriken_storm>=4&variable.effective_combo_points>=4
    if enemies(tagged=1) >= 4 and effective_combo_points() >= 4 subtletyfinishcdactions()
...
AddFunction subtletystealth_cdsshortcdactions
{
 ...
 unless race(nightelf)
 {
  ...
  #shadow_dance,if=variable.shd_combo_points&(variable.shd_threshold|buff.symbols_of_death.remains>=1.2|buff.chaos_bane.up|spell_targets.shuriken_storm>=4&cooldown.symbols_of_death.remains>10)&(buff.perforated_veins.stack<4|spell_targets.shuriken_storm>2)
  if shd_combo_points() and { shd_threshold() or buffremaining(symbols_of_death) >= 1.2 or buffpresent(chaos_bane_buff) or enemies(tagged=1) >= 4 and spellcooldown(symbols_of_death) > 10 } and { buffstacks(perforated_veins_buff) < 4 or enemies(tagged=1) > 2 } spell(shadow_dance)
...
{
 not race(nightelf) and { shd_combo_points() and { shd_threshold() or buffremaining(symbols_of_death) >= 1.2 or buffpresent(chaos_bane_buff) or enemies(tagged=1) >= 4 and spellcooldown(symbols_of_death) > 10 } and { buffstacks(perforated_veins_buff) < 4 or enemies(tagged=1) > 2 } and spell(shadow_dance) or shd_combo_points() and fightremains() < spellcooldown(symbols_of_death) and spell(shadow_dance) }
}
...
AddFunction subtletyfinishmainactions
{
 ...
 if not skip_rupture() and not use_priority_rotation() and enemies(tagged=1) >= 2 and target.timetodie() >= 5 + 2 * combopoints() and target.refreshable(rupture) spell(rupture text=cycle)
...
AddFunction subtletybuildmainactions
{
 ...
 #shuriken_storm,if=spell_targets>=2&(!covenant.necrolord|cooldown.serrated_bone_spike.max_charges-charges_fractional>=0.25|spell_targets.shuriken_storm>4)&buff.perforated_veins.stack<=4
 if enemies(tagged=1) >= 2 and { not iscovenant("necrolord") or spellmaxcharges(serrated_bone_spike) - charges(shuriken_storm count=0) >= 0.25 or enemies(tagged=1) > 4 } and buffstacks(perforated_veins_buff) <= 4 spell(shuriken_storm)
 #gloomblade
 spell(gloomblade)
 #backstab
 spell(backstab)
...
{
 unless not hastalent(nightstalker_talent) and runeforge(tiny_toxic_blade_runeforge) and enemies(tagged=1) < 5 and spell(shiv) or enemies(tagged=1) >= 2 and { not iscovenant("necrolord") or spellmaxcharges(serrated_bone_spike) - charges(shuriken_storm count=0) >= 0.25 or enemies(tagged=1) > 4 } and buffstacks(perforated_veins_buff) <= 4 and spell(shuriken_storm)
 {
...
AddFunction subtlety_defaultmainactions
{
 ...
 #variable,name=effective_combo_points,if=covenant.kyrian&effective_combo_points>combo_points&combo_points.deficit>2&time_to_sht.4.plus<0.5&!(combo_points=2&buff.echoing_reprimand_3.up|combo_points=3&buff.echoing_reprimand_4.up|combo_points=4&buff.echoing_reprimand_5.up),value=combo_points
 #call_action_list,name=cds
   ...
   unless use_priority_rotation() and subtletystealth_cdsmainpostconditions()
   {
    ...
    unless energydeficit() <= stealth_threshold() and subtletystealth_cdsmainpostconditions()
    {
     ...
     unless effective_combo_points() >= maxcombopoints() and subtletyfinishmainpostconditions()
     {
      ...
      if combopointsdeficit() <= 1 or fightremains() <= 1 and effective_combo_points() >= 3 or buffpresent(symbols_of_death_autocrit) and effective_combo_points() >= 4 subtletyfinishmainactions()

      unless { combopointsdeficit() <= 1 or fightremains() <= 1 and effective_combo_points() >= 3 or buffpresent(symbols_of_death_autocrit) and effective_combo_points() >= 4 } and subtletyfinishmainpostconditions()
      {
       #call_action_list,name=finish,if=spell_targets.shuriken_storm>=4&variable.effective_combo_points>=4
       if enemies(tagged=1) >= 4 and effective_combo_points() >= 4 subtletyfinishmainactions()
   ...
   unless buffpresent(stealthed_buff) and subtletystealthedshortcdpostconditions() or enemies(tagged=1) < 6 and fightremains() > 6 and buffremaining(slice_and_dice) < gcd() and combopoints() >= 4 - { timeincombat() < 10 } * 2 and spell(slice_and_dice)
   {
    ...
    unless use_priority_rotation() and subtletystealth_cdsshortcdpostconditions()
    {
     ...
     unless energydeficit() <= stealth_threshold() and subtletystealth_cdsshortcdpostconditions()
     {
      ...
      {
       #call_action_list,name=finish,if=combo_points.deficit<=1|fight_remains<=1&variable.effective_combo_points>=3|buff.symbols_of_death_autocrit.up&variable.effective_combo_points>=4
       if combopointsdeficit() <= 1 or fightremains() <= 1 and effective_combo_points() >= 3 or buffpresent(symbols_of_death_autocrit) and effective_combo_points() >= 4 subtletyfinishshortcdactions()

       unless { combopointsdeficit() <= 1 or fightremains() <= 1 and effective_combo_points() >= 3 or buffpresent(symbols_of_death_autocrit) and effective_combo_points() >= 4 } and subtletyfinishshortcdpostconditions()
       {
        #call_action_list,name=finish,if=spell_targets.shuriken_storm>=4&variable.effective_combo_points>=4
        if enemies(tagged=1) >= 4 and effective_combo_points() >= 4 subtletyfinishshortcdactions()
   ...
   unless buffpresent(stealthed_buff) and subtletystealthedcdpostconditions() or enemies(tagged=1) < 6 and fightremains() > 6 and buffremaining(slice_and_dice) < gcd() and combopoints() >= 4 - { timeincombat() < 10 } * 2 and spell(slice_and_dice)
   {
    ...
    unless use_priority_rotation() and subtletystealth_cdscdpostconditions()
    {
     ...
     unless energydeficit() <= stealth_threshold() and subtletystealth_cdscdpostconditions()
     {
      ...
      {
       #call_action_list,name=finish,if=combo_points.deficit<=1|fight_remains<=1&variable.effective_combo_points>=3|buff.symbols_of_death_autocrit.up&variable.effective_combo_points>=4
       if combopointsdeficit() <= 1 or fightremains() <= 1 and effective_combo_points() >= 3 or buffpresent(symbols_of_death_autocrit) and effective_combo_points() >= 4 subtletyfinishcdactions()
...
# deeper_stratagem_talent
# echoing_reprimand
# echoing_reprimand_3_buff
# echoing_reprimand_4_buff
# echoing_reprimand_5_buff
