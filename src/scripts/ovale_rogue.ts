...
{
	const name = "sc_t23_rogue_assassination"
	const desc = "[8.2] Simulationcraft: T23_Rogue_Assassination"
	const code = `
...
{
 HasAzeriteTrait(shrouded_suffocation_trait) and { FIXME_non_ss_buffed_targets >= 1 or Enemies() == 3 } and { FIXME_ss_buffed_targets_above_pandemic == 0 or Enemies() >= 6 }
}
...
AddFunction AssassinationStealthedMainActions
{
 ...
 if HasAzeriteTrait(shrouded_suffocation_trait) and BuffPresent(subterfuge_buff) and BuffRemaining(subterfuge_buff) < 1.3 and not False() Spell(garrote)
 unless HasAzeriteTrait(shrouded_suffocation_trait) and BuffPresent(subterfuge_buff) and BuffRemaining(subterfuge_buff) < 1.3 and not False() and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote)
 {
  #pool_resource,for_next=1
  ...
  unless Talent(subterfuge_talent) and { target.DebuffRemaining(garrote_debuff) < 12 or PersistentMultiplier(garrote_debuff) <= 1 } and target.TimeToDie() - target.DebuffRemaining(garrote_debuff) > 2 and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote)
  {
   ...
   if Talent(subterfuge_talent) and HasAzeriteTrait(shrouded_suffocation_trait) and target.TimeToDie() > target.DebuffRemaining(garrote_debuff) and { target.DebuffRemaining(garrote_debuff) < 18 or not False() } Spell(garrote)
   unless Talent(subterfuge_talent) and HasAzeriteTrait(shrouded_suffocation_trait) and target.TimeToDie() > target.DebuffRemaining(garrote_debuff) and { target.DebuffRemaining(garrote_debuff) < 18 or not False() } and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote)
   {
    #pool_resource,for_next=1
...
{
 ComboPoints() >= 4 and { Talent(nightstalker_talent) or Talent(subterfuge_talent) and { Talent(exsanguinate_talent) and SpellCooldown(exsanguinate) <= 2 or not target.DebuffPresent(rupture_debuff) } and single_target() } and target.TimeToDie() - target.DebuffRemaining(rupture_debuff) > 6 and Spell(rupture) or HasAzeriteTrait(shrouded_suffocation_trait) and BuffPresent(subterfuge_buff) and BuffRemaining(subterfuge_buff) < 1.3 and not False() and Spell(garrote) or not { HasAzeriteTrait(shrouded_suffocation_trait) and BuffPresent(subterfuge_buff) and BuffRemaining(subterfuge_buff) < 1.3 and not False() and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote) } and { Talent(subterfuge_talent) and { target.DebuffRemaining(garrote_debuff) < 12 or PersistentMultiplier(garrote_debuff) <= 1 } and target.TimeToDie() - target.DebuffRemaining(garrote_debuff) > 2 and Spell(garrote) or not { Talent(subterfuge_talent) and { target.DebuffRemaining(garrote_debuff) < 12 or PersistentMultiplier(garrote_debuff) <= 1 } and target.TimeToDie() - target.DebuffRemaining(garrote_debuff) > 2 and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote) } and { Talent(subterfuge_talent) and HasAzeriteTrait(shrouded_suffocation_trait) and not target.DebuffPresent(rupture_debuff) and single_target() and Spell(rupture) or Talent(subterfuge_talent) and HasAzeriteTrait(shrouded_suffocation_trait) and target.TimeToDie() > target.DebuffRemaining(garrote_debuff) and { target.DebuffRemaining(garrote_debuff) < 18 or not False() } and Spell(garrote) or not { Talent(subterfuge_talent) and HasAzeriteTrait(shrouded_suffocation_trait) and target.TimeToDie() > target.DebuffRemaining(garrote_debuff) and { target.DebuffRemaining(garrote_debuff) < 18 or not False() } and SpellUsable(garrote) and SpellCooldown(garrote) < TimeToEnergyFor(garrote) } and Talent(subterfuge_talent) and Talent(exsanguinate_talent) and SpellCooldown(exsanguinate) < 1 and PreviousGCDSpell(rupture) and target.DebuffRemaining(rupture_debuff) > 5 + 4 * MaxComboPoints() and Spell(garrote) } }
}
...
{
	const name = "sc_t23_rogue_outlaw"
	const desc = "[8.2] Simulationcraft: T23_Rogue_Outlaw"
	const code = `
...
{
	const name = "sc_t23_rogue_subtlety"
	const desc = "[8.2] Simulationcraft: T23_Rogue_Subtlety"
	const code = `
...
AddFunction SubtletyStealthedMainActions
{
 ...
 unless ComboPointsDeficit() <= 1 - { Talent(deeper_stratagem_talent) and { BuffPresent(vanish_buff) or HasAzeriteTrait(the_first_dance_trait) and not Talent(dark_shadow_talent) and not Talent(subterfuge_talent) and Enemies() < 3 } } and SubtletyFinishMainPostConditions()
 {
  ...
  if AzeriteTraitRank(perforate_trait) >= 2 and Enemies() <= 2 and FIXME_position_back Spell(gloomblade)
...
{
 { Talent(find_weakness_talent) or Enemies() < 3 } and { BuffPresent(stealthed_buff any=1) or BuffPresent(vanish_buff) } and Spell(shadowstrike) or ComboPointsDeficit() <= 1 - { Talent(deeper_stratagem_talent) and { BuffPresent(vanish_buff) or HasAzeriteTrait(the_first_dance_trait) and not Talent(dark_shadow_talent) and not Talent(subterfuge_talent) and Enemies() < 3 } } and SubtletyFinishCdPostConditions() or AzeriteTraitRank(perforate_trait) >= 2 and Enemies() <= 2 and FIXME_position_back and Spell(gloomblade) or Talent(secret_technique_talent) and Talent(find_weakness_talent) and target.DebuffRemaining(find_weakness_debuff) < 1 and Enemies() == 2 and target.TimeToDie() - target.DebuffRemaining(shadowstrike) > 6 and Spell(shadowstrike) or not Talent(deeper_stratagem_talent) and AzeriteTraitRank(blade_in_the_shadows_trait) == 3 and Enemies() == 3 and Spell(shadowstrike) or use_priority_rotation() and { Talent(find_weakness_talent) and target.DebuffRemaining(find_weakness_debuff) < 1 or Talent(weaponmaster_talent) and Enemies() <= 4 or HasAzeriteTrait(inevitability_trait) and BuffPresent(symbols_of_death_buff) and Enemies() <= 3 + HasAzeriteTrait(blade_in_the_shadows_trait) } and Spell(shadowstrike) or Enemies() >= 3 and Spell(shuriken_storm) or Spell(shadowstrike)
}
...
AddFunction SubtletyBuildMainActions
{
 ...
 if Enemies() >= 2 + { Talent(gloomblade_talent) and AzeriteTraitRank(perforate_trait) >= 2 and FIXME_position_back } Spell(shuriken_storm)
 #gloomblade
 Spell(gloomblade)
 #backstab
 Spell(backstab)
...
{
 Enemies() >= 2 + { Talent(gloomblade_talent) and AzeriteTraitRank(perforate_trait) >= 2 and FIXME_position_back } and Spell(shuriken_storm) or Spell(gloomblade) or Spell(backstab)
}
