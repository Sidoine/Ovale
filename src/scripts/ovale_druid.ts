...
{
 not hasequippeditem(empyreal_ordnance_item) or spellcooldown(empyreal_ordnance) < 160 and not spellcooldown(empyreal_ordnance) <= 0 or iscovenant("kyrian")
}

AddFunction convoke_desync
{
 { fightremains() - 20 - spellcooldown(convoke_the_spirits) } / 120 > { fightremains() - 25 - 10 * talentpoints(incarnation_chosen_of_elune_talent) - conduitvalue(precise_alignment_conduit) - spellcooldown(celestial_alignment) } / 180 or spellcooldown(celestial_alignment) > fightremains() or spellcooldown(convoke_the_spirits) > fightremains() or not iscovenant("night_fae")
}
...
{
 buffpresent(eclipse_lunar) and { enemies(tagged=1) > 4 and hastalent(soul_of_the_forest_talent) or enemies(tagged=1) > 6 }
}
...
{
 buffpresent(eclipse_lunar) * 8 / executetime(starfire) + { not buffpresent(eclipse_lunar) * 6 / executetime(wrath_balance) + 0.2 / { 100 / { 100 + spellcastspeedpercent() } } }
}
...
AddFunction balancestmainactions
{
 ...
 if target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } spell(sunfire)
 ...
 if not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } spell(incarnation)
 ...
 if iscovenant("night_fae") and convoke_desync() and spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") spell(starsurge)
 ...
 if buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) spell(starfire)
 #wrath
 spell(wrath_balance)
...
{
 { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(celestial_alignment) <= 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord) and texture(starlord text=cancel) or iscovenant("night_fae") and convoke_desync() and spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and buffpresent(eclipse_lunar) and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(celestial_alignment) <= 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize) or buffpresent(celestial_alignment) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) <= 0 or not iscovenant("venthyr") } or astralpower() > 90 and buffpresent(eclipse_any) } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord) or astralpower() > 90 } and buffstacks(starlord) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(celestial_alignment) > 10 or not convoke_desync() and iscovenant("night_fae") } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(celestial_alignment) > 7 and { spellcooldown(kindred_spirits) > 7 or not iscovenant("kyrian") } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance) or balancefallthrushortcdpostconditions()
}
...
AddFunction balancestcdactions
{
 unless { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm)
 {
  ...
  if not checkboxon("no_cds") and { { convoke_desync() and not spellcooldown(celestial_alignment) <= 0 or buffpresent(celestial_alignment) } and astralpower() < 40 and { buffremaining(eclipse_lunar) > 10 or buffremaining(eclipse_solar) > 10 } or fightremains() < 10 } spell(convoke_the_spirits)

  unless target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
  {
   ...
   unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(celestial_alignment) <= 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits)
   {
    ...
    if not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 20 + conduitvalue(precise_alignment_conduit) } spell(celestial_alignment)

    unless not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage) or not iscovenant("necrolord") } and save_for_ca_inc() and spell(fury_of_elune) or buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord) and texture(starlord text=cancel) or iscovenant("night_fae") and convoke_desync() and spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and buffpresent(eclipse_lunar) and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(celestial_alignment) <= 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize) or buffpresent(celestial_alignment) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) <= 0 or not iscovenant("venthyr") } or astralpower() > 90 and buffpresent(eclipse_any) } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord) or astralpower() > 90 } and buffstacks(starlord) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(celestial_alignment) > 10 or not convoke_desync() and iscovenant("night_fae") } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(celestial_alignment) > 7 and { spellcooldown(kindred_spirits) > 7 or not iscovenant("kyrian") } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance)
    {
     #run_action_list,name=fallthru
...
{
 { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 and { buffamount(primordial_arcanic_pulsar) < 240 or not equippedruneforge(primordial_arcanic_pulsar_runeforge) } or buffamount(primordial_arcanic_pulsar) >= 270 or spellcooldown(celestial_alignment) <= 0 and { astralpower() > 90 or is_aoe() } } and spell(kindred_spirits) or not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + 9 * equippedruneforge(primordial_arcanic_pulsar_runeforge) + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 240 and { target.debuffpresent(adaptive_swarm_damage) or not iscovenant("necrolord") } and save_for_ca_inc() and spell(fury_of_elune) or buffpresent(oneths_perception) and buffrefreshable(starfall) and spell(starfall) or buffremaining(starlord) < 5 and { buffremaining(eclipse_solar) > 5 or buffremaining(eclipse_lunar) > 5 } and astralpower() > 90 and buffpresent(starlord) and texture(starlord text=cancel) or iscovenant("night_fae") and convoke_desync() and spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and spell(starsurge) or hastalent(stellar_drift_talent) and not hastalent(starlord_talent) and buffrefreshable(starfall) and { buffremaining(eclipse_lunar) > 6 and buffpresent(eclipse_lunar) and buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 and astralpower() > 90 or target.debuffremaining(adaptive_swarm_damage) > 8 or inflighttotarget(adaptive_swarm_damage) } and not spellcooldown(celestial_alignment) <= 0 and spell(starfall) or { buffpresent(oneths_clear_vision) or buffpresent(kindred_empowerment_energize) or buffpresent(celestial_alignment) and { buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or not buffpresent(ravenous_frenzy) and not spellcooldown(ravenous_frenzy) <= 0 or not iscovenant("venthyr") } or astralpower() > 90 and buffpresent(eclipse_any) } and spell(starsurge) or hastalent(starlord_talent) and { buffpresent(starlord) or astralpower() > 90 } and buffstacks(starlord) < 3 and { buffpresent(eclipse_solar) or buffpresent(eclipse_lunar) } and buffamount(primordial_arcanic_pulsar) < 270 and { spellcooldown(celestial_alignment) > 10 or not convoke_desync() and iscovenant("night_fae") } and spell(starsurge) or { buffamount(primordial_arcanic_pulsar) < 270 or buffamount(primordial_arcanic_pulsar) < 250 and hastalent(stellar_drift_talent) } and buffremaining(eclipse_solar) > 7 and buffpresent(eclipse_solar) and not buffpresent(oneths_perception) and not hastalent(starlord_talent) and spellcooldown(celestial_alignment) > 7 and { spellcooldown(kindred_spirits) > 7 or not iscovenant("kyrian") } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance) or balancefallthrucdpostconditions()
}
...
AddFunction balancefallthrumainactions
{
 ...
 if target.debuffremaining(moonfire_debuff) > target.debuffremaining(sunfire_debuff) spell(sunfire)
 #moonfire
 spell(moonfire)
}
...
{
 not equippedruneforge(balance_of_all_things_runeforge) and spell(starsurge) or target.debuffremaining(moonfire_debuff) > target.debuffremaining(sunfire_debuff) and spell(sunfire) or spell(moonfire)
}
...
AddFunction balancedreambindermainactions
{
 ...
 if target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } spell(sunfire)
 ...
 #incarnation,if=!druid.no_cds&variable.cd_condition&((buff.kindred_empowerment_energize.up|!covenant.kyrian)|covenant.night_fae|variable.is_aoe|buff.bloodlust.up&buff.bloodlust.remains<30+(conduit.precise_alignment.time_value))&!buff.ca_inc.up&(!covenant.night_fae|cooldown.convoke_the_spirits.up|interpolated_fight_remains<cooldown.convoke_the_spirits.remains+6|interpolated_fight_remains%%180<30+(conduit.precise_alignment.time_value))
 if not checkboxon("no_cds") and cd_condition() and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } spell(incarnation)
 #starsurge,if=covenant.night_fae&variable.convoke_desync&astral_power>=40&cooldown.convoke_the_spirits.remains<gcd.max*ceil(astral_power%30)&!druid.no_cds
 if iscovenant("night_fae") and convoke_desync() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and not checkboxon("no_cds") spell(starsurge)
 #new_moon,if=(buff.eclipse_lunar.up|(charges=2&recharge_time<5)|charges=3)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } spell(new_moon)
 #half_moon,if=(buff.eclipse_lunar.up&!covenant.kyrian|(buff.kindred_empowerment_energize.up&covenant.kyrian)|(charges=2&recharge_time<5)|charges=3|buff.ca_inc.up)&ap_check&variable.save_for_ca_inc
 if { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) spell(half_moon)
...
{
 unless { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)

  unless { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } and spell(kindred_spirits) or not checkboxon("no_cds") and cd_condition() and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation)
  {
   #variable,name=save_for_ca_inc,value=!cooldown.ca_inc.ready|!variable.convoke_desync&covenant.night_fae|druid.no_cds
...
{
 { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } and spell(kindred_spirits) or not checkboxon("no_cds") and cd_condition() and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or iscovenant("night_fae") and convoke_desync() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and not checkboxon("no_cds") and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance) or balancefallthrushortcdpostconditions()
}
...
AddFunction balancedreambindercdactions
{
 ...
 {
  #convoke_the_spirits,if=!druid.no_cds&((variable.convoke_desync&interpolated_fight_remains>130&!cooldown.ca_inc.ready|buff.ca_inc.up)&astral_power<40&(buff.eclipse_lunar.remains>10|buff.eclipse_solar.remains>10)|fight_remains<10)
  if not checkboxon("no_cds") and { { convoke_desync() and fightremains() > 130 and not spellcooldown(celestial_alignment) <= 0 or buffpresent(celestial_alignment) } and astralpower() < 40 and { buffremaining(eclipse_lunar) > 10 or buffremaining(eclipse_solar) > 10 } or fightremains() < 10 } spell(convoke_the_spirits)

  unless { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
  {
   #ravenous_frenzy,if=buff.ca_inc.up
   if buffpresent(celestial_alignment) spell(ravenous_frenzy)
   ...
   {
    #celestial_alignment,if=!druid.no_cds&variable.cd_condition&((buff.kindred_empowerment_energize.up|!covenant.kyrian)|covenant.night_fae|variable.is_aoe|buff.bloodlust.up&buff.bloodlust.remains<20+(conduit.precise_alignment.time_value))&!buff.ca_inc.up&(!covenant.night_fae|cooldown.convoke_the_spirits.up|interpolated_fight_remains<cooldown.convoke_the_spirits.remains+6|interpolated_fight_remains%%180<20+(conduit.precise_alignment.time_value))
    if not checkboxon("no_cds") and cd_condition() and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 20 + conduitvalue(precise_alignment_conduit) } spell(celestial_alignment)
...
{
 { not safe_to_use_spell() or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or astralpower() > 90 } and spell(starsurge) or { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 5 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or target.refreshable(sunfire_debuff) and target.timetodie() > 12 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(sunfire) } and spell(sunfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 and { { buffremaining(celestial_alignment) > 5 and { buffremaining(ravenous_frenzy) > 5 or not buffpresent(ravenous_frenzy) } or not buffpresent(celestial_alignment) or astralpower() < 30 } and { not buffpresent(kindred_empowerment_energize) or astralpower() < 30 } and astralpower() >= astralpowercost(stellar_flare) } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { { buffremaining(eclipse_solar) > 10 or buffremaining(eclipse_lunar) > 10 } and spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } and spell(kindred_spirits) or not checkboxon("no_cds") and cd_condition() and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") or iscovenant("night_fae") or is_aoe() or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and { target.debuffpresent(adaptive_swarm_damage) or not iscovenant("necrolord") } and save_for_ca_inc() and spell(fury_of_elune) or iscovenant("night_fae") and convoke_desync() and astralpower() >= 40 and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 30 } and not checkboxon("no_cds") and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and save_for_ca_inc() and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(half_moon) and save_for_ca_inc() and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) and not iscovenant("kyrian") or buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 or buffpresent(celestial_alignment) } and astralpower() >= astralpowercost(full_moon) and save_for_ca_inc() and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance) or balancefallthrucdpostconditions()
}
...
AddFunction balanceboatmainactions
{
 ...
 if { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and buffpresent(starlord) texture(starlord text=cancel)
 ...
 if not critnotup() and { iscovenant("night_fae") or spellcooldown(celestial_alignment) > 7 or not cd_condition() and not iscovenant("kyrian") or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } spell(starsurge)
 ...
 if spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(celestial_alignment) < 5 } and astralpower() > 40 and iscovenant("night_fae") and not checkboxon("no_cds") spell(starsurge)
 #variable,name=dot_requirements,value=(buff.ca_inc.remains>5&(buff.ravenous_frenzy.remains>5|!buff.ravenous_frenzy.up)|!buff.ca_inc.up|astral_power<30)&(!buff.kindred_empowerment_energize.up|astral_power<30)&(buff.eclipse_solar.remains>gcd.max|buff.eclipse_lunar.remains>gcd.max)
 ...
 if { counter(lunar) == 1 or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } or fightremains() < 10 spell(kindred_spirits)
 ...
 if not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 } or fightremains() < 30 + conduitvalue(precise_alignment_conduit) spell(incarnation)
 ...
 if fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not buffpresent(celestial_alignment) or not hastalent(starlord_talent) } and { not { not spellcooldown(celestial_alignment) > 0 } or iscovenant("kyrian") and not { not spellcooldown(empower_bond) > 0 } or iscovenant("night_fae") } and { not iscovenant("venthyr") or not buffpresent(celestial_alignment) or astralpower() > 90 } or hastalent(starlord_talent) and buffpresent(celestial_alignment) and { buffstacks(starlord) < 3 or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) and not hastalent(starlord_talent) spell(starsurge)
...
{
 unless buffstacks(balance_of_all_things_nature_buff) < 4 and buffstacks(balance_of_all_things_arcane_buff) < 4 and { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 3 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 5 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and buffpresent(starlord) and texture(starlord text=cancel) or not critnotup() and { iscovenant("night_fae") or spellcooldown(celestial_alignment) > 7 or not cd_condition() and not iscovenant("kyrian") or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and spell(starsurge) or spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(celestial_alignment) < 5 } and astralpower() > 40 and iscovenant("night_fae") and not checkboxon("no_cds") and spell(starsurge) or target.refreshable(sunfire_debuff) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + target.debuffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare)
 {
  #force_of_nature,if=ap_check
  if astralpower() >= astralpowercost(force_of_nature) spell(force_of_nature)
  ...
  if { buffpresent(eclipse_any) or counter(solar) == 1 or counter(lunar) == 1 } and { not iscovenant("night_fae") or checkboxon("no_cds") or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(celestial_alignment) > 0 } } } and { spellcooldown(celestial_alignment) > 30 or checkboxon("no_cds") or astralpower() > 90 and not spellcooldown(celestial_alignment) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not iscovenant("kyrian") } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage) > 4 or not iscovenant("necrolord") } spell(fury_of_elune)

  unless { { counter(lunar) == 1 or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } or fightremains() < 10 } and spell(kindred_spirits) or { not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 } or fightremains() < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not buffpresent(celestial_alignment) or not hastalent(starlord_talent) } and { not { not spellcooldown(celestial_alignment) > 0 } or iscovenant("kyrian") and not { not spellcooldown(empower_bond) > 0 } or iscovenant("night_fae") } and { not iscovenant("venthyr") or not buffpresent(celestial_alignment) or astralpower() > 90 } or hastalent(starlord_talent) and buffpresent(celestial_alignment) and { buffstacks(starlord) < 3 or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) and not hastalent(starlord_talent) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon)
  {
   #warrior_of_elune
   spell(warrior_of_elune)
...
{
 buffstacks(balance_of_all_things_nature_buff) < 4 and buffstacks(balance_of_all_things_arcane_buff) < 4 and { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 3 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 5 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and buffpresent(starlord) and texture(starlord text=cancel) or not critnotup() and { iscovenant("night_fae") or spellcooldown(celestial_alignment) > 7 or not cd_condition() and not iscovenant("kyrian") or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and spell(starsurge) or spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(celestial_alignment) < 5 } and astralpower() > 40 and iscovenant("night_fae") and not checkboxon("no_cds") and spell(starsurge) or target.refreshable(sunfire_debuff) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + target.debuffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare) or { { counter(lunar) == 1 or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } or fightremains() < 10 } and spell(kindred_spirits) or { not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 } or fightremains() < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not buffpresent(celestial_alignment) or not hastalent(starlord_talent) } and { not { not spellcooldown(celestial_alignment) > 0 } or iscovenant("kyrian") and not { not spellcooldown(empower_bond) > 0 } or iscovenant("night_fae") } and { not iscovenant("venthyr") or not buffpresent(celestial_alignment) or astralpower() > 90 } or hastalent(starlord_talent) and buffpresent(celestial_alignment) and { buffstacks(starlord) < 3 or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) and not hastalent(starlord_talent) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceboatcdactions
{
 ...
 unless buffstacks(balance_of_all_things_nature_buff) < 4 and buffstacks(balance_of_all_things_arcane_buff) < 4 and { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 3 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 5 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm)
 {
  ...
  if not checkboxon("no_cds") and { { convoke_desync() and not spellcooldown(celestial_alignment) <= 0 or buffpresent(celestial_alignment) } and { buffstacks(balance_of_all_things_nature_buff) == 5 or buffstacks(balance_of_all_things_arcane_buff) == 5 } or fightremains() < 10 } spell(convoke_the_spirits)

  unless { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and buffpresent(starlord) and texture(starlord text=cancel) or not critnotup() and { iscovenant("night_fae") or spellcooldown(celestial_alignment) > 7 or not cd_condition() and not iscovenant("kyrian") or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and spell(starsurge) or spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(celestial_alignment) < 5 } and astralpower() > 40 and iscovenant("night_fae") and not checkboxon("no_cds") and spell(starsurge) or target.refreshable(sunfire_debuff) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + target.debuffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(eclipse_any) or counter(solar) == 1 or counter(lunar) == 1 } and { not iscovenant("night_fae") or checkboxon("no_cds") or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(celestial_alignment) > 0 } } } and { spellcooldown(celestial_alignment) > 30 or checkboxon("no_cds") or astralpower() > 90 and not spellcooldown(celestial_alignment) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not iscovenant("kyrian") } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage) > 4 or not iscovenant("necrolord") } and spell(fury_of_elune) or { { counter(lunar) == 1 or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } or fightremains() < 10 } and spell(kindred_spirits)
  {
   #celestial_alignment,if=!druid.no_cds&variable.cd_condition&(astral_power>90&(buff.kindred_empowerment_energize.up|!covenant.kyrian)|covenant.night_fae|buff.bloodlust.up&buff.bloodlust.remains<20+(conduit.precise_alignment.time_value))&(variable.convoke_desync|cooldown.convoke_the_spirits.ready)|interpolated_fight_remains<20+(conduit.precise_alignment.time_value)
   if not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or buffpresent(bloodlust) and buffremaining(bloodlust) < 20 + conduitvalue(precise_alignment_conduit) } and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 } or fightremains() < 20 + conduitvalue(precise_alignment_conduit) spell(celestial_alignment)

   unless { not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 } or fightremains() < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not buffpresent(celestial_alignment) or not hastalent(starlord_talent) } and { not { not spellcooldown(celestial_alignment) > 0 } or iscovenant("kyrian") and not { not spellcooldown(empower_bond) > 0 } or iscovenant("night_fae") } and { not iscovenant("venthyr") or not buffpresent(celestial_alignment) or astralpower() > 90 } or hastalent(starlord_talent) and buffpresent(celestial_alignment) and { buffstacks(starlord) < 3 or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) and not hastalent(starlord_talent) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance)
   {
    #run_action_list,name=fallthru
    balancefallthrucdactions()
   }
...
{
 buffstacks(balance_of_all_things_nature_buff) < 4 and buffstacks(balance_of_all_things_arcane_buff) < 4 and { not target.debuffpresent(adaptive_swarm_damage) and not inflighttotarget(adaptive_swarm_damage) and { not target.debuffpresent(adaptive_swarm) or target.debuffremaining(adaptive_swarm) > 3 } or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 5 and target.debuffpresent(adaptive_swarm_damage) } and spell(adaptive_swarm) or { buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 } and { spellcooldown(celestial_alignment) > 7 or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and buffpresent(starlord) and texture(starlord text=cancel) or not critnotup() and { iscovenant("night_fae") or spellcooldown(celestial_alignment) > 7 or not cd_condition() and not iscovenant("kyrian") or spellcooldown(empower_bond) > 7 and not buffpresent(kindred_empowerment_energize) and iscovenant("kyrian") } and spell(starsurge) or spellcooldown(convoke_the_spirits) < 5 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(celestial_alignment) < 5 } and astralpower() > 40 and iscovenant("night_fae") and not checkboxon("no_cds") and spell(starsurge) or target.refreshable(sunfire_debuff) and target.timetodie() > 16 and { astralpower() >= astralpowercost(sunfire) and dot_requirements() } and spell(sunfire) or target.refreshable(moonfire_debuff) and target.timetodie() > 13.5 and { astralpower() >= astralpowercost(moonfire) and dot_requirements() } and spell(moonfire) or target.refreshable(stellar_flare) and target.timetodie() > 16 + target.debuffremaining(stellar_flare) and { astralpower() >= astralpowercost(stellar_flare) and dot_requirements() } and spell(stellar_flare) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature) or { buffpresent(eclipse_any) or counter(solar) == 1 or counter(lunar) == 1 } and { not iscovenant("night_fae") or checkboxon("no_cds") or astralpower() < 95 and { critnotup() or astralpower() < 30 or is_aoe() } and { convoke_desync() and not { not spellcooldown(convoke_the_spirits) > 0 } or not convoke_desync() and not { not spellcooldown(celestial_alignment) > 0 } } } and { spellcooldown(celestial_alignment) > 30 or checkboxon("no_cds") or astralpower() > 90 and not spellcooldown(celestial_alignment) > 0 and { spellcooldown(empower_bond) < executetime(starfire) or not iscovenant("kyrian") } or fightremains() < 10 } and { target.debuffremaining(adaptive_swarm_damage) > 4 or not iscovenant("necrolord") } and spell(fury_of_elune) or { { counter(lunar) == 1 or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffremaining(balance_of_all_things_nature_buff) > 4.5 or buffremaining(balance_of_all_things_arcane_buff) > 4.5 or astralpower() > 90 and spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") } and { spellcooldown(celestial_alignment) > 30 or spellcooldown(celestial_alignment) <= 0 } or fightremains() < 10 } and spell(kindred_spirits) or { not checkboxon("no_cds") and cd_condition() and { astralpower() > 90 and { buffpresent(kindred_empowerment_energize) or not iscovenant("kyrian") } or iscovenant("night_fae") or buffpresent(bloodlust) and buffremaining(bloodlust) < 30 + conduitvalue(precise_alignment_conduit) } and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 } or fightremains() < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or { fightremains() < 4 or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) or { astralpower() + aspPerSec() * buffremaining(eclipse_solar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 or astralpower() + aspPerSec() * buffremaining(eclipse_lunar) + target.ticksremaining(fury_of_elune) * 2.5 > 120 } and buffpresent(eclipse_any) and { not buffpresent(celestial_alignment) or not hastalent(starlord_talent) } and { not { not spellcooldown(celestial_alignment) > 0 } or iscovenant("kyrian") and not { not spellcooldown(empower_bond) > 0 } or iscovenant("night_fae") } and { not iscovenant("venthyr") or not buffpresent(celestial_alignment) or astralpower() > 90 } or hastalent(starlord_talent) and buffpresent(celestial_alignment) and { buffstacks(starlord) < 3 or astralpower() > 90 } or buffremaining(celestial_alignment) > 8 and not buffpresent(ravenous_frenzy) and not hastalent(starlord_talent) } and spell(starsurge) or { buffpresent(eclipse_lunar) or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_lunar) or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_lunar) or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or counter(lunar) + counter(solar) == 1 or buffpresent(warrior_of_elune) and buffpresent(eclipse_lunar) or buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) } and spell(starfire) or spell(wrath_balance) or balancefallthrucdpostconditions()
}
...
{
 #sunfire,target_if=refreshable&target.time_to_die>14-spell_targets+remains&(eclipse.in_any|remains<gcd.max)
 if target.refreshable(sunfire_debuff) and target.timetodie() > 14 - enemies(tagged=1) + target.debuffremaining(sunfire_debuff) and { buffpresent(eclipse_any) or target.debuffremaining(sunfire_debuff) < gcd() } spell(sunfire)
 #starfall,if=buff.starfall.refreshable&(spell_targets.starfall<3|!runeforge.timeworn_dreambinder.equipped)&(!runeforge.lycaras_fleeting_glimpse.equipped|time%%45>buff.starfall.remains+2)&target.time_to_die>5
 if buffrefreshable(starfall) and { enemies(tagged=1) < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and target.timetodie() > 5 spell(starfall)
 ...
 if equippedruneforge(timeworn_dreambinder_runeforge) and enemies(tagged=1) >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies(tagged=1) > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } spell(starfall)
 ...
 #starsurge,if=variable.dream_will_fall_off&variable.starfall_wont_fall_off&!variable.ignore_starsurge|(buff.balance_of_all_things_nature.stack>3|buff.balance_of_all_things_arcane.stack>3)&spell_targets.starfall<4
 if dream_will_fall_off() and starfall_wont_fall_off() and not ignore_starsurge() or { buffstacks(balance_of_all_things_nature_buff) > 3 or buffstacks(balance_of_all_things_arcane_buff) > 3 } and enemies(tagged=1) < 4 spell(starsurge)
 ...
 #moonfire,target_if=refreshable&target.time_to_die>(14+(spell_targets.starfire*1.5))%spell_targets+remains,if=(cooldown.ca_inc.ready&!druid.no_cds&(variable.convoke_desync|cooldown.convoke_the_spirits.ready|!covenant.night_fae)|spell_targets.starfire<(5*(1+talent.twin_moons.enabled))|(eclipse.in_solar|(eclipse.in_both|eclipse.in_lunar)&!talent.soul_of_the_forest.enabled|buff.primordial_arcanic_pulsar.value>=250)&(spell_targets.starfire<10*(1+talent.twin_moons.enabled))&astral_power>50-buff.starfall.remains*6)&(!buff.kindred_empowerment_energize.up|eclipse.in_solar|!covenant.kyrian)&ap_check
 if target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies(tagged=1) * 1.5 } / enemies(tagged=1) + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 or not iscovenant("night_fae") } or enemies(tagged=1) < 5 * { 1 + talentpoints(twin_moons_talent) } or { buffpresent(eclipse_solar) or { buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) } and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies(tagged=1) < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and { not buffpresent(kindred_empowerment_energize) or buffpresent(eclipse_solar) or not iscovenant("kyrian") } and astralpower() >= astralpowercost(moonfire) } spell(moonfire)
 ...
 if fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and { spellcooldown(celestial_alignment) > 50 or checkboxon("no_cds") } spell(kindred_spirits)
 ...
 if target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies(tagged=1) < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } spell(stellar_flare)
 ...
 if iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffremaining(starfall) < 4 and not checkboxon("no_cds") spell(starfall)
 ...
 if iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < 6 and buffpresent(starfall) and buffpresent(eclipse_any) and not ignore_starsurge() and not checkboxon("no_cds") spell(starsurge)
 ...
 if buffpresent(oneths_clear_vision) or { not astralpower() >= astralpowercost(starfire) or { buffremaining(celestial_alignment) < 5 and buffpresent(celestial_alignment) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies(tagged=1) < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies(tagged=1) < 3 } spell(starsurge)
...
AddFunction balanceaoeshortcdactions
{
 unless target.refreshable(sunfire_debuff) and target.timetodie() > 14 - enemies(tagged=1) + target.debuffremaining(sunfire_debuff) and { buffpresent(eclipse_any) or target.debuffremaining(sunfire_debuff) < gcd() } and spell(sunfire) or buffrefreshable(starfall) and { enemies(tagged=1) < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and target.timetodie() > 5 and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies(tagged=1) >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies(tagged=1) > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or { dream_will_fall_off() and starfall_wont_fall_off() and not ignore_starsurge() or { buffstacks(balance_of_all_things_nature_buff) > 3 or buffstacks(balance_of_all_things_arcane_buff) > 3 } and enemies(tagged=1) < 4 } and spell(starsurge) or { not buffpresent(adaptive_swarm) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies(tagged=1) * 1.5 } / enemies(tagged=1) + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 or not iscovenant("night_fae") } or enemies(tagged=1) < 5 * { 1 + talentpoints(twin_moons_talent) } or { buffpresent(eclipse_solar) or { buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) } and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies(tagged=1) < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and { not buffpresent(kindred_empowerment_energize) or buffpresent(eclipse_solar) or not iscovenant("kyrian") } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire)
 {
  ...
  unless not checkboxon("no_cds") and cd_condition() and { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or convoke_desync() or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and { spellcooldown(celestial_alignment) > 50 or checkboxon("no_cds") } } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies(tagged=1) < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } and spell(stellar_flare)
  {
   ...
   if buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage) or not iscovenant("necrolord") or enemies(tagged=1) > 2 } spell(fury_of_elune)

   unless buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffremaining(starfall) < 4 and not checkboxon("no_cds") and spell(starfall) or iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < 6 and buffpresent(starfall) and buffpresent(eclipse_any) and not ignore_starsurge() and not checkboxon("no_cds") and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not astralpower() >= astralpowercost(starfire) or { buffremaining(celestial_alignment) < 5 and buffpresent(celestial_alignment) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies(tagged=1) < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies(tagged=1) < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon)
   {
    #warrior_of_elune
    spell(warrior_of_elune)
...
{
 target.refreshable(sunfire_debuff) and target.timetodie() > 14 - enemies(tagged=1) + target.debuffremaining(sunfire_debuff) and { buffpresent(eclipse_any) or target.debuffremaining(sunfire_debuff) < gcd() } and spell(sunfire) or buffrefreshable(starfall) and { enemies(tagged=1) < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and target.timetodie() > 5 and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies(tagged=1) >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies(tagged=1) > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or { dream_will_fall_off() and starfall_wont_fall_off() and not ignore_starsurge() or { buffstacks(balance_of_all_things_nature_buff) > 3 or buffstacks(balance_of_all_things_arcane_buff) > 3 } and enemies(tagged=1) < 4 } and spell(starsurge) or { not buffpresent(adaptive_swarm) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies(tagged=1) * 1.5 } / enemies(tagged=1) + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 or not iscovenant("night_fae") } or enemies(tagged=1) < 5 * { 1 + talentpoints(twin_moons_talent) } or { buffpresent(eclipse_solar) or { buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) } and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies(tagged=1) < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and { not buffpresent(kindred_empowerment_energize) or buffpresent(eclipse_solar) or not iscovenant("kyrian") } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or not checkboxon("no_cds") and cd_condition() and { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or convoke_desync() or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and { spellcooldown(celestial_alignment) > 50 or checkboxon("no_cds") } } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies(tagged=1) < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } and spell(stellar_flare) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffremaining(starfall) < 4 and not checkboxon("no_cds") and spell(starfall) or iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < 6 and buffpresent(starfall) and buffpresent(eclipse_any) and not ignore_starsurge() and not checkboxon("no_cds") and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not astralpower() >= astralpowercost(starfire) or { buffremaining(celestial_alignment) < 5 and buffpresent(celestial_alignment) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies(tagged=1) < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies(tagged=1) < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or { counter(lunar) == 1 or counter(lunar) + counter(solar) == 1 and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(celestial_alignment) < executetime(starfire) and not is_cleave() and buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies(tagged=1) <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(celestial_alignment) > executetime(wrath_balance) } and spell(wrath_balance) or spell(starfire) or balancefallthrushortcdpostconditions()
}
...
AddFunction balanceaoecdactions
{
 ...
 #variable,name=ignore_starsurge,value=buff.eclipse_lunar.up&(spell_targets.starfire>4&talent.soul_of_the_forest.enabled|spell_targets.starfire>6)
 ...
 if not checkboxon("no_cds") and { { convoke_desync() and not spellcooldown(celestial_alignment) <= 0 or buffpresent(celestial_alignment) } and { astralpower() < 50 or ignore_starsurge() } and { buffremaining(eclipse_lunar) > 6 or buffremaining(eclipse_solar) > 6 } and { not runeforge(balance_of_all_things_runeforge) or buffstacks(balance_of_all_things_nature_buff) > 3 or buffstacks(balance_of_all_things_arcane_buff) > 3 } or fightremains() < 10 } spell(convoke_the_spirits)

 unless target.refreshable(sunfire_debuff) and target.timetodie() > 14 - enemies(tagged=1) + target.debuffremaining(sunfire_debuff) and { buffpresent(eclipse_any) or target.debuffremaining(sunfire_debuff) < gcd() } and spell(sunfire) or buffrefreshable(starfall) and { enemies(tagged=1) < 3 or not equippedruneforge(timeworn_dreambinder_runeforge) } and { not equippedruneforge(lycaras_fleeting_glimpse_runeforge) or timeincombat() % 45 > buffremaining(starfall) + 2 } and target.timetodie() > 5 and spell(starfall) or equippedruneforge(timeworn_dreambinder_runeforge) and enemies(tagged=1) >= 3 and { not buffpresent(timeworn_dreambinder_buff) and buffrefreshable(starfall) or dream_will_fall_off() and { buffremaining(starfall) < 3 or enemies(tagged=1) > 2 and hastalent(stellar_drift_talent) and buffremaining(starfall) < 5 } } and spell(starfall) or { dream_will_fall_off() and starfall_wont_fall_off() and not ignore_starsurge() or { buffstacks(balance_of_all_things_nature_buff) > 3 or buffstacks(balance_of_all_things_arcane_buff) > 3 } and enemies(tagged=1) < 4 } and spell(starsurge) or { not buffpresent(adaptive_swarm) and not inflighttotarget(adaptive_swarm_damage) or target.debuffstacks(adaptive_swarm_damage) < 3 and target.debuffremaining(adaptive_swarm_damage) < 3 } and spell(adaptive_swarm) or target.refreshable(moonfire_debuff) and target.timetodie() > { 14 + enemies(tagged=1) * 1.5 } / enemies(tagged=1) + target.debuffremaining(moonfire_debuff) and { { spellcooldown(celestial_alignment) <= 0 and not checkboxon("no_cds") and { convoke_desync() or spellcooldown(convoke_the_spirits) <= 0 or not iscovenant("night_fae") } or enemies(tagged=1) < 5 * { 1 + talentpoints(twin_moons_talent) } or { buffpresent(eclipse_solar) or { buffpresent(eclipse_solar) and buffpresent(eclipse_lunar) or buffpresent(eclipse_lunar) } and not hastalent(soul_of_the_forest_talent) or buffamount(primordial_arcanic_pulsar) >= 250 } and enemies(tagged=1) < 10 * { 1 + talentpoints(twin_moons_talent) } and astralpower() > 50 - buffremaining(starfall) * 6 } and { not buffpresent(kindred_empowerment_energize) or buffpresent(eclipse_solar) or not iscovenant("kyrian") } and astralpower() >= astralpowercost(moonfire) } and spell(moonfire) or astralpower() >= astralpowercost(force_of_nature) and spell(force_of_nature)
 {
  #ravenous_frenzy,if=buff.ca_inc.up
  if buffpresent(celestial_alignment) spell(ravenous_frenzy)
  #celestial_alignment,if=!druid.no_cds&variable.cd_condition&(buff.starfall.up|astral_power>50)&!buff.solstice.up&!buff.ca_inc.up&(!covenant.night_fae|variable.convoke_desync|cooldown.convoke_the_spirits.up|interpolated_fight_remains<20+(conduit.precise_alignment.time_value))
  if not checkboxon("no_cds") and cd_condition() and { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or convoke_desync() or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < 20 + conduitvalue(precise_alignment_conduit) } spell(celestial_alignment)

  unless not checkboxon("no_cds") and cd_condition() and { buffpresent(starfall) or astralpower() > 50 } and not buffpresent(solstice_buff) and not buffpresent(celestial_alignment) and { not iscovenant("night_fae") or convoke_desync() or not spellcooldown(convoke_the_spirits) > 0 or fightremains() < spellcooldown(convoke_the_spirits) + 6 or fightremains() % 180 < 30 + conduitvalue(precise_alignment_conduit) } and spell(incarnation) or { fightremains() < 15 or { buffamount(primordial_arcanic_pulsar) < 250 or buffamount(primordial_arcanic_pulsar) >= 250 } and buffpresent(starfall) and { spellcooldown(celestial_alignment) > 50 or checkboxon("no_cds") } } and spell(kindred_spirits) or target.refreshable(stellar_flare) and target.timetodie() > 15 and { enemies(tagged=1) < 4 and astralpower() >= astralpowercost(stellar_flare) and { buffremaining(celestial_alignment) > 10 or not buffpresent(celestial_alignment) } } and spell(stellar_flare) or buffpresent(eclipse_any) and astralpower() >= astralpowercost(fury_of_elune) and buffamount(primordial_arcanic_pulsar) < 250 and { target.debuffpresent(adaptive_swarm_damage) or not iscovenant("necrolord") or enemies(tagged=1) > 2 } and spell(fury_of_elune) or buffpresent(oneths_perception) and { buffrefreshable(starfall) or astralpower() > 90 } and spell(starfall) or iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < gcd() * { astralpower() / 50 } and buffremaining(starfall) < 4 and not checkboxon("no_cds") and spell(starfall) or iscovenant("night_fae") and { convoke_desync() or not spellcooldown(celestial_alignment) > 0 or buffpresent(celestial_alignment) } and spellcooldown(convoke_the_spirits) < 6 and buffpresent(starfall) and buffpresent(eclipse_any) and not ignore_starsurge() and not checkboxon("no_cds") and spell(starsurge) or { buffpresent(oneths_clear_vision) or { not astralpower() >= astralpowercost(starfire) or { buffremaining(celestial_alignment) < 5 and buffpresent(celestial_alignment) or buffremaining(ravenous_frenzy) < gcd() * { astralpower() / 30 } and buffpresent(ravenous_frenzy) } and starfall_wont_fall_off() and enemies(tagged=1) < 3 } and { not equippedruneforge(timeworn_dreambinder_runeforge) or enemies(tagged=1) < 3 } } and spell(starsurge) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(new_moon) == 2 and spellchargecooldown(new_moon) < 5 or charges(new_moon) == 3 } and astralpower() >= astralpowercost(new_moon) and { not spellknown(half_moon) and not spellknown(full_moon) } and spell(new_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(half_moon) == 2 and spellchargecooldown(half_moon) < 5 or charges(half_moon) == 3 } and astralpower() >= astralpowercost(half_moon) and spellknown(half_moon) and spell(half_moon) or { buffpresent(eclipse_any) and spellcooldown(celestial_alignment) > 50 or charges(full_moon) == 2 and spellchargecooldown(full_moon) < 5 or charges(full_moon) == 3 } and astralpower() >= astralpowercost(full_moon) and spellknown(full_moon) and spell(full_moon) or spell(warrior_of_elune) or { counter(lunar) == 1 or counter(lunar) + counter(solar) == 1 and is_cleave() or buffremaining(eclipse_solar) < executetime(starfire) and buffpresent(eclipse_solar) or buffpresent(eclipse_solar) and not starfire_in_solar() or buffremaining(celestial_alignment) < executetime(starfire) and not is_cleave() and buffremaining(celestial_alignment) < executetime(wrath_balance) and buffpresent(celestial_alignment) or buffpresent(ravenous_frenzy) and 100 / { 100 + spellcastspeedpercent() } > 0.6 and { enemies(tagged=1) <= 3 or not hastalent(soul_of_the_forest_talent) } or not is_cleave() and buffremaining(celestial_alignment) > executetime(wrath_balance) } and spell(wrath_balance) or spell(starfire)
  {
   #run_action_list,name=fallthru
   balancefallthrucdactions()
  }
...
AddFunction balance_defaultcdactions
{
 ...
 unless { not iscovenant("night_fae") or not { not spellcooldown(convoke_the_spirits) > 0 } } and buffpresent(celestial_alignment) and spell(berserking)
 {
  ...
  #variable,name=cd_condition,value=(!equipped.empyreal_ordnance|cooldown.empyreal_ordnance.remains<160&!cooldown.empyreal_ordnance.ready)|covenant.kyrian
  #use_item,name=empyreal_ordnance,if=cooldown.ca_inc.remains<20&variable.convoke_desync|cooldown.convoke_the_spirits.remains<20
  if spellcooldown(celestial_alignment) < 20 and convoke_desync() or spellcooldown(convoke_the_spirits) < 20 balanceuseitemactions()
  #use_item,name=soulletting_ruby,if=cooldown.ca_inc.remains<6&variable.convoke_desync|cooldown.convoke_the_spirits.remains<6
  if spellcooldown(celestial_alignment) < 6 and convoke_desync() or spellcooldown(convoke_the_spirits) < 6 balanceuseitemactions()
  #use_item,name=inscrutable_quantum_device,if=buff.ca_inc.up
  if buffpresent(celestial_alignment) balanceuseitemactions()
  #use_items,slots=trinket1,if=!trinket.1.has_proc.any&(!trinket.2.has_proc.any|!trinket.2.ready_cooldown|!trinket.2.cooldown.duration)|(!trinket.1.has_proc.any&!trinket.2.has_proc.any&!trinket.2.cooldown.duration|trinket.1.has_proc.any)&(buff.ca_inc.up|cooldown.ca_inc.remains-10>trinket.1.cooldown.duration&!covenant.kyrian|covenant.night_fae&variable.convoke_desync&cooldown.convoke_the_spirits.up&!cooldown.ca_inc.up&((buff.eclipse_lunar.remains>10|buff.eclipse_solar.remains>10)&!runeforge.balance_of_all_things|runeforge.balance_of_all_things&(buff.balance_of_all_things_nature.stack>3|buff.balance_of_all_things_arcane.stack>3))|buff.kindred_empowerment_energize.up)|fight_remains<20
  if not itemrppm(slot="trinket0slot") > 0 and { not itemrppm(slot="trinket1slot") > 0 or not 0 or not itemcooldownduration(slot="trinket1slot") } or { not itemrppm(slot="trinket0slot") > 0 and not itemrppm(slot="trinket1slot") > 0 and not itemcooldownduration(slot="trinket1slot") or itemrppm(slot="trinket0slot") > 0 } and { buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) - 10 > itemcooldownduration(slot="trinket0slot") and not iscovenant("kyrian") or iscovenant("night_fae") and convoke_desync() and not spellcooldown(convoke_the_spirits) > 0 and not { not spellcooldown(celestial_alignment) > 0 } and { { buffremaining(eclipse_lunar) > 10 or buffremaining(eclipse_solar) > 10 } and not runeforge(balance_of_all_things_runeforge) or runeforge(balance_of_all_things_runeforge) and { buffstacks(balance_of_all_things_nature_buff) > 3 or buffstacks(balance_of_all_things_arcane_buff) > 3 } } or buffpresent(kindred_empowerment_energize) } or fightremains() < 20 balanceuseitemactions()
  #use_items,slots=trinket2,if=!trinket.2.has_proc.any&(!trinket.1.has_proc.any|!trinket.1.ready_cooldown|!trinket.1.cooldown.duration)|trinket.1.has_proc.any&trinket.2.has_proc.any&!trinket.1.ready_cooldown|buff.ca_inc.up|cooldown.ca_inc.remains-10>trinket.2.cooldown.duration&!covenant.kyrian|covenant.night_fae&variable.convoke_desync&cooldown.convoke_the_spirits.up&!cooldown.ca_inc.up&((buff.eclipse_lunar.remains>10|buff.eclipse_solar.remains>10)&!runeforge.balance_of_all_things|runeforge.balance_of_all_things&(buff.balance_of_all_things_nature.stack>3|buff.balance_of_all_things_arcane.stack>3))|buff.kindred_empowerment_energize.up|fight_remains<20
  if not itemrppm(slot="trinket1slot") > 0 and { not itemrppm(slot="trinket0slot") > 0 or not 0 or not itemcooldownduration(slot="trinket0slot") } or itemrppm(slot="trinket0slot") > 0 and itemrppm(slot="trinket1slot") > 0 and not 0 or buffpresent(celestial_alignment) or spellcooldown(celestial_alignment) - 10 > itemcooldownduration(slot="trinket1slot") and not iscovenant("kyrian") or iscovenant("night_fae") and convoke_desync() and not spellcooldown(convoke_the_spirits) > 0 and not { not spellcooldown(celestial_alignment) > 0 } and { { buffremaining(eclipse_lunar) > 10 or buffremaining(eclipse_solar) > 10 } and not runeforge(balance_of_all_things_runeforge) or runeforge(balance_of_all_things_runeforge) and { buffstacks(balance_of_all_things_nature_buff) > 3 or buffstacks(balance_of_all_things_arcane_buff) > 3 } } or buffpresent(kindred_empowerment_energize) or fightremains() < 20 balanceuseitemactions()
  #use_items
...
AddFunction feralstealthmainactions
{
 ...
 unless { target.debuffpersistentmultiplier(rake_debuff) < 1.5 or target.refreshable(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > 2 and { spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake) }
 {
  ...
  if target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and enemies(tagged=1) > 3 spell(thrash_cat)
  unless target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and enemies(tagged=1) > 3 and { spellusable(thrash_cat) and spellcooldown(thrash_cat) < timetoenergyfor(thrash_cat) }
  {
...
{
 { target.debuffpersistentmultiplier(rake_debuff) < 1.5 or target.refreshable(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > 2 and spell(rake) or not { { target.debuffpersistentmultiplier(rake_debuff) < 1.5 or target.refreshable(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > 2 and { spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake) } } and { target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and enemies(tagged=1) > 3 and spell(thrash_cat) or not { target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and enemies(tagged=1) > 3 and { spellusable(thrash_cat) and spellcooldown(thrash_cat) < timetoenergyfor(thrash_cat) } } and { enemies(tagged=1) > 2 and spell(brutal_slash) or combopoints() < 4 and spell(shred) } }
}
...
AddFunction feralowlweavemainactions
{
 ...
 #sunfire,if=!prev_gcd.1.sunfire&!prev_gcd.2.sunfire
 if not previousgcdspell(sunfire) and not previousgcdspell(sunfire count=2) spell(sunfire)
 #moonkin_form,if=energy<40&(dot.rip.remains>4.5|combo_points<5)&cooldown.tigers_fury.remains>=6.5&buff.clearcasting.stack<1&!buff.apex_predators_craving.up&!buff.bloodlust.up&(buff.bs_inc.remains>5|!buff.bs_inc.up)&(!cooldown.convoke_the_spirits.up|!covenant.night_fae)
 if energy() < 40 and { target.debuffremaining(rip) > 4.5 or combopoints() < 5 } and spellcooldown(tigers_fury) >= 6.5 and buffstacks(clearcasting) < 1 and not buffpresent(apex_predators_craving_buff) and not buffpresent(bloodlust) and { buffremaining(bs_inc_buff) > 5 or not buffpresent(bs_inc_buff) } and { not { not spellcooldown(convoke_the_spirits) > 0 } or not iscovenant("night_fae") } spell(moonkin_form)
}
...
{
 buffpresent(heart_of_the_wild) and spell(starsurge) or not previousgcdspell(sunfire) and not previousgcdspell(sunfire count=2) and spell(sunfire) or energy() < 40 and { target.debuffremaining(rip) > 4.5 or combopoints() < 5 } and spellcooldown(tigers_fury) >= 6.5 and buffstacks(clearcasting) < 1 and not buffpresent(apex_predators_craving_buff) and not buffpresent(bloodlust) and { buffremaining(bs_inc_buff) > 5 or not buffpresent(bs_inc_buff) } and { not { not spellcooldown(convoke_the_spirits) > 0 } or not iscovenant("night_fae") } and spell(moonkin_form)
}

AddFunction feralowlweavecdactions
{
 unless buffpresent(heart_of_the_wild) and spell(starsurge) or not previousgcdspell(sunfire) and not previousgcdspell(sunfire count=2) and spell(sunfire)
 {
  #heart_of_the_wild,if=energy<40&(dot.rip.remains>4.5|combo_points<5)&cooldown.tigers_fury.remains>=6.5&buff.clearcasting.stack<1&!buff.apex_predators_craving.up&!buff.bloodlust.up&(buff.bs_inc.remains>5|!buff.bs_inc.up)&(!cooldown.convoke_the_spirits.up|!covenant.night_fae)
  if energy() < 40 and { target.debuffremaining(rip) > 4.5 or combopoints() < 5 } and spellcooldown(tigers_fury) >= 6.5 and buffstacks(clearcasting) < 1 and not buffpresent(apex_predators_craving_buff) and not buffpresent(bloodlust) and { buffremaining(bs_inc_buff) > 5 or not buffpresent(bs_inc_buff) } and { not { not spellcooldown(convoke_the_spirits) > 0 } or not iscovenant("night_fae") } spell(heart_of_the_wild)
 }
}
...
AddFunction feralfinishermainactions
{
 ...
 if enemies(tagged=1) > 2 and enemies(tagged=1) > 1 spell(primal_wrath)
...
{
 { buffexpires(savage_roar) or buffremaining(savage_roar) < { combopoints() * 6 + 1 } * 0.3 } and spell(savage_roar) or enemies(tagged=1) > 2 and enemies(tagged=1) > 1 and spell(primal_wrath) or target.refreshable(rip) and target.ticksgainedonrefresh(rip) > rip_ticks() and { { buffpresent(tigers_fury) or spellcooldown(tigers_fury) > 5 } and { buffpresent(bloodtalons) or not hastalent(bloodtalons_talent) } and target.debuffpersistentmultiplier(rip) <= persistentmultiplier(rip) or not hastalent(sabertooth_talent) } and spell(rip) or energy() >= energycost(ferocious_bite max=1) and spell(ferocious_bite)
}
...
AddFunction feralfillermainactions
{
 ...
 if filler() == 3 spell(lunar_inspiration_feral)
...
{
 filler() == 1 and target.debuffpersistentmultiplier(rake_debuff) <= persistentmultiplier(rake_debuff) and spell(rake) or filler() == 2 and spell(rake) or filler() == 3 and spell(lunar_inspiration_feral) or filler() == 4 and spell(swipe) or spell(shred)
}
...
{
 combopoints() >= 3 and spell(berserk) or combopoints() >= 3 and spell(incarnation) or { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking) or { buffpresent(tigers_fury) or conduit(deep_allegiance_conduit) } and spell(kindred_spirits) or spell(adaptive_swarm)
}

AddFunction feralcooldowncdactions
{
 unless combopoints() < 3 and spell(feral_frenzy) or combopoints() >= 3 and spell(berserk) or combopoints() >= 3 and spell(incarnation) or { energydeficit() > 40 or buffpresent(bs_inc_buff) or hastalent(predator_talent) and shortest_ttd() < 3 } and spell(tigers_fury)
 {
  ...
  unless { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking)
  {
   ...
   if buffpresent(bs_inc_buff) and { checkboxon(opt_use_consumables) and target.classification(worldboss) } item(potion_of_spectral_agility_item usable=1)
   ...
   if target.debuffremaining(rip) > 4 and combopoints() < 3 and target.debuffpresent(rake_debuff) and energydeficit() >= 20 or fightremains() < 5 spell(convoke_the_spirits)
...
{
 combopoints() < 3 and spell(feral_frenzy) or combopoints() >= 3 and spell(berserk) or combopoints() >= 3 and spell(incarnation) or { energydeficit() > 40 or buffpresent(bs_inc_buff) or hastalent(predator_talent) and shortest_ttd() < 3 } and spell(tigers_fury) or { buffpresent(tigers_fury) or buffpresent(bs_inc_buff) } and spell(berserking) or { buffpresent(tigers_fury) or conduit(deep_allegiance_conduit) } and spell(kindred_spirits) or spell(adaptive_swarm)
}
...
AddFunction feralbloodtalonsmainactions
{
 ...
 if { not target.debuffpresent(rake_debuff) or target.refreshable(rake_debuff) and persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) or buffcount(bt_buffs) == 2 and persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) or buffcount(bt_buffs) == 2 and target.refreshable(rake_debuff) } and buffexpires(bt_rake_buff) and target.ticksgainedonrefresh(rake_debuff) >= 2 spell(rake)
 ...
 #thrash_cat,target_if=refreshable&buff.bt_thrash.down&druid.thrash_cat.ticks_gained_on_refresh>variable.thrash_ticks
 if target.refreshable(thrash_cat) and buffexpires(bt_thrash_buff) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() spell(thrash_cat)
...
{
 { not target.debuffpresent(rake_debuff) or target.refreshable(rake_debuff) and persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) or buffcount(bt_buffs) == 2 and persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) or buffcount(bt_buffs) == 2 and target.refreshable(rake_debuff) } and buffexpires(bt_rake_buff) and target.ticksgainedonrefresh(rake_debuff) >= 2 and spell(rake) or target.refreshable(lunar_inspiration_feral) and buffexpires(bt_moonfire_buff) and spell(lunar_inspiration_feral) or target.refreshable(thrash_cat) and buffexpires(bt_thrash_buff) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and spell(thrash_cat) or buffexpires(bt_brutal_slash_buff) and spell(brutal_slash) or buffexpires(bt_swipe_buff) and enemies(tagged=1) > 1 and spell(swipe_cat) or buffexpires(bt_shred_buff) and spell(shred) or buffexpires(bt_swipe_buff) and spell(swipe_cat) or buffexpires(bt_thrash_buff) and spell(thrash_cat)
}
...
{
 #call_action_list,name=owlweave,if=variable.owlweave=1
 if owlweave() == 1 feralowlweavemainactions()

 unless owlweave() == 1 and feralowlweavemainpostconditions()
 {
  #prowl
  spell(prowl)
      ...
      unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) < 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and buffcount(bt_buffs) == 0
      {
       ...
       unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and feralbloodtalonsmainpostconditions()
       {
        ...
        unless { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > enemies(tagged=1) * 2 - 2 and { spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake) }
        {
         ...
         if target.refreshable(moonfire_cat) and target.ticksgainedonrefresh(moonfire_debuff) > enemies(tagged=1) * 2 - 2 spell(moonfire_cat)
         #pool_resource,for_next=1
         #thrash_cat,target_if=refreshable&druid.thrash_cat.ticks_gained_on_refresh>variable.thrash_ticks&!buff.bs_inc.up
         if target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and not buffpresent(bs_inc_buff) spell(thrash_cat)
         unless target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and not buffpresent(bs_inc_buff) and { spellusable(thrash_cat) and spellcooldown(thrash_cat) < timetoenergyfor(thrash_cat) }
         {
          #pool_resource,for_next=1
     ...
     unless combopoints() >= 5 - _4cp_bite() and feralfinishershortcdpostconditions()
     {
      ...
      unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthshortcdpostconditions()
      {
       ...
       unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) < 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and buffcount(bt_buffs) == 0
       {
        ...
        unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and feralbloodtalonsshortcdpostconditions() or buffpresent(apex_predators_craving_buff) and spell(ferocious_bite)
        {
         #pool_resource,for_next=1
         #rake,target_if=(refreshable|persistent_multiplier>dot.rake.pmultiplier)&druid.rake.ticks_gained_on_refresh>spell_targets.swipe_cat*2-2
         unless { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > enemies(tagged=1) * 2 - 2 and { spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake) }
         {
          unless target.refreshable(moonfire_cat) and target.ticksgainedonrefresh(moonfire_debuff) > enemies(tagged=1) * 2 - 2 and spell(moonfire_cat)
          {
           #pool_resource,for_next=1
           #thrash_cat,target_if=refreshable&druid.thrash_cat.ticks_gained_on_refresh>variable.thrash_ticks&!buff.bs_inc.up
           unless target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and not buffpresent(bs_inc_buff) and { spellusable(thrash_cat) and spellcooldown(thrash_cat) < timetoenergyfor(thrash_cat) }
...
{
 owlweave() == 1 and feralowlweaveshortcdpostconditions() or spell(prowl) or buffexpires(cat_form) and spell(cat_form) or { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthshortcdpostconditions() or feralcooldownshortcdpostconditions() or combopoints() >= 5 - _4cp_bite() and feralfinishershortcdpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthshortcdpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) < 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and buffcount(bt_buffs) == 0 } and { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and feralbloodtalonsshortcdpostconditions() or buffpresent(apex_predators_craving_buff) and spell(ferocious_bite) or not { { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > enemies(tagged=1) * 2 - 2 and { spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake) } } and { target.refreshable(moonfire_cat) and target.ticksgainedonrefresh(moonfire_debuff) > enemies(tagged=1) * 2 - 2 and spell(moonfire_cat) or not { target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and not buffpresent(bs_inc_buff) and { spellusable(thrash_cat) and spellcooldown(thrash_cat) < timetoenergyfor(thrash_cat) } } and { not { 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies(tagged=1) * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) and { spellusable(brutal_slash) and spellcooldown(brutal_slash) < timetoenergyfor(brutal_slash) } } and { enemies(tagged=1) > 1 + buffpresent(bs_inc_buff) * 2 and spell(swipe_cat) or buffpresent(clearcasting) and spell(shred) or buffpresent(bs_inc_buff) and target.ticksgainedonrefresh(rake_debuff) > 2 and spell(rake) or feralfillershortcdpostconditions() } } } }
}
...
AddFunction feral_defaultcdactions
{
 ...
 #call_action_list,name=owlweave,if=variable.owlweave=1
 if owlweave() == 1 feralowlweavecdactions()

 unless owlweave() == 1 and feralowlweavecdpostconditions() or spell(prowl) or buffexpires(cat_form) and spell(tigers_fury) or buffexpires(cat_form) and spell(cat_form)
 {
  #variable,name=shortest_ttd,value=target.time_to_die,if=variable.shortest_ttd=0
  #cycling_variable,name=shortest_ttd,op=min,value=target.time_to_die
  #run_action_list,name=stealth,if=buff.shadowmeld.up|buff.prowl.up
    ...
    unless combopoints() >= 5 - _4cp_bite() and feralfinishercdpostconditions()
    {
     ...
     unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthcdpostconditions()
     {
      ...
      unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) < 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and buffcount(bt_buffs) == 0
      {
       ...
       unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and feralbloodtalonscdpostconditions() or buffpresent(apex_predators_craving_buff) and spell(ferocious_bite)
       {
        #pool_resource,for_next=1
        #rake,target_if=(refreshable|persistent_multiplier>dot.rake.pmultiplier)&druid.rake.ticks_gained_on_refresh>spell_targets.swipe_cat*2-2
        unless { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > enemies(tagged=1) * 2 - 2 and { spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake) }
...
{
 owlweave() == 1 and feralowlweavecdpostconditions() or spell(prowl) or buffexpires(cat_form) and spell(tigers_fury) or buffexpires(cat_form) and spell(cat_form) or { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthcdpostconditions() or feralcooldowncdpostconditions() or combopoints() >= 5 - _4cp_bite() and feralfinishercdpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthcdpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) < 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and buffcount(bt_buffs) == 0 } and { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and feralbloodtalonscdpostconditions() or buffpresent(apex_predators_craving_buff) and spell(ferocious_bite) or not { { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and target.ticksgainedonrefresh(rake_debuff) > enemies(tagged=1) * 2 - 2 and { spellusable(rake) and spellcooldown(rake) < timetoenergyfor(rake) } } and { target.refreshable(moonfire_cat) and target.ticksgainedonrefresh(moonfire_debuff) > enemies(tagged=1) * 2 - 2 and spell(moonfire_cat) or not { target.refreshable(thrash_cat) and target.ticksgainedonrefresh(thrash_cat) > thrash_ticks() and not buffpresent(bs_inc_buff) and { spellusable(thrash_cat) and spellcooldown(thrash_cat) < timetoenergyfor(thrash_cat) } } and { not { 600 > { 1 + spellmaxcharges(brutal_slash) - charges(brutal_slash count=0) } * spellchargecooldown(brutal_slash) and enemies(tagged=1) * damage(brutal_slash) / powercost(brutal_slash) > damage(shred) / powercost(shred) and { spellusable(brutal_slash) and spellcooldown(brutal_slash) < timetoenergyfor(brutal_slash) } } and { enemies(tagged=1) > 1 + buffpresent(bs_inc_buff) * 2 and spell(swipe_cat) or buffpresent(clearcasting) and spell(shred) or buffpresent(bs_inc_buff) and target.ticksgainedonrefresh(rake_debuff) > 2 and spell(rake) or feralfillercdpostconditions() } } } }
}
...
AddFunction guardian_defaultmainactions
{
 ...
 unless checkboxon("catweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and target.debuffremaining(moonfire_debuff) >= gcd() + 0.5 and rage() < 40 and buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk_bear) and buffexpires(galactic_guardian) or buffpresent(cat_form) and energy() > 25 or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) and buffpresent(cat_form) and energy() > 20 or iscovenant("kyrian") and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardiancatweavemainpostconditions()
 {
  ...
  if checkboxon("owlweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 15 and buffexpires(incarnation) and buffexpires(berserk_bear) and buffexpires(galactic_guardian) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire_debuff) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or iscovenant("night_fae") and spellcooldown(convoke_the_spirits) <= 1 or iscovenant("kyrian") and spellcooldown(empower_bond) <= 1 and enemies() < 2 } guardianowlweavemainactions()

  unless checkboxon("owlweave_bear") and { spellcooldown(thrash_bear) > 0 and spellcooldown(mangle) > 0 and rage() < 15 and buffexpires(incarnation) and buffexpires(berserk_bear) and buffexpires(galactic_guardian) or buffpresent(moonkin_form) and target.debuffrefreshable(sunfire_debuff) or buffpresent(moonkin_form) and buffpresent(heart_of_the_wild) or equippedruneforge(oath_of_the_elder_druid_runeforge) and not buffpresent(oath_of_the_elder_druid) or iscovenant("night_fae") and spellcooldown(convoke_the_spirits) <= 1 or iscovenant("kyrian") and spellcooldown(empower_bond) <= 1 and enemies() < 2 } and guardianowlweavemainpostconditions()
  {
...
AddFunction guardian_defaultcdactions
{
 ...
 if { hastalent(heart_of_the_wild_talent) and buffpresent(heart_of_the_wild) and { checkboxon("catweave_bear") or checkboxon("owlweave_bear") } and not buffpresent(prowl) or { buffpresent(berserk_bear) or buffpresent(incarnation_guardian_of_ursoc) } and not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") } and { checkboxon(opt_use_consumables) and target.classification(worldboss) } item(potion_of_spectral_agility_item usable=1)
 #run_action_list,name=catweave,if=druid.catweave_bear&((cooldown.thrash_bear.remains>0&cooldown.mangle.remains>0&dot.moonfire.remains>=gcd+0.5&rage<40&buff.incarnation_guardian_of_ursoc.down&buff.berserk_bear.down&buff.galactic_guardian.down)|(buff.cat_form.up&energy>25)|(runeforge.oath_of_the_elder_druid.equipped&!buff.oath_of_the_elder_druid.up&(buff.cat_form.up&energy>20))|(covenant.kyrian&cooldown.empower_bond.remains<=1&active_enemies<2))
