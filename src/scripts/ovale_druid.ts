...
{
 hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and feralbloodtalonsshortcdpostconditions() or target.debuffpersistentmultiplier(rake_debuff) < 1.6 and ticksgainedonrefresh(rake) > 2 and spell(rake) or spell(shred)
}
...
{
 hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and feralbloodtalonscdpostconditions() or target.debuffpersistentmultiplier(rake_debuff) < 1.6 and ticksgainedonrefresh(rake) > 2 and spell(rake) or spell(shred)
}
   ...
   unless combopoints() >= 5 - _4cp_bite() and feralfinishermainpostconditions()
   {
    ...
    unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthmainpostconditions()
    {
     ...
     unless hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and buffcount(bt_buffs) == 0
     {
      ...
      if hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or buffcount(bt_buffs) == 2 } feralbloodtalonsmainactions()

      unless hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or buffcount(bt_buffs) == 2 } and feralbloodtalonsmainpostconditions()
      {
...
{
 { buffpresent(shadowmeld) or buffpresent(prowl) } and feralstealthmainpostconditions() or feralcooldownmainpostconditions() or combopoints() >= 5 - _4cp_bite() and feralfinishermainpostconditions() or { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthmainpostconditions() or not { hastalent(bloodtalons_talent) and buffexpires(bloodtalons) and energy() + 3.5 * energyregenrate() + 40 * buffpresent(clearcasting) >= 115 - 23 * buffpresent(incarnation_king_of_the_jungle) and buffcount(bt_buffs) == 0 } and { hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or buffcount(bt_buffs) == 2 } and feralbloodtalonsmainpostconditions() or feralfillermainpostconditions() }
}
   ...
   unless feralcooldownshortcdpostconditions()
   {
    ...
    unless combopoints() >= 5 - _4cp_bite() and feralfinishershortcdpostconditions()
    {
     ...
     unless { buffpresent(bs_inc_buff) or buffpresent(sudden_ambush_buff) } and feralstealthshortcdpostconditions()
     {
      ...
      {
       #run_action_list,name=bloodtalons,if=talent.bloodtalons.enabled&(buff.bloodtalons.down|active_bt_triggers=2)
       if hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or buffcount(bt_buffs) == 2 } feralbloodtalonsshortcdactions()

       unless hastalent(bloodtalons_talent) and { buffexpires(bloodtalons) or buffcount(bt_buffs) == 2 } and feralbloodtalonsshortcdpostconditions() or { target.refreshable(rake_debuff) or persistentmultiplier(rake_debuff) > target.debuffpersistentmultiplier(rake_debuff) } and spell(rake)
       {
...
{
 not buffpresent(moonkin_form) and spell(moonkin_form) or checkboxon("owlweave_bear") and spell(empower_bond) or target.refreshable(adaptive_swarm) and spell(adaptive_swarm) or { target.refreshable(moonfire_debuff) or buffpresent(galactic_guardian) } and spell(moonfire) or target.refreshable(sunfire) and spell(sunfire) or { buffpresent(eclipse_lunar) or buffpresent(eclipse_solar) } and spell(starsurge) or { buffpresent(eclipse_lunar) or counter(solar) == 1 or buffpresent(eclipse_lunar) and buffpresent(starsurge_empowerment_buff) } and spell(starfire) or spell(wrath)
}
...
AddFunction guardianbearmainactions
{
 ...
 if buffpresent(galactic_guardian) and checkboxon("owlweave_bear") and enemies() <= 3 spell(moonfire)
 ...
 if buffpresent(galactic_guardian) and checkboxon("catweave_bear") and enemies() <= 3 or buffpresent(galactic_guardian) and not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") and enemies() <= 3 spell(moonfire)
...
{
 not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not iscovenant("venthyr") } and spell(incarnation) or buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm) and spell(adaptive_swarm) or buffpresent(galactic_guardian) and checkboxon("owlweave_bear") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk_bear) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire_debuff) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian) and checkboxon("catweave_bear") and enemies() <= 3 or buffpresent(galactic_guardian) and not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle) or spell(thrash_bear) or spell(maul) or spell(swipe)
}
...
{
 not buffpresent(bear_form) and spell(bear_form) or { buffpresent(ravenous_frenzy) or not iscovenant("venthyr") } and spell(incarnation) or { not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") or enemies() >= 2 } and spell(empower_bond) or hastalent(brambles_talent) and buffpresent(bear_form) and spell(barkskin) or buffremaining(ironfur) < 0.5 and spell(ironfur) or target.refreshable(adaptive_swarm) and spell(adaptive_swarm) or buffpresent(galactic_guardian) and checkboxon("owlweave_bear") and enemies() <= 3 and spell(moonfire) or { target.refreshable(thrash_bear_debuff) or target.debuffstacks(thrash_bear_debuff) < 3 or target.debuffstacks(thrash_bear_debuff) < 4 and equippedruneforge(luffainfused_embrace_runeforge) or enemies() > 5 } and spell(thrash_bear) or buffexpires(incarnation_guardian_of_ursoc) and buffexpires(berserk_bear) and enemies() >= 4 and spell(swipe) or buffpresent(incarnation) and enemies() < 2 and spell(maul) or buffstacks(savage_combatant_buff) >= 1 and buffpresent(tooth_and_claw_buff) and buffpresent(incarnation) and enemies() == 2 and spell(maul) or buffpresent(incarnation) and enemies() <= 3 and spell(mangle) or target.refreshable(moonfire_debuff) and enemies() <= 3 and spell(moonfire) or { buffstacks(tooth_and_claw_buff) >= 2 or buffpresent(tooth_and_claw_buff) and buffremaining(tooth_and_claw_buff) < 1.5 or buffstacks(savage_combatant_buff) >= 3 } and spell(maul) or enemies() > 1 and spell(thrash_bear) or { buffpresent(galactic_guardian) and checkboxon("catweave_bear") and enemies() <= 3 or buffpresent(galactic_guardian) and not checkboxon("catweave_bear") and not checkboxon("owlweave_bear") and enemies() <= 3 } and spell(moonfire) or rage() < 80 and enemies() < 4 and spell(mangle) or target.debuffstacks(thrash_bear_debuff) > 2 and target.debuffgain(thrash_bear_debuff) <= baseduration(thrash_bear_debuff) and spell(pulverize) or spell(thrash_bear) or spell(maul) or spell(swipe)
}
