...
AddFunction ShadowSingleMainActions
{
 ...
 if DebuffStacks(voidform_shadow) > 20 + 5 * BuffPresent(burst_haste_buff any=1) and Insanity() <= 50 Spell(memory_of_lucid_dreams)
...
AddFunction ShadowSingleShortCdActions
{
 unless Spell(void_eruption)
 {
  ...
  unless Spell(void_bolt) or DebuffStacks(voidform_shadow) > 20 + 5 * BuffPresent(burst_haste_buff any=1) and Insanity() <= 50 and Spell(memory_of_lucid_dreams) or BuffPresent(harvested_thoughts_buff) and SpellCooldown(void_bolt) >= 1.5 and AzeriteTraitRank(searing_dialogue_trait) >= 1 and Spell(mind_sear) or { target.TimeToDie() < 3 or SpellCharges(shadow_word_death) == 2 or SpellCharges(shadow_word_death) == 1 and SpellCooldown(shadow_word_death) < GCD() } and Spell(shadow_word_death)
  {
...
AddFunction ShadowCleaveShortCdActions
{
 unless Spell(void_eruption)
 {
  ...
  unless not target.DebuffPresent(vampiric_touch_debuff) and AzeriteTraitRank(thought_harvester_trait) >= 1 and Spell(vampiric_touch) or BuffPresent(harvested_thoughts_buff) and Spell(mind_sear) or Spell(void_bolt) or DebuffStacks(voidform_shadow) > 20 + 5 * BuffPresent(burst_haste_buff any=1) and Insanity() <= 50 and Spell(memory_of_lucid_dreams) or { target.TimeToDie() < 3 or DebuffExpires(voidform_shadow) } and Spell(shadow_word_death)
  {
...
{
 unless Spell(blood_of_the_enemy) or Spell(guardian_of_azeroth) or Spell(focused_azerite_beam) or Spell(purifying_blast) or Spell(the_unbound_force) or Spell(ripple_in_space) or Spell(worldvein_resonance)
 {
  #run_action_list,name=cleave,if=active_enemies>1
...
{
 Spell(blood_of_the_enemy) or Spell(guardian_of_azeroth) or Spell(focused_azerite_beam) or Spell(purifying_blast) or Spell(the_unbound_force) or Spell(ripple_in_space) or Spell(worldvein_resonance) or Enemies() > 1 and ShadowCleaveShortCdPostConditions() or Enemies() == 1 and ShadowSingleShortCdPostConditions()
}
...
{
 Spell(blood_of_the_enemy) or Spell(guardian_of_azeroth) or Spell(focused_azerite_beam) or Spell(purifying_blast) or Spell(the_unbound_force) or Spell(ripple_in_space) or Spell(worldvein_resonance) or Enemies() > 1 and ShadowCleaveCdPostConditions() or Enemies() == 1 and ShadowSingleCdPostConditions()
}
