...
AddFunction elementalsingle_targetmainactions
{
 ...
 if buffpresent(primordial_wave_buff) and target.refreshable(flame_shock) spell(flame_shock text=cycle)
...
{
 unless buffremaining(stormkeeper) < 1.1 * gcd() * buffstacks(stormkeeper) and spell(lightning_bolt) or hastalent(icefury_talent) and buffpresent(icefury) and buffremaining(icefury) < 1.1 * gcd() * buffstacks(icefury) and spell(frost_shock) or { not target.debuffpresent(flame_shock) or target.debuffremaining(flame_shock) <= gcd() or hastalent(ascendance_talent) and target.debuffremaining(flame_shock) < spellcooldown(ascendance) + baseduration(ascendance) and spellcooldown(ascendance) < 4 } and { buffpresent(lava_surge_buff) or not buffpresent(bloodlust) } and spell(flame_shock) or buffpresent(primordial_wave_buff) and target.refreshable(flame_shock) and spell(flame_shock text=cycle)
 {
...
AddFunction elemental_defaultcdactions
{
 ...
 unless not hastalent(elemental_blast_talent_elemental) and not target.debuffpresent(flame_shock) and not pet.present() and { enemies(tagged=1) < 3 or hastalent(master_of_the_elements_talent) or equippedruneforge(skybreakers_fiery_demise_runeforge) } and spell(flame_shock) or not buffpresent(primordial_wave_buff) and { not pet.present() or enemies(tagged=1) < 3 and buffstacks(wind_gust_buff) < 20 or enabledsoulbind(lead_by_example_soulbind) } and { enemies(tagged=1) < 5 or hastalent(master_of_the_elements_talent) or equippedruneforge(skybreakers_fiery_demise_runeforge) or enabledsoulbind(lead_by_example_soulbind) } and spell(primordial_wave text=cycle) or not target.debuffpresent(flame_shock) and { not pet.present() or enemies(tagged=1) < 3 and buffstacks(wind_gust_buff) < 20 } and { enemies(tagged=1) < 3 or hastalent(master_of_the_elements_talent) or equippedruneforge(skybreakers_fiery_demise_runeforge) } and spell(flame_shock)
 {
  #fire_elemental
  spell(fire_elemental)
...
AddFunction enhancementaoemainactions
{
 ...
 if target.refreshable(flame_shock) and { hastalent(fire_nova_talent) or hastalent(lashing_flames_talent) or iscovenant("necrolord") or equippedruneforge(primal_lava_actuators_runeforge) } spell(flame_shock text=cycle)
 ...
 if hastalent(lashing_flames_talent) spell(lava_lash text=cycle)
 ...
 if target.refreshable(flame_shock) spell(flame_shock text=cycle)
 #frost_shock
 spell(frost_shock)
 #ice_strike
 spell(ice_strike)
...
AddFunction enhancementaoeshortcdactions
{
 unless buffpresent(crash_lightning) and spell(windstrike) or equippedruneforge(doom_winds_runeforge) and buffpresent(doom_winds_buff) and spell(crash_lightning) or buffpresent(hailstorm_buff) and spell(frost_shock)
 {
  ...
  unless target.refreshable(flame_shock) and { hastalent(fire_nova_talent) or hastalent(lashing_flames_talent) or iscovenant("necrolord") or equippedruneforge(primal_lava_actuators_runeforge) } and spell(flame_shock text=cycle)
  {
   #primordial_wave,target_if=min:dot.flame_shock.remains,cycle_targets=1,if=!buff.primordial_wave.up
   if not buffpresent(primordial_wave_buff) spell(primordial_wave text=cycle)
   ...
   unless debuffcountonany(flame_shock) >= 3 and spell(fire_nova)
   {
    ...
    unless buffpresent(primordial_wave_buff) and buffstacks(maelstrom_weapon_buff) >= 5 and spell(lightning_bolt) or buffpresent(stormkeeper_enhancement) and spell(chain_lightning) or { hastalent(crashing_storm_talent) or buffexpires(crash_lightning) } and spell(crash_lightning) or hastalent(lashing_flames_talent) and spell(lava_lash text=cycle) or buffpresent(crash_lightning) and { buffpresent(hot_hand_buff) or equippedruneforge(primal_lava_actuators_runeforge) and buffstacks(primal_lava_actuators_buff) > 6 } and spell(lava_lash) or buffpresent(crash_lightning) and spell(stormstrike) or spell(crash_lightning)
    {
...
{
 buffpresent(crash_lightning) and spell(windstrike) or equippedruneforge(doom_winds_runeforge) and buffpresent(doom_winds_buff) and spell(crash_lightning) or buffpresent(hailstorm_buff) and spell(frost_shock) or target.refreshable(flame_shock) and { hastalent(fire_nova_talent) or hastalent(lashing_flames_talent) or iscovenant("necrolord") or equippedruneforge(primal_lava_actuators_runeforge) } and spell(flame_shock text=cycle) or debuffcountonany(flame_shock) >= 3 and spell(fire_nova) or buffpresent(primordial_wave_buff) and buffstacks(maelstrom_weapon_buff) >= 5 and spell(lightning_bolt) or buffpresent(stormkeeper_enhancement) and spell(chain_lightning) or { hastalent(crashing_storm_talent) or buffexpires(crash_lightning) } and spell(crash_lightning) or hastalent(lashing_flames_talent) and spell(lava_lash text=cycle) or buffpresent(crash_lightning) and { buffpresent(hot_hand_buff) or equippedruneforge(primal_lava_actuators_runeforge) and buffstacks(primal_lava_actuators_buff) > 6 } and spell(lava_lash) or buffpresent(crash_lightning) and spell(stormstrike) or spell(crash_lightning) or buffstacks(maelstrom_weapon_buff) >= 5 and spell(elemental_blast) or buffstacks(maelstrom_weapon_buff) == 10 and spell(chain_lightning) or spell(windstrike) or spell(stormstrike) or spell(lava_lash) or target.refreshable(flame_shock) and spell(flame_shock text=cycle) or spell(frost_shock) or spell(ice_strike) or buffstacks(maelstrom_weapon_buff) >= 5 and spell(chain_lightning) or debuffcountonany(flame_shock) > 1 and spell(fire_nova) or spell(earthen_spike) or buffremaining(windfury_totem) < 30 and spell(windfury_totem)
}
...
AddFunction enhancementaoecdactions
{
 unless buffpresent(crash_lightning) and spell(windstrike)
 {
  ...
  unless equippedruneforge(doom_winds_runeforge) and buffpresent(doom_winds_buff) and spell(crash_lightning) or buffpresent(hailstorm_buff) and spell(frost_shock) or spell(sundering) or target.refreshable(flame_shock) and { hastalent(fire_nova_talent) or hastalent(lashing_flames_talent) or iscovenant("necrolord") or equippedruneforge(primal_lava_actuators_runeforge) } and spell(flame_shock text=cycle) or not buffpresent(primordial_wave_buff) and spell(primordial_wave text=cycle) or debuffcountonany(flame_shock) >= 3 and spell(fire_nova) or spell(vesper_totem) or buffpresent(primordial_wave_buff) and buffstacks(maelstrom_weapon_buff) >= 5 and spell(lightning_bolt) or buffpresent(stormkeeper_enhancement) and spell(chain_lightning) or { hastalent(crashing_storm_talent) or buffexpires(crash_lightning) } and spell(crash_lightning) or hastalent(lashing_flames_talent) and spell(lava_lash text=cycle) or buffpresent(crash_lightning) and { buffpresent(hot_hand_buff) or equippedruneforge(primal_lava_actuators_runeforge) and buffstacks(primal_lava_actuators_buff) > 6 } and spell(lava_lash) or buffpresent(crash_lightning) and spell(stormstrike) or spell(crash_lightning) or buffstacks(maelstrom_weapon_buff) >= 5 and spell(chain_harvest) or buffstacks(maelstrom_weapon_buff) >= 5 and spell(elemental_blast) or buffstacks(maelstrom_weapon_buff) >= 5 and spell(stormkeeper_enhancement) or buffstacks(maelstrom_weapon_buff) == 10 and spell(chain_lightning) or spell(windstrike) or spell(stormstrike) or spell(lava_lash)
  {
   #fleshcraft,interrupt=1,if=soulbind.volatile_solvent
   if soulbind(volatile_solvent_soulbind) spell(fleshcraft)

   unless target.refreshable(flame_shock) and spell(flame_shock text=cycle)
   {
    #fae_transfusion
