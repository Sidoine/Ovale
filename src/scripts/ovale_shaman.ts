...
{
 unless { not target.debuffpresent(flame_shock) or target.debuffremaining(flame_shock) <= gcd() or hastalent(ascendance_talent) and target.debuffremaining(flame_shock) < spellcooldown(ascendance) + baseduration(ascendance_buff) and spellcooldown(ascendance) < 4 } and { buffpresent(lava_surge) or not buffpresent(bloodlust) } and spell(flame_shock) or hastalent(ascendance_talent) and { timeincombat() >= 60 or buffpresent(bloodlust) } and spellcooldown(lava_burst) > 0 and { not hastalent(icefury_talent) or not buffpresent(icefury) and not { not spellcooldown(icefury) > 0 } } and spell(ascendance) or hastalent(elemental_blast_talent_elemental) and { hastalent(master_of_the_elements_talent) and { buffpresent(master_of_the_elements_buff) and maelstrom() < 60 or not buffpresent(master_of_the_elements_buff) } or not hastalent(master_of_the_elements_talent) } and spell(elemental_blast) or hastalent(stormkeeper_talent) and { 0 < 3 or 600 > 50 } and maelstrom() < 44 and spell(stormkeeper)
 {
...
{
 unless target.debuffremaining(flame_shock) <= gcd() and { buffpresent(lava_surge) or not buffpresent(bloodlust) } and spell(flame_shock) or hastalent(elemental_blast_talent_elemental) and spell(elemental_blast) or hastalent(stormkeeper_talent) and maelstrom() < 44 and spell(stormkeeper)
 {
...
AddFunction elementalaoeshortcdactions
{
 unless buffpresent(echoing_shock) and spell(earthquake)
 {
  ...
  unless hastalent(stormkeeper_talent) and spell(stormkeeper) or { debuffcountonany(flame_shock) < 3 and enemies() <= 5 or equippedruneforge(skybreakers_fiery_demise_runeforge) } and target.refreshable(flame_shock) and spell(flame_shock) or not debuffcountonany(flame_shock) and spell(flame_shock)
  {
   ...
   unless hastalent(ascendance_talent) and not pet.present() and { not hastalent(icefury_talent) or not buffpresent(icefury) and not { not spellcooldown(icefury) > 0 } } and spell(ascendance)
   {
    #liquid_magma_totem,if=talent.liquid_magma_totem.enabled
    if hastalent(liquid_magma_totem_talent) spell(liquid_magma_totem)
   }
...
{
 buffpresent(echoing_shock) and spell(earthquake) or hastalent(stormkeeper_talent) and spell(stormkeeper) or { debuffcountonany(flame_shock) < 3 and enemies() <= 5 or equippedruneforge(skybreakers_fiery_demise_runeforge) } and target.refreshable(flame_shock) and spell(flame_shock) or not debuffcountonany(flame_shock) and spell(flame_shock) or hastalent(ascendance_talent) and not pet.present() and { not hastalent(icefury_talent) or not buffpresent(icefury) and not { not spellcooldown(icefury) > 0 } } and spell(ascendance) or equippedruneforge(echoes_of_great_sundering_runeforge) and not buffpresent(echoes_of_great_sundering_buff) and spell(earth_shock) or target.debuffremaining(flame_shock) and { enemies() < 4 or buffpresent(lava_surge) or hastalent(master_of_the_elements_talent) and not buffpresent(master_of_the_elements_buff) and maelstrom() >= 60 } and spell(lava_burst) or { not hastalent(master_of_the_elements_talent) or buffpresent(stormkeeper) or maelstrom() >= 100 - 4 * enemies() or buffpresent(master_of_the_elements_buff) or enemies() > 3 } and spell(earthquake) or buffremaining(stormkeeper) < 3 * gcd() * buffstacks(stormkeeper) and spell(chain_lightning) or buffpresent(lava_surge) and enemies() < 4 and not pet.present() and target.debuffpresent(flame_shock) and spell(lava_burst) or hastalent(elemental_blast_talent_elemental) and enemies() < 5 and not pet.present() and spell(elemental_blast) or hastalent(ascendance_talent) and spell(lava_beam) or spell(chain_lightning) or speed() > 0 and { buffpresent(lava_surge) and not spellcooldown(lava_burst) > 0 } and spell(lava_burst) or speed() > 0 and target.refreshable(flame_shock) and spell(flame_shock) or speed() > 0 and spell(frost_shock)
}

AddFunction elementalaoecdactions
{
 unless buffpresent(echoing_shock) and spell(earthquake) or spell(chain_harvest) or hastalent(stormkeeper_talent) and spell(stormkeeper) or { debuffcountonany(flame_shock) < 3 and enemies() <= 5 or equippedruneforge(skybreakers_fiery_demise_runeforge) } and target.refreshable(flame_shock) and spell(flame_shock) or not debuffcountonany(flame_shock) and spell(flame_shock) or hastalent(echoing_shock_talent) and maelstrom() >= 60 and spell(echoing_shock) or hastalent(ascendance_talent) and not pet.present() and { not hastalent(icefury_talent) or not buffpresent(icefury) and not { not spellcooldown(icefury) > 0 } } and spell(ascendance) or hastalent(liquid_magma_totem_talent) and spell(liquid_magma_totem) or equippedruneforge(echoes_of_great_sundering_runeforge) and not buffpresent(echoes_of_great_sundering_buff) and spell(earth_shock)
 {
...
{
 buffpresent(echoing_shock) and spell(earthquake) or spell(chain_harvest) or hastalent(stormkeeper_talent) and spell(stormkeeper) or { debuffcountonany(flame_shock) < 3 and enemies() <= 5 or equippedruneforge(skybreakers_fiery_demise_runeforge) } and target.refreshable(flame_shock) and spell(flame_shock) or not debuffcountonany(flame_shock) and spell(flame_shock) or hastalent(echoing_shock_talent) and maelstrom() >= 60 and spell(echoing_shock) or hastalent(ascendance_talent) and not pet.present() and { not hastalent(icefury_talent) or not buffpresent(icefury) and not { not spellcooldown(icefury) > 0 } } and spell(ascendance) or hastalent(liquid_magma_totem_talent) and spell(liquid_magma_totem) or equippedruneforge(echoes_of_great_sundering_runeforge) and not buffpresent(echoes_of_great_sundering_buff) and spell(earth_shock) or target.debuffremaining(flame_shock) and { enemies() < 4 or buffpresent(lava_surge) or hastalent(master_of_the_elements_talent) and not buffpresent(master_of_the_elements_buff) and maelstrom() >= 60 } and spell(lava_burst) or { not hastalent(master_of_the_elements_talent) or buffpresent(stormkeeper) or maelstrom() >= 100 - 4 * enemies() or buffpresent(master_of_the_elements_buff) or enemies() > 3 } and spell(earthquake) or buffremaining(stormkeeper) < 3 * gcd() * buffstacks(stormkeeper) and spell(chain_lightning) or buffpresent(lava_surge) and enemies() < 4 and not pet.present() and target.debuffpresent(flame_shock) and spell(lava_burst) or hastalent(elemental_blast_talent_elemental) and enemies() < 5 and not pet.present() and spell(elemental_blast) or hastalent(ascendance_talent) and spell(lava_beam) or spell(chain_lightning) or speed() > 0 and { buffpresent(lava_surge) and not spellcooldown(lava_burst) > 0 } and spell(lava_burst) or speed() > 0 and target.refreshable(flame_shock) and spell(flame_shock) or speed() > 0 and spell(frost_shock)
}
