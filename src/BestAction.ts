...
export class OvaleBestActionClass {
    ...
    private GetActionMacroInfo: ActionInfoHandler = (
        ...
        let macro = <string>element.cachedParams.positional[1];
    ...
    private GetActionTextureInfo: ActionInfoHandler = (
        ...
        let actionTexture;
        {
            let texture = element.cachedParams.positional[1];
    ...
    public GetAction(node: AstIconNode, atTime: number): AstNodeSnapshot {
        ...
        let element = this.runner.PostOrderCompute(groupNode, atTime);
        if (element.type == "state" && element.timeSpan.HasTime(atTime)) {
            let [variable, value] = [element.name, element.value];
            let isFuture = !element.timeSpan.HasTime(atTime);
