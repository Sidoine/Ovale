...
type ComputerFunction = (
    element: Element,
    atTime: number
) => [OvaleTimeSpan, Element?];

export class OvaleBestActionClass {
    private self_serial = 0;
    private self_timeSpan: LuaObj<OvaleTimeSpan> = {};
    private self_valuePool = new OvalePool<Element>(
        "OvaleBestAction_valuePool"
    );
    ...
    private GetTimeSpan(node: AstNode, defaultTimeSpan?: OvaleTimeSpan) {
        ...
        if (timeSpan) {
            ...
            this.self_timeSpan[node.nodeId] = newTimeSpanFromArray(
                defaultTimeSpan
            );
    ...
    private getSpellActionInfo(
        ...
        let [isUsable, noMana] = this.OvaleSpells.IsUsableSpell(
            spellId,
            atTime,
            targetGUID
        );
        this.tracer.Log(
            "OvaleSpells:IsUsableSpell(%d, %f, %s) returned %d, %d",
            spellId,
            atTime,
            targetGUID,
            isUsable,
        ...
        if (si) {
            ...
            if (actionCooldownStart && actionCooldownDuration) {
                ...
                let runes = this.ovaleData.GetSpellInfoProperty(
                    spellId,
                    atTime,
                    "runes",
                    targetGUID
                ...
                if (runes) {
                    let timeToRunes = this.ovaleRunes.GetRunesCooldown(
                        atTime,
                        <number>runes
                    );
    ...
    public GetActionInfo(
        ...
        if (element && element.type == "action") {
            if (element.serial && element.serial >= this.self_serial) {
                ...
                let target =
                    <string>element.namedParams.target ||
    ...
    public GetAction(
        node: AstNode,
        atTime: number
    ): [OvaleTimeSpan, Element | undefined] {
    ...
    private PostOrderCompute(
        ...
        if (
            postOrder &&
            !(element.serial && element.serial >= this.self_serial)
        ) {
            let index = 1;
        ...
        if (element) {
            if (element.serial == -1) {
                ...
                if (visitor) {
                    ...
                    this.tracer.Log(
                        "[%d] Runtime error: unable to compute node of type '%s'.",
                        element.nodeId,
                        element.type
                    );
                    timeSpan = EMPTY_SET;
                ...
                if (result && isNodeType(result, "value")) {
                    this.tracer.Log(
                        "[%d] <<< '%s' returns %s with value = %s, %s, %s",
                        element.nodeId,
                        element.type,
                        timeSpan,
                        result.value,
    ...
    private ComputeAction: ComputerFunction = (
        element,
        atTime: number
    ): [OvaleTimeSpan, any] => {
        ...
        let result;
        this.tracer.Log(
            "[%d]    evaluating action: %s(%s)",
            nodeId,
            element.name,
            element.paramsAsString
        );
        let [
            actionTexture,
            actionInRange,
            ...
            actionResourceExtend,
            actionCharges,
        ] = this.GetActionInfo(element, atTime);
        ...
        if (!actionTexture) {
            ...
            if (
                ...
                (actionCharges == undefined || actionCharges == 0)
            ) {
                this.tracer.Log(
            ...
            let offgcd =
                element.namedParams.offgcd ||
                (spellInfo && spellInfo.offgcd) ||
                0;
    ...
    private ComputeArithmetic: ComputerFunction = (
        element,
        atTime
    ): [OvaleTimeSpan, Element?] => {
    ...
    private ComputeFunction: ComputerFunction = (
        element,
        atTime: number
    ): [OvaleTimeSpan, Element | undefined] => {
    ...
    private ComputeGroup: ComputerFunction = (
        element,
        atTime
    ): [OvaleTimeSpan, Element?] => {
        ...
        for (const [, node] of ipairs(element.child)) {
            ...
            if (current.Measure() > 0) {
                ...
                if (best.Measure() == 0) {
                    ...
                    let threshold =
                        (bestElement &&
                            bestElement.namedParams &&
                            bestElement.namedParams.wait) ||
                        0;
        ...
        if (bestElement) {
            ...
            this.tracer.Log(
                "[%d]    group best action %s remains %s",
                element.nodeId,
                id,
                timeSpan
            );
            ...
            this.tracer.Log(
                "[%d]    group no best action returns %s",
                element.nodeId,
                timeSpan
            );
        }
    ...
    private ComputeIf: ComputerFunction = (
        element,
        atTime
    ): [OvaleTimeSpan, Element?] => {
    ...
    private ComputeState: ComputerFunction = (
        element: Element,
        atTime
    ): [OvaleTimeSpan, Element?] => {
    ...
    private ComputeValue: ComputerFunction = (
        element: Element,
        atTime
    ): [OvaleTimeSpan, Element?] => {
