...
export class OvalePowerClass extends States<PowerState> implements StateModule {
    ...
    private OnInitialize = () => {
        ...
        this.module.RegisterEvent("UNIT_POWER_FREQUENT", this.UNIT_POWER_UPDATE);
        ...
        for (const [powerType] of pairs(this.POWER_INFO)) {
            this.requirement.RegisterRequirement(powerType, this.RequirePowerHandler);
        }
    }
    ...
    initializePower() {
        ...
        for (const [powerType, powerId] of pairs(Enum.PowerType)) {
            ...
            let powerToken = this.ovale.playerClass != undefined && possiblePowerTypes[this.ovale.playerClass][powerTypeLower];
            if (powerToken) {
    ...
    private UNIT_MAXPOWER = (event: string, unitId: string, powerToken: string) => {
        if (unitId == "player") {
    ...
    private UNIT_POWER_UPDATE = (event: string, unitId: string, powerToken: string) => {
        if (unitId == "player") {
            let powerType = this.POWER_TYPE[powerToken];
            if (powerType) {
    ...
    RequirePowerHandler = (spellId: number, atTime: number, requirement: string, tokens: Tokens, index: number, targetGUID: string | undefined): [boolean, string, number] => {
        return this.getPowerRequirementAt(this.GetState(atTime), spellId, atTime, requirement, tokens, index, targetGUID);
    }
    ...
    TimeToPower(spellId: number, atTime: number, targetGUID: string | undefined, powerType: PowerType | undefined, extraPower?: number) {
    ...
    ApplyPowerCost(spellId: number, targetGUID: string, atTime:number , spellcast: SpellCast) {
        ...
        if (si) {
            for (const [powerType, powerInfo] of kpairs(this.POWER_INFO)) {
                let [cost, refund] = this.getPowerCostAt(this.next, spellId, powerInfo.type, atTime, targetGUID);
                ...
                if (seconds > 0) {
                    let powerRate = this.getPowerRateAt(this.next, powerType, atTime) || 0;
    ...
    PowerCost(spellId: number, powerType: PowerType, atTime: number, targetGUID: string, maximumCost?: boolean) {
        return this.getPowerCostAt(this.GetState(atTime), spellId, powerType, atTime, targetGUID, maximumCost);
    }
     ...
     * @param atTime 
     */
    getPowerAt(state: PowerState, powerType: PowerType, atTime: number): number {
        let power = state.power[powerType] || 0;
        if (atTime) {
            let now = this.baseState.next.currentTime;
            let seconds = atTime - now;
            if (seconds > 0) {
                let powerRate = this.getPowerRateAt(state, powerType, atTime) || 0;
                power = power + powerRate * seconds;
            }
        }
        return power;
     ...
     */
    private getPowerCostAt(state: PowerState, spellId: number, powerType: PowerType, atTime:number, targetGUID:string | undefined, maximumCost?:boolean): [number, number] {
     ...
     */
    private getPowerRequirementAt(state: PowerState, spellId: number, atTime: number, requirement: string, tokens: Tokens, index: number, targetGUID: string | undefined): [boolean, string, number] {
        let verified = false;
        ...
        if (baseCost) {
            if (baseCost > 0) { // Don't check power requirements for abilities that generate power
                ...
                let [cost] = this.getPowerCostAt(state, spellId, powerType, atTime, targetGUID);
                if (cost > 0) {
                    let power = this.getPowerAt(state, powerType, atTime);
                    if (power >= cost) {
                        verified = true;
                ...
                if (cost > 0) {
                    ...
                    this.tracer.Log("    Require %f %s at time=%f: %s", cost, powerType, atTime, result);
                }
            } else {
                verified = true;
            }
        } else {
            this.ovale.OneTimeMessage("Warning: requirement '%s' power is missing a cost argument.", requirement);
     ...
     */
    private getTimeToPowerStateAt(state: PowerState, spellId:number, atTime:number, targetGUID:string | undefined, powerType:PowerType | undefined, extraPower?:number) {
        let seconds = 0;
