...
import { getSpellData, PowerType, SpellPowerData, isFriendlyTarget, EffectType, SpellData } from "./importspells";
...
${limitLine2}
    ...
    for (const spellId of spellIds) {
        ...
        if (spell.talent) {
            ...
            if (talentIds.indexOf(spell.talent.talentId) <= 0) talentIds.push(spell.talent.talentId);
        }
...
// END`;
    ...
    const existing = readFileSync(fileName, { encoding: 'utf8'});
    output = existing.replace(/\/\/ THE REST OF THIS FILE IS AUTOMATICALLY GENERATED[^]*\/\/ END/, output);
