	...
	elseif action == "snapshot_stats" then
		...
		elseif action == "potion" then
			...
			if name then
				...
				annotation.opt_use_consumables = class
		...
		elseif action == "use_item" then
			...
			elseif modifier.name then
				...
				if strmatch(name, "legendary_ring") then
					legendaryRing = name
				elseif false then -- TODO: use all other items here
...
EmitOperandSpecial = function(operand, parseNode, nodeList, annotation, action, target)
	...
	elseif class == "MAGE" and operand == "firestarter.active" then
		code = "(target.HealthPercent() >= 90 and Talent(firestarter_talent))"
		ok = true
...
function OvaleSimulationCraft:ParseProfile(simc)
	...
	for k,v in pairs(CONSUMABLE_ITEMS) do
		if v then
			if profile[k] ~= nil then consumables[k] = profile[k] end
		end
	end
