...
AddFunction ArmsSingleMainActions
{
	...
	#thunder_clap,if=!set_bonus.tier18_4pc&!talent.slam.enabled&target.health.pct>20&(rage>=40|debuff.colossus_smash.up)&glyph.resonating_power.enabled&cooldown.colossus_smash.remains>gcd
	if not ArmorSetBonus(T18 4) and not Talent(slam_talent) and target.HealthPercent() > 20 and { Rage() >= 40 or target.DebuffPresent(colossus_smash_debuff) } and Glyph(glyph_of_resonating_power) and SpellCooldown(colossus_smash) > GCD() Spell(thunder_clap)
	#whirlwind,if=!set_bonus.tier18_4pc&!talent.slam.enabled&target.health.pct>20&(rage>=40|debuff.colossus_smash.up)&cooldown.colossus_smash.remains>gcd
	if not ArmorSetBonus(T18 4) and not Talent(slam_talent) and target.HealthPercent() > 20 and { Rage() >= 40 or target.DebuffPresent(colossus_smash_debuff) } and SpellCooldown(colossus_smash) > GCD() Spell(whirlwind)
}
