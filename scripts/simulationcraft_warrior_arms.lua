...
AddFunction ArmsSingleMainActions
{
	...
	if { not ArmorSetBonus(T18 4) or FIXME_rage.deficit < 30 } and not Talent(slam_talent) and target.HealthPercent() > 20 and { Rage() >= 40 or target.DebuffPresent(colossus_smash_debuff) } and Glyph(glyph_of_resonating_power) and SpellCooldown(colossus_smash) > GCD() Spell(thunder_clap)
	...
	if { not ArmorSetBonus(T18 4) or FIXME_rage.deficit < 30 } and not Talent(slam_talent) and target.HealthPercent() > 20 and { Rage() >= 40 or target.DebuffPresent(colossus_smash_debuff) } and SpellCooldown(colossus_smash) > GCD() Spell(whirlwind)
}
