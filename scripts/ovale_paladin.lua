...
AddFunction ProtectionDefaultShortCDActions
{
	...
	if ((Charges(shield_of_the_righteous) >= 2) or (Charges(shield_of_the_righteous) >= 1 and BuffRemaining(shield_of_the_righteous_buff) > SpellChargeCooldown(shield_of_the_righteous))) 
	{
...
AddFunction RetributionPriorityMainActions
{
	...
	if Enemies() <= 3 and { SpellCooldown(judgment) < GCD() * 4.5 or target.DebuffRemaining(judgment_debuff) > GCD() * 4.5 } Spell(execution_sentence)
	...
	if target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 Spell(divine_storm)
	#divine_storm,if=debuff.judgment.up&variable.ds_castable&holy_power>=5&buff.divine_purpose.react
	if target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) Spell(divine_storm)
	...
	if target.DebuffPresent(judgment_debuff) and Enemies() >= 2 and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } Spell(divine_storm)
	#divine_storm,if=debuff.judgment.up&variable.ds_castable&holy_power>=5
	if target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 Spell(divine_storm)
	#justicars_vengeance,if=debuff.judgment.up&buff.divine_purpose.up&buff.divine_purpose.remains<gcd*2&!equipped.137020
	if target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and not HasEquippedItem(137020) Spell(justicars_vengeance)
	#justicars_vengeance,if=debuff.judgment.up&holy_power>=5&buff.divine_purpose.react&!equipped.137020
	if target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) Spell(justicars_vengeance)
	...
	if target.DebuffPresent(judgment_debuff) and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } Spell(templars_verdict)
	...
	if target.DebuffPresent(judgment_debuff) and ds_castable() and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 Spell(divine_storm)
	...
	if target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 Spell(divine_storm)
	...
	if { HasEquippedItem(137020) or target.DebuffPresent(judgment_debuff) } and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 Spell(templars_verdict)
	#templars_verdict,if=debuff.judgment.up&buff.whisper_of_the_nathrezim.up&buff.whisper_of_the_nathrezim.remains<gcd*1.5
	if target.DebuffPresent(judgment_debuff) and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 Spell(templars_verdict)
	...
	if SpellCharges(zeal count=0) >= 1.65 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() Spell(zeal)
	...
	if SpellCharges(crusader_strike count=0) >= 1.65 - TalentPoints(the_fires_of_justice_talent) * 0.25 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() Spell(crusader_strike)
	#consecration
	Spell(consecration)
	#divine_storm,if=debuff.judgment.up&variable.ds_castable&buff.divine_purpose.react
	if target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) Spell(divine_storm)
	#divine_storm,if=debuff.judgment.up&variable.ds_castable&buff.the_fires_of_justice.react
	if target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(the_fires_of_justice_buff) Spell(divine_storm)
	#divine_storm,if=debuff.judgment.up&variable.ds_castable
	if target.DebuffPresent(judgment_debuff) and ds_castable() Spell(divine_storm)
	#justicars_vengeance,if=debuff.judgment.up&buff.divine_purpose.react&!equipped.137020
	if target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) Spell(justicars_vengeance)
	...
	if target.DebuffPresent(judgment_debuff) and { not Talent(execution_sentence_talent) or SpellCooldown(execution_sentence) > GCD() * 2 } Spell(templars_verdict)
...
{
	unless Enemies() <= 3 and { SpellCooldown(judgment) < GCD() * 4.5 or target.DebuffRemaining(judgment_debuff) > GCD() * 4.5 } and Spell(execution_sentence) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and Enemies() >= 2 and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and ds_castable() and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 and Spell(divine_storm) or { HasEquippedItem(137020) or target.DebuffPresent(judgment_debuff) } and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 and Spell(templars_verdict) or target.DebuffPresent(execution_sentence_debuff) and target.DebuffRemaining(execution_sentence_debuff) < GCD() * 2 and target.DebuffRemaining(judgment_debuff) < GCD() * 2 and Spell(judgment) or { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and Spell(consecration) or { not False(raid_event_adds_exists) or 600 > 15 } and { HolyPower() <= 0 or HolyPower() == 1 and { SpellCooldown(blade_of_justice) > GCD() or SpellCooldown(divine_hammer) > GCD() } or HolyPower() == 2 and { SpellCharges(zeal count=0) <= 0.65 or SpellCharges(crusader_strike count=0) <= 0.65 } } and Spell(wake_of_ashes) or HolyPower() <= 3 - ArmorSetBonus(T20 4) and Spell(blade_of_justice) or HolyPower() <= 3 - ArmorSetBonus(T20 4) and Spell(divine_hammer) or Spell(judgment) or SpellCharges(zeal count=0) >= 1.65 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() and Spell(zeal) or SpellCharges(crusader_strike count=0) >= 1.65 - TalentPoints(the_fires_of_justice_talent) * 0.25 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() and Spell(crusader_strike) or Spell(consecration) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(the_fires_of_justice_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and BuffPresent(the_fires_of_justice_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and { not Talent(execution_sentence_talent) or SpellCooldown(execution_sentence) > GCD() * 2 } and Spell(templars_verdict)
	{
...
{
	Enemies() <= 3 and { SpellCooldown(judgment) < GCD() * 4.5 or target.DebuffRemaining(judgment_debuff) > GCD() * 4.5 } and Spell(execution_sentence) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and Enemies() >= 2 and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and ds_castable() and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 and Spell(divine_storm) or { HasEquippedItem(137020) or target.DebuffPresent(judgment_debuff) } and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 and Spell(templars_verdict) or target.DebuffPresent(execution_sentence_debuff) and target.DebuffRemaining(execution_sentence_debuff) < GCD() * 2 and target.DebuffRemaining(judgment_debuff) < GCD() * 2 and Spell(judgment) or { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and Spell(consecration) or { not False(raid_event_adds_exists) or 600 > 15 } and { HolyPower() <= 0 or HolyPower() == 1 and { SpellCooldown(blade_of_justice) > GCD() or SpellCooldown(divine_hammer) > GCD() } or HolyPower() == 2 and { SpellCharges(zeal count=0) <= 0.65 or SpellCharges(crusader_strike count=0) <= 0.65 } } and Spell(wake_of_ashes) or HolyPower() <= 3 - ArmorSetBonus(T20 4) and Spell(blade_of_justice) or HolyPower() <= 3 - ArmorSetBonus(T20 4) and Spell(divine_hammer) or Spell(judgment) or SpellCharges(zeal count=0) >= 1.65 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() and Spell(zeal) or SpellCharges(crusader_strike count=0) >= 1.65 - TalentPoints(the_fires_of_justice_talent) * 0.25 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() and Spell(crusader_strike) or Spell(consecration) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(the_fires_of_justice_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and BuffPresent(the_fires_of_justice_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and { not Talent(execution_sentence_talent) or SpellCooldown(execution_sentence) > GCD() * 2 } and Spell(templars_verdict) or HolyPower() <= 4 and Spell(zeal) or HolyPower() <= 4 and Spell(crusader_strike)
}
...
{
	Enemies() <= 3 and { SpellCooldown(judgment) < GCD() * 4.5 or target.DebuffRemaining(judgment_debuff) > GCD() * 4.5 } and Spell(execution_sentence) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and Enemies() >= 2 and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and HolyPower() >= 5 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and BuffRemaining(divine_purpose_buff) < GCD() * 2 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and BuffPresent(divine_purpose_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 3 and { BuffPresent(crusade_buff) and BuffStacks(crusade_buff) < 15 or BuffPresent(liadrins_fury_unleashed_buff) } and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and HolyPower() >= 5 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and ds_castable() and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 and Spell(divine_storm) or { HasEquippedItem(137020) or target.DebuffPresent(judgment_debuff) } and HasArtifactTrait(wake_of_ashes) and SpellCooldown(wake_of_ashes) < GCD() * 2 and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and BuffPresent(whisper_of_the_nathrezim_buff) and BuffRemaining(whisper_of_the_nathrezim_buff) < GCD() * 1.5 and Spell(templars_verdict) or target.DebuffPresent(execution_sentence_debuff) and target.DebuffRemaining(execution_sentence_debuff) < GCD() * 2 and target.DebuffRemaining(judgment_debuff) < GCD() * 2 and Spell(judgment) or { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and Spell(consecration) or { not False(raid_event_adds_exists) or 600 > 15 } and { HolyPower() <= 0 or HolyPower() == 1 and { SpellCooldown(blade_of_justice) > GCD() or SpellCooldown(divine_hammer) > GCD() } or HolyPower() == 2 and { SpellCharges(zeal count=0) <= 0.65 or SpellCharges(crusader_strike count=0) <= 0.65 } } and Spell(wake_of_ashes) or HolyPower() <= 3 - ArmorSetBonus(T20 4) and Spell(blade_of_justice) or HolyPower() <= 3 - ArmorSetBonus(T20 4) and Spell(divine_hammer) or Spell(judgment) or SpellCharges(zeal count=0) >= 1.65 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() and Spell(zeal) or SpellCharges(crusader_strike count=0) >= 1.65 - TalentPoints(the_fires_of_justice_talent) * 0.25 and HolyPower() <= 4 and { SpellCooldown(blade_of_justice) > GCD() * 2 or SpellCooldown(divine_hammer) > GCD() * 2 } and target.DebuffRemaining(judgment_debuff) > GCD() and Spell(crusader_strike) or Spell(consecration) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(divine_purpose_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and BuffPresent(the_fires_of_justice_buff) and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and ds_castable() and Spell(divine_storm) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and not HasEquippedItem(137020) and Spell(justicars_vengeance) or target.DebuffPresent(judgment_debuff) and BuffPresent(divine_purpose_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and BuffPresent(the_fires_of_justice_buff) and Spell(templars_verdict) or target.DebuffPresent(judgment_debuff) and { not Talent(execution_sentence_talent) or SpellCooldown(execution_sentence) > GCD() * 2 } and Spell(templars_verdict) or HasEquippedItem(137065) and target.HealthPercent() >= 75 and HolyPower() <= 4 and Spell(hammer_of_justice) or HolyPower() <= 4 and Spell(zeal) or HolyPower() <= 4 and Spell(crusader_strike)
}
