...
AddFunction AfflictionDefaultMainActions
{
	...
	#mortal_coil,if=equipped.132452
	if HasEquippedItem(132452) Spell(mortal_coil)
	...
	if Talent(malefic_grasp_talent) and not PreviousGCDSpell(3) and target.DebuffRemaining(agony_debuff) > CastTime(unstable_affliction) + 6.5 and { target.DebuffRemaining(corruption_debuff) > CastTime(unstable_affliction) + 6.5 or Talent(absolute_corruption_talent) } and { target.DebuffRemaining(siphon_life_debuff) > CastTime(unstable_affliction) + 6.5 or not Talent(siphon_life_talent) } Spell(unstable_affliction)
	...
	if not BuffPresent(deadwind_harvester_buff) and not True(trinket_has_stacking_stat_any) and not True(trinket_has_stat_any) and PreviousGCDSpell(1) Spell(reap_souls)
	...
	if target.DebuffRemaining(agony_debuff) <= BaseDuration(agony_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(agony_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 Spell(agony)
	...
	if target.DebuffRemaining(corruption_debuff) <= BaseDuration(corruption_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(corruption_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 Spell(corruption)
	...
	if target.DebuffRemaining(siphon_life_debuff) <= BaseDuration(siphon_life_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(siphon_life_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 Spell(siphon_life)
...
{
	not BuffPresent(deadwind_harvester_buff) and { BuffPresent(soul_harvest_buff) or BuffStacks(tormented_souls_buff) >= 8 or target.TimeToDie() <= BuffStacks(tormented_souls_buff) * 5 or BuffPresent(trinket_proc_any_buff) or BuffPresent(trinket_stacking_proc_any_buff) } and Spell(reap_souls) or not pet.Present() and Spell(soul_effigy) or target.DebuffRemaining(agony_debuff) <= target.TickTime(agony_debuff) + GCD() and Spell(agony) or HasEquippedItem(132452) and Spell(mortal_coil) or target.DebuffRemaining(corruption_debuff) <= target.TickTime(corruption_debuff) + GCD() and Spell(corruption) or { Talent(absolute_corruption_talent) or not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(corruption_debuff) <= target.TickTime(corruption_debuff) + GCD() and Spell(corruption) or target.DebuffRemaining(siphon_life_debuff) <= target.TickTime(siphon_life_debuff) + GCD() and Spell(siphon_life) or { not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(siphon_life_debuff) <= target.TickTime(siphon_life_debuff) + GCD() and Spell(siphon_life) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) <= GCD() and Spell(life_tap) or Spell(haunt) or Talent(writhe_in_agony_talent) and Talent(contagion_talent) and Spell(unstable_affliction) or Talent(writhe_in_agony_talent) and { SoulShards() >= 4 or BuffPresent(trinket_proc_intellect_buff) or BuffPresent(trinket_stacking_proc_mastery_buff) or BuffPresent(trinket_proc_mastery_buff) or BuffPresent(trinket_proc_crit_buff) or BuffPresent(trinket_proc_versatility_buff) or BuffPresent(soul_harvest_buff) or BuffPresent(deadwind_harvester_buff) or BuffStacks(compounding_horror_buff) == 5 or target.TimeToDie() <= 20 } and Spell(unstable_affliction) or Talent(malefic_grasp_talent) and target.TimeToDie() < 30 and Spell(unstable_affliction) or Talent(malefic_grasp_talent) and SoulShards() >= 4 and Spell(unstable_affliction) or Talent(malefic_grasp_talent) and not PreviousGCDSpell(3) and target.DebuffRemaining(agony_debuff) > CastTime(unstable_affliction) + 6.5 and { target.DebuffRemaining(corruption_debuff) > CastTime(unstable_affliction) + 6.5 or Talent(absolute_corruption_talent) } and { target.DebuffRemaining(siphon_life_debuff) > CastTime(unstable_affliction) + 6.5 or not Talent(siphon_life_talent) } and Spell(unstable_affliction) or Talent(haunt_talent) and { SoulShards() >= 4 or target.DebuffPresent(haunt_debuff) or target.TimeToDie() < 30 } and Spell(unstable_affliction) or not BuffPresent(deadwind_harvester_buff) and not True(trinket_has_stacking_stat_any) and not True(trinket_has_stat_any) and PreviousGCDSpell(1) and Spell(reap_souls) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) < BaseDuration(empowered_life_tap_buff) * 0.3 and Spell(life_tap) or not Talent(malefic_grasp_talent) and target.DebuffRemaining(agony_debuff) <= BaseDuration(agony_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(agony_debuff) and Spell(agony) or target.DebuffRemaining(agony_debuff) <= BaseDuration(agony_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(agony_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 and Spell(agony) or not Talent(malefic_grasp_talent) and target.DebuffRemaining(corruption_debuff) <= BaseDuration(corruption_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(corruption_debuff) and Spell(corruption) or target.DebuffRemaining(corruption_debuff) <= BaseDuration(corruption_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(corruption_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 and Spell(corruption) or { Talent(absolute_corruption_talent) or not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(corruption_debuff) <= BaseDuration(corruption_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(corruption_debuff) and Spell(corruption) or not Talent(malefic_grasp_talent) and target.DebuffRemaining(siphon_life_debuff) <= BaseDuration(siphon_life_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(siphon_life_debuff) and Spell(siphon_life) or target.DebuffRemaining(siphon_life_debuff) <= BaseDuration(siphon_life_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(siphon_life_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 and Spell(siphon_life) or { not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(siphon_life_debuff) <= BaseDuration(siphon_life_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(siphon_life_debuff) and Spell(siphon_life) or ManaPercent() <= 10 and Spell(life_tap) or Spell(drain_soul) or Spell(life_tap)
}
...
{
	not BuffPresent(deadwind_harvester_buff) and { BuffPresent(soul_harvest_buff) or BuffStacks(tormented_souls_buff) >= 8 or target.TimeToDie() <= BuffStacks(tormented_souls_buff) * 5 or BuffPresent(trinket_proc_any_buff) or BuffPresent(trinket_stacking_proc_any_buff) } and Spell(reap_souls) or not pet.Present() and Spell(soul_effigy) or target.DebuffRemaining(agony_debuff) <= target.TickTime(agony_debuff) + GCD() and Spell(agony) or target.DebuffRemaining(corruption_debuff) and target.DebuffRemaining(agony_debuff) and Spell(service_felhunter) or HasEquippedItem(132452) and Spell(mortal_coil) or target.DebuffRemaining(corruption_debuff) <= target.TickTime(corruption_debuff) + GCD() and Spell(corruption) or { Talent(absolute_corruption_talent) or not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(corruption_debuff) <= target.TickTime(corruption_debuff) + GCD() and Spell(corruption) or target.DebuffRemaining(siphon_life_debuff) <= target.TickTime(siphon_life_debuff) + GCD() and Spell(siphon_life) or { not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(siphon_life_debuff) <= target.TickTime(siphon_life_debuff) + GCD() and Spell(siphon_life) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) <= GCD() and Spell(life_tap) or Spell(phantom_singularity) or Spell(haunt) or Talent(writhe_in_agony_talent) and Talent(contagion_talent) and Spell(unstable_affliction) or Talent(writhe_in_agony_talent) and { SoulShards() >= 4 or BuffPresent(trinket_proc_intellect_buff) or BuffPresent(trinket_stacking_proc_mastery_buff) or BuffPresent(trinket_proc_mastery_buff) or BuffPresent(trinket_proc_crit_buff) or BuffPresent(trinket_proc_versatility_buff) or BuffPresent(soul_harvest_buff) or BuffPresent(deadwind_harvester_buff) or BuffStacks(compounding_horror_buff) == 5 or target.TimeToDie() <= 20 } and Spell(unstable_affliction) or Talent(malefic_grasp_talent) and target.TimeToDie() < 30 and Spell(unstable_affliction) or Talent(malefic_grasp_talent) and SoulShards() >= 4 and Spell(unstable_affliction) or Talent(malefic_grasp_talent) and not PreviousGCDSpell(3) and target.DebuffRemaining(agony_debuff) > CastTime(unstable_affliction) + 6.5 and { target.DebuffRemaining(corruption_debuff) > CastTime(unstable_affliction) + 6.5 or Talent(absolute_corruption_talent) } and { target.DebuffRemaining(siphon_life_debuff) > CastTime(unstable_affliction) + 6.5 or not Talent(siphon_life_talent) } and Spell(unstable_affliction) or Talent(haunt_talent) and { SoulShards() >= 4 or target.DebuffPresent(haunt_debuff) or target.TimeToDie() < 30 } and Spell(unstable_affliction) or not BuffPresent(deadwind_harvester_buff) and not True(trinket_has_stacking_stat_any) and not True(trinket_has_stat_any) and PreviousGCDSpell(1) and Spell(reap_souls) or Talent(empowered_life_tap_talent) and BuffRemaining(empowered_life_tap_buff) < BaseDuration(empowered_life_tap_buff) * 0.3 and Spell(life_tap) or not Talent(malefic_grasp_talent) and target.DebuffRemaining(agony_debuff) <= BaseDuration(agony_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(agony_debuff) and Spell(agony) or target.DebuffRemaining(agony_debuff) <= BaseDuration(agony_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(agony_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 and Spell(agony) or not Talent(malefic_grasp_talent) and target.DebuffRemaining(corruption_debuff) <= BaseDuration(corruption_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(corruption_debuff) and Spell(corruption) or target.DebuffRemaining(corruption_debuff) <= BaseDuration(corruption_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(corruption_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 and Spell(corruption) or { Talent(absolute_corruption_talent) or not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(corruption_debuff) <= BaseDuration(corruption_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(corruption_debuff) and Spell(corruption) or not Talent(malefic_grasp_talent) and target.DebuffRemaining(siphon_life_debuff) <= BaseDuration(siphon_life_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(siphon_life_debuff) and Spell(siphon_life) or target.DebuffRemaining(siphon_life_debuff) <= BaseDuration(siphon_life_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(siphon_life_debuff) and not target.DebuffCount(unstable_affliction_debuff) >= 1 and not target.DebuffCount(unstable_affliction_debuff) >= 2 and not target.DebuffCount(unstable_affliction_debuff) >= 3 and not target.DebuffCount(unstable_affliction_debuff) >= 4 and not target.DebuffCount(unstable_affliction_debuff) >= 5 and Spell(siphon_life) or { not Talent(malefic_grasp_talent) or not Talent(soul_effigy_talent) } and target.DebuffRemaining(siphon_life_debuff) <= BaseDuration(siphon_life_debuff) * 0.3 and target.TimeToDie() >= target.DebuffRemaining(siphon_life_debuff) and Spell(siphon_life) or ManaPercent() <= 10 and Spell(life_tap) or Spell(drain_soul) or Spell(life_tap)
}
...
AddFunction DemonologyDefaultMainActions
{
	...
	if PreviousGCDSpell(1) and { DemonDuration(wild_imp) <= 3 and BuffPresent(demonic_synergy_buff) or DemonDuration(wild_imp) <= 4 and Enemies() > 2 } Spell(implosion)
	...
	if PreviousGCDSpell(1) or PreviousGCDSpell(1) or Talent(power_trip_talent) Spell(summon_darkglare)
	...
	if Talent(summon_darkglare_talent) and { Enemies() < 3 or not Talent(implosion_talent) } and { SpellCooldown(summon_darkglare) > 2 or PreviousGCDSpell(1) or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 3 or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 1 and BuffPresent(demonic_calling_buff) } Spell(call_dreadstalkers)
	...
	if SoulShards() >= 3 and PreviousGCDSpell(1) or SoulShards() >= 5 or SoulShards() >= 4 and SpellCooldown(summon_darkglare) > 2 Spell(hand_of_guldan)
	...
	if { Talent(power_trip_talent) and { not Talent(implosion_talent) or Enemies() <= 1 } or not Talent(implosion_talent) or Talent(implosion_talent) and not Talent(soul_conduit_talent) and Enemies() <= 3 } and { NotDeDemons(wild_imp) > 3 or PreviousGCDSpell(1) } or PreviousGCDSpell(1) and NotDeDemons(wild_imp) == 0 and DemonDuration(wild_imp) <= 0 or PreviousGCDSpell(1) and NotDeDemons(wild_imp) > 0 Spell(demonic_empowerment)
...
AddFunction DemonologyDefaultShortCdActions
{
	unless DemonDuration(wild_imp) <= ExecuteTime(shadow_bolt) and { BuffPresent(demonic_synergy_buff) or Talent(soul_conduit_talent) or not Talent(soul_conduit_talent) and Enemies() > 1 or Demons(wild_imp) <= 4 } and Spell(implosion) or PreviousGCDSpell(1) and { DemonDuration(wild_imp) <= 3 and BuffPresent(demonic_synergy_buff) or DemonDuration(wild_imp) <= 4 and Enemies() > 2 } and Spell(implosion) or { target.DebuffStacks(shadowflame_debuff) > 0 and target.DebuffRemaining(shadowflame_debuff) < CastTime(shadow_bolt) + TravelTime(shadowflame) or Charges(shadowflame) == 2 and SoulShards() < 5 } and Enemies() < 5 and Spell(shadowflame)
	{
		#service_pet
		Spell(service_felguard)

		unless { not Talent(summon_darkglare_talent) or Talent(power_trip_talent) } and { Enemies() < 3 or not Talent(implosion_talent) } and Spell(call_dreadstalkers) or SoulShards() >= 4 and not Talent(summon_darkglare_talent) and Spell(hand_of_guldan) or { PreviousGCDSpell(1) or PreviousGCDSpell(1) or Talent(power_trip_talent) } and Spell(summon_darkglare) or SpellCooldown(call_dreadstalkers) > 5 and SoulShards() < 3 and Spell(summon_darkglare) or SpellCooldown(call_dreadstalkers) <= CastTime(summon_darkglare) and { SoulShards() >= 3 or SoulShards() >= 1 and BuffPresent(demonic_calling_buff) } and Spell(summon_darkglare) or Talent(summon_darkglare_talent) and { Enemies() < 3 or not Talent(implosion_talent) } and { SpellCooldown(summon_darkglare) > 2 or PreviousGCDSpell(1) or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 3 or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 1 and BuffPresent(demonic_calling_buff) } and Spell(call_dreadstalkers) or { SoulShards() >= 3 and PreviousGCDSpell(1) or SoulShards() >= 5 or SoulShards() >= 4 and SpellCooldown(summon_darkglare) > 2 } and Spell(hand_of_guldan) or { { Talent(power_trip_talent) and { not Talent(implosion_talent) or Enemies() <= 1 } or not Talent(implosion_talent) or Talent(implosion_talent) and not Talent(soul_conduit_talent) and Enemies() <= 3 } and { NotDeDemons(wild_imp) > 3 or PreviousGCDSpell(1) } or PreviousGCDSpell(1) and NotDeDemons(wild_imp) == 0 and DemonDuration(wild_imp) <= 0 or PreviousGCDSpell(1) and NotDeDemons(wild_imp) > 0 } and Spell(demonic_empowerment) or { NotDeDemons(dreadstalker) > 0 or NotDeDemons(darkglare) > 0 or NotDeDemons(doomguard) > 0 or NotDeDemons(infernal) > 0 or 0 > 0 } and Spell(demonic_empowerment) or not Talent(hand_of_doom_talent) and target.TimeToDie() > BaseDuration(doom_debuff) and { not target.DebuffPresent(doom_debuff) or target.DebuffRemaining(doom_debuff) < BaseDuration(doom_debuff) * 0.3 } and Spell(doom) or Charges(shadowflame) == 2 and Enemies() < 5 and Spell(shadowflame)
		{
...
{
	DemonDuration(wild_imp) <= ExecuteTime(shadow_bolt) and { BuffPresent(demonic_synergy_buff) or Talent(soul_conduit_talent) or not Talent(soul_conduit_talent) and Enemies() > 1 or Demons(wild_imp) <= 4 } and Spell(implosion) or PreviousGCDSpell(1) and { DemonDuration(wild_imp) <= 3 and BuffPresent(demonic_synergy_buff) or DemonDuration(wild_imp) <= 4 and Enemies() > 2 } and Spell(implosion) or { target.DebuffStacks(shadowflame_debuff) > 0 and target.DebuffRemaining(shadowflame_debuff) < CastTime(shadow_bolt) + TravelTime(shadowflame) or Charges(shadowflame) == 2 and SoulShards() < 5 } and Enemies() < 5 and Spell(shadowflame) or { not Talent(summon_darkglare_talent) or Talent(power_trip_talent) } and { Enemies() < 3 or not Talent(implosion_talent) } and Spell(call_dreadstalkers) or SoulShards() >= 4 and not Talent(summon_darkglare_talent) and Spell(hand_of_guldan) or { PreviousGCDSpell(1) or PreviousGCDSpell(1) or Talent(power_trip_talent) } and Spell(summon_darkglare) or SpellCooldown(call_dreadstalkers) > 5 and SoulShards() < 3 and Spell(summon_darkglare) or SpellCooldown(call_dreadstalkers) <= CastTime(summon_darkglare) and { SoulShards() >= 3 or SoulShards() >= 1 and BuffPresent(demonic_calling_buff) } and Spell(summon_darkglare) or Talent(summon_darkglare_talent) and { Enemies() < 3 or not Talent(implosion_talent) } and { SpellCooldown(summon_darkglare) > 2 or PreviousGCDSpell(1) or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 3 or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 1 and BuffPresent(demonic_calling_buff) } and Spell(call_dreadstalkers) or { SoulShards() >= 3 and PreviousGCDSpell(1) or SoulShards() >= 5 or SoulShards() >= 4 and SpellCooldown(summon_darkglare) > 2 } and Spell(hand_of_guldan) or { { Talent(power_trip_talent) and { not Talent(implosion_talent) or Enemies() <= 1 } or not Talent(implosion_talent) or Talent(implosion_talent) and not Talent(soul_conduit_talent) and Enemies() <= 3 } and { NotDeDemons(wild_imp) > 3 or PreviousGCDSpell(1) } or PreviousGCDSpell(1) and NotDeDemons(wild_imp) == 0 and DemonDuration(wild_imp) <= 0 or PreviousGCDSpell(1) and NotDeDemons(wild_imp) > 0 } and Spell(demonic_empowerment) or { NotDeDemons(dreadstalker) > 0 or NotDeDemons(darkglare) > 0 or NotDeDemons(doomguard) > 0 or NotDeDemons(infernal) > 0 or 0 > 0 } and Spell(demonic_empowerment) or not Talent(hand_of_doom_talent) and target.TimeToDie() > BaseDuration(doom_debuff) and { not target.DebuffPresent(doom_debuff) or target.DebuffRemaining(doom_debuff) < BaseDuration(doom_debuff) * 0.3 } and Spell(doom) or Charges(shadowflame) == 2 and Enemies() < 5 and Spell(shadowflame) or ManaPercent() <= 30 and Spell(life_tap) or Enemies() >= 3 and Spell(demonwrath) or Speed() > 0 and Spell(demonwrath) or Spell(demonbolt) or Spell(shadow_bolt) or Spell(life_tap)
}
...
{
	DemonDuration(wild_imp) <= ExecuteTime(shadow_bolt) and { BuffPresent(demonic_synergy_buff) or Talent(soul_conduit_talent) or not Talent(soul_conduit_talent) and Enemies() > 1 or Demons(wild_imp) <= 4 } and Spell(implosion) or PreviousGCDSpell(1) and { DemonDuration(wild_imp) <= 3 and BuffPresent(demonic_synergy_buff) or DemonDuration(wild_imp) <= 4 and Enemies() > 2 } and Spell(implosion) or { target.DebuffStacks(shadowflame_debuff) > 0 and target.DebuffRemaining(shadowflame_debuff) < CastTime(shadow_bolt) + TravelTime(shadowflame) or Charges(shadowflame) == 2 and SoulShards() < 5 } and Enemies() < 5 and Spell(shadowflame) or Spell(service_felguard) or { not Talent(summon_darkglare_talent) or Talent(power_trip_talent) } and { Enemies() < 3 or not Talent(implosion_talent) } and Spell(call_dreadstalkers) or SoulShards() >= 4 and not Talent(summon_darkglare_talent) and Spell(hand_of_guldan) or { PreviousGCDSpell(1) or PreviousGCDSpell(1) or Talent(power_trip_talent) } and Spell(summon_darkglare) or SpellCooldown(call_dreadstalkers) > 5 and SoulShards() < 3 and Spell(summon_darkglare) or SpellCooldown(call_dreadstalkers) <= CastTime(summon_darkglare) and { SoulShards() >= 3 or SoulShards() >= 1 and BuffPresent(demonic_calling_buff) } and Spell(summon_darkglare) or Talent(summon_darkglare_talent) and { Enemies() < 3 or not Talent(implosion_talent) } and { SpellCooldown(summon_darkglare) > 2 or PreviousGCDSpell(1) or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 3 or SpellCooldown(summon_darkglare) <= CastTime(call_dreadstalkers) and SoulShards() >= 1 and BuffPresent(demonic_calling_buff) } and Spell(call_dreadstalkers) or { SoulShards() >= 3 and PreviousGCDSpell(1) or SoulShards() >= 5 or SoulShards() >= 4 and SpellCooldown(summon_darkglare) > 2 } and Spell(hand_of_guldan) or { { Talent(power_trip_talent) and { not Talent(implosion_talent) or Enemies() <= 1 } or not Talent(implosion_talent) or Talent(implosion_talent) and not Talent(soul_conduit_talent) and Enemies() <= 3 } and { NotDeDemons(wild_imp) > 3 or PreviousGCDSpell(1) } or PreviousGCDSpell(1) and NotDeDemons(wild_imp) == 0 and DemonDuration(wild_imp) <= 0 or PreviousGCDSpell(1) and NotDeDemons(wild_imp) > 0 } and Spell(demonic_empowerment) or { NotDeDemons(dreadstalker) > 0 or NotDeDemons(darkglare) > 0 or NotDeDemons(doomguard) > 0 or NotDeDemons(infernal) > 0 or 0 > 0 } and Spell(demonic_empowerment) or not Talent(hand_of_doom_talent) and target.TimeToDie() > BaseDuration(doom_debuff) and { not target.DebuffPresent(doom_debuff) or target.DebuffRemaining(doom_debuff) < BaseDuration(doom_debuff) * 0.3 } and Spell(doom) or Charges(shadowflame) == 2 and Enemies() < 5 and Spell(shadowflame) or { DemonDuration(dreadstalker) > ExecuteTime(thalkiels_consumption) or Talent(implosion_talent) and Enemies() >= 3 } and Demons(wild_imp) > 3 and DemonDuration(wild_imp) > ExecuteTime(thalkiels_consumption) and Spell(thalkiels_consumption) or ManaPercent() <= 30 and Spell(life_tap) or Enemies() >= 3 and Spell(demonwrath) or Speed() > 0 and Spell(demonwrath) or Spell(demonbolt) or Spell(shadow_bolt) or Spell(life_tap)
}
