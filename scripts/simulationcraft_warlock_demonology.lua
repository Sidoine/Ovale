...
do
	...
	local desc = "[6.0.2] SimulationCraft: Warlock_Demonology_T16M"
	local code = [[
...
AddFunction DemonologyDefaultActions
{
	...
	if not Talent(demonbolt_talent) and { Charges(dark_soul_knowledge) == 2 or target.TimeToDie() <= 20 and not Glyph(glyph_of_dark_soul_knowledge) or target.TimeToDie() <= 10 or target.TimeToDie() <= 60 and DemonicFury() > 400 or BuffPresent(trinket_proc_intellect_buff) and DemonicFury() > 400 } Spell(dark_soul_knowledge)
	...
	if BuffPresent(metamorphosis_buff) and target.TimeToDie() >= 30 * { SpellHaste() / 100 } and target.DebuffRemaining(doom_debuff) <= target.DebuffDurationIfApplied(doom_debuff) * 0.3 and { target.DebuffRemaining(doom_debuff) < SpellCooldown(cataclysm) or not Talent(cataclysm_talent) } and { BuffExpires(dark_soul_knowledge_buff) or not Glyph(glyph_of_dark_soul_knowledge) } Spell(doom)
	#corruption,if=target.time_to_die>=6&remains<=(0.3*duration)&buff.metamorphosis.down
	if target.TimeToDie() >= 6 and target.DebuffRemaining(corruption_debuff) <= 0.3 * target.DebuffDurationIfApplied(corruption_debuff) and BuffExpires(metamorphosis_buff) Spell(corruption)
	...
	if BuffPresent(metamorphosis_buff) and { DemonicFury() < 650 and not Glyph(glyph_of_dark_soul_knowledge) or DemonicFury() < 450 } and BuffExpires(dark_soul_knowledge_buff) and BuffExpires(trinket_proc_intellect_buff) and target.TimeToDie() > SpellCooldown(dark_soul_knowledge) Texture(spell_shadow_demonform text=cancel)
	...
	if BuffPresent(metamorphosis_buff) and Charges(hand_of_guldan) > 0 and target.DebuffPresent(shadowflame_debuff) < 0.5 + CastTime(shadow_bolt) and DemonicFury() < 100 and BuffRemaining(dark_soul_knowledge_buff) > 10 Texture(spell_shadow_demonform text=cancel)
	...
	if BuffPresent(metamorphosis_buff) and Charges(hand_of_guldan) == 3 and { not BuffRemaining(dark_soul_knowledge_buff) > GCD() or SpellCooldown(metamorphosis) < GCD() } Texture(spell_shadow_demonform text=cancel)
...
AddFunction DemonologyDbActions
{
	...
	if BuffPresent(metamorphosis_buff) and target.TimeToDie() >= 30 * { SpellHaste() / 100 } and target.DebuffRemaining(doom_debuff) <= target.DebuffDurationIfApplied(doom_debuff) * 0.3 and { target.DebuffRemaining(doom_debuff) < SpellCooldown(cataclysm) or not Talent(cataclysm_talent) } and { BuffExpires(dark_soul_knowledge_buff) or not Glyph(glyph_of_dark_soul_knowledge) } and BuffRemaining(demonbolt_buff) and { BuffRemaining(demonbolt_buff) < 40 * { SpellHaste() / 100 } - ExecuteTime(demonbolt) or DemonicFury() < 80 + 80 * BuffStacks(demonbolt_buff) } Spell(doom)
	#corruption,if=target.time_to_die>=6&remains<=(0.3*duration)&buff.metamorphosis.down
	if target.TimeToDie() >= 6 and target.DebuffRemaining(corruption_debuff) <= 0.3 * target.DebuffDurationIfApplied(corruption_debuff) and BuffExpires(metamorphosis_buff) Spell(corruption)
	...
	if BuffPresent(metamorphosis_buff) and BuffStacks(demonbolt_buff) > 3 and DemonicFury() <= 600 and target.TimeToDie() > BuffRemaining(demonbolt_buff) and BuffExpires(dark_soul_knowledge_buff) Texture(spell_shadow_demonform text=cancel)
