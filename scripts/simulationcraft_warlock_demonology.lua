...
AddFunction DemonologyDefaultMainActions
{
	...
	#metamorphosis,if=buff.nithramus.remains>4&demonic_fury>200
	if BuffRemaining(nithramus_buff) > 4 and DemonicFury() > 200 Spell(metamorphosis)
	...
	#soul_fire,if=buff.molten_core.react&(buff.demon_rush.remains<=4|buff.demon_rush.stack<5)&set_bonus.tier18_2pc=1
	if BuffPresent(molten_core_buff) and { BuffRemaining(demon_rush_buff) <= 4 or BuffStacks(demon_rush_buff) < 5 } and ArmorSetBonus(T18 2) == 1 Spell(soul_fire)
...
AddFunction DemonologyMetaMainActions
{
	...
	#chaos_wave,if=buff.dark_soul.up&spell_targets.chaos_wave>=2
	if BuffPresent(dark_soul_knowledge_buff) and Enemies() >= 2 Spell(chaos_wave)
