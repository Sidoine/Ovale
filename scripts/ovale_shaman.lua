...
AddFunction EnhancementDefaultMainActions
{
	...
	#frostbrand,if=talent.hailstorm.enabled&buff.frostbrand.remains<gcd&((!talent.fury_of_air.enabled)|(talent.fury_of_air.enabled&maelstrom>25))
	if Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < GCD() and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } Spell(frostbrand)
	...
	if Talent(overcharge_talent) and Maelstrom() >= 40 and not Talent(fury_of_air_talent) or Talent(overcharge_talent) and Talent(fury_of_air_talent) and Maelstrom() > 46 Spell(lightning_bolt_enhancement)
	...
	#frostbrand,if=equipped.137084&talent.hot_hand.enabled&buff.hot_hand.react&!buff.frostbrand.up&((!talent.fury_of_air.enabled)|(talent.fury_of_air.enabled&maelstrom>25))
	if HasEquippedItem(137084) and Talent(hot_hand_talent) and BuffPresent(hot_hand_buff) and not BuffPresent(frostbrand_buff) and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } Spell(frostbrand)
	...
	#windstrike,if=talent.overcharge.enabled&cooldown.lightning_bolt.remains<gcd&maelstrom>80
	if Talent(overcharge_talent) and SpellCooldown(lightning_bolt_enhancement) < GCD() and Maelstrom() > 80 Spell(windstrike)
	#windstrike,if=talent.fury_of_air.enabled&maelstrom>46&(cooldown.lightning_bolt.remains>gcd|!talent.overcharge.enabled)
	if Talent(fury_of_air_talent) and Maelstrom() > 46 and { SpellCooldown(lightning_bolt_enhancement) > GCD() or not Talent(overcharge_talent) } Spell(windstrike)
	#windstrike,if=!talent.overcharge.enabled&!talent.fury_of_air.enabled
	if not Talent(overcharge_talent) and not Talent(fury_of_air_talent) Spell(windstrike)
	#stormstrike,if=talent.overcharge.enabled&cooldown.lightning_bolt.remains<gcd&maelstrom>80
	if Talent(overcharge_talent) and SpellCooldown(lightning_bolt_enhancement) < GCD() and Maelstrom() > 80 Spell(stormstrike)
	#stormstrike,if=talent.fury_of_air.enabled&maelstrom>46&(cooldown.lightning_bolt.remains>gcd|!talent.overcharge.enabled)
	if Talent(fury_of_air_talent) and Maelstrom() > 46 and { SpellCooldown(lightning_bolt_enhancement) > GCD() or not Talent(overcharge_talent) } Spell(stormstrike)
	...
	#frostbrand,if=talent.hailstorm.enabled&buff.frostbrand.remains<4.8&((!talent.fury_of_air.enabled)|(talent.fury_of_air.enabled&maelstrom>25))
	if Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < 4.8 and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } Spell(frostbrand)
	#frostbrand,if=equipped.137084&!buff.frostbrand.up&((!talent.fury_of_air.enabled)|(talent.fury_of_air.enabled&maelstrom>25))
	if HasEquippedItem(137084) and not BuffPresent(frostbrand_buff) and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } Spell(frostbrand)
...
AddFunction EnhancementDefaultShortCdActions
{
	...
	unless ArtifactTraitRank(alpha_wolf) and PreviousGCDSpell(feral_spirit) and Spell(crash_lightning) or { BuffRemaining(boulderfist_buff) < GCD() or Maelstrom() <= 50 and Enemies() >= 3 } and Spell(boulderfist) or { BuffRemaining(boulderfist_buff) < GCD() or Charges(boulderfist count=0) > 1.75 and Maelstrom() <= 100 and Enemies() <= 2 } and Spell(boulderfist) or Talent(landslide_talent) and BuffRemaining(landslide_buff) < GCD() and Spell(rockbiter) or not target.DebuffPresent(fury_of_air_debuff) and Maelstrom() > 22 and Spell(fury_of_air) or Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < GCD() and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or { BuffRemaining(flametongue_buff) < GCD() or SpellCooldown(doom_winds) < 6 and BuffRemaining(flametongue_buff) < 4 } and Spell(flametongue)
	{
		#doom_winds
		Spell(doom_winds)

		unless Talent(crashing_storm_talent) and Enemies() >= 3 and { not Talent(hailstorm_talent) or BuffRemaining(frostbrand_buff) > GCD() } and Spell(crash_lightning) or Spell(earthen_spike) or { Talent(overcharge_talent) and Maelstrom() >= 40 and not Talent(fury_of_air_talent) or Talent(overcharge_talent) and Talent(fury_of_air_talent) and Maelstrom() > 46 } and Spell(lightning_bolt_enhancement) or BuffRemaining(crash_lightning_buff) < GCD() and Enemies() >= 2 and Spell(crash_lightning)
		{
			#windsong
			Spell(windsong)
		}
...
{
	ArtifactTraitRank(alpha_wolf) and PreviousGCDSpell(feral_spirit) and Spell(crash_lightning) or { BuffRemaining(boulderfist_buff) < GCD() or Maelstrom() <= 50 and Enemies() >= 3 } and Spell(boulderfist) or { BuffRemaining(boulderfist_buff) < GCD() or Charges(boulderfist count=0) > 1.75 and Maelstrom() <= 100 and Enemies() <= 2 } and Spell(boulderfist) or Talent(landslide_talent) and BuffRemaining(landslide_buff) < GCD() and Spell(rockbiter) or not target.DebuffPresent(fury_of_air_debuff) and Maelstrom() > 22 and Spell(fury_of_air) or Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < GCD() and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or { BuffRemaining(flametongue_buff) < GCD() or SpellCooldown(doom_winds) < 6 and BuffRemaining(flametongue_buff) < 4 } and Spell(flametongue) or Talent(crashing_storm_talent) and Enemies() >= 3 and { not Talent(hailstorm_talent) or BuffRemaining(frostbrand_buff) > GCD() } and Spell(crash_lightning) or Spell(earthen_spike) or { Talent(overcharge_talent) and Maelstrom() >= 40 and not Talent(fury_of_air_talent) or Talent(overcharge_talent) and Talent(fury_of_air_talent) and Maelstrom() > 46 } and Spell(lightning_bolt_enhancement) or BuffRemaining(crash_lightning_buff) < GCD() and Enemies() >= 2 and Spell(crash_lightning) or BuffPresent(stormbringer_buff) and { Talent(fury_of_air_talent) and Maelstrom() >= 26 or not Talent(fury_of_air_talent) } and Spell(windstrike) or BuffPresent(stormbringer_buff) and { Talent(fury_of_air_talent) and Maelstrom() >= 26 or not Talent(fury_of_air_talent) } and Spell(stormstrike) or HasEquippedItem(137084) and Talent(hot_hand_talent) and BuffPresent(hot_hand_buff) and not BuffPresent(frostbrand_buff) and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or Talent(hot_hand_talent) and BuffPresent(hot_hand_buff) and Spell(lava_lash) or Enemies() >= 3 and Spell(sundering) or Enemies() >= 4 and Spell(crash_lightning) or Talent(overcharge_talent) and SpellCooldown(lightning_bolt_enhancement) < GCD() and Maelstrom() > 80 and Spell(windstrike) or Talent(fury_of_air_talent) and Maelstrom() > 46 and { SpellCooldown(lightning_bolt_enhancement) > GCD() or not Talent(overcharge_talent) } and Spell(windstrike) or not Talent(overcharge_talent) and not Talent(fury_of_air_talent) and Spell(windstrike) or Talent(overcharge_talent) and SpellCooldown(lightning_bolt_enhancement) < GCD() and Maelstrom() > 80 and Spell(stormstrike) or Talent(fury_of_air_talent) and Maelstrom() > 46 and { SpellCooldown(lightning_bolt_enhancement) > GCD() or not Talent(overcharge_talent) } and Spell(stormstrike) or not Talent(overcharge_talent) and not Talent(fury_of_air_talent) and Spell(stormstrike) or { { Enemies() > 1 or Talent(crashing_storm_talent) or Talent(boulderfist_talent) } and not ArmorSetBonus(T19 4) or TotemRemaining(sprit_wolf) > 5 } and Spell(crash_lightning) or Enemies() >= 3 and Spell(crash_lightning) or Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < 4.8 and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or HasEquippedItem(137084) and not BuffPresent(frostbrand_buff) and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or Talent(fury_of_air_talent) and Talent(overcharge_talent) and ArmorSetBonus(T19 4) and Maelstrom() >= 80 and Spell(lava_lash) or Talent(fury_of_air_talent) and not Talent(overcharge_talent) and ArmorSetBonus(T19 4) and Maelstrom() >= 53 and Spell(lava_lash) or { not ArmorSetBonus(T19 4) and Maelstrom() >= 120 or not Talent(fury_of_air_talent) and ArmorSetBonus(T19 4) and Maelstrom() >= 40 } and Spell(lava_lash) or BuffRemaining(flametongue_buff) < 4.8 and Spell(flametongue) or Spell(rockbiter) or Spell(flametongue) or Spell(boulderfist)
}
...
AddFunction EnhancementDefaultCdActions
{
	...
	unless ArtifactTraitRank(alpha_wolf) and PreviousGCDSpell(feral_spirit) and Spell(crash_lightning)
	{
		...
		#potion,if=feral_spirit.remains>5|target.time_to_die<=60
		if { TotemRemaining(sprit_wolf) > 5 or target.TimeToDie() <= 60 } and CheckBoxOn(opt_use_consumables) and target.Classification(worldboss) Item(prolonged_power_potion usable=1)

		unless { BuffRemaining(boulderfist_buff) < GCD() or Maelstrom() <= 50 and Enemies() >= 3 } and Spell(boulderfist) or { BuffRemaining(boulderfist_buff) < GCD() or Charges(boulderfist count=0) > 1.75 and Maelstrom() <= 100 and Enemies() <= 2 } and Spell(boulderfist) or Talent(landslide_talent) and BuffRemaining(landslide_buff) < GCD() and Spell(rockbiter) or not target.DebuffPresent(fury_of_air_debuff) and Maelstrom() > 22 and Spell(fury_of_air) or Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < GCD() and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or { BuffRemaining(flametongue_buff) < GCD() or SpellCooldown(doom_winds) < 6 and BuffRemaining(flametongue_buff) < 4 } and Spell(flametongue) or Talent(crashing_storm_talent) and Enemies() >= 3 and { not Talent(hailstorm_talent) or BuffRemaining(frostbrand_buff) > GCD() } and Spell(crash_lightning) or Spell(earthen_spike) or { Talent(overcharge_talent) and Maelstrom() >= 40 and not Talent(fury_of_air_talent) or Talent(overcharge_talent) and Talent(fury_of_air_talent) and Maelstrom() > 46 } and Spell(lightning_bolt_enhancement) or BuffRemaining(crash_lightning_buff) < GCD() and Enemies() >= 2 and Spell(crash_lightning) or Spell(windsong)
		{
			#ascendance,if=buff.stormbringer.react
			if BuffPresent(stormbringer_buff) and BuffExpires(ascendance_enhancement_buff) Spell(ascendance_enhancement)
		}
	}
}
...
{
	ArtifactTraitRank(alpha_wolf) and PreviousGCDSpell(feral_spirit) and Spell(crash_lightning) or { BuffRemaining(boulderfist_buff) < GCD() or Maelstrom() <= 50 and Enemies() >= 3 } and Spell(boulderfist) or { BuffRemaining(boulderfist_buff) < GCD() or Charges(boulderfist count=0) > 1.75 and Maelstrom() <= 100 and Enemies() <= 2 } and Spell(boulderfist) or Talent(landslide_talent) and BuffRemaining(landslide_buff) < GCD() and Spell(rockbiter) or not target.DebuffPresent(fury_of_air_debuff) and Maelstrom() > 22 and Spell(fury_of_air) or Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < GCD() and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or { BuffRemaining(flametongue_buff) < GCD() or SpellCooldown(doom_winds) < 6 and BuffRemaining(flametongue_buff) < 4 } and Spell(flametongue) or Talent(crashing_storm_talent) and Enemies() >= 3 and { not Talent(hailstorm_talent) or BuffRemaining(frostbrand_buff) > GCD() } and Spell(crash_lightning) or Spell(earthen_spike) or { Talent(overcharge_talent) and Maelstrom() >= 40 and not Talent(fury_of_air_talent) or Talent(overcharge_talent) and Talent(fury_of_air_talent) and Maelstrom() > 46 } and Spell(lightning_bolt_enhancement) or BuffRemaining(crash_lightning_buff) < GCD() and Enemies() >= 2 and Spell(crash_lightning) or Spell(windsong) or BuffPresent(stormbringer_buff) and { Talent(fury_of_air_talent) and Maelstrom() >= 26 or not Talent(fury_of_air_talent) } and Spell(windstrike) or BuffPresent(stormbringer_buff) and { Talent(fury_of_air_talent) and Maelstrom() >= 26 or not Talent(fury_of_air_talent) } and Spell(stormstrike) or HasEquippedItem(137084) and Talent(hot_hand_talent) and BuffPresent(hot_hand_buff) and not BuffPresent(frostbrand_buff) and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or Talent(hot_hand_talent) and BuffPresent(hot_hand_buff) and Spell(lava_lash) or Enemies() >= 3 and Spell(sundering) or Enemies() >= 4 and Spell(crash_lightning) or Talent(overcharge_talent) and SpellCooldown(lightning_bolt_enhancement) < GCD() and Maelstrom() > 80 and Spell(windstrike) or Talent(fury_of_air_talent) and Maelstrom() > 46 and { SpellCooldown(lightning_bolt_enhancement) > GCD() or not Talent(overcharge_talent) } and Spell(windstrike) or not Talent(overcharge_talent) and not Talent(fury_of_air_talent) and Spell(windstrike) or Talent(overcharge_talent) and SpellCooldown(lightning_bolt_enhancement) < GCD() and Maelstrom() > 80 and Spell(stormstrike) or Talent(fury_of_air_talent) and Maelstrom() > 46 and { SpellCooldown(lightning_bolt_enhancement) > GCD() or not Talent(overcharge_talent) } and Spell(stormstrike) or not Talent(overcharge_talent) and not Talent(fury_of_air_talent) and Spell(stormstrike) or { { Enemies() > 1 or Talent(crashing_storm_talent) or Talent(boulderfist_talent) } and not ArmorSetBonus(T19 4) or TotemRemaining(sprit_wolf) > 5 } and Spell(crash_lightning) or Enemies() >= 3 and Spell(crash_lightning) or Talent(hailstorm_talent) and BuffRemaining(frostbrand_buff) < 4.8 and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or HasEquippedItem(137084) and not BuffPresent(frostbrand_buff) and { not Talent(fury_of_air_talent) or Talent(fury_of_air_talent) and Maelstrom() > 25 } and Spell(frostbrand) or Talent(fury_of_air_talent) and Talent(overcharge_talent) and ArmorSetBonus(T19 4) and Maelstrom() >= 80 and Spell(lava_lash) or Talent(fury_of_air_talent) and not Talent(overcharge_talent) and ArmorSetBonus(T19 4) and Maelstrom() >= 53 and Spell(lava_lash) or { not ArmorSetBonus(T19 4) and Maelstrom() >= 120 or not Talent(fury_of_air_talent) and ArmorSetBonus(T19 4) and Maelstrom() >= 40 } and Spell(lava_lash) or BuffRemaining(flametongue_buff) < 4.8 and Spell(flametongue) or Spell(rockbiter) or Spell(flametongue) or Spell(boulderfist)
}
