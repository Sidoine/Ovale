...
AddFunction HavocDefaultMainActions
{
	...
	if Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and { not Talent(first_blood_talent) or DemonicFury() >= 80 or DemonicFuryDeficit() < 30 } Spell(eye_beam)
	#demons_bite,sync=metamorphosis,if=fury.deficit>=25
	if BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } and Spell(metamorphosis) and DemonicFuryDeficit() >= 25 Spell(demons_bite)
	...
	unless HavocCooldownMainPostConditions()
	{
		...
		if FIXME_death_sweep_worth_using Spell(death_sweep)
		#demons_bite,if=buff.metamorphosis.remains>gcd&cooldown.blade_dance.remains<gcd&fury<70&death_sweep_worth_using
		if BuffRemaining(metamorphosis_buff) > GCD() and SpellCooldown(blade_dance) < GCD() and DemonicFury() < 70 and FIXME_death_sweep_worth_using Spell(demons_bite)
		#blade_dance,if=blade_dance_worth_using
		if FIXME_blade_dance_worth_using Spell(blade_dance)
		...
		if not Talent(momentum_talent) or BuffPresent(momentum_buff) or DemonicFuryDeficit() <= 30 + BuffPresent(prepared_buff) * 8 or BuffRemaining(metamorphosis_buff) < 2 Spell(annihilation)
		...
		if not Talent(demonic_talent) and { Enemies() > Enemies(tagged=1) or 600 > 45 and BuffExpires(metamorphosis_buff) and { BuffPresent(anguish_of_the_deceiver_buff) or Enemies() > 1 or Level() == 100 } } Spell(eye_beam)
		#demons_bite,if=buff.metamorphosis.down&cooldown.blade_dance.remains<gcd&fury<55&blade_dance_worth_using
		if BuffExpires(metamorphosis_buff) and SpellCooldown(blade_dance) < GCD() and DemonicFury() < 55 and FIXME_blade_dance_worth_using Spell(demons_bite)
		#demons_bite,if=talent.demonic.enabled&buff.metamorphosis.down&cooldown.eye_beam.remains<gcd&fury.deficit>=20
		if Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and SpellCooldown(eye_beam) < GCD() and DemonicFuryDeficit() >= 20 Spell(demons_bite)
		#demons_bite,if=talent.demonic.enabled&buff.metamorphosis.down&cooldown.eye_beam.remains<2*gcd&fury.deficit>=45
		if Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and SpellCooldown(eye_beam) < 2 * GCD() and DemonicFuryDeficit() >= 45 Spell(demons_bite)
		#throw_glaive,if=buff.metamorphosis.down&spell_targets>=3
		if BuffExpires(metamorphosis_buff) and Enemies() >= 3 Spell(throw_glaive)
		...
		if Charges(fel_barrage) == 4 and BuffExpires(metamorphosis_buff) and { BuffPresent(momentum_buff) or not Talent(momentum_talent) } and { Enemies() > Enemies(tagged=1) or 600 > 30 } Spell(fel_barrage)
...
AddFunction HavocDefaultShortCdActions
{
	...
	unless Talent(demonic_appetite_talent) and DemonicFuryDeficit() >= 30 and Spell(pick_up_fragment)
	{
		...
		unless { Talent(prepared_talent) or Talent(momentum_talent) } and BuffExpires(prepared_buff) and BuffExpires(momentum_buff) and Spell(vengeful_retreat) or { Talent(momentum_talent) or Talent(fel_mastery_talent) } and { not Talent(momentum_talent) or { Charges(fel_rush) == 2 or SpellCooldown(vengeful_retreat) > 4 } and BuffExpires(momentum_buff) } and { not Talent(fel_mastery_talent) or DemonicFuryDeficit() >= 25 } and 600 > Charges(fel_rush) * 10 and Spell(fel_rush) or Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and { not Talent(first_blood_talent) or DemonicFury() >= 80 or DemonicFuryDeficit() < 30 } and Spell(eye_beam) or BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } and Spell(metamorphosis) and DemonicFuryDeficit() >= 25 and Spell(demons_bite)
		{
			#call_action_list,name=cooldown
			HavocCooldownShortCdActions()

			unless HavocCooldownShortCdPostConditions() or { Enemies() > Enemies(tagged=1) or 600 > 55 } and Spell(fury_of_the_illidari) or FIXME_death_sweep_worth_using and Spell(death_sweep) or BuffRemaining(metamorphosis_buff) > GCD() and SpellCooldown(blade_dance) < GCD() and DemonicFury() < 70 and FIXME_death_sweep_worth_using and Spell(demons_bite) or FIXME_blade_dance_worth_using and Spell(blade_dance) or Charges(fel_barrage) >= 5 and { BuffPresent(momentum_buff) or not Talent(momentum_talent) } and { Enemies() > Enemies(tagged=1) or 600 > 30 } and Spell(fel_barrage) or Talent(bloodlet_talent) and Enemies() >= 2 + TalentPoints(chaos_cleave_talent) and { not Talent(master_of_the_glaive_talent) or not Talent(momentum_talent) or BuffPresent(momentum_buff) } and Spell(throw_glaive)
			{
				#fel_eruption
				Spell(fel_eruption)
			}
...
{
	Talent(demonic_appetite_talent) and DemonicFuryDeficit() >= 30 and Spell(pick_up_fragment) or { Talent(prepared_talent) or Talent(momentum_talent) } and BuffExpires(prepared_buff) and BuffExpires(momentum_buff) and Spell(vengeful_retreat) or { Talent(momentum_talent) or Talent(fel_mastery_talent) } and { not Talent(momentum_talent) or { Charges(fel_rush) == 2 or SpellCooldown(vengeful_retreat) > 4 } and BuffExpires(momentum_buff) } and { not Talent(fel_mastery_talent) or DemonicFuryDeficit() >= 25 } and 600 > Charges(fel_rush) * 10 and Spell(fel_rush) or Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and { not Talent(first_blood_talent) or DemonicFury() >= 80 or DemonicFuryDeficit() < 30 } and Spell(eye_beam) or BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } and Spell(metamorphosis) and DemonicFuryDeficit() >= 25 and Spell(demons_bite) or HavocCooldownShortCdPostConditions() or { Enemies() > Enemies(tagged=1) or 600 > 55 } and Spell(fury_of_the_illidari) or FIXME_death_sweep_worth_using and Spell(death_sweep) or BuffRemaining(metamorphosis_buff) > GCD() and SpellCooldown(blade_dance) < GCD() and DemonicFury() < 70 and FIXME_death_sweep_worth_using and Spell(demons_bite) or FIXME_blade_dance_worth_using and Spell(blade_dance) or Charges(fel_barrage) >= 5 and { BuffPresent(momentum_buff) or not Talent(momentum_talent) } and { Enemies() > Enemies(tagged=1) or 600 > 30 } and Spell(fel_barrage) or Talent(bloodlet_talent) and Enemies() >= 2 + TalentPoints(chaos_cleave_talent) and { not Talent(master_of_the_glaive_talent) or not Talent(momentum_talent) or BuffPresent(momentum_buff) } and Spell(throw_glaive) or DemonicFuryDeficit() >= 30 + BuffPresent(prepared_buff) * 8 and Spell(felblade) or { not Talent(momentum_talent) or BuffPresent(momentum_buff) or DemonicFuryDeficit() <= 30 + BuffPresent(prepared_buff) * 8 or BuffRemaining(metamorphosis_buff) < 2 } and Spell(annihilation) or Talent(bloodlet_talent) and { not Talent(master_of_the_glaive_talent) or not Talent(momentum_talent) or BuffPresent(momentum_buff) } and Spell(throw_glaive) or not Talent(demonic_talent) and { Enemies() > Enemies(tagged=1) or 600 > 45 and BuffExpires(metamorphosis_buff) and { BuffPresent(anguish_of_the_deceiver_buff) or Enemies() > 1 or Level() == 100 } } and Spell(eye_beam) or BuffExpires(metamorphosis_buff) and SpellCooldown(blade_dance) < GCD() and DemonicFury() < 55 and FIXME_blade_dance_worth_using and Spell(demons_bite) or Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and SpellCooldown(eye_beam) < GCD() and DemonicFuryDeficit() >= 20 and Spell(demons_bite) or Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and SpellCooldown(eye_beam) < 2 * GCD() and DemonicFuryDeficit() >= 45 and Spell(demons_bite) or BuffExpires(metamorphosis_buff) and Enemies() >= 3 and Spell(throw_glaive) or { not Talent(momentum_talent) or BuffPresent(momentum_buff) or DemonicFuryDeficit() <= 30 + BuffPresent(prepared_buff) * 8 } and Spell(chaos_strike) or Charges(fel_barrage) == 4 and BuffExpires(metamorphosis_buff) and { BuffPresent(momentum_buff) or not Talent(momentum_talent) } and { Enemies() > Enemies(tagged=1) or 600 > 30 } and Spell(fel_barrage) or not Talent(momentum_talent) and 600 > Charges(fel_rush) * 10 and Spell(fel_rush) or Spell(demons_bite) or Spell(throw_glaive) or { 0 > 15 or BuffPresent(out_of_range_buff) and not Talent(momentum_talent) } and Spell(fel_rush) or 0 > 15 and Spell(vengeful_retreat)
}
...
{
	Talent(demonic_appetite_talent) and DemonicFuryDeficit() >= 30 and Spell(pick_up_fragment) or { Talent(prepared_talent) or Talent(momentum_talent) } and BuffExpires(prepared_buff) and BuffExpires(momentum_buff) and Spell(vengeful_retreat) or { Talent(momentum_talent) or Talent(fel_mastery_talent) } and { not Talent(momentum_talent) or { Charges(fel_rush) == 2 or SpellCooldown(vengeful_retreat) > 4 } and BuffExpires(momentum_buff) } and { not Talent(fel_mastery_talent) or DemonicFuryDeficit() >= 25 } and 600 > Charges(fel_rush) * 10 and Spell(fel_rush) or Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and { not Talent(first_blood_talent) or DemonicFury() >= 80 or DemonicFuryDeficit() < 30 } and Spell(eye_beam) or BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } and Spell(metamorphosis) and DemonicFuryDeficit() >= 25 and Spell(demons_bite) or HavocCooldownCdPostConditions() or { Enemies() > Enemies(tagged=1) or 600 > 55 } and Spell(fury_of_the_illidari) or FIXME_death_sweep_worth_using and Spell(death_sweep) or BuffRemaining(metamorphosis_buff) > GCD() and SpellCooldown(blade_dance) < GCD() and DemonicFury() < 70 and FIXME_death_sweep_worth_using and Spell(demons_bite) or FIXME_blade_dance_worth_using and Spell(blade_dance) or Charges(fel_barrage) >= 5 and { BuffPresent(momentum_buff) or not Talent(momentum_talent) } and { Enemies() > Enemies(tagged=1) or 600 > 30 } and Spell(fel_barrage) or Talent(bloodlet_talent) and Enemies() >= 2 + TalentPoints(chaos_cleave_talent) and { not Talent(master_of_the_glaive_talent) or not Talent(momentum_talent) or BuffPresent(momentum_buff) } and Spell(throw_glaive) or Spell(fel_eruption) or DemonicFuryDeficit() >= 30 + BuffPresent(prepared_buff) * 8 and Spell(felblade) or { not Talent(momentum_talent) or BuffPresent(momentum_buff) or DemonicFuryDeficit() <= 30 + BuffPresent(prepared_buff) * 8 or BuffRemaining(metamorphosis_buff) < 2 } and Spell(annihilation) or Talent(bloodlet_talent) and { not Talent(master_of_the_glaive_talent) or not Talent(momentum_talent) or BuffPresent(momentum_buff) } and Spell(throw_glaive) or not Talent(demonic_talent) and { Enemies() > Enemies(tagged=1) or 600 > 45 and BuffExpires(metamorphosis_buff) and { BuffPresent(anguish_of_the_deceiver_buff) or Enemies() > 1 or Level() == 100 } } and Spell(eye_beam) or BuffExpires(metamorphosis_buff) and SpellCooldown(blade_dance) < GCD() and DemonicFury() < 55 and FIXME_blade_dance_worth_using and Spell(demons_bite) or Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and SpellCooldown(eye_beam) < GCD() and DemonicFuryDeficit() >= 20 and Spell(demons_bite) or Talent(demonic_talent) and BuffExpires(metamorphosis_buff) and SpellCooldown(eye_beam) < 2 * GCD() and DemonicFuryDeficit() >= 45 and Spell(demons_bite) or BuffExpires(metamorphosis_buff) and Enemies() >= 3 and Spell(throw_glaive) or { not Talent(momentum_talent) or BuffPresent(momentum_buff) or DemonicFuryDeficit() <= 30 + BuffPresent(prepared_buff) * 8 } and Spell(chaos_strike) or Charges(fel_barrage) == 4 and BuffExpires(metamorphosis_buff) and { BuffPresent(momentum_buff) or not Talent(momentum_talent) } and { Enemies() > Enemies(tagged=1) or 600 > 30 } and Spell(fel_barrage) or not Talent(momentum_talent) and 600 > Charges(fel_rush) * 10 and Spell(fel_rush) or Spell(demons_bite) or Spell(throw_glaive) or { 0 > 15 or BuffPresent(out_of_range_buff) and not Talent(momentum_talent) } and Spell(fel_rush) or 0 > 15 and Spell(vengeful_retreat)
}
...
AddFunction HavocCooldownMainActions
{
	...
	if not False(raid_event_adds_exists) and { SpellCooldown(metamorphosis) > 100 or target.TimeToDie() < 70 } Spell(nemesis)
	#nemesis,sync=metamorphosis,if=!raid_event.adds.exists
	if BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } and Spell(metamorphosis) and not False(raid_event_adds_exists) Spell(nemesis)
	#chaos_blades,if=buff.metamorphosis.up|cooldown.metamorphosis.remains>100|target.time_to_die<20
	if BuffPresent(metamorphosis_buff) or SpellCooldown(metamorphosis) > 100 or target.TimeToDie() < 20 Spell(chaos_blades)
	...
	if BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } Spell(metamorphosis)
}
...
{
	False(raid_event_adds_exists) and target.DebuffExpires(nemesis_debuff) and { Enemies() > Enemies(tagged=1) or 600 > 60 } and Spell(nemesis) or not False(raid_event_adds_exists) and { SpellCooldown(metamorphosis) > 100 or target.TimeToDie() < 70 } and Spell(nemesis) or BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } and Spell(metamorphosis) and not False(raid_event_adds_exists) and Spell(nemesis) or { BuffPresent(metamorphosis_buff) or SpellCooldown(metamorphosis) > 100 or target.TimeToDie() < 20 } and Spell(chaos_blades) or BuffExpires(metamorphosis_buff) and { not Talent(demonic_talent) or not SpellCooldownDuration(eye_beam) } and { not Talent(chaos_blades_talent) or SpellCooldownDuration(chaos_blades) } and { not Talent(nemesis_talent) or target.DebuffPresent(nemesis_debuff) or SpellCooldownDuration(nemesis) } and Spell(metamorphosis)
}
...
AddFunction VengeanceDefaultMainActions
{
	...
	if not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and BuffPresent(fiery_demise_buff) and target.DebuffPresent(fiery_brand_debuff) Spell(infernal_strike)
	...
	if not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and { not BuffPresent(fiery_demise_buff) or { SpellMaxCharges(infernal_strike) - Charges(infernal_strike count=0) } * SpellChargeCooldown(infernal_strike) < SpellCooldown(fiery_brand) + 5 } and { SpellCooldown(sigil_of_flame) > 7 or Charges(infernal_strike) == 2 } Spell(infernal_strike)
	#spirit_bomb,if=debuff.frailty.down
	if target.DebuffExpires(frailty_debuff) Spell(spirit_bomb)
	#immolation_aura,if=pain<=80
	if FIXME_pain <= 80 Spell(immolation_aura)
	#felblade,if=pain<=70
	if FIXME_pain <= 70 Spell(felblade)
	#soul_barrier
	Spell(soul_barrier)
	#soul_cleave,if=soul_fragments=5
	if FIXME_soul_fragments == 5 Spell(soul_cleave)
	...
	if BuffExpires(demon_spikes_buff) and not target.DebuffPresent(fiery_brand_debuff) and BuffExpires(metamorphosis_buff) and IncomingDamage(5) > MaxHealth() * 0.7 Spell(metamorphosis)
	...
	if target.DebuffRemaining(sigil_of_flame_debuff) - FIXME_delay <= 0.3 * BaseDuration(sigil_of_flame_debuff) Spell(sigil_of_flame)
	#fracture,if=pain>=80&soul_fragments<4&incoming_damage_4s<=health.max*0.20
	if FIXME_pain >= 80 and FIXME_soul_fragments < 4 and IncomingDamage(4) <= MaxHealth() * 0.2 Spell(fracture)
	#soul_cleave,if=pain>=80
	if FIXME_pain >= 80 Spell(soul_cleave)
	#shear
	Spell(shear)
}
...
AddFunction VengeanceDefaultShortCdActions
{
	...
	if BuffExpires(demon_spikes_buff) and BuffExpires(metamorphosis_buff) Spell(fiery_brand)
	#demon_spikes,if=charges=2|buff.demon_spikes.down&!dot.fiery_brand.ticking&buff.metamorphosis.down
	if Charges(demon_spikes) == 2 or BuffExpires(demon_spikes_buff) and not target.DebuffPresent(fiery_brand_debuff) and BuffExpires(metamorphosis_buff) Spell(demon_spikes)
	#empower_wards,if=debuff.casting.up
	if target.DebuffPresent(casting_debuff) Spell(empower_wards)

	unless not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and BuffPresent(fiery_demise_buff) and target.DebuffPresent(fiery_brand_debuff) and Spell(infernal_strike) or not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and { not BuffPresent(fiery_demise_buff) or { SpellMaxCharges(infernal_strike) - Charges(infernal_strike count=0) } * SpellChargeCooldown(infernal_strike) < SpellCooldown(fiery_brand) + 5 } and { SpellCooldown(sigil_of_flame) > 7 or Charges(infernal_strike) == 2 } and Spell(infernal_strike) or target.DebuffExpires(frailty_debuff) and Spell(spirit_bomb)
	{
		#soul_carver,if=dot.fiery_brand.ticking
		if target.DebuffPresent(fiery_brand_debuff) Spell(soul_carver)

		unless FIXME_pain <= 80 and Spell(immolation_aura) or FIXME_pain <= 70 and Spell(felblade) or Spell(soul_barrier) or FIXME_soul_fragments == 5 and Spell(soul_cleave) or BuffExpires(demon_spikes_buff) and not target.DebuffPresent(fiery_brand_debuff) and BuffExpires(metamorphosis_buff) and IncomingDamage(5) > MaxHealth() * 0.7 and Spell(metamorphosis) or IncomingDamage(5) > MaxHealth() * 0.7 and Spell(fel_devastation) or IncomingDamage(5) >= MaxHealth() * 0.7 and Spell(soul_cleave)
		{
			#fel_eruption
			Spell(fel_eruption)
		}
...
{
	not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and BuffPresent(fiery_demise_buff) and target.DebuffPresent(fiery_brand_debuff) and Spell(infernal_strike) or not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and { not BuffPresent(fiery_demise_buff) or { SpellMaxCharges(infernal_strike) - Charges(infernal_strike count=0) } * SpellChargeCooldown(infernal_strike) < SpellCooldown(fiery_brand) + 5 } and { SpellCooldown(sigil_of_flame) > 7 or Charges(infernal_strike) == 2 } and Spell(infernal_strike) or target.DebuffExpires(frailty_debuff) and Spell(spirit_bomb) or FIXME_pain <= 80 and Spell(immolation_aura) or FIXME_pain <= 70 and Spell(felblade) or Spell(soul_barrier) or FIXME_soul_fragments == 5 and Spell(soul_cleave) or BuffExpires(demon_spikes_buff) and not target.DebuffPresent(fiery_brand_debuff) and BuffExpires(metamorphosis_buff) and IncomingDamage(5) > MaxHealth() * 0.7 and Spell(metamorphosis) or IncomingDamage(5) > MaxHealth() * 0.7 and Spell(fel_devastation) or IncomingDamage(5) >= MaxHealth() * 0.7 and Spell(soul_cleave) or target.DebuffRemaining(sigil_of_flame_debuff) - FIXME_delay <= 0.3 * BaseDuration(sigil_of_flame_debuff) and Spell(sigil_of_flame) or FIXME_pain >= 80 and FIXME_soul_fragments < 4 and IncomingDamage(4) <= MaxHealth() * 0.2 and Spell(fracture) or FIXME_pain >= 80 and Spell(soul_cleave) or Spell(shear)
}
...
{
	not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and BuffPresent(fiery_demise_buff) and target.DebuffPresent(fiery_brand_debuff) and Spell(infernal_strike) or not FIXME_sigil_placed and not InFlightToTarget(infernal_strike) and target.DebuffRemaining(infernal_strike_debuff) - TravelTime(infernal_strike) - FIXME_delay < 0.3 * BaseDuration(infernal_strike_debuff) and { not BuffPresent(fiery_demise_buff) or { SpellMaxCharges(infernal_strike) - Charges(infernal_strike count=0) } * SpellChargeCooldown(infernal_strike) < SpellCooldown(fiery_brand) + 5 } and { SpellCooldown(sigil_of_flame) > 7 or Charges(infernal_strike) == 2 } and Spell(infernal_strike) or target.DebuffExpires(frailty_debuff) and Spell(spirit_bomb) or target.DebuffPresent(fiery_brand_debuff) and Spell(soul_carver) or FIXME_pain <= 80 and Spell(immolation_aura) or FIXME_pain <= 70 and Spell(felblade) or Spell(soul_barrier) or FIXME_soul_fragments == 5 and Spell(soul_cleave) or BuffExpires(demon_spikes_buff) and not target.DebuffPresent(fiery_brand_debuff) and BuffExpires(metamorphosis_buff) and IncomingDamage(5) > MaxHealth() * 0.7 and Spell(metamorphosis) or IncomingDamage(5) > MaxHealth() * 0.7 and Spell(fel_devastation) or IncomingDamage(5) >= MaxHealth() * 0.7 and Spell(soul_cleave) or Spell(fel_eruption) or target.DebuffRemaining(sigil_of_flame_debuff) - FIXME_delay <= 0.3 * BaseDuration(sigil_of_flame_debuff) and Spell(sigil_of_flame) or FIXME_pain >= 80 and FIXME_soul_fragments < 4 and IncomingDamage(4) <= MaxHealth() * 0.2 and Spell(fracture) or FIXME_pain >= 80 and Spell(soul_cleave) or Spell(shear)
}
