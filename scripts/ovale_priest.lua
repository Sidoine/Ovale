...
{
	#mindbender,if=!set_bonus.tier20_4pc&(talent.mindbender.enabled&((talent.surrender_to_madness.enabled&target.time_to_die>variable.s2mcheck+60)|!talent.surrender_to_madness.enabled))
	if not ArmorSetBonus(T20 4) and Talent(mindbender_talent) and { Talent(surrender_to_madness_talent) and target.TimeToDie() > s2mcheck() + 60 or not Talent(surrender_to_madness_talent) } Spell(mindbender)
...
{
	unless not ArmorSetBonus(T20 4) and Talent(mindbender_talent) and { Talent(surrender_to_madness_talent) and target.TimeToDie() > s2mcheck() + 60 or not Talent(surrender_to_madness_talent) } and Spell(mindbender) or Speed() > 0 and Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < GCD() and Spell(shadow_word_pain) or Talent(misery_talent) and { target.DebuffRemaining(vampiric_touch_debuff) < 3 * GCD() or target.DebuffRemaining(shadow_word_pain_debuff) < 3 * GCD() } and Spell(vampiric_touch) or not Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < { 3 + 4 / 3 } * GCD() and Spell(shadow_word_pain) or not Talent(misery_talent) and target.DebuffRemaining(vampiric_touch_debuff) < { 4 + 4 / 3 } * GCD() and Spell(vampiric_touch) or Spell(void_eruption)
	{
...
{
	not ArmorSetBonus(T20 4) and Talent(mindbender_talent) and { Talent(surrender_to_madness_talent) and target.TimeToDie() > s2mcheck() + 60 or not Talent(surrender_to_madness_talent) } and Spell(mindbender) or Speed() > 0 and Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < GCD() and Spell(shadow_word_pain) or Talent(misery_talent) and { target.DebuffRemaining(vampiric_touch_debuff) < 3 * GCD() or target.DebuffRemaining(shadow_word_pain_debuff) < 3 * GCD() } and Spell(vampiric_touch) or not Talent(misery_talent) and target.DebuffRemaining(shadow_word_pain_debuff) < { 3 + 4 / 3 } * GCD() and Spell(shadow_word_pain) or not Talent(misery_talent) and target.DebuffRemaining(vampiric_touch_debuff) < { 4 + 4 / 3 } * GCD() and Spell(vampiric_touch) or Spell(void_eruption) or Talent(shadow_crash_talent) and Spell(shadow_crash) or { Enemies() <= 4 or Talent(reaper_of_souls_talent) and Enemies() <= 2 } and SpellCharges(shadow_word_death) == 2 and Insanity() <= 85 - 15 * TalentPoints(reaper_of_souls_talent) and Spell(shadow_word_death) or Enemies() <= 4 and Talent(legacy_of_the_void_talent) and { Insanity() <= 81 or Insanity() <= 75.2 and Talent(fortress_of_the_mind_talent) } and Spell(mind_blast) or { Enemies() <= 4 and not Talent(legacy_of_the_void_talent) or Insanity() <= 96 or Insanity() <= 95.2 and Talent(fortress_of_the_mind_talent) } and Spell(mind_blast) or not Talent(misery_talent) and not target.DebuffPresent(shadow_word_pain_debuff) and target.TimeToDie() > 10 and Enemies() < 5 and { Talent(auspicious_spirits_talent) or Talent(shadowy_insight_talent) } and Spell(shadow_word_pain) or Enemies() > 1 and not Talent(misery_talent) and not target.DebuffPresent(vampiric_touch_debuff) and 85.2 * { 1 + 0.2 + MasteryRating() / 16000 } * { 1 + 0.2 * TalentPoints(sanlayn_talent) } * 0.5 * target.TimeToDie() / { GCD() * { 138 + 80 * { Enemies() - 1 } } } > 1 and Spell(vampiric_touch) or Enemies() > 1 and not Talent(misery_talent) and not target.DebuffPresent(shadow_word_pain_debuff) and 47.12 * { 1 + 0.2 + MasteryRating() / 16000 } * 0.75 * target.TimeToDie() / { GCD() * { 138 + 80 * { Enemies() - 1 } } } > 1 and Spell(shadow_word_pain) or Talent(shadow_word_void_talent) and Insanity() <= 75 - 10 * TalentPoints(legacy_of_the_void_talent) and Spell(shadow_word_void) or Spell(mind_flay) or Spell(shadow_word_pain)
}
...
AddFunction ShadowVfMainActions
{
	...
	#mindbender,if=set_bonus.tier20_4pc&buff.insanity_drain_stacks.value>=(25+2*buff.bloodlust.up+2*talent.fortress_of_the_mind.enabled)&(!talent.surrender_to_madness.enabled|(talent.surrender_to_madness.enabled&target.time_to_die>variable.s2mcheck-buff.insanity_drain_stacks.value))
	if ArmorSetBonus(T20 4) and BuffAmount(insanity_drain_stacks_buff) >= 25 + 2 * BuffPresent(burst_haste_buff any=1) + 2 * TalentPoints(fortress_of_the_mind_talent) and { not Talent(surrender_to_madness_talent) or Talent(surrender_to_madness_talent) and target.TimeToDie() > s2mcheck() - BuffAmount(insanity_drain_stacks_buff) } Spell(mindbender)
	#mindbender,if=!set_bonus.tier20_4pc&(!talent.surrender_to_madness.enabled|(talent.surrender_to_madness.enabled&target.time_to_die>variable.s2mcheck-(buff.insanity_drain_stacks.value)+30))
	if not ArmorSetBonus(T20 4) and { not Talent(surrender_to_madness_talent) or Talent(surrender_to_madness_talent) and target.TimeToDie() > s2mcheck() - BuffAmount(insanity_drain_stacks_buff) + 30 } Spell(mindbender)
	#void_bolt
	Spell(void_bolt)
...
AddFunction ShadowVfCdActions
{
	...
	unless Spell(void_bolt)
	{
		#dispersion,if=talent.legacy_of_the_void.enabled&current_insanity_drain*gcd.max>insanity&!buff.power_infusion.up&buff.voidform.stack>=51&cooldown.void_torrent.remains<=8
		if Talent(legacy_of_the_void_talent) and CurrentInsanityDrain() * GCD() > Insanity() and not BuffPresent(power_infusion_buff) and BuffStacks(voidform_buff) >= 51 and SpellCooldown(void_torrent) <= 8 Spell(dispersion)
